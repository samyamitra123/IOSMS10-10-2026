...
from 
prd_employee_master mas 
WHERE SUBSTRING(cast(mas.emp_retirement_date as text),1,7)>='2020-01'  and SUBSTRING(cast(mas.emp_retirement_date as text),1,7)<='".date("Y-m", strtotime("+11 months"))."'
AND mas.emp_status in('1') AND mas.emp_pension_status in('0') AND mas.gp_id_fk NOT IN('3355','3380','3383','3400','3401','3402','3403') AND mas.ps_id_fk NOT IN ('342','343')
AND (mas.zp_id_fk is null or mas.zp_id_fk not in('21')) 
and (mas.ps_id_fk!='0' or mas.gp_id_fk!='0' or (mas.zp_id_fk!='0' and mas.zp_emp_type='367'))
...
mas.ropa_level
from prd_employee_master mas  WHERE 
 mas.emp_status in('2') AND mas.emp_pension_status in('0') and mas.ropa_status='1' AND mas.gp_id_fk NOT IN('3355','3380','3383','3400','3401','3402','3403')
  AND(mas.zp_id_fk is null or mas.zp_id_fk not in('21') )
  AND (mas.ps_id_fk!='0' or mas.gp_id_fk!='0' or (mas.zp_id_fk!='0' and mas.zp_emp_type='367'))
 AND mas.ps_id_fk NOT IN ('342','343') AND mas.emp_id_const!='0' 
...
LEFT JOIN 
(select * from (select emp_id_fk,date,reason,claimant_name,relationship_incumbent,claiment_mobile_no,max(date) over (partition by emp_id_fk) max_my_date from prd_stop_sal_reason)A 
where date = max_my_date and reason='1993' AND (SUBSTRING(cast(date as text),1,7)>='2020-01') and SUBSTRING(cast(date as text),1,7)<='".$curMonthYear."') as stop ON mas1.emp_id_pk=stop.emp_id_fk )";
//print_r($Query); exit;

$sql_pension_employee_data = $db->fetch_table($Query);


//print_r($sql_pension_employee_data); exit;
...
foreach ($sql_pension_employee_data as $mas_data) 
{
	...
	$sql_pension_data= $db->fetch_table($Query);
    //print_r(count($sql_pension_data) <= 0); exit;	
	//pg_query('BEGIN');
	if(count($sql_pension_data) <= 0 && $mas_data['emp_pension_status']==0)
	{
       ...
       $Query = "INSERT INTO prd_pension_employee 
								...
								emp_pay_in_payband, emp_father_name,emp_mother_name, emp_religion,
								...
								mas.emp_termination_date, mas.emp_group, mas.emp_desig_first_app, mas.emp_pay_in_payband, mas.emp_father_name,mas.emp_mother_name,
								...
								from prd_employee_master mas WHERE  mas.emp_id_pk='".$mas_data['emp_id_pk']."' and SUBSTRING(cast(mas.emp_retirement_date as text),1,7)>='2020-01'  and SUBSTRING(cast(mas.emp_retirement_date as text),1,7)<='".date("Y-m", strtotime("+11 months"))."'
								...
								mas.emp_pay_in_payband, mas.emp_father_name,mas.emp_mother_name, mas.emp_religion,
								...
								from prd_stop_sal_reason)A where date = max_my_date and reason='1993' AND (SUBSTRING(cast(date as text),1,7)>='2020-01') and SUBSTRING(cast(date as text),1,7)<='".$curMonthYear."') as stop ON mas1.emp_id_pk=stop.emp_id_fk ))b"; 

		//print( $Query); exit; 
		$sql_pension_employee_data_insert=$db->insert($Query);										
...
$Query ="select emp.emp_id_pk, emp.gp_id_fk , emp.emp_first_name , emp.emp_second_name , 
...
emp.emp_desig_first_app, emp.emp_pay_in_payband, emp.emp_father_name,emp.emp_mother_name,
...
from prd_stop_sal_reason)A where date = max_my_date and reason='1993') as stop ON pen.emp_id_fk=stop.emp_id_fk 
 where emp.emp_pension_status='0' and emp.ropa_status='1'
 and 
 SUBSTRING(cast(emp.emp_termination_date as text),1,7)>='2020-01'  and SUBSTRING(cast(emp.emp_termination_date as text),1,7)<='".date("Y-m", strtotime("+11 months"))."'
...
$sql_pension_employee_data_check = $db->fetch_table($Query);

//print_r($sql_pension_employee_data_check); exit;
		...
		if($mas_data_up['emp_pension_status']==2 && count($sql_pension_data)>=1)
				...
				if(count($sql_pension_employee_data_insert_archive)==1 && $sql_pension_data[0]['emp_pension_status']==0)
				{
					...
					$update_penstion_data=$db->update("UPDATE prd_pension_employee SET
														...
														emp_mother_name='".$mas_data_up['emp_mother_name']."',
