  ...
  authenticat_hoo_remarks
   ...
   $present_address_state=$data[0]['present_address_state'];
    ...
    <div class="col-sm-3">
    <p class="text-muted text-show"><?= $emp_id_const; ?></p>
    </div>
    </div>
    ...
    <div class="col-md-12">
    <label class="col-md-12 control-label">Proposal Name</label>  
    <p class="text-muted text-show"><?php echo $proposal ?></p>
    </div>
 
    <div class="col-md-12 row">
    <div class="col-md-4">  
    <label class="control-label">Name</label>  
    <?php $emp_second_name = ($emp_second_name != '')?$emp_second_name.' ':''; ?>
    <p class="text-muted text-show"><?php echo $emp_first_name.' '.$emp_second_name.$emp_last_name ?></p>
     </div>
    <div class="col-md-4">  
    <label class="control-label">Designation</label>  
    
    <p class="text-muted text-show"><?php echo $arr_desig_d[0]['description'] ?></p>
     </div> 
    <div class="col-md-4">  
    <label class="control-label">First Joining in Service</label>  
    
    <p class="text-muted text-show"><?php echo ($emp_first_join_date=='0001-01-01') ? '' : dateshow($emp_first_join_date); ?></p>
     </div>          
    </div>
    
     <div class="col-md-12 row">
    <div class="col-md-4">  
    <?php if($data[0]['gp_id_fk']!='0'): ?>  
    <label class="control-label">Place Of Posting In GP</label>  
    <p class="text-muted text-show"><?php echo code_gp($gp_id_fk);?></p>
    <?php elseif($data[0]['ps_id_fk']!='0'): ?>
    <label class="control-label">Place Of Posting In PS</label>  
    <p class="text-muted text-show"><?php echo code_ps($ps_id_fk);?></p>    
    <?php elseif($data[0]['ZP_id_fk']!='0'): ?>
    <label class="control-label">Place Of Posting In ZP</label>  
    <p class="text-muted text-show"><?php echo code_district($zp_id_fk);?></p>    
    <?php endif; ?>    
     </div>
    <div class="col-md-4">  
    <?php if($data[0]['gp_id_fk']!='0'): ?>   
    <label class="control-label">Block Name</label>  
    <p class="text-muted text-show"><?php echo  code_block($gp_id_fk);?></p>
    <?php endif; ?> 
     </div> 
    <div class="col-md-4">  
    <label class="control-label">First Joining in Service</label>  
   ...
   <?php
    if($group > 0)
      {
    $arr = $db->fetch_table("select code,description from prd_dise_code_master where code='".$group."'");
    ?>   
    <p class="text-muted text-show"><?php echo $arr[0]['description']; ?></p>
    <?php } ?>  
     </div>
    <div class="col-md-4">  
   ...
   <?php
    if($employee_type > 0)
      {
    $arr = $db->fetch_table("select code,description from intra_pri_cg_relation_master where status='2' AND code='".$employee_type."'");
    ?>   
    <p class="text-muted text-show"><?php echo $arr[0]['description']; ?></p>
    <?php } ?> 
     </div> 
    <div class="col-md-4">  
    <label class="control-label">Last Pay Drawn(GROSS)</label>  
    
    <p class="text-muted text-show"><?php echo $last_pay; ?></p>
     </div>          
    </div>   
    
    <div class="col-md-12 row">
    <div class="col-md-4">  
    <label class="control-label">Nomine Details</label>  
    <p class="text-muted text-show"><?php echo ($nomine_details != '')?$nomine_details:'NIL'; ?></p>
     </div>
    <div class="col-md-4">  
    <label class="control-label">Date of Death</label>  
    
    <p class="text-muted text-show">
    <?php if($employee_type=='1993') {?>  
      <?php echo ($death_date=='0001-01-01') ? 'NIL' : dateshow($death_date) ?>
    <?php } ?>  
        
    </p>
     </div> 
    <div class="col-md-4">  
    <label class="control-label">Premature retirement date</label>  
    ...
    $Qulification = array();
    foreach($arr_desig as $key){
      $Qulification[$key['code']] = $key['description'];
    }
    $relationship = array();
    foreach($arr_relation as $key){ 
      $relationship[$key['code']] = $key['description'];  
     } 
   // print_r($Qulification); exit;
	?>
    ...
    <table id="tbl_family" class="table table-striped table-bordered dataTable no-footer">
    <thead>  
    <td>Name</td>
    <td>Age</td>
    <td>Qualification</td>
    ...
    <?php if(count($data_relastion)){ 
      foreach($data_relastion as $item){?>
    <tr id="tr0">
    <td>
    <?php echo $item['family_name'];  ?>
    </td>
    
    
    <td>
    <?php echo $item['family_age']?>
    </td>
    
    
    <td>
    
        <?php echo $Qulification[$item['family_quali']] ?>

    </td>
    
   
    <td>
      <?php echo $relationship[$item['family_relation']]; ?>
    </td>
    
    <td>
      <?php echo ($item['family_dependent']=='1')?'YES':'NO'; ?> 
    </td>
    </tr>
    <?php  } ?>
    ...
    <label for="inputPassword3" class="col-sm-3 control-label">Name Of the Applicant<span class="star_color">*</span>
    <small>Note: Age must be greater then 18+ is only applicable.</small>
    </label>
    ...
    <?
    $db = new database();
    $arr = $db->fetch_table("select family_name from intra_pri_cg_relation_master_submit where family_age>='18' where employee_id='".$emp_id_const."' order by family_name");
    ?>
...
<?php }?>
 ...
 if($data[0]['status']=='1'){ ?>
    ...
    <a href="<?= $config['base_url']?>page/intra_pri/pdf_cg.php?emp_id_const=<?php echo $crypto->encode($emp_id_const,4) ?>" target="_blank"><img src="<?= $config['base_url'] ?>themes/default/image/pdf_download.png"/></a>
    </div>
    </div>
<?php }?>
