    ...
    <div class="row mb-3" >
        ...
        <?php 
										$transfer_level=$db->fetch_table("SELECT code, description, code_master_id_pk
																		...
																		");
                                     ?>
        	<select name="stake_lvl_select" style="width:100%;" id="stake_lvl_select" class="form-control" onChange="gp_ps(this.value);">
 ...
 ?>
  ...
  $arr = $db->fetch_table("select ps_id_pk,ps_name,ps_code,district_id_fk from prd_location_master_panchayat_samiti where ps_id_pk='".$ps_code."'");?>
                            	<option value="">-Please Select-</option>
                                <? foreach($arr as $key){ $key['ps_id_pk']. '<br />'; ?>
                               <option value="<?= $crypto->encode($key['ps_id_pk'],4); ?>" <? if($ps_code==$key['ps_id_pk']){ echo "selected";}?> ><?= $key['ps_name']; ?></option>
...
<? } ?>
    ...
    <!--<input type="text"  id="district_id_fk" name="district_id_fk" value="<?php //echo $district_id_fk; ?>" />-->
    </div>  
    </div>
...
<? } ?>
	...
	<label for="inputPassword3" class="col-sm-3 col-form-label">BRIF NOTE<span class="star_color">*</span></label>
	<div class="col-sm-3">
	<textarea type="text" class="form-control upper_case" name="note" id="note" placeholder="BRIF NOTE" rows="5" cols="40" ></textarea>
	</div>
	...
	<label for="inputPassword3" class="col-sm-3 col-form-label">BRIF NOTE<span class="star_color">*</span></label>
	<div class="col-sm-3">
	<textarea type="text" class="form-control upper_case" name="note" id="note" placeholder="BRIF NOTE" rows="5" cols="40" ><?php echo $data[0]['note'];?></textarea>
	</div>
  ...
  <?php if(count($arr_file_1)==1){?>
	...
	function gp_ps(val)
	{
		...
		if(val=='555')
		{
			$.post('<?= $config['base_url'] ?>page/intra_pri/ajax_ps_block_fetch.php?stake='+val, function(data){
...
// alert(8888);
  ...
  $.ajax({
      url : 'ajax_preview_tran_form.php',
      type : 'POST',
...
.modal-backdrop fade in{
 ...
 <!---------------------------------------Final submit modal start--------------------------->
    ...
    <div class="modal-body"> 
    <form action="intra_pri_district__transfer_profile_submit.php" method="post">
    <p class="alert alert-warning"><strong><i class="fa fa-exclamation-triangle"></i> Are You Sure To Submit Employee INTRA DISTRICT TRANSFER APPLICATION Profile ?</strong></p>
    </div>
    <div class="modal-footer">
