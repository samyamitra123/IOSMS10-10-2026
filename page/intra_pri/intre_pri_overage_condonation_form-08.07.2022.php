...
<form name="myForm" id="myForm" method="post" class="w3_form_post" action="ajax_intra_pri_overage_condonation_submit.php#scroll_div" onsubmit="return validateForm()" enctype="multipart/form-data" >
...
<div class="row mb-3">
  ...
  <div class="row mb-3" >
	...
	<?php 
      ...
      <input type="file" class="form-control " autocomplete="off" name="Case_Details"  id="Case_Details" value="" onchange="return file_upload(this.id, 26);">
		<?php } 
		else{ ?>
    ...
    <div class="col-sm-1"><a id="del"  onclick="del('<?php echo $crypto->encode("26",4); ?>','<?php echo $crypto->encode("delete",4); ?>');" name='del' class='del' style="color:red">Remove File</a></div>
    </div>
    <div class="row mb-3">
	<div class="col-sm-5"></div>
    <div class="col-sm-offset-5 col-sm-7">
      ...
      <span style="color:#838683;font-weight:bold;">File Size must not be 0 and maximum file size should be 1 MB.</span>
		...
		<div class="row mb-3" id="scroll_div" style="margin-left: 41%;">
			...
			<?php if($emp_id_detail_ocon[0]['application_id'] !=''){
				...
				?>
					<button type="button" class="btn btn-warning" style='width: 25%; margin-left: -10%;' > <span class="spinner-grow spinner-grow-sm"></span> UPDATE </button>
				<?php } ?>
				<button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#gpprofModal" style='width: auto; margin-left: 2%;' onclick="return sub_prev(this.id)"> <span class="spinner-grow spinner-grow-sm"></span> PREVIEW & FORWARD </button>
			<?php }
			else{			?>
