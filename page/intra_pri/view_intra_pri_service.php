...
.menu {margin: 0 auto; width: 440px;}
...
<?php require 'common_back_btns_intra_pri.php'; ?>
   <div class="welcome_msg">
		...
		$Query = " SELECT forw.*, role.description as role_desc, forw.status as status, forw.for_app_rej as for_app_rej,
											...
											WHERE (forw.status = 1) AND forw.delete=0 AND forw.from_officer_id_const = '".$_SESSION['user_info']['officer_id_const']."' order by forwarding_id_pk asc"; 
	  ...
	  array_multisort($appDate,SORT_ASC,$Applications);
...
<ul class="menu">
	<li><a href="javascript:void(0);" class="btn btn-sm btn-primary active">Inbox</a></li>
	<li><a href="view_intra_pri_service_other.php" class="btn btn-sm btn-primary">Ongoing</a></li>
	<li><a href="approved_intra_pri_service.php" class="btn btn-sm btn-primary">Approved</a></li>
	<li><a href="rejected_intra_pri_service.php" class="btn btn-sm btn-primary">Rejected</a></li>
</ul>
</br>
</br>
<?php 
...
if($value['status'] == 2) {
			...
			if($officer_name_pendingTemp['for_app_rej'] == 'A')
			{
				continue;
			}
}
...
<tr>
   ...
   // echo $value['status'];
	?>		
  <p><small>dated:<?php echo date('dS M,Y',strtotime($value['submitted_on']));?></small></p>
	</td>
	<td>
		<?php if($value['status'] == 1){ echo "Pending in your end."; } else if($value['status'] == 2) { 
