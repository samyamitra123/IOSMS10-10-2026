...
<?php
	...
	if(!isset($_POST['changepassword_submit'])){
		header('Location: '. $config['base_url'] . "page/intra_pri/change_password_intra_pri.php");
		exit;
	...
	if($sec_time_token!=$enc_session){
		...
		header('location:change_password_form_intra_pri_user.php');
		exit(0);
		}else{
			...
			if($conf_pass==$new_pass)
			{
				//echo 12222; die;
					...
					$update= $db->update("UPDATE intra_pri_master
									 ...
									 user_pin='".$new_pass."',
									 active_status = '1'
									 WHERE officer_id_const='".$_SESSION['user_info']['officer_id_const']."'
									 AND active_status='1'
								   ");
						if($update==true)
						{
