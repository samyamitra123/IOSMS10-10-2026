...
$appId=(isset($_POST['appId']))?$crypto->decode($_POST['appId'],4):'';  
	 if($delete_id=="delete")
			{
				$query=$db->update("UPDATE intra_pri_file_upload SET 
			status='0'
			WHERE 
			emp_id_const='".$empId."' and flag='".$delete_f."' and application_id='".$appId."' and status='1'");
				
				if($query==true)
				{
					$_SESSION["msg"]='<div class="alert alert-success" style="text-align:center;"><strong>DOCUMENT SUCCESSFULLY REMOVED </strong></div>';
					header('Location:edit_district_transfer.php?empId='.$crypto->encode($empId,4));
				}
				else
				{
