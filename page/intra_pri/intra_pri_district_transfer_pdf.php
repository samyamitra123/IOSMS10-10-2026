...
$officer_desig =$db->fetch_table(" SELECT * FROM intra_pri_designation_master WHERE designation_code = '".$_SESSION['user_info']['stake_level_code']."' ");
	   ...
	   if($pdf_fetch_DT[0]['memo_no']=='')
	   {
		  // echo 1222; die;
		   $memo_no=rand(000,999).'/'.date('Y').'/'.'Intra District Transfer';
		   $memo_insert=$db->update("UPDATE intra_pri_district_transfer set
								memo_no='".$memo_no."'
								where application_id = '".$id."'
								");
		$pdf_memo =$db->fetch_table(" SELECT memo_no FROM intra_pri_district_transfer WHERE application_id = '".$id."' and status='2' and district_level='ID' ");
										
	   
	   
	   $memo=$pdf_memo[0]['memo_no'];
	   }
	   else
	   {
		  // echo 222; die;
		  $memo=$pdf_fetch_DT[0]['memo_no'];
	   }
...
<tr>
<td width="370" class="layout">Order: Intra Dstrict Transfer</td>
</tr>
</table>
...
$mpdf->WriteHTML('<li>The Treasury Officers,_______________________________ Treasury, '.fun_district($pdf_fetch_DT[0]['pre_district_id_fk']).'&nbsp;'.'</li>',2);
$db=new database();
