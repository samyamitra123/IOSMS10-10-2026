...
font-weight: bold;
	...
	stack_user = '".$user."' and district_level='".$stake_level_district."' and status in('0','1','2','3','4','5','6') order by emp_first_name");
	   }
	   else
	   {
		     $prv='2';
		   $db=new database();
	$data = $db->fetch_table(" SELECT * FROM intra_pri_district_transfer WHERE from_status in('2') and
	 application_id='".$app_no."' and status in('0','1','2','3','4','5','6') order by emp_first_name");
	   }
    ...
    <?php if($user_stack=="DM" || $user_stack=="ADM"|| $user_stack=="AEO"){
		//echo 333; die;
		 ?>
    <button type="button" class="btn btn-success " id="emp_approve<?=$item['emp_id_const']?>" style="margin-left: 89%;" onclick="emp_approve_modal_all();">Approve All</button>
    <?php }?>
	...
	<td id="block"><?php if($item['transfer_block_id_fk']=='')
	{    
	$db= new database();
	...
	$arr_file_40 = $db->fetch_table("select file_name,flag from intra_pri_file_upload where emp_id_const = '".$item['emp_id_const']."' AND status in('1','2','3') AND flag= 40 "); ?>
   ...
   <a style="text-decoration:none" href="<?= $config['base_url']?>page/intra_pri/doc_download_trasfer.php?employee_id=<?= $crypto->encode($item['emp_id_const'],4)?>&flag=<?=$arr_file_40[0]['flag']?>"><?php echo substr($arr_file_40[0]['file_name'],6); ?></a>
    <?php }?>
    ...
    <td><?= $status.$officer_name_pending[0]['officer_name'] ?></td>
        ...
        <button type="button" class="btn btn-success " id="emp_approve<?=$item['emp_id_const']?>" style="display: block;" onclick="emp_approve_modal();">Approve</button>
        <button type="button" class="btn btn-danger" id="emp_reject<?=$item['emp_id_const']?>"style="display: block;" onclick="emp_reject_modal();">Reject</button>
        </div>
        </td>
...
<?php }?>
   ...
   <?php }?>
    ...
    $db=new database();
		...
		$arr_file_42 = $db->fetch_table("select file_name,flag from intra_pri_file_upload where emp_id_const = '".$item['emp_id_const']."' AND status = '1' AND flag= 42 "); ?>
   ...
   <a style="text-decoration:none" href="<?= $config['base_url']?>page/intra_pri/doc_download_trasfer.php?employee_id=<?= $crypto->encode($item['emp_id_const'],4)?>&flag=<?=$arr_file_42[0]['flag']?>"><?php echo substr($arr_file_42[0]['file_name'],6); ?></a>
    <?php }}?>
    </td>
   ...
   <a style="text-decoration:none" href="<?= $config['base_url']?>page/intra_pri/doc_download_trasfer.php?employee_id=<?= $crypto->encode($item['emp_id_const'],4)?>&flag=<?=$arr_file_43[0]['flag']?>"><?php echo substr($arr_file_43[0]['file_name'],6); ?></a>
    <?php }}?>
    </td>
    <?php
  ...
  <button type="button" class="btn btn-warning " id="emp_approve<?=$item['emp_id_const']?>" style="display: block;" onclick="emp_f_modal();">FORWARD</button>
   <button type="button" class="btn btn-success "  disabled="disabled"id="emp_approve<?=$item['emp_id_const']?>" style="display: block;" onclick="emp_approve_modal();">Approve</button>
     <button type="button" class="btn btn-danger"  disabled="disabled" id="emp_reject<?=$item['emp_id_const']?>"style="display: block;" onclick="emp_reject_modal();">Reject</button>
</div>
	</td>
    <?php }
	else if($item['status']=='2'){?>
  ...
  <button type="button" class="btn btn-success "   disabled="disabled" style="display: block;" >Approve</button>
    ...
    <div id="forward_div" style="display:none;">		
		...
		<div class="col-mb-3" >
		<input type="hidden" id="application_id_o" name="application_id_o" value="<?php echo $a_id; ?>" >
...
function may_be_rej_send(k){
    ...
    <input type="button"  value="YES" onclick="emp_approve('<?=$crypto->encode($item['emp_id_const'],4);?>','<?=$crypto->encode("approve",4)?>');" class="btn btn-success finalize" />
    ...
    <input type="button" value="YES"  onclick="emp_reject('<?=$crypto->encode($item['emp_id_const'],4);?>','<?=$crypto->encode("reject",4)?>');" class="btn btn-success finalize" />
    <button type="button" class="btn btn-warning" data-bs-dismiss="modal">NO</button>      
    </div>
    </div>
    </div>
