...
$db=new database();
 $application_id_check=$db->fetch_table("SELECT application_id FROM intra_pri_district_transfer where stack_user='".$user."' and from_status='2' and district_level='".$stack_final."' and status='0'");
 
 if(count($application_id_check)!='0')
		{
...
application_id='".$application_id."'
 ...
 WHERE stack_user='".$user."' and from_status='2' and status='0' and district_level='".$stack_final."'");
 ...
 if($update_oc_final){
        ...
        $insert_request_forwarding = $db->insert("INSERT INTO intra_pri_forwarding(application_id, from_officer_id_const, status, service_type, emp_id_const, sub_menu,forward_head)
            VALUES ('".$application_id."', '".$_SESSION['user_info']['officer_id_const']."','1','".$s."', '".$statement."','".$s."','1')");
    }
 ...
 if($insert_request_forwarding)
			...
			$status='1';
			echo json_encode(array($status, $application_id));
			}
			else
			{
			echo "0";
