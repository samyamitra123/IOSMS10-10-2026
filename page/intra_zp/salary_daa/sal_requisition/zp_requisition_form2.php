...
function getAmount($dise,$emp_id_pk,$type,$basic)
{
	...
	if($type=='other_loan_deduction')
	{
		$arr = $db->fetch_table("select other_loan_deduction from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND zp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'"); 
...
if(count($tch_final) == 0 )
{
	...
	$tch= $db->fetch_table("
							...
							AND tch.emp_status in('1','9') order by tch.emp_first_name
							");
        ...
        if(count($tch))
		{
			...
			foreach ($tch as $key) 
			{
				...
				$sal_chk = $db->fetch_table("SELECT 
												...
												save.interim_relief,save.hra_deduction,save.zp_id_fk,save.total_loan_deduction, save.other_loan_deduction
...
//					}
					...
					{ 
					   echo 13;
						...
						//echo $gross_salary.'--'.$total_deduct;
					...
					else
					{
						...
						*/
						...
						$overdrawn = getAmount($dise,$emp_id_pk,'ovd','');
						$festival_loan = getAmount($dise,$emp_id_pk,'festival_loan','');  
						$total_deduct = $gpf+$pfl+$cpf_deduct+$ptax+$itax+$gsli+$hra_deduction+$festival_loan+$other_loan_deduction;
						$net_salary = $gross_salary-$total_deduct;
						...
						$total_deduct = $gpf+$pfl+$cpf_deduct+$ptax+$itax+$gsli+$overdrawn+$reduct1+$reduct2+$reduct3+$hra_deduction+$festival_loan+$other_loan_deduction;
						$net_salary = $gross_salary-$total_deduct;
					}
				}
				...
				<tr>
                    ...
                    <td class="loan">
						...
						{ ?>
                            <a data-toggle="modal" data-target=".bs-example-modal-lg-loan" href="" id="<?php echo $crypto->encode($key['emp_id_pk'],4) ?> ">
	...
	<div class="form-group" id="sal_save" style="display:none;">
        <div class="col-sm-offset-5 col-sm-7" style="margin-top:1%">
        	<a href="<?= $config['base_url'] ?>page/intra_zp/salary_daa/sal_requisition/ajax_zp_rq_finalize.php" class="btn btn-success btn-sm">Salary Finalize</a>
        </div>
	</div>
...
{  ?>
	<script>
        ...
        window.setTimeout(function(){
            window.location.replace("<?php echo $config['base_url'] . 'page/intra_zp/salary_daa/sal_requisition/view_zp_sal_requisition.php'; ?>");
        },3000);
...
<script>
	$(document).ready(function()
	{
		...
		{	
			...
			var link1= $("#zp_id").val();
			$.post('<?= $config['base_url'] ?>page/intra_zp/salary_daa/sal_requisition/ajax_requisition_edit1.php?id='+link+'&zp_id='+link1, function(data)
			{                    
				$("#mbody").html(data);
			});
		});
		...
		$(".save a").click(function() 
		{
			...
			$.post('<?= $config['base_url'] ?>page/intra_zp/salary_daa/sal_requisition/ajax_zp_rq_save.php?emp_id_pk='+emp_id_pk+'&sec_tok='+sec_tok, function(data)
			{
		...
		{	
			$.post('<?= $config['base_url'] ?>page/intra_zp/salary_daa/sal_requisition/ajax_zp_rq_finalize.php', function(data){
			//$("#mbody").html(data);
			});
		});
		$("#example").DataTable({		
