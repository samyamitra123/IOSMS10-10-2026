...
else
{
	...
	//echo $total_loan_deduction;exit;
	...
	$p_tax = $_POST['p_tax'];//200
	$i_tax = $_POST['i_tax'];//0
	$gsli = $_POST['gsli'];//0
	$other_loan_deduction = $_POST['other_loan_deduction'];//500
	$is_overdrawn = $_POST['is_overdrawn'];
	$overdrawn = $_POST['overdrawn'];//0
	...
	{       
		$hra_deduction= $_POST['hra_deduc'];//2360
		if($hra_deduction>0)
		{
			$hra_loan=$_POST['n7'];
		}
	...
	$total_allowance=$consolidated_pay+$pay_in_band+$da+$hra+$ma+$conv_allow+$hill_allow+$interim_relief+$allow;
	$total_deductions=$gpf+$pf_loan+$p_tax+$i_tax+$gsli+$overdrawn+$festival_val+$hra_deduction+$total_loan_deduction+$other_loan_deduction; 

						//8000+""+200+0+0+0+0+2360+0+500
	...
	if($validator->blank_select($emp_id_pk) == FALSE || $validator->pattern_number($crypto->decode($emp_id_pk,4)) == FALSE)
	{
		...
		header('location:zp_emp_sal_requisition_ropa_2019.php'.$query_string);
		exit(0);
	}
	...
	else if($allow != 0 && $zp_emp_type == 367){
		$_SESSION['msg']= '<div class="alert alert-danger" style="text-align:center"><strong>Employee not elegible.<strong></div>';
		header('location:zp_emp_sal_requisition_ropa_2019.php'.$query_string);
		exit(0);
	}
	
	else if($allow != 0  && $zp_emp_type == 366)
	{
		if($allow > 1200){
			$_SESSION['msg']= '<div class="alert alert-danger" style="text-align:center"><strong>Please enter valid Allowance.<strong></div>';
			header('location:zp_emp_sal_requisition_ropa_2019.php'.$query_string);
			exit(0);
		}
	}
	
	/*else if(($allow != 1200 || $validator->pattern_number($allow) == FALSE))
	{
		$_SESSION['msg']= '<div class="alert alert-danger" style="text-align:center"><strong>Please enter valid Allowance.<strong></div>';
		header('location:zp_emp_sal_requisition_ropa_2019.php'.$query_string);
		exit(0);
	...
	else if(($validator->blank_select($other_loan_deduction) == FALSE || $validator->pattern_number($other_loan_deduction) == FALSE))
	{
		$_SESSION['msg']= '<div class="alert alert-danger" style="text-align:center"><strong>Out of Account Deduction amount is not valid.<strong></div>';
		header('location:zp_emp_sal_requisition_ropa_2019.php'.$query_string);
		exit(0);
	}
	...
	//var_dump($festival_val); die;
	if($salary_type=='8' && $festival_val!='0')
	{
	...
	//echo 	$total_allowance."</br>";exit;//63940
	//echo 	$gross;"</br>";exit;//63940

	//echo 	$total_deductions;"</br>";exit;//11060
	//echo 	$net;"</br>";exit;//55240
	...
	$hra_details_fetch=$db->fetch_table(" SELECT emp_gvt_hra_type,emp_gvt_hra_ammount FROM prd_employee_master WHERE emp_id_pk='".$crypto->decode($emp_id_pk,4)."' AND emp_spouse_res='251' AND zp_id_fk='".$_SESSION['location']['district_id']."' 
	AND (ropa_status = '1' OR consolidated_status='1')");
	...
	{
		$_SESSION['msg']= '<div class="alert alert-danger" style="text-align:center"><strong>HRA Deduction/License Fee should be equal to HRA<strong></div>';
		header('location:zp_emp_sal_requisition_ropa_2019.php'.$query_string);
		exit(0);
	}
	else if($hra_schm=='202' && $hra_deduction!=0)
	{
		$_SESSION['msg']= '<div class="alert alert-danger" style="text-align:center"><strong>HRA Deduction/License Fee should be equal to 0<strong></div>';
		header('location:zp_emp_sal_requisition_ropa_2019.php'.$query_string);
		exit(0);
	}
	...
	{ 
		...
		$upd = $db->update("UPDATE prd_employee_salary_save
							SET status_flag='0', delete_status='0',is_saved=0
							WHERE emp_id_fk='".$crypto->decode($emp_id_pk,4)."' AND zp_id_fk='".$_SESSION['location']['district_id']."' 
							AND salary_monthyear='".date('Ym')."' AND requisition_type='".$requisition_type."' AND ropa_status='1'");	
		
		
		$ins = $db->insert("INSERT INTO prd_employee_salary_save(slno,latestupdate_time, latestupdate_ip_address, zp_id_fk, empcd, bankname, accountno, basic, da, hra, ma, cpf, pf_loan, p_tax, 
		...
		hra_deduction,hra_loan_cause,other_deduction_cause,zp_emp_type,emp_desig,other_loan_deduction, ropa_level, ropa_status,allowance
		)
		VALUES ('0', 'now()', '$user_ip', '".$_SESSION['location']['district_id']."', '0', 
		'$bankname', '$accountno', '$basic', '$da', '$hra', '$ma', '0', '0', '$p_tax', 
		'$i_tax', '$net', '$bank_ifsc', 'NA', '0', '$p_tax', '0', '0', '1', '$salary_monthyear', 
		'1', '0', '".$crypto->decode($emp_id_pk,4)."', '$pay_in_band', '0', '$hill_allow','$gpf', '0', '$gross', '0',
		'$conv_allow','$overdrawn','$salary_type','$cause_msg','".$_SESSION['user_info']['stake_user']."',
		'$part_day','$gsli','1','$consolidated_pay','$festival_val','$festival_loan_cause','$part_salary_cause_msg',
		'$no_salary_cause_msg','$interim_relief','".$requisition_type."','0','".$total_loan_deduction."','".$hra_deduction."','".$hra_loan."','".$other_deduction_cause."','".$zp_emp_type."','".$emp_desig."','".$other_loan_deduction."', '$grade_pay', '$ropa_status','$allow')");	
	...
	{ 
		pg_query("begin");
		$Query = "INSERT INTO prd_employee_salary_save(slno,latestupdate_time, latestupdate_ip_address, zp_id_fk, empcd, bankname, accountno, basic, da, hra, ma, cpf, pf_loan, p_tax, 
		i_tax, net, bank_ifsc, sal_source, spl_pay, pf_deduct, code, spl_alo, status_flag, salary_monthyear, category_id, block_code, emp_id_fk, pay_payband, tch_grade_pay, hill_allowance, gpf, cpf_deduct, gross_salary, is_saved,conv_allow,
		overdrawn,salary_type,cause,gp_code,part_day,gsli,delete_status,
		consolidated_pay,festival_loan,festival_loan_cause,part_salary_cause,
		no_salary_cause,interim_relief,requisition_type,gp_id_fk,total_loan_deduction,
		hra_deduction,hra_loan_cause,other_deduction_cause,zp_emp_type,emp_desig,other_loan_deduction, ropa_level, ropa_status,allowance
		...
		'".$other_deduction_cause."','".$zp_emp_type."','".$emp_desig."','".$other_loan_deduction."', '$grade_pay', '$ropa_status', '$allow')";
		//print_r($Query); exit;
		$ins = $db->insert($Query);
