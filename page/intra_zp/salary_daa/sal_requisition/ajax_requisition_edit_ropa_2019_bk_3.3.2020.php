...
if($type=='gpf')
{
$arr = $db->fetch_table("select gpf from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND zp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='1'");
if($arr[0]['gpf'])
{
return $arr[0]['gpf'];
}
else
{
	...
	$arr_emp = $db->fetch_table("select ropa_9_emp_pay_in_payband,emp_grade_pay from prd_employee_master where ropa_status=1  AND emp_id_pk='".$emp_id_pk."' AND zp_id_fk='".$dise."'  AND ropa_status='1'");
...
<input maxlength="5" style='background-color: #EEE;' type="hidden" id="pay_in_band_2009" name="pay_in_band_2009" value="<?php echo  $tch[0]['ropa_9_emp_pay_in_payband']; ?>" readonly size="5" />
