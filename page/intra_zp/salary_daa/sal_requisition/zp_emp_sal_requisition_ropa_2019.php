	...
	if($config['appointment'] == 1)
	{
		foreach ($id_const as $const) {
			if(strlen($const['emp_first_memo_no']) <=1 || strlen($const['emp_present_memo_no']) <=1 || $const['recruitment_type'] == '0')
				$appMemo[] = $const['emp_first_name'].' '.$const['emp_second_name'].' '.$const['emp_last_name'].'  '.'['.'Employee Id='.$const['emp_id_const'].']'.'      ';
		}
  }  
...
$('#sess_msg').hide();
	...
	function gpfCal(){
			...
			//if($('#cpf').val() == 0){
				...
				//alert(pay_band); alert(grade_pay);
	...
	function allowCal(){ 
		...
		var consolidated_pay =$('#consolidated_pay').val();
		var pay_in_band =$('#pay_in_band').val();
		var da =$('#da').val();
		var hra =$('#hra').val();
		var ma =$('#ma').val();
		var allow =$('#allow').val();
		
		var total_gross= parseInt(consolidated_pay)+parseInt(pay_in_band)+parseInt(da)+parseInt(hra)+parseInt(ma)+parseInt(allow);
		...
		var total_net = parseInt(total_gross)-parseInt(deduct);
		//alert(deduct);
		$('#deduct').val(deduct);
		//$('#net').val(total_deduct);
