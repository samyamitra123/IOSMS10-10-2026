...
$hra_details_fetch=$db->fetch_table(" SELECT emp_gvt_hra_type,emp_gvt_hra_ammount FROM prd_employee_master WHERE emp_id_pk='".$emp_id_pk."' AND emp_spouse_res='251' AND zp_id_fk='".$_SESSION['location']['district_id']."' AND (ropa_status = '1' OR consolidated_status='1') ");
...
else
{
	...
	else
	{
		...
		if($arr[0]['net'])
		{
			...
			$upd = $db->update("UPDATE prd_employee_salary_save 
				...
				AND emp_id_fk='$emp_id_pk' AND ropa_status='1'
	...
	$ins = $db->insert("INSERT INTO prd_employee_salary_save
				...
				ropa_level, ropa_status, allowance)
	...
	'".$crypto->decode($_SESSION['ropa_level'.$emp_id_pk],4)."', '1', '".$crypto->decode($_SESSION['allow'.$emp_id_pk],4)."')");
			...
			if($ins)
			{
				...
				unset($_SESSION['allow'.$emp_id_pk]);
	...
	$sal_emp_count = $db->fetch_table("SELECT count(emp_id_pk) as emp_cnt 
							...
							WHERE
							prd_emp.emp_status in (1) and gp.district_id_pk='".$_SESSION['location']['district_id']."'
