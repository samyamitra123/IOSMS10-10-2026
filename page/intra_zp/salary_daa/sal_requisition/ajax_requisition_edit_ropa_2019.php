...
$arr_gp_val = $db->fetch_table("select interim_relief,emp_pay_in_payband,emp_grade_pay,grade_amount from prd_employee_master as emp INNER JOIN prd_dise_gradepay_master as gd 
ON trim(emp.emp_grade_pay)=gd.grade_code
where emp_id_pk='".$emp_id_pk."' AND emp.ropa_status='1'");         //Checked   //interim_relief, emp_pay_in_payband has been added by 
...
$arr = $db->fetch_table("select cpf from prd_employee_salary_save where (status_flag=1 or status_flag=2 or status_flag=3) AND delete_status=1  AND emp_id_fk='$emp_id_pkk' AND salary_monthyear='".date('Ym')."' AND zp_id_fk='".$dise."' AND ropa_status='1' ");
...
//echo ($type == 'gpf')?'GPF':'NO GPF';

if($type=='gpf')
{
	
$arr = $db->fetch_table("select gpf from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND zp_id_fk='".$dise."' AND ropa_status='1'");
...
else
{
	...
	$arr_emp = $db->fetch_table("select ropa_9_emp_pay_in_payband,emp_grade_pay, emp_pay_in_payband from prd_employee_master where ropa_status=1  AND emp_id_pk='".$emp_id_pk."' AND zp_id_fk='".$dise."'  AND ropa_status='1'");
...
$arr = $db->fetch_table("select i_tax from prd_employee_salary_save where (status_flag=1 or status_flag=2 or status_flag=3) AND delete_status=1  AND emp_id_fk='$emp_id_pkk' AND salary_monthyear='".date('Ym')."' AND zp_id_fk='".$dise."' AND ropa_status='1'");
...
$arr = $db->fetch_table("select overdrawn from prd_employee_salary_save where (status_flag=1 or status_flag=2 or status_flag=3) AND delete_status=1  AND emp_id_fk='$emp_id_pkk' AND salary_monthyear='".date('Ym')."' AND zp_id_fk='".$dise."' AND ropa_status='1'");
...
$arrear_amount = $db->fetch_table("SELECT remaning_amt FROM prd_arrear_calculation WHERE emp_id_fk='".$emp_id_pkk."' 																												
AND status in ('1')
...
$arr = $db->fetch_table("select conv_allow from prd_employee_salary_save where (status_flag=1 or status_flag=2 or status_flag=3) AND delete_status=1  AND emp_id_fk='$emp_id_pkk' AND salary_monthyear='".date('Ym')."' AND zp_id_fk='".$dise."' AND ropa_status='1' ");
if($arr[0]['conv_allow']){
...
function festival_adv_recovery($emp_id_pk)
{
	// for festival advance by nd on 04072017	
		//die;
	$db = new database();
	/*$arr = $db->fetch_table("select festival_loan from prd_employee_salary_save where status_flag=1 AND delete_status=1 AND salary_monthyear='".date('Ym')."' AND emp_id_fk = '".$emp_id_pk."'"); 
...
overdrawn, salary_type, cause,gp_code, part_day,gsli,consolidated_pay,festival_loan,festival_loan_cause,total_loan_deduction,hra_loan_cause,other_loan_deduction,allowance from prd_employee_salary_save
where 
(status_flag=1 or status_flag=2 or status_flag=3) AND delete_status=1 AND salary_monthyear='".date('Ym')."'  
AND emp_id_fk='$emp_id_pk' AND zp_id_fk='".$dise."' AND ropa_status='1'");

 

//---------- Start Gpf=0 before retirement--------------------
$retirement_date=$tch[0]['emp_retirement_date'];
if($tch[0]['zp_emp_type']=='366')
{
...
if($sal_chk[0]['zp_id_fk'] && $sal_chk[0]['emp_id_fk'] && ($sal_chk[0]['salary_type']=='1' || $sal_chk[0]['salary_type']=='2'))
{
    ...
    {	
	    ...
	    $full_gross_salary = round($full_basic+$full_da+$full_interim_relief+$full_hra+$full_ma+$full_conveyance_allowance+$full_hill_allowance+$allow);

    }
    else
    {

    //echo "I am not in Des";
    ...
    foreach ($tch as $key) 
    {
		...
		if(count($promotion_data_cal) > 0){
			...
			if($emp_effective_day_for_promotion_cal != 01 && $emp_id_for_promotion_part != "" && $dop_doi == 3 && $promotion_eff_yr_mnth == date("Ym"))
			{
				...
				$full_da = round(($full_basic/100)*$da_per);//04.12.2020
			...
			else
			{
				...
				$full_da = ($full_basic_sus/100)*$da_per;//04.12.2020//
				$full_interim_relief =0;
	...
	$full_da = round(($full_basic/100)*$da_per); */   
		...
		$consolidated_pay=0;
			...
			//echo "HRA";
			$full_hra = 0;
			}else if($tch[0]['emp_spouse_hra'] < 12000){ //spouse HRA < 6000
			$hra_emp = ($full_basic/100)*$hra_per;
			//echo $hra_emp;
			if($hra_emp >= 12000){
	...
	//$full_hra=$hra;
			...
			if($tch[0]['emp_diff_able']=='1' && $tch[0]['zp_emp_type']=='367')
			{
			...
			else if($tch[0]['emp_diff_able']=='1' && $tch[0]['zp_emp_type']=='366')
			{
			if($sal_save[0]['conv_allow']!=''){
				$conveyance_allowance = $sal_save[0]['conv_allow'];	
			}
		...
		if(($state10=='3219' || $state10=='3299')&& $tch[0]['zp_emp_type']=='366')
		{									
			$hill_p = ($full_pay_band/100)*12;
			//$hill_g = ($full_grade_pay/100)*$hill_allowance_per;
			$full_hill_allowance_amt = $hill_p;
		...
		else if(($state10=='3219' || $state10=='3299')&& $tch[0]['zp_emp_type']=='367')
		{									
	...
	//******************GPF  *************
		...
		else
		{
			if($sal_chk[0]['gpf']!=0)
			{
				...
				$full_gpf = getAmount($dise,$emp_id_pk,'gpf',$basic);
				}
		}
	...
	//****************** P TAX  *************
		if($key['emp_diff_able']=='1')
		{
			...
			if(strtotime($suspend_effect_date)<=strtotime(date('Y-m-d')) && strtotime($suspend_effect_date)>0){
				$full_gross_salary_sus =round($full_pay_in_band_sus+$full_da_sus+$interim_relief+$hra+$ma+$conveyance_allowance+$allow);
	...
	//****************** I TAX  *************
		if($sal_chk[0]['i_tax'])
		{
			...
			$full_itax = getAmount($dise,$emp_id_pk,'itax','').'';
		}
...
else if($sal_chk[0]['salary_type']=='8')
{
   ...
   if($tch[0]['spouse_medical_allowance']=='1'){
    ...
    if(($state10=='3219' || $state10=='3299')&&  $tch[0]['zp_emp_type']=='367'){								
	...
	else  if(($state10=='3219' || $state10=='3299')&&  $tch[0]['zp_emp_type']=='366'){								
		$hill_p = ($pay_in_band/100)*12;
		$hill_g = ($pay_in_band/100)*12;
		$hill_allowance_amt = $hill_p+$hill_g;
    ...
    $gross_salary = round($consolidated_pay+$basic+$da+$interim_relief+$hra+$ma+$conveyance_allowance+$cpf+$hill_allowance+$allow);
    //Deduction part
    //$gpf_amt = ($basic/100)*6;
...
{
  //echo "Test1"; //Subikar Working Here
...
$pfl = 0;
$cpf_deduct = 0;
$gross_salary = round($bas+$da+$interim_relief+$hra+$ma+$conveyance_allowance+$cpf+$hill_allowance+$allow);
if($tch[0]['emp_diff_able']=='1')
{
	$ptax = 0;
}
...
//$pay_in_band = $tch[0]['emp_pay_in_payband'];
	...
	$basic = $pay_in_band;
	$da = round(($basic/100)*$da_per);//04.12.2020//
		
		//$interim_relief = $tch[0]['interim_relief']28.12.2018;
...
if($tch[0]['emp_diff_able']=='1')
{
			if($tch[0]['conv_allow_status']==1 && $tch[0]['zp_emp_type']=='367'){
			$conveyance_allowance = $conveyance_allowance_max;
			}
			else if($tch[0]['conv_allow_status']==1 && $tch[0]['zp_emp_type']=='366')
			{
...
$gross_salary = round($pay_in_band+$da+$interim_relief+$hra+$ma+$conveyance_allowance+$hill_allowance+$allow);
//echo (strtotime($emp_first_join_match_date)>0);
if(strtotime($emp_first_join_match_date)>=strtotime(date('Y-m-d')) && strtotime($emp_first_join_match_date)>0)
{
	
$gpf=0;
}	
//else if(strtotime($date)<strtotime(date('Y-m-d')) && strtotime($date)>0) Done changes on 30.05.2024 for North 24 Employee becuase employee GPF not dedcuting when retirement date still left 7 month
else if(strtotime($date)<strtotime(date('Y-m-d')) && strtotime($date)>0)
{ 
...
<form id="form" method="post" action="ajax_zp_rq_submit_ropa_2019.php" enctype="multipart/form-data" onsubmit="return validate_sal_req();">
...
<tr>
	...
	<td>
		...
		where status_flag=1 AND delete_status=1  AND emp_id_fk='".$tch[0]['emp_id_pk']."' AND zp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='1'");
...
*/
	...
	$dedact_cause = $db->fetch_table("select festival_loan_cause,hra_loan_cause from prd_employee_salary_save where status_flag=1 AND delete_status=1 AND emp_id_fk='".$tch[0]['emp_id_pk']."' AND zp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'
	AND ropa_status='1'");
...
<td class="row_heading">BASIC PAY</td>
<td>
...
<tr>
	<td class="row_heading"> ADMINISTRATIVE ALLOWANCE</td>
	<td><input maxlength="5" type="text" id="allow" name="allow" <?php if($zp_emp_type=='366' &&($tch[0]['emp_desig']=='31' || $tch[0]['emp_desig']=='32')) {?> value="<?php echo round($allow); ?>" <?php } else {?> style='background-color: #EEE;' value='0' readonly <?php } ?> size="5" onKeyUp="return allowCal();" autocomplete="off" /></td>
</tr>
...
<td><input maxlength="6" type="text" id="i_tax" name="i_tax" value="<?php echo $itax; ?>"size="5" onKeyUp="return itaxCal();" onkeypress="return keyRestrict(event,'0123456789');" /></td>
</tr>
<tr>
...
/*echo "SELECT prd_loan_deduction.installment_amount,prd_loan_deduction.no_of_installment,prd_loan_deduction.counter, prd_loan_deduction.reminder_amount, prd_master_loan_type.loan_type,prd_loan_deduction.deduction_loan_type_variable FROM prd_loan_deduction INNER JOIN prd_master_loan_type ON prd_master_loan_type.dise_code = prd_loan_deduction.dise_code_fk WHERE prd_loan_deduction.emp_id_fk = '".$emp_id_pk."' AND prd_loan_deduction.status in (2) AND prd_loan_deduction.approval_status in (3)";exit;*/
...
<td>
	...
	<input maxlength="5" style='background-color: #EEE;' type="text" name="<?php echo $deductions['deduction_loan_type_variable']; ?>" readonly size="5" class="loan_values" id="<?php echo $deductions['deduction_loan_type_variable']; ?>" value="<?php if($sal_caus[0]['salary_type']!= '8'){
		echo $loan_amount;
	}else{
		echo 0;
	} /*THIS IS FOR NO SALARY FOR LEAVE TAKEN*/?> " size="5" autocomplete="off" onKeyUp="return reduc3(this.value);" onkeypress="return keyRestrict(event,'0123456789');" /></td>
</tr>
<?php } ?>
...
$('#allow').attr('readonly','readonly');
$('#allow').css('background-color','#EEE');
<?php } ?>
</script>
<script>
...
{ 
	//var gross = $('#gross').val();
	//var allow = $('#allow').val();
	//var total_gross = parseInt(gross)-parseInt(allow);
	//alert(total_gross);
	//$('#gross').val(total_gross);
	$('#ovd_is').show();
	$('#is_overdrawn').show();	
	$('#allow').val(0);
...
{ 
	...
	var gross = parseInt(pay_band)+parseInt(da)+parseInt(interim_relief)+parseInt(hra)+parseInt(ma)+parseInt(conv_allow)+parseInt(allow);
	$('#gross').val(gross);
	...
	if(desig=='1'){
		$('#pay_in_band,#consolidated_pay,#grade_pay,#da,#interim_relief,#hra,#ma,#conv_allow,#gross,#gpf,#pf_loan,#p_tax,#i_tax,#gsli,#allow').attr('readonly','readonly').css('background-color','#EEE;');
	}
}
if(id == '2'){
	...
	//alert($('#pay_in_band').val());
	if(desig=='1'){
		$('#pay_in_band,#consolidated_pay,#grade_pay,#da,#interim_relief,#hra,#ma,#conv_allow,#gross,#gpf,#pf_loan,#p_tax,#i_tax,#gsli,#other_loan_deduction,#allow').attr('readonly','readonly').css('background-color','#EEE;');
	
	}
}
if(id == '8'){
	//alert(11111111);
	$('#pay_in_band,#consolidated_pay,#grade_pay,#da,#interim_relief,#hra,#ma,#conv_allow,#gross,#gpf,#pf_loan,#p_tax,#i_tax,#gsli,#other_loan_deduction,#hra_deduc,#allow').attr('readonly','readonly').css('background-color','#EEE;');
...
if(id == '2'){
	...
	if(desig=='1'){
		$('#pay_in_band,#consolidated_pay,#grade_pay,#da,#interim_relief,#hra,#ma,#conv_allow,#gross,#gpf,#pf_loan,#p_tax,#i_tax,#gsli,#allow').attr('readonly','readonly')
...
if(id == '8'){
	//alert($('#reduction').val());
	//alert(42);
	$('#pay_in_band,#consolidated_pay,#grade_pay,#da,#hra,#ma,#conv_allow,#gross,#gpf,#pf_loan,#p_tax,#i_tax,#gsli,#hra_deduc,#other_loan_deduction,#allow').attr('readonly','readonly').css('background-color','#EEE;');
...
<script>
	$(document).ready(function(e1) 
	{
		...
		//alert("4645");
			$('#pay_in_band,#consolidated_pay,#grade_pay,#da,#interim_relief,#hra,#ma,#conv_allow,#gross,#gpf,#pf_loan,#p_tax,#i_tax,#gsli,#other_loan_deduction,#overdrawn,#allow').attr('readonly','readonly').css('background-color','#EEE;');
...
?>
	<script>
		...
		$('#working_days').keyup(function(e)
		{
			...
			var gross = parseInt(pay_band)+parseInt(hill_allow)+parseInt(da)+parseInt(interim_relief)+parseInt(hra)+parseInt(ma)+parseInt(conv_allow);
			//alert("1");
			...
			var gross = parseInt(pay_band)+parseInt(da)+parseInt(interim_relief)+parseInt(hra)+parseInt(ma)+parseInt(conv_allow);
			//var gross = 2200;
			//alert("2");					
			//cal_ptax(gross);
			$('#gross').val(gross);
			...
			$('#grade_pay').val('<?php echo $tch[0]['ropa_level']; ?>');
	...
	<script>
		$('#working_days').keyup(function(e) 
		{
			...
			if($('#working_days').val() >= parseInt($('#month_last_date').val()) || $('#working_days').val() == '0')
			{
				...
				var gross = parseInt(pay_band)+parseInt(da)+parseInt(interim_relief)+parseInt(hra)+parseInt(ma)+parseInt(conv_allow)+parseInt(hill_allaw);
				$('#gross').val(gross);
				//alert("4");
				var gpf = $('#gpf').val();
				var pfl = $('#pf_loan').val();
			...
			else
			{
				if(work_days=='')
				{
				  //alert(76);
					...
					//var hill_allaw=$('#hill_allow').val();
					var hill_allaw=0;
					var gross = parseInt(consolidated_pay)+parseInt(pay_band)+parseInt(da)+parseInt(interim_relief)+parseInt(hra)+parseInt(ma)+parseInt(conv_allow)+parseInt(hill_allaw)+parseInt(allow);
					$('#gross').val(gross);
					var gpf = $('#gpf').val();
				...
				else
				{
					...
					var gross = parseInt(consolidated_pay)+parseInt(pay_band)+parseInt(da)+parseInt(interim_relief)+parseInt(hra)+parseInt(ma)+parseInt(conv_allow);
					//alert(gross);
					<?php if($tch[0]['emp_diff_able']!='1'){ ?>
					//alert(444);
					cal_ptax(gross);
...
<script>
	$(document).ready(function()
	{
		...
		$("#form").submit(function()
		{
			...
			var gross = Number(pay_in_payband)+Number(da)+Number(hra)+Number(ma)+Number(conv_allow)+Number(interim_relief)+Number(emp_bonus);
			//alert(gross);
