...
$hra_details_fetch=$db->fetch_table(" SELECT emp_gvt_hra_type,emp_gvt_hra_ammount FROM prd_employee_master WHERE emp_id_pk='".$emp_id_pk."' AND emp_spouse_res='251' AND zp_id_fk='".$_SESSION['location']['district_id']."' AND ropa_status='2' ");
	...
	else
	{
		...
		if($arr[0]['net'])
		{
			...
			$upd = $db->update("UPDATE prd_employee_salary_save 
								...
								AND requisition_type='".$requisition_type."' AND salary_monthyear='".date('Ym')."' AND ropa_status='2'");
			if($upd)
			{
		...
		else
		{
			...
			//echo $crypto->decode($_SESSION['basic'.$emp_id_pk],4);die;
			...
			$ins = $db->insert("INSERT INTO prd_employee_salary_save
								...
								total_loan_deduction,hra_deduction,zp_emp_type,emp_desig,other_loan_deduction, ropa_status)
								...
								VALUES ('0', 'now()', '$user_ip', '".$zpcd."', '$empcd', 
										...
										'".$crypto->decode($_SESSION['emp_desig'.$emp_id_pk],4)."','".$crypto->decode($_SESSION['other_loan_deduction'.$emp_id_pk],4)."','2')");
