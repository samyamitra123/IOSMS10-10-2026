...
$fun_store=new zp_ps_gp_class();
 ...
 $employee_type=$crypto->decode($_GET['type'],4); 
	...
	if($monthyear==date('Ym'))
	{
       ...
       sal.overdrawn, sal.salary_type, sal.cause, sal.part_day,sal.allowance, sal.total_loan_deduction,sal.gsli,sal.consolidated_pay,sal.other_deduction,sal.cooperative_loan,sal.hbl_loan,sal.hbl_loan,festival_loan,sal.interim_relief,sal.hra_deduction,sal.other_loan_deduction,
										tch.emp_first_name,
										tch.emp_second_name,
										tch.emp_desig,
										tch.emp_last_name,
       ...
       sal.overdrawn, sal.salary_type, sal.cause, sal.part_day,sal.allowance, sal.total_loan_deduction,sal.gsli,sal.consolidated_pay,sal.other_deduction,sal.cooperative_loan,sal.hbl_loan,sal.hbl_loan,festival_loan,sal.interim_relief,sal.hra_deduction,sal.other_loan_deduction,
									...
									INNER JOIN 
										...
										WHERE
											...
											AND sal.zp_id_fk = '".$_SESSION['location']['district_id']."'
											AND sal.zp_emp_type='".$employee_type."'
											AND sal.status_flag in('2','3','4') AND is_saved='1' AND sal.delete_status='1' AND salary_monthyear='".$monthyear."'  AND requisition_type='".$requisition_type."' order by tch.emp_first_name ;
								");
	}		
...
$tot_conv_allow=$tot_payband=$tot_basic=$tot_da=$tot_hra=$tot_ma=$tot_ca=$tot_ha=$tot_cpf=$tot_gross=$tot_gpf=$tot_cpfdeduct=$tot_ptax=$tot_itax=$tot_od=$tot_gsli=$total_loan=$other_loan=$tot_net=0;
   if(count($tch)){foreach ($tch as $key) {
	   ...
	   ?>
  <tr>
    <td scope="row" class="tr_style" ><?= $cnt;?></td>
    ...
    <td>CONSOLIDATED PAY : <?= $key['consolidated_pay']?><br/>
    
    <?php if($monthyear<='201912')
{?>
    PAY IN PAY BAND : <?= $key['pay_payband']?><br/>
    <?php }else {?>
     BASIC : <?= $key['pay_payband']?>
      <?php }?>
       <?php if($monthyear<='201912')
{?>
    GRADE PAY : <?= $key['tch_grade_pay']?>
    
     <?php }?>
    <br/>DA : <?= $key['da']?><br/>HRA : <?= $key['hra']?><br/>MA : <?= $key['ma']?><br/>CONV ALLOW: <?= $key['conv_allow']?><br/>HILL ALLOW : <?= $key['hill_allowance']?> <br/>ADMINISTRATIVE ALLOW : <?= $key['allowance']?><?php if($monthyear<=201812)
{ ?><br/>INTERIM RELIEF : <?= $key['interim_relief']?><?php }?><br/><span  style="color:#CD1316;font-weight:bold;">OVERDRAWN : <?=$key['overdrawn']?></span></td>
    <td class="tr_style" ><?= $key['gross_salary']-$key['overdrawn']?></td>
    <td >GPF : <?= $key['gpf']?><br/>P-TAX : <?= $key['p_tax']?><br/>I-TAX : <?= $key['i_tax']?><br/>GSLI : <?= $key['gsli']?><br/>HRA DEDUCTION : <?= $key['hra_deduction']?><br/>FESTIVAL LOAN RECOVERY : <?= $key['festival_loan']?><br/>TOTAL LOAN DEDUCTION: <?= $key['total_loan_deduction'] ?><br/>OUT OF ACCOUNT DEDUCTION: <?= $key['other_loan_deduction'] ?></td> 
...
<tr style="text-align:center;font-weight:bold;">
	<td style="border:none;">&nbsp;</td>
	<td style="text-align:right;">TOTAL BASIC</td>
    <td><?= $tot_payband?></td>
</tr> 
<?php }?>
...
<tr style="text-align:center;font-weight:bold;">
	<td>&nbsp;</td>
	<td style="text-align:right;">TOTAL ADMINISTRATIVE ALLOWANCE</td>
    <td><?= $tot_al?></td>
</tr> 
<?php
if($monthyear<201812)
