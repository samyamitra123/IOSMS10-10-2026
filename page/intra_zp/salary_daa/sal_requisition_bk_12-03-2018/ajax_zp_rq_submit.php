...
else
{
	...
	echo $emp_id_pk=$_POST['emp_id_pk']; die;
	...
	echo "SELECT pay_payband,salary_type 
							FROM prd_employee_salary_save 
							WHERE emp_id_fk='$emp_id_pk' AND zp_id_fk='".$schcd."' 
							AND salary_monthyear='".date('Ym')."' AND requisition_type='".$requisition_type."'"; die;
	...
	if($arr[0]['salary_type'])
	{
		...
		$upd = $db->update("UPDATE prd_employee_salary_save
							...
							WHERE emp_id_fk='$emp_id_pk' AND gp_id_fk='".$schcd."' 
							AND salary_monthyear='".date('Ym')."' AND requisition_type='".$requisition_type."'
						");	
		...
		$ins = $db->insert("INSERT INTO prd_employee_salary_save(slno,latestupdate_time, latestupdate_ip_address, gp_id_fk, empcd, 
								...
								conv_allow,overdrawn,salary_type,cause,gp_code,
								part_day,gsli,delete_status,consolidated_pay,cooperative_loan,
								hbl_loan,festival_loan,cooperative_loan_cause,hbl_loan_cause,
								festival_loan_cause,part_salary_cause,no_salary_cause,interim_relief,requisition_type)
								...
								'1', '".substr($_SESSION['user_info']['stake_user'],0,7)."', '$teacher_id_pk', 
								'$pay_in_band', '$grade_pay', '$hill_allow','$gpf', '0', '$gross', '0','$conv_allow',
								'$overdrawn','$salary_type','$cause_msg','".$_SESSION['user_info']['stake_user']."',
								'$part_day','$gsli','1','$consolidated_pay','$cooperative_loan_val','$hbl_val',
								'$festival_val','$cooperative_loan','$hbl','$festival','$part_salary_cause_msg',
								'$no_salary_cause_msg','$interim_relief','".$requisition_type."')");			
	...
	else
	{
		...
		$ins = $db->insert("INSERT INTO prd_employee_salary_save(slno,latestupdate_time, latestupdate_ip_address, gp_id_fk, empcd, 
								...
								gsli,delete_status,consolidated_pay,cooperative_loan,hbl_loan,
								festival_loan,cooperative_loan_cause,hbl_loan_cause,festival_loan_cause,
								part_salary_cause,no_salary_cause,interim_relief,requisition_type)
