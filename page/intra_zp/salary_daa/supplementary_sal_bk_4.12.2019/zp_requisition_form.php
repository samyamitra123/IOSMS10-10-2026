...
function getAmount($dise,$emp_id_pk,$type,$basic)
{
	...
	if($type=='cpf')
	{
		$arr = $db->fetch_table("select cpf from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND zp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
		if($arr[0]['cpf'])
		{
			return $arr[0]['cpf'];
		}
	...
	if($type=='gpf')
	{
		$arr = $db->fetch_table("select gpf from prd_employee_salary_save where status_flag=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND AND zp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
		if($arr[0]['gpf'])
		{
			return $arr[0]['gpf'];
		}
	...
	if($type=='pfl')
	{
		$arr = $db->fetch_table("select pf_loan from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND zp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
		if($arr[0]['pf_loan'])
		{
	...
	if($type=='itax')
	{
		$arr = $db->fetch_table("select i_tax from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND zp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
		if($arr[0]['i_tax'])
		{
			return $arr[0]['i_tax'];
		}
	...
	if($type=='ovd')
	{
		$arr = $db->fetch_table("select overdrawn from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND zp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
		if($arr[0]['overdrawn'])
		{
	...
	if($type=='gsli')
	{
		$arr = $db->fetch_table("select gsli from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND zp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'"); 
...
function getEmpAmount($type,$dise,$emp_id_pk)
{
	...
	{
		$arr = $db->fetch_table("select emp_pay_in_payband from prd_employee_master where emp_id_pk='".$emp_id_pk."' AND zp_id_fk='".$dise."'");
...
///////////////////////////////////////////// List of Supplementary Employee /////////////////////////////////////////////////////////


$supplementary_employee_fetch=$db->fetch_table("
												SELECT
													emp_id_pk
												FROM
													prd_employee_master
												...
												WHERE 
													status_flag in (3,4) AND delete_status=1 AND is_saved=1 
		...
		$count=count($supplementary_employee_fetch);
		$sup_emp_id='';
		for ($i=0;$i<=$count-1;$i++)
		{
			$sup_emp_id.=$supplementary_employee_fetch[$i]['emp_id_pk'];
...
if(count($tch_final) == 0 )
{
	...
	//															WHERE
	//															tch.zp_id_fk = '".$dise."'
	//															AND (tch.emp_status='1' OR tch.emp_status='9' AND tch.emp_id_pk in ($sup_emp_id))");	
	//																
	//																		
	...
	$tch= $db->fetch_table("
							...
							AND tch.emp_id_pk in ($sup_emp_id)
							");
	...
	$sal_saved = $db->fetch_table("SELECT prd_emp.emp_desig,save.emp_id_fk FROM prd_employee_salary_save 
									...
									AND save.is_saved=1 
									AND save.zp_id_fk = '".$dise."' AND (prd_emp.emp_status=1 or prd_emp.emp_status=9)
	...
	<table width="100%">
        <tr>
            <th></th>
            <th></th>
            <th colspan="7"><strong>PAY & ALLOWANCE</strong></th>
            <!--<th colspan="1"></th>-->
            <th></th>
        ...
        if(count($tch))
		{
			...
			foreach ($tch as $key) 
			{
				...
				$sal_chk = $db->fetch_table("SELECT 
												slno, latestupdate_time, latestupdate_ip_address, save.ps_id_fk, save.empcd, bankname, 
												...
												save.interim_relief,save.hra_deduction,save.zp_id_fk 
											...
											AND emp_id_fk='$emp_id_pk' AND save.zp_id_fk='".$key['zp_id_fk']."' 
					...
					else
					{
						if($sal_chk[0]['salary_type']=='1')
						{
							...
							if($emp_effective_day_for_promotion_cal != 01 && $emp_id_for_promotion_part != "" && $dop_doi == 3 && $promotion_eff_yr_mnth == date("Ym"))
							{
								...
								//2.1.2019 $full_interim_relief = $key['interim_relief'];
						...
						if($sal_chk[0]['salary_type']=='2')
						{
							...
							$da = $sal_chk[0]['da'];
							$hra = $sal_chk[0]['hra'];
							//2.1.2019 $interim_relief = $sal_chk[0]['interim_relief'];
							$interim_relief=0;
							$ma = $sal_chk[0]['ma'];
				...
				else
				{
					$sal_save=	$db->fetch_table("SELECT 
														...
														part_day,gsli,consolidated_pay,festival_loan,festival_loan_cause,interim_relief  
													FROM prd_employee_salary_save 
													WHERE status_flag=3 AND delete_status=1 
													AND salary_monthyear='".date('Ym',strtotime('-1 month'))."' 
													AND ps_id_fk='".$key['ps_id_fk']."' 
					...
					else
					{
						...
						else
						{
							...
							//2.1.2019 $interim_relief = $key['interim_relief'];
							$interim_relief=0;
						...
						$hbl_loan = $sal_chk[0]['hbl_loan'];
						$festival_loan = $sal_chk[0]['festival_loan'];*/
						$festival_loan = getAmount($dise,$emp_id_pk,'festival_loan','');  
						...
						*/
						...
						$total_deduct = $gpf+$pfl+$cpf_deduct+$ptax+$itax+$gsli+$overdrawn+$reduct1+$reduct2+$reduct3+$hra_deduction+$festival_loan;
						$net_salary = $gross_salary-$total_deduct;
					}
				}
				...
				<tr>
                    ...
                    <!--<td><?php echo round($emp_bonus); ?></td>-->
                    ...
                    <td class="loan">
						<?php if($total_loan_deduction != 0)
						{ ?>
                            <a data-toggle="modal" data-target=".bs-example-modal-lg-loan" href="" id="<? echo $crypto->encode($key['emp_id_pk'],4) ?> ">
	...
	<div class="form-group" id="sal_save" style="display:none;">
        <div class="col-sm-offset-5 col-sm-7" style="margin-top:1%">
        	<a href="<?= $config['base_url'] ?>page/intra_zp/salary_daa/supplementary_sal/ajax_zp_rq_finalize.php" class="btn btn-success btn-sm">Salary Finalize</a>
        </div>
	</div>
...
{  ?>
	<script>
        ...
        window.setTimeout(function(){
            window.location.replace("<?php echo $config['base_url'] . 'page/intra_zp/salary_daa/supplementary_sal/view_zp_sal_requisition.php'; ?>");
        },3000);
...
<script>
	$(document).ready(function()
	{
		...
		{	
			...
			var link1= $("#zp_id").val();
			$.post('<?= $config['base_url'] ?>page/intra_zp/salary_daa/supplementary_sal/ajax_requisition_edit.php?id='+link+'&zp_id='+link1, function(data)
			{                    
				$("#mbody").html(data);
			});
		});
		...
		$(".loan a").click(function() 
		{
			...
			var link = $(this).attr('id');
			var link1= $("#gp_id").val();
			//$.post('<?= $config['base_url'] ?>page/intra_mad/ddo_deo/supplementary_sal/ajax_emp_loan_details_vew.php?id='+link+'&gp_id='+link1, function(data)
			$.post('<?= $config['base_url'] ?>page/intra_zp/salary_daa/supplementary_sal/ajax_emp_loan_details_vew.php?id='+link+'&mu_id='+link1, function(data){
				$("#loanmbody").html(data);
			});
		});
		...
		$(".save a").click(function() 
		{
			...
			$.post('<?= $config['base_url'] ?>page/intra_zp/salary_daa/supplementary_sal/ajax_zp_rq_save.php?emp_id_pk='+emp_id_pk+'&sec_tok='+sec_tok, function(data)
			{
		...
		{	
			$.post('<?= $config['base_url'] ?>page/intra_zp/salary_daa/supplementary_sal/ajax_zp_rq_finalize.php', function(data){
			//$("#mbody").html(data);
			});
		});
