...
<script>
	...
	function hra_deduction()
	{
		...
		if(hra_deduc=='' || hra_deduc=='0')
		{
			...
			var hra_dedu=0;
			var total_loan_deduction=$('#total_loan_deduction').val();
			var deduct = parseInt(gpf)+parseInt(total_loan_deduction)+parseInt(ptax)+parseInt(itax)+parseInt(overdrawn)+parseInt(gsli)+parseInt(reduct3)+parseInt(hra_dedu);
		...
		else
		{
			...
			{
				alert('Please enter the valid HRA Deduction amount.\nHRA Deduction amonut should not be greater than HRA amount.');
				$('#hra_deduc').val(0);
				var hra = $('#hra').val();
				var gross = $('#gross').val();
				//var pfl = $('#pf_loan').val();
	...
	{
		$.post('<?= $config['base_url'] ?>page/intra_zp/salary_daa/supplementary_sal/cal_ptax.php?gross='+gross, function(data){
			// alert(data);
			$('#p_tax').val(data);
			/* var data=data;
			return(data);*/
