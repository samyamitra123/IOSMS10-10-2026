...
$paychange = $db->fetch_table("SELECT paychange_id_pk, paychange_ip, paychange_fromdate, paychange_todate, 
       ...
       overdrawn, salary_type, cause,gp_code, part_day,gsli,consolidated_pay,festival_loan,festival_loan_cause,total_loan_deduction,hra_loan_cause from prd_employee_salary_save
	    where 
		...
		AND emp_id_fk='$emp_id_pk' AND zp_id_fk='".$dise."'");
			...
			//----------- End Gpf=0 before retirement ---------------------
				     ...
				     if(strtotime($suspend_effect_date)<=strtotime(date('Y-m-d')) && strtotime($suspend_effect_date)>0)
			          {
						 ...
						 {
							 $full_interim_relief=0;
						 	//2.1.2019 $full_interim_relief = $tch[0]['interim_relief'];
						 }
						 else
						 {
							 $full_interim_relief = 0;
	           ...
	           if($tch[0]['emp_spouse_res']=='251'){ 
					...
					if($state10=='3219'){									//1120 -> 999999 Final Check 121217
				  ...
				  //  $festival_loan = getAmount($dise,$empcd,'festival_loan','');     //changed 
					...
					$festival_loan_cause=$sal_chk[0]['festival_loan_cause'];
					$total_loan_deduction=$sal_chk[0]['total_loan_deduction'];
				     $total_deduct = $gpf+$ptax+$itax+$gsli+$overdrawn+$festival_loan+$hra_deduction;
					 
				     $total_deductions = $total_deduct;   
				     $full_net_salary = $full_gross_salary-$total_deduct;
					 //$net_salary_sal_save=$full_net_salary; 
					...
					//$total_loan_deduction=$sal_chk[0]['total_loan_deduction'];
				    //$festival_loan = getAmount($dise,$empcd,'festival_loan','');     //changed 
				    $total_deduct = $gpf+$cpf_deduct+$ptax+$itax+$gsli+$overdrawn+$festival_loan+$hra_deduction+$total_loan_deduction;
				    $net_salary_sal_save = $sal_chk[0]['net'];
	         ...
	         //}
				...
				else if($sal_chk[0]['salary_type']=='8')
				{
				 ...
				 // This condition for return full salary of employee zero salary and display saved salary of employee in edit mode
					...
					//$interim_relief = $sal_chk[0]['interim_relief'];
					//2.1.2019 $interim_relief = getEmpAmount('interim_relief',$dise,$emp_id_pk); 	//Changed by 
					$interim_relief=0;
				...
				if($tch[0]['spouse_medical_allowance']=='1'){
					...
					$festival_loan = getAmount($dise,$empcd,'festival_loan','');     //changed 
				    $total_deduct = $gpf+$cpf_deduct+$ptax+$itax+$gsli+$overdrawn+$festival_loan+$ssl+$scl+$scc+$arrear_amount+$hra_deduction;
	   ...
	   $emp_suspend=$db->fetch_table("SELECT 
				  ...
				  if(strtotime($suspend_effect_date)<=strtotime(date('Y-m-d')) && strtotime($suspend_effect_date)>0)
					  ...
					  {  
						...
						if($tch[0]['interim_relief'] != '')
						{
							//2.1.2019 $interim_relief = $tch[0]['interim_relief'];					//Commented by 
							$interim_relief=0;
						}
						else
						{
				...
				else
				{
	              ...
	              /*if($tch[0]['emp_desig']=='1120')					
				  {
					...
					//$cooperative_loan = 0;
						//$hbl_loan = 0;
				   $festival_loan =0;
					$total_deduct = $gpf+$pfl+$cpf_deduct+$ptax+$itax+$gsli+$overdrawn+$festival_loan+$festival_loan;	
...
input[type="checkbox"] {
	display:inline !important;
          ...
          <select name="reduction" id="reduction" class="form-control" style="width:150px;border: 1px solid #3E4255; " >
                             <option id="no_ovd1" value="no_ovd1" <?php if(!$other_deduction_cause){ echo 'selected';} ?>>NO</option>
                      <option id="yes_ovd1" value="yes_ovd1" <?php if($other_deduction_cause=="yes_ovd1" ){ echo 'selected';} ?> >YES</option>   
                           
                        </select>
                    </td></tr>
   <tr id="reduction_type" >
                	<td>Deduction Cause <span class="star_color">*</span>:</td>   
                    <td width="65%" style="text-align:left; "><p><input type="checkbox" name="n7" id="reduction_type1" class="cls6" value="1" <?php  if($hra_loan_cause==1) { ?>checked="checked" <?php } ?>>&nbsp;&nbsp;&nbsp;<b>HRA Deduction</b></p>
                    <p><input type="checkbox" name="n8" id="reduction_type2" class="cls6" value="1" <?php  if($festival_loan_cause==1) { ?>checked="checked" <?php } ?>>&nbsp;&nbsp;&nbsp;<b>Festival Advance</b></p>
	...
	width:17%;
                 ...
                 <div style="width:100%; float:left;">
			       ...
			       <input type="hidden" name="emp_desig" id="emp_desig" value="<?php echo $cryp->encode($tch[0]['emp_desig'],4); ?>" />
                                ...
                                <input maxlength="5" type="hidden" style='background-color: #EEE;' id="interim_relief" name="interim_relief" value="<?php echo round($interim_relief); ?>" size="5" readonly='readonly' />
			    ...
			    <tr>
				<td class="row_heading">HRA DEDUCTION </td>
...
//				$('#scc').val('<?php echo $scc; ?>');
			...
			if(id == '8'){
				//alert(42);
				$('#pay_in_band,#consolidated_pay,#grade_pay,#da,#hra,#ma,#conv_allow,#gross,#gpf,#pf_loan,#p_tax,#i_tax,#gsli,#hra_deduc').attr('readonly','readonly').css('background-color','#EEE;');
...
//					var scc=$('#scc').val();
					...
					var deduct = parseInt(gpf)+parseInt(gsli)+parseInt(ptax)+parseInt(itax)+parseInt(overdrawn)+parseInt(reduct3)+parseInt(total_arrear_deduction);
					$('#deduct').val(deduct);
					var net = parseInt(gross)-parseInt(deduct);	
					$('#net').val(net);
