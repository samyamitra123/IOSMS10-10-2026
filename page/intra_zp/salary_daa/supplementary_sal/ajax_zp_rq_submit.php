...
else
{
	...
	$total_allowance=$consolidated_pay+$pay_in_band+$grade_pay+$da+$hra+$ma+$conv_allow+$hill_allow+$interim_relief;
	...
	if($arr[0]['salary_type'])
	{
		...
		$ins = $db->insert("INSERT INTO prd_employee_salary_save(slno,latestupdate_time, latestupdate_ip_address, zp_id_fk, empcd, 
							...
							hra_deduction,hra_loan_cause,other_deduction_cause,zp_emp_type,emp_desig,other_loan_deduction
							)
							...
							'$no_salary_cause_msg','$interim_relief','".$requisition_type."','0','".$total_loan_deduction."','".$hra_deduction."',
							'".$hra_loan."','".$other_deduction_cause."','".$zp_emp_type."','".$emp_desig."','".$other_loan_deduction."')");	
		
		
		if($upd && $ins)
		{					
			pg_query("commit"); 
			$_SESSION['msg']= '<div class="alert alert-success" style="text-align:center"><strong>The Supplementary Salary requisition of '.$tchname.' has been successfully updated.</strong></div>';
			header('location:zp_emp_sal_requisition.php'.$query_string);
			exit(0);
		}
		else
		{
			pg_query("rollback");
			$_SESSION['msg']= '<div class="alert alert-danger" style="text-align:center"><strong>Sorry !!! Supplementary Salary Requisition Updation Fails.</strong></div>';
			header('location:zp_emp_sal_requisition.php'.$query_string);
			exit(0);
		}
	}
	else
	{
		...
		$ins = $db->insert("INSERT INTO prd_employee_salary_save(slno,latestupdate_time, latestupdate_ip_address, zp_id_fk, empcd, 
						...
						'".$other_deduction_cause."','".$zp_emp_type."','".$emp_desig."','".$other_loan_deduction."')");
