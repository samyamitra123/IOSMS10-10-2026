...
$tch= $db->fetch_table("SELECT sal.ps_id_fk,tch.emp_status, sal.empcd, 
       ...
       sal.overdrawn, sal.salary_type, sal.cause, sal.part_day, sal.consolidated_pay,sal.total_loan_deduction,
	   sal.gsli,sal.consolidated_pay,sal.other_deduction,sal.cooperative_loan,sal.hbl_loan,sal.hbl_loan,festival_loan,
	   sal.interim_relief,sal.hra_deduction,
		tch.emp_first_name,
		tch.emp_second_name,
		tch.emp_last_name,
	...
	INNER JOIN 
		...
		WHERE
			...
			AND sal.status_flag in('1','2','3','4')  AND is_saved='1' AND sal.delete_status='1' AND salary_monthyear='".date('Ym')."' 
			AND is_saved='1' AND requisition_type='".$requisition_type."' order by tch.emp_first_name ;
								");
			...
			<br>
				<?php if(count($tch)){
					if($tch[0]['status_flag'] == 1){ ?>
						<div class="alert alert-success" style="text-align:center"><strong> Supplementary Salary Requisition Not Finalized</strong></div>
						
					<?php }
					elseif($tch[0]['status_flag'] == 2){ ?>
						<div class="statusf">
                        <div class="alert alert-success" style="text-align:center"><strong> Supplementary Salary Requisition Finalized by DAA</strong></div>
                        </div>
						
					<?php } 
						elseif($tch[0]['status_flag'] == 3){ ?>
						<div class="alert alert-success" style="text-align:center"><strong>Supplementary Salary Requisition Forwarded by ACCOUNTANT</strong></div>
					<?php } 
					} ?>
            <div class="emplist">
				<div class="school">	
