...
if(count($tch_final) == 0 )
{
	...
	$supplementary_employee_fetch=$db->fetch_table("
										...
										WHERE emp_status=1 AND zp_id_fk = '".$dise."'
										EXCEPT
										SELECT emp_id_fk
										FROM prd_employee_salary_save
										WHERE status_flag=4 AND delete_status=1 AND is_saved=1 AND zp_id_fk = '".$dise."' AND 
	...
	$tch= $db->fetch_table("
							...
							AND tch.emp_status in('1','9') AND tch.emp_id_pk in ($sup_emp_id)
							");
...
<script>
	$(document).ready(function()
	{
		...
		var tch=(<? if(isset($tch)){ echo count($tch);}?>);
		...
		$(".save a").click(function() 
		{
			...
			$.post('<?= $config['base_url'] ?>page/intra_zp/salary_daa/supplementary_sal/ajax_zp_rq_save.php?emp_id_pk='+emp_id_pk+'&sec_tok='+sec_tok, function(data)
			{
				...
				if(result[0]==2)
				{ 
					$('.border_val').html('<div class="alert alert-success" style="text-align:center"><strong>The Supplementary salary requisition of '+name+' has been successfully saved.</strong></div>');
					$('.save_id'+show+'').html('<img width="25" src="<?= $config['base_url'] ?>themes/default/image/saved_icon.png" alt="Edit">');
				}
				
				if(result[0]==3)
				{
					$('.border_val').html('<div class="alert alert-danger" style="text-align:center"><strong>Sorry!!! Supplementary Salary Requisition Not Saved. Please try again.</strong></div>');
				}
