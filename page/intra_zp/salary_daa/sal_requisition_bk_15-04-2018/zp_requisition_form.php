...
<!--            <th>Consolidated Pay</th>-->
       ...
       overdrawn, salary_type, cause,gp_code, part_day, gsli,consolidated_pay,other_deduction,festival_loan,festival_loan_cause,save.interim_relief,save.hra_deduction,save.zp_id_fk from 
       prd_employee_master inner join
prd_employee_salary_save save  on emp_id_pk=emp_id_fk
where emp_status=1 and status_flag=1 AND delete_status=1 AND salary_monthyear='".date('Ym')."'  AND emp_id_fk='$emp_id_pk' AND save.zp_id_fk='".$key['zp_id_fk']."' AND requisition_type='".$requisition_type."'");
...
*/							else{
				...
				if($key['conv_allow_status']=='1'){
					$ptax = 0;
					  ...
					  if(strtotime($suspend_effect_date)<=strtotime(date('Y-m-d')) && strtotime($suspend_effect_date)>0){
					 $gross_salary_sus=round($pay_in_band+$grade_pay+$da_sus+$interim_relief+$hra+$ma+$conveyance_allowance+$cpf+$hill_allowance+$emp_bonus);
            ...
            <!--<td><?php //if($cooperative_loan){echo $cooperative_loan;} else {echo 0;} ?></td>
             ...
             <td><?php if($festival_loan){echo $festival_loan;} else {echo 0;} ?></td>
              ...
              <a data-toggle="modal" data-target=".bs-example-modal-lg-loan" href="" id="<? echo $crypto->encode($key['emp_id_pk'],4) ?> ">
				...
				//echo $emp_id_pk;
				 ?></a>
                 <?php }else{ echo $total_loan_deduction; } ?>
		...
		<?php } }}  else { ?>
			...
			alert ('eee');
					...
					window.setTimeout(function(){
							window.location.replace("<?php echo $config['base_url'] . 'page/intra_zp/salary_daa/sal_requisition/view_zp_salary_requisition.php'; ?>");
					},3000);
            </script>
