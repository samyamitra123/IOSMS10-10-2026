...
$arr=$db->fetch_table("SELECT 
							...
							emp_last_name,emp_desig,
							emp_status,emp_id_pk,
							emp_form_status,emp_id_const,emp_unlock_status 
						FROM prd_employee_master 
						WHERE emp_form_status='5' AND zp_id_fk='".$_SESSION['location']['district_id']."' 
						ORDER BY emp_status,emp_id_const,emp_first_name");
...
<script>
	$(document).ready(function(){
		...
		$(".view a").click(function() {
			...
			$(".mbody").load('<?= $config['base_url'] ?>page/intra_zp/aeo/ajax_emp_view.php?id='+link, function(responseTxt,statusTxt,xhr){
			
				var status= $('#emp_status').val();
				if(status=='1' || status=='7' || status=='10' || status=='6')
				{
					$(".finz_but").hide();
					$(".rej_but").hide();
					//$(".delete_but").hide();
		...
		$(".finalize").click(function(){
			...
			$.post('<?= $config['base_url'] ?>page/intra_zp/aeo/ajax_finalize_reject.php?id='+link1+'&flag=finalize', function(data){
               //alert(data);
		...
		$(".reject").click(function(){
			...
			{
				$.post('<?= $config['base_url'] ?>page/intra_zp/aeo/ajax_finalize_reject.php?id='+link2+'&flag=reject&reject_reason='+reason, function(data){
				//alert(data);
				$('#myModal').modal('hide');
				$('#reject').modal('show');
				$('.msg').html(data);
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <div class="btn-group">
                    <a class="btn btn-success finz_but" data-bs-toggle="modal" data-bs-target="#finalize">Approve</a>
                    <a class="btn btn-danger rej_but" data-bs-toggle="modal" data-bs-target="#reject">Reject</a>  
...
<form name="unlock_request" id="unlock_request" action="unlock_reject_request_employee.php" method="post">
    <div class="modal fade bs-example-modal-sm" id="unlock_r" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true" data-modal-parent="#schprfModal">
        <div class="modal-dialog modal-sm">
            <div class="modal-content" >
                <div class="modal-header">
                ...
                <div class="modal-body"> 
                    <p class="alert alert-warning"><strong><i class="fa fa-exclamation-triangle"></i> Are You Sure To <span id="unlock_show" style="display:none;"> Approve Unlock</span><span id="reject_show" style="display:none;">Reject Unlock</span>  Request ?</strong></p>
