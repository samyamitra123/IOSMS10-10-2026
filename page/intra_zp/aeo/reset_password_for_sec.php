...
<script>
		...
		alert("Please Insert Secretary Name.");
...
<? require '../../../page/common_back_btns.php'; ?>
  <div class="welcome_msg">
       ...
       <?php
	    ...
	    $data11 = $db->fetch_table("SELECT COUNT(*) AS result_count FROM prd_stack_user_login where stake_level_id_fk = '51'");
...
<div class="row" id="cont">
 ...
 <?php if($count11<1)
		{
		 	echo '<h1 class="heading"> Create Profile For Secretary </h1>';	
		}
		else 
		{
		    
			echo '<h1 class="heading">Reset Password For Secretary </h1>';
		} ?>
...
<div id="msg"></div>
    ...
    <label for="inputPassword3" class="col-sm-3 control-label">Secretary CODE: <span class="star_color">*</span></label>
    <div class="col-sm-3">
    <?php
			$db=new database();
			...
			//$zp_fetch=$db->fetch_table("SELECT stake_user FROM prd_stack_user_login WHERE stake_level_id_fk='51' AND stake_user like '".$zp_code[0]['district_code']."%'");
	 ?>
	
	<input type="text" class="form-control" readonly name="sec_code" id="sec_code" value="<?php echo $_SESSION['location']['district_code'].'ZPSEC'; ?>" />
	
	<input type="hidden" class="form-control" readonly name="sec" id="sec" value="<?php echo $zp_code[0]['district_code']?>" />
    <input type="hidden" class="form-control" readonly name="stake_level_id" id="stake_level_id" value="51" />
  ...
  border: 1px solid #C9C9C9;margin-left:3%" id="captcha_img" src="'.$config['base_url'].'page/CaptchaSecurityImages.php?width=115&height=40&characters=5" style="border:1px solid #FA6F1B; margin:0 0 0 10px;" alt="CAPTCHA code">'; ?> <span class="star_color">*</span></label>
    ...
    <div class="col-sm-offset-5 col-sm-7">
      <input type="submit" name="change_sec" value="SUBMIT" class="btn btn-info">
    </div>
  </div>
