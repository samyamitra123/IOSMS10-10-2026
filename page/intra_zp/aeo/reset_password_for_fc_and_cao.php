...
<script>
	...
	if(cap_code!=''){
		...
		if(document.getElementById('fc_and_cao_code').value==''){
		alert("Please Insert FC And CAO Name.");
		document.getElementById('fc_and_cao_code').focus();
		return false;
	}
...
<? require '../../../page/common_back_btns.php'; ?>
  <div class="welcome_msg">
       ...
       <?php
	    ...
	    $data11 = $db->fetch_table("SELECT COUNT(*) AS result_count FROM prd_stack_user_login where stake_level_id_fk = '53'");
...
<div class="row" id="cont">
      <div class="col-lg-12 col-md-8 col-sm-8" id="sm-pad"> 
        ...
        <?php if($count11<1)
		{
		 	echo '<h1 class="heading"> Create Profile For FC & CAO </h1>';	
		}
		else 
		{
			echo '<h1 class="heading">Reset Password For FC & CAO </h1>';
		} ?>
...
<?php 
    ...
    <label for="inputPassword3" class="col-sm-3 control-label">FC & CAO CODE: <span class="star_color">*</span></label>
    <div class="col-sm-3">
    <?php
			$db=new database();
			...
			$zp_fetch=$db->fetch_table("SELECT stake_user FROM prd_stack_user_login WHERE stake_level_id_fk='53' AND stake_user like '".$zp_code[0]['district_code']."%'");
	 ?>
	
	<input type="text" class="form-control" readonly name="fc_and_cao_code" id="fc_and_cao_code" value="<?php echo $_SESSION['location']['district_code'].'ZPDDO'; ?>" />
	
	<input type="hidden" class="form-control" readonly name="fc_and_cao" id="fc_and_cao" value="<?php echo $zp_code[0]['district_code']?>" />
    <input type="hidden" class="form-control" readonly name="stake_level_id" id="stake_level_id" value="53" />
  ...
  border: 1px solid #C9C9C9;margin-left:3%" id="captcha_img" src="'.$config['base_url'].'page/CaptchaSecurityImages.php?width=115&height=40&characters=5" style="border:1px solid #FA6F1B; margin:0 0 0 10px;" alt="CAPTCHA code">'; ?> <span class="star_color">*</span></label>
    ...
    <div class="col-sm-offset-5 col-sm-7">
      <input type="submit" name="change_fc_and_cao" value="SUBMIT" class="btn btn-info">
    </div>
  </div>
