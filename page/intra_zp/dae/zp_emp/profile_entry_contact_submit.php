...
else
{
	...
	if($_POST['permanent_city_district']=='')
	{
		$permanent_city_district=0;
	}
	else
	{
	...
	$permanent_police_station=strtoupper(trim($_POST['permanent_police_station']));
	...
	if($validator->pattern_match_alphanumeric($present_street)==FALSE || $validator->pattern_match_alphanumeric($present_town_vill)==FALSE || $validator->pattern_match_alphanumeric($present_post_office)==FALSE || $validator->pattern_match_alphanumeric($present_others_dist)==FALSE ||  $validator->pattern_match_alphanumeric($permanent_street)==FALSE || $validator->pattern_match_alphanumeric($permanent_town_vill)==FALSE || $validator->pattern_match_alphanumeric($permanent_post_office)==FALSE || $validator->pattern_match_alphanumeric($permanent_others_dist)==FALSE || $validator->pattern_match_alphanumeric($permanent_police_station)==FALSE || $validator->pattern_match_alphanumeric($present_police_station)==FALSE)
	{
	...
	else if($validator->blank_select($present_police_station)==FALSE)
	{
		$error_msg='<div class="alert alert-danger" style="text-align:center"><strong>Please Enter Present Police Station.</strong></div>';
		$emp_id=$cryptoGraph->encode($emp_id_pk,4);
		include 'profile_entry_contact_form.php';
		exit;
	}
	...
	else if($validator->blank_select($permanent_police_station)==FALSE)
	{
		$error_msg='<div class="alert alert-danger" style="text-align:center"><strong>Please Enter Permanent Police Station.</strong></div>';
		$emp_id=$cryptoGraph->encode($emp_id_pk,4);
		include 'profile_entry_contact_form.php';
		exit;
	}
	...
	if($edit_status == 2)
	{
		...
		{	
			...
			$insrt_contact=$db->update("update prd_employee_master set 
										...
										emp_pre_ps='".$present_police_station."',
										emp_per_ps='".$permanent_police_station."',
	...
	if($insrt_contact)
	{
		...
		$emp_id_const=$db->fetch_table("select emp_id_const from prd_employee_master where emp_id_pk='".$emp_id_pk."' ");
		$emp_id_con=$emp_id_const[0]['emp_id_const'];
		if($emp_id_con!='0')
		{
			header('Location:employee_edit_details.php?emp_id='.$cryptoGraph->encode($emp_id_pk,4).'');
			exit(0);
		}
		else
		{
