...
$tot_conv_allow=$tot_payband=$tot_basic=$tot_da=$tot_hra=$tot_ma=$tot_ca=$tot_ha=$tot_cpf=$tot_gross=$tot_gpf=$tot_cpfdeduct=$tot_ptax=$tot_itax=$tot_od=$tot_gsli=$total_loan=$other_loan=$tot_net=0;
  ...
  <tr>
    ...
    <td>CONSOLIDATED PAY : <?= $key['consolidated_pay']?>
    <br/>
    
     <?php if($monthyear<='201912')
{?>
    PAY IN PAY BAND : <?= $key['pay_payband']?>
    <?php }else {?>
...
{?>
    GRADE PAY : <?= $key['tch_grade_pay']?>
     <br/>
     <?php }?>
   DA : <?= $key['da']?><br/>HRA : <?= $key['hra']?><br/>MA : <?= $key['ma']?><br/>CONV ALLOW: <?= $key['conv_allow']?><br/>HILL ALLOW : <?= $key['hill_allowance']?><br/>ADMINISTRATIVE ALLOW : <?= $key['allowance']?><?php if($monthyear<='201812'){?><br/>INTERIM RELIEF : <?= $key['interim_relief']?><?php }?><br/><span  style="color:#CD1316;font-weight:bold;">OVERDRAWN : <?php if($key['overdrawn']=='0'){ echo $key['overdrawn'];}else{ '-'.$key['overdrawn'];}?><span/></td>
