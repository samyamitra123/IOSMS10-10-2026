...
//$path = '../../../../readwrite/xml_file/ifmszp/webservice/';
$path = $_SERVER['DOCUMENT_ROOT'].'/readwrite/xml_file/ifmszp/webservice/';

//$path = $_SERVER['DOCUMENT_ROOT'].'/prd/readwrite/xml_file/ifmszp/webservice/';
...
$ddo_sql = $db->fetch_table("Select ddo_code,pl_code from zpemp_zp_profile where district_id_fk = '".$_SESSION['location']['district_id']."'");
	...
	$treasury_code=substr($ddo_sql[0]['ddo_code'],0,3);
	$pl_code=str_pad($ddo_sql[0]['pl_code'],6,'0',STR_PAD_LEFT);
	$schm_code='999998';
...
//echo ("select bill_no,bill_entry_time,salary_monthyear, block_bill_pk from prd_block_bill_details where zp_id_fk='" . $_SESSION['location']['district_id'] . "' and salary_monthyear= '".date('Ym')."' AND requisition_type='".$bill_type."' AND status='1' AND drn_number='".$drn_number."' AND zp_emp_type='".$emp_type."'");die;
if($bill_type == '1001' ||$bill_type == '1002')
{
$bill_det=$db->fetch_table("select bill_no,bill_entry_time,salary_monthyear, block_bill_pk from prd_block_bill_details where zp_id_fk='" . $_SESSION['location']['district_id'] . "' and salary_monthyear= '".$monthyear."' AND requisition_type='".$bill_type."' AND status='1' AND drn_number='".$drn_number."' AND zp_emp_type='".$emp_type."'");
}
else if ($bill_type == '1004' || $bill_type == '1005'   || $bill_type == '1003')
{
$bill_det=$db->fetch_table("select bill_no,bill_entry_time,salary_monthyear, block_bill_pk from prd_block_bill_details where zp_id_fk='" . $_SESSION['location']['district_id'] . "' and salary_monthyear= '".$monthyear."' AND requisition_type='".$bill_type."' AND status='1' AND drn_number='".$drn_number."' ");
}
$det_head=$db->fetch_table("select * from prd_head_details where block_code='3299001'");
...
$teacher_dtls=$db->fetch_table("select distinct(emp.emp_system_code, emp.empcd), 
								...
								INNER JOIN ".$sal_table_name." sal ON (sal.emp_id_fk=emp.emp_id_pk and sal.zp_id_fk=emp.zp_id_fk)
							where 
...
$billdate=dateshow_slash($bill_det[0]['bill_entry_time']);
	...
	$ern_amount=($gross_salary)-($overdrawn)-($advance_amount);
	$ern_amount1=($gross_salary);
...
else if($bill_type == '1005')
{
	...
	$teacher_dtls=$db->fetch_table("select distinct(emp.emp_system_code, emp.empcd), 
									...
									sal.festival_advance_total_amount
...
$total_beneficiary=count($teacher_dtls);
	...
	//$subsflag = $xml->createElement('SUBS_FLAG','N'); // Once Live Need to set to Y
