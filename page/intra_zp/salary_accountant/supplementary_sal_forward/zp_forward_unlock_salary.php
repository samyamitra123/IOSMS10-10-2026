...
if($sal_action=='forward')
{
	...
	if(count($unlocked_emp_check)>0)
	{
		...
		$_SESSION["msg"]='<div class="alert alert-danger" style="text-align:center;"><strong>Supplementary Salary Forward failed Due to following employees:<br>'.$emp_list.'</strong></div>';
	...
	else
	{
		...
		{		
			$security = $db->insert("
									...
									");
			$_SESSION["msg"]='<div class="alert alert-success" style="text-align:center;"><strong>Supplementary Salary Forwarded Successfully...</strong></div>';
			header('Location:zp_salary_requisition_view.php');
		}
		
		else
		{
			$_SESSION["msg"]='<div class="alert alert-danger" style="text-align:center;"><strong>Supplementary Salary Forward failed. Please try again...</strong></div>';
			header('Location:zp_salary_requisition_view.php');
		}
	}
}
else if($sal_action=='unlock')
{
	...
	if($query_unlock)
	{
		...
		");
		$_SESSION["msg"]='<div class="alert alert-success" style="text-align:center;"><strong>Supplementary Salary Unlocked Successfully...</strong></div>';
		header('Location:zp_salary_requisition_view.php');
		
	}
	else
	{
		$_SESSION["msg"]='<div class="alert alert-danger" style="text-align:center;"><strong>Supplementary Salary Unlock failed. Please try again...</strong></div>';
		header ('Location:zp_salary_requisition_view.php');
	}
}
?>