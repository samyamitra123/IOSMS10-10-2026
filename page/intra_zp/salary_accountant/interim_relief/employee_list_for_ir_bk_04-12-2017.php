...
/*echo("SELECT 
				...
				ON(mas.emp_id_pk=ir.emp_id_fk AND mas.gp_id_fk=ir.gp_id_fk)
				WHERE ir.ir_status in('3','4' ) AND ir.delete_status ='1' AND ir.gp_id_fk='".$cryptoGraph->decode($_GET['id'],4)."' order by mas.emp_id_const"); die;*/
...
$arr=$db->fetch_table("SELECT 
				...
				INNER JOIN psemp_interim_relief ir 
				ON(mas.emp_id_pk=ir.emp_id_fk AND mas.gp_id_fk=ir.gp_id_fk)
				WHERE ir.ir_status in('3','4' ) AND ir.delete_status ='1' AND ir.gp_id_fk='".$cryptoGraph->decode($_GET['id'],4)."' order by mas.emp_id_const");
?>
<style>
h1 {
display: block;
...
<input type="hidden" id="emp_id_const<?=$cnt; ?>" name="emp_id_const" value="<?=$item['emp_id_const'];?>"  />
<input type="hidden" id="emp_gp_id<?=$cnt; ?>" name="emp_gp_id" value="<?=$cryptoGraph->encode($item['gp_id_fk'],4);?>"  />
...
<td><? if($item['ir_status']=='4' && $item['delete_status']=='1'&& $item['unlock_req']=='1'){ ?><a data-toggle="modal" onClick="ir_edit(<?=$cnt;?>);" style="cursor:pointer; "><img width="25" src="<?= $config['base_url'] ?>themes/default/image/edit_icon.png" alt="Edit" ></a>
<? } 
else{ ?>
...
if($item['ir_status']=='3') 
{ ?>
	<a class="btn btn-danger" onClick="request_reject(<?=$cnt;?>)">REJECT</a></td>
<?php }
else
{ ?>
    ...
    <div class="modal-content" >
     ...
     <div class="modal-body"> 
      ...
      <input type="hidden" id="flag" name="flag"/>
       <input type="hidden" name="ir_gp" id="ir_gp"/>
     </div>
