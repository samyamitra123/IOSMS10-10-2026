...
$cryptoGraph=new cryptography();
		...
		$gp_id=$cryptoGraph->decode($_POST['ir_gp_id'],4);
  
	$db=new database();
	pg_query('BEGIN');
	$emp_id_pk_check=$db->fetch_table("select emp_id_fk,delete_status from psemp_interim_relief where emp_id_fk='".$emp_id_fk."' and gp_id_fk='".$gp_id."' and ir_status='4' and delete_status='1'");
	...
	{ 
		...
		$ir_submit=$db->insert("INSERT INTO psemp_interim_relief
	       (emp_id_fk,ir_status,entry_time,interim_relief,gp_id_fk,delete_status,unlock_req)
		VALUES('".$emp_id_fk."','4','now()','".$ir."','".$gp_id."','1','1')");
		if($ir_submit)
	{
	...
	else
	{
		...
		$ir_submit=$db->insert("INSERT INTO psemp_interim_relief
	       (emp_id_fk,ir_status,entry_time,interim_relief,gp_id_fk,delete_status)
		VALUES('".$emp_id_fk."','1','now()','".$ir."','".$gp_id."','1')");
	...
	if($ir_status_up && $ir_submit && $emp_master_ir_update)
		{
			...
			header('Location:employee_list_for_ir.php?id='.$_POST['ir_gp_id']);
		}
		else
		{
			pg_query('ROLLBACK');
