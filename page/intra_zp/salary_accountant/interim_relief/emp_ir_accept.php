...
if($emp_id_fk!="" && $flag!="" && $zp_id!="")
{
	if($flag=='accept')
	{
		...
		$ir_status_update=$db->update("UPDATE psemp_interim_relief SET ir_status='4' where emp_id_fk='".$emp_id_fk."' AND ir_status='3' AND zp_id_fk='".$zp_id."'");
	...
	{
		$db=new database();
		$ir_status=$db->update("UPDATE psemp_interim_relief SET ir_status='1',delete_status='1' WHERE emp_id_fk='".$emp_id_fk."' AND ir_status='3' AND zp_id_fk='".$zp_id."'");
		
		if($ir_status)
		{
			$_SESSION['msg']='<div class="alert alert-success" style="text-align:center;"><strong> Interim Relief Reject successfully...</strong></div>';
			header('Location:employee_list_for_ir.php?id='.$_POST['ir_gp']);
		}
		else
		{
	...
	{     
		...
		else
		{
			...
			header('Location:employee_list_for_ir?id='.$_POST['ir_gp']);
		}
	}	 
}
