...
$finz_checked = $db->fetch_table("SELECT prd_emp.emp_desig,save.emp_id_fk FROM prd_employee_salary_save 
											as save 
											INNER JOIN prd_employee_master as prd_emp
											ON save.emp_id_fk=prd_emp.emp_id_pk
											WHERE
												save.status_flag = 2 AND save.delete_status=1 
												AND save.is_saved=1 
												AND save.zp_id_fk = '".$dise."' AND (prd_emp.emp_status=1 or prd_emp.emp_status=9)
												AND salary_monthyear='".date('Ym')."' AND requisition_type='".$requisition_type."'
										");
  ...
  #save
{
    cursor: pointer;
		...
		if(count($tch) == count($finz_checked)) {  ?>
            <script>
			//alert ('eee');
