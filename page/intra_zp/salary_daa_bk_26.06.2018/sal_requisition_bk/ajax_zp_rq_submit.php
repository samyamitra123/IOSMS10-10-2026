...
$enc_session=md5('371371371'.$session_token);
 ...
 if($deduction=="no_ovd1") 
	...
	$arr = $db->fetch_table("select ps_id_fk,emp_id_fk from prd_employee_salary_save where emp_id_fk='$emp_id_pk' AND ps_id_fk='".$schcd."' AND salary_monthyear='".date('Ym')."' AND requisition_type='".$requisition_type."'");
	if($arr[0]['emp_id_fk'] && $arr[0]['ps_id_fk']){
		pg_query("begin");
	
		$upd = $db->update("UPDATE prd_employee_salary_save
			SET delete_status='0',is_saved=0
			WHERE emp_id_fk='$emp_id_pk' AND ps_id_fk='".$schcd."' AND salary_monthyear='".date('Ym')."' AND requisition_type='".$requisition_type."'
		");	
	...
	$ins = $db->insert("INSERT INTO prd_employee_salary_save(slno,latestupdate_time, latestupdate_ip_address, ps_id_fk, empcd, 
				...
				hill_allowance, gpf, cpf_deduct, gross_salary, is_saved,conv_allow,overdrawn,salary_type,cause,part_day,gsli,delete_status,consolidated_pay,festival_loan,festival_loan_cause,part_salary_cause,no_salary_cause,interim_relief,requisition_type,gp_id_fk,hra_deduction,hra_loan_cause)
    		VALUES ('$rank', 'now()', '$user_ip','".$schcd."', '$tchcd', 
				...
				'1', '0', '$teacher_id_pk', '$pay_in_band', '$grade_pay', '$hill_allow','$gpf', '0', '$gross', '0','$conv_allow','$overdrawn','$salary_type','$cause_msg','$part_day','$gsli','1','$consolidated_pay','$festival_val',
				'$festival','$part_salary_cause_msg','$no_salary_cause_msg','$interim_relief','".$requisition_type."','0','".$hra_deduction."','".$hra_loan."')");			
	   ...
	   pg_query("begin");
    		...
    		VALUES ('$rank', 'now()', '$user_ip', '".$schcd."', '$tchcd', 
				...
				'$i_tax', '$net', '$bank_ifsc', 'NA', '0', '$p_tax', '$code', 
				'0', '1', '$salary_monthyear', 
				'1', '0', '$teacher_id_pk', '$pay_in_band', '$grade_pay', '$hill_allow','$gpf', '0', '$gross', '0','$conv_allow','$overdrawn','$salary_type','$cause_msg','$part_day','$gsli','1','$consolidated_pay','$festival_val','$festival','$part_salary_cause_msg','$no_salary_cause_msg','$interim_relief','".$requisition_type."','0','".$hra_deduction."','".$hra_loan."')");
				if($ins){					
					pg_query("commit"); 
					$_SESSION['msg']= '<div class="alert alert-success" style="text-align:center"><strong>The salary requisition of '.$tchname.' has been successfully updated.</strong></div>';
					header('location:ps_emp_sal_requisition.php'.$query_string);
