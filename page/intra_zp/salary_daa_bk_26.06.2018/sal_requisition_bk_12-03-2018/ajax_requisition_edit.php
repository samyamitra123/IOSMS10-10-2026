...
function getEmpAmount($type,$dise,$emp_id_pk){
	...
	//Function for interim relief when salary type == 8 by Rupesh Start
	...
	//Function for interim relief when salary type == 8 by Rupesh End
}
...
$arr_gp_val = $db->fetch_table("select interim_relief,emp_pay_in_payband,emp_grade_pay,grade_amount from prd_employee_master as emp
	INNER JOIN prd_dise_gradepay_master as gd 
	ON trim(emp.emp_grade_pay)=gd.grade_code
	where emp_id_pk='".$emp_id_pk."'");         //Checked   //interim_relief, emp_pay_in_payband has been added by Rupesh
...
$paychange = $db->fetch_table("SELECT paychange_id_pk, paychange_ip, paychange_fromdate, paychange_todate, 
       ...
       overdrawn, salary_type, cause,gp_code, part_day,gsli,consolidated_pay,festival_loan,festival_loan_cause from prd_employee_salary_save
			...
			//----------- End Gpf=0 before retirement ---------------------
				...
				if($sal_chk[0]['zp_id_fk'] && $sal_chk[0]['emp_id_fk'] && ($sal_chk[0]['salary_type']=='1' || $sal_chk[0]['salary_type']=='2')){
					...
					// This condition for return full salary of employee and display saved salary of employee in edit mode
					  if($tch[0]['emp_desig']=='999999') //by nirupam     //1120 -> 999999 Final Check 121217
					  // if($tch[0]['emp_desig']!='')
					  {	
					...
					$full_gross_salary = round($full_basic+$full_da+$full_interim_relief+$full_hra+$full_ma+$full_conveyance_allowance+$full_hill_allowance+$emp_bonus);
				     ...
				     if(strtotime($suspend_effect_date)<=strtotime(date('Y-m-d')) && strtotime($suspend_effect_date)>0)
			          {
						 ...
						 $full_grade_pay= $sal_chk[0]['tch_grade_pay'];
			             $full_basic = $full_pay_band+$full_grade_pay;
                         $full_da = round(($full_basic/100)*$da_per);    //round by Rupesh
	           ...
	           if($tch[0]['emp_spouse_res']=='251'){ 
					...
					if($state10=='999999'){									//1120 -> 999999 Final Check 121217
					...
					else
					{
					 ...
					 $full_ptax = 0;
					    ...
					    $full_ptax = empPtax($full_gross_salary);  //Changed from full_gross_salary_sus to full_gross_salary by Rupesh
					  }
					  else{
						  $full_ptax = empPtax($full_gross_salary);
					  }
					...
					$itax=$full_itax;
					//$gsli = $sal_chk[0]['gsli'];
					$gsli = getAmount($dise,$empcd,'gsli','');     //changed Rupesh
					$full_gsli = $gsli;     					   //Added by Rupesh
					...
					//$pfl = getAmount($dise,$empcd,'pfl','');       //changed Rupesh
					
					//$full_pfl = $pfl;                              //Added by Rupesh
					$overdrawn = $sal_chk[0]['overdrawn'];
					//$cooperative_loan = $sal_chk[0]['cooperative_loan'];
				  //  $hbl_loan = $sal_chk[0]['hbl_loan'];
				    $festival_loan = getAmount($dise,$empcd,'festival_loan','');     //changed Rupesh
					...
					//$full_net_salary = $full_gross_salary-$total_deduct;
					 ...
					 $total_deductions = $total_deduct;   //Done by Rupesh
				     $full_net_salary = $full_gross_salary-$total_deduct;
					 //$net_salary_sal_save=$full_net_salary;  //Commented By Rupesh
					...
					$cpf = $sal_chk[0]['cpf'];
					$gross_salary = $sal_chk[0]['gross_salary'];
					//$gpf = $sal_chk[0]['gpf'];                    //changed Rupesh   
					$gpf = getAmount($dise,$empcd,'gpf','');
					//$pfl = $sal_chk[0]['pf_loan'];
					$pfl = getAmount($dise,$empcd,'pfl','');       //changed Rupesh
					$cpf_deduct = $sal_chk[0]['cpf_deduct'];
				...
				else if($sal_chk[0]['salary_type']=='8')
				{
					...
					//$interim_relief = $sal_chk[0]['interim_relief'];
					$interim_relief = getEmpAmount('interim_relief',$dise,$emp_id_pk); 	//Changed by Rupesh
					
					  if($tch[0]['emp_spouse_res']=='251'){
						$full_hra = 0; 		//If obtain any govt. housing scheme.
					}else{
						if($tch[0]['emp_spouse_hra']=='0' || $tch[0]['emp_spouse_hra']=='' || !$tch[0]['emp_spouse_hra']){
							$hra_emp = ($basic/100)*$hra_per;			//changed from $full_basic this by Rupesh
							if($hra_emp > 6000){
								$full_hra = 6000;
							}else{
								  $full_hra = round($hra_emp);
					...
					$hra = $full_hra;    //added this by Rupesh
				if($tch[0]['spouse_medical_allowance']=='1'){
						$ma = 0;
					}else{
						$ma = $max_ma;
					...
					$gross_salary = round($basic+$da+$interim_relief+$hra+$ma+$conveyance_allowance+$cpf+$hill_allowance+$emp_bonus);
					//Deduction part
					//$gpf_amt = ($basic/100)*6;
					...
					$net_salary = $net_salary_sal_save;                //by Rupesh on 28_03_2017 Because javascript is calling $net_salary;
		...
		else
		{
			...
			if($month_by_n == 01)
			{
			$pre_yr = $year_by_n - 1; //echo removed by Rupesh 06.04.2017
			}	
			else
			{
			$pre_yr = $year_by_n;	
...
<?php				     
					  ...
					  {  
					    //$pay_in_band =  $tch[0]['emp_pay_in_payband'];				//Commented by Rupesh
						...
						if($tch[0]['interim_relief'] != '')
						{
							$interim_relief = $tch[0]['interim_relief'];					//Commented by Rupesh
						}
						else
						{
							$interim_relief = 0;
			  ...
			  else
			  {
					...
					$gross_salary = round($pay_in_band+$grade_pay+$da+$interim_relief+$hra+$ma+$conveyance_allowance+$hill_allowance+$emp_bonus);
					//Deduction part
					//$gpf_amt = ($basic/100)*6;
				 ...
				 //  $check_gpf=$db->fetch_table("select tch_first_joining from ehrms_dise_teacher_primary where status=1  AND tchcd='$tchcd' AND schcd='".$dise."'");
					...
					//Commented on 10.07.2017 By Rupesh Start 
					/*if(strtotime($emp_first_join_match_date)>=strtotime(date('Y-m-d')) && strtotime($emp_first_join_match_date)>0)
					{
					$gpf=0;
					}	
					...
					//Commented on 10.07.2017 By Rupesh End
				...
				else
				{
				    if(strtotime($suspend_effect_date)<=strtotime(date('Y-m-d')) && strtotime($suspend_effect_date)>0)
					{
						 ...
						 //commented by Rupesh
						 $gross_salary;   //echo removed by Rupesh 06.04.2017
				...
				/*	if($sal_save[0]['overdrawn']){
	              ...
	              if($tch[0]['emp_desig']=='999999')					//1120 -> 999999 Final Check 121217
				  {
					...
					$pfl = 0;
					$cpf_deduct = 0;
					$gross_salary = round($bas+$da+$interim_relief+$hra+$ma+$conveyance_allowance+$cpf+$hill_allowance+$emp_bonus);
					if($tch[0]['emp_diff_able']=='1')
					{
					$ptax = 0;
					}
					...
					//$cooperative_loan = 0;
						//$hbl_loan = 0;
				   $festival_loan =0;
					$total_deduct = $gpf+$pfl+$cpf_deduct+$ptax+$itax+$gsli+$overdrawn+$festival_loan+$festival_loan;	//echo removed by Rupesh 06.04.2017
					$net_salary = $gross_salary-$total_deduct;
				}
...
input[type="checkbox"] {
	display:inline !important;
                ...
                <input type="hidden" name="gp_id_fk" id="gp_id_fk" value="<?php echo $cryp->encode($dise,4); ?>" >
                        ...
                        <table width="99%">
                            ...
                            <tr>
                                ...
                                <input type="hidden" name="empcd" id="empcd" value="<?php echo $cryp->encode($tch[0]['empcd'],4); ?>" />
                                 <input type="hidden" name="emp_id_pk" id="emp_id_pk" value="<?php echo $cryp->encode($tch[0]['emp_id_pk'],4); ?>" />
                        ...
                        {?>   value="<?php echo 1; ?>"      <?php }?>  />
                            ...
                            <tr>
                            	<td class="row_heading">Grade Pay</td>
                               	<!-- Changed by Rupesh For Grade Pay Value Start-->
                                ...
                                <td><input maxlength="5" style='background-color: #EEE;' type="text" id="grade_pay" name="grade_pay" value="<?php echo round($grade_pay); // echo round($arr_gp_val[0]['grade_amount']); ?>" readonly size="5" /></td>
                                <!-- Changed by Rupesh For Grade Pay Value End-->	
                            </tr>
                            <tr>
                    	...
                    	<table width="99%">
                           ...
                           <!-- <tr>
                            ...
                            <tr>
                                ...
                                <td><input readonly="readonly" maxlength="5" style='background-color: #EEE;' type="text" name="reduct3"  size="5" id="reduct3" value="<?php if($festival_loan==0){echo '0';} else {echo trim($festival_loan);} ?>" autocomplete="off" onKeyUp="return reduc3(this.value);" onkeypress="return keyRestrict(event,'0123456789');"/></td>
                            </tr>
                            <!--<tr>
	 ...
	 if($('.cls8').is(':checked')==true)
		...
		if($('#exist_part_day').val()!=''){
			...
			$('#ovd_is').show();
				...
				if(desig!='1120'){
					...
					$('#pay_in_band').val('<?php echo $full_pay_band; //echo $full_pay_band; ?>');
						...
						//$('#gsli').val('<?php echo $gsli; ?>');   //By Rupesh
						
						$('#gpf').val('<?php echo $full_gpf; ?>');
						//$('#pf_loan').val('<?php echo $full_pfl; ?>');  //By Rupesh
						$('#i_tax').val('<?php echo $full_itax; ?>');
						$('#gsli').val('<?php echo $full_gsli; ?>');    //By Rupesh
						...
						$('.loan_d').show();     //by Rupesh
						$('#total_arrear_deduction').val('<?php echo $arrear_amount; ?>');     //by Rupesh
						$('#reduct3').val('<?php echo $festival_loan; ?>');     //by Rupesh
						$('#deduct').val('<?php echo $total_loan_amount+$full_deductions; ?>');	 //by Rupesh
					...
					$('#pay_in_band').val(0);
						...
						$('#scl').val(0);
						$('#scc').val(0);
						$('#total_arrear_deduction').val(0);     //by Rupesh
						$('#reduct3').val(0);     //by Rupesh
				}
						...
						var gross = parseInt(pay_band)+parseInt(grade_pay)+parseInt(da)+parseInt(interim_relief)+parseInt(hra)+parseInt(ma)+parseInt(conv_allow)+parseInt(bonus);
						$('#gross').val(gross);
						...
						var deduct = parseInt(gpf)+parseInt(gsli)+parseInt(ptax)+parseInt(itax)+parseInt(overdrawn)+parseInt(reduct3)+parseInt(total_loan_amount)+parseInt(ssl)+parseInt(scl)+parseInt(scc)+parseInt(total_arrear_deduction);
						//Loan amount hasbeen added by Rupesh
						//var net = parseInt(gross)-parseInt(deduct);	//commented by Rupesh for calculation
						$('#deduct').val(deduct);   //by Rupesh
			...
			if(id == '2'){
				...
				var desig=<?=$tch[0]['emp_desig']?>;
				$('#part_day_holder').show();
				$('#part_salary_cause_holder').show();    //Added by Rupesh because part salary cause was not coming after changing from full salary from part salary.
				$('#no_salary_cause_holder').hide(); //Added by Rupesh for part to no and no to part again.
				$('#is_reduct').show(); //Added by Rupesh for part to no and no to part again.
				//$('#cause_holder').show();     //Added by Rupesh because part salary cause was coming after changing from full salary from part salary.
				$('#cause_msg').val('');
				$('#limit').text(500);
				//$('#conv_allow').removeAttr('readonly');
				//$('#conv_allow').css('background-color','#FFF');
			...
			$('#pay_in_band').val('<?php echo $full_pay_band; ?>');
						...
						$('#emp_bonus').val('<?php echo $emp_bonus; ?>'); //by Rupesh
						$('#net').val('<?php echo $full_net_salary; ?>');
						$('.loan_d').show();     //by Rupesh
						$('#total_arrear_deduction').val('<?php echo $arrear_amount; ?>');     //by Rupesh
						$('#reduct3').val('<?php echo $festival_loan; ?>');     //by Rupesh
						$('#deduct').val('<?php echo $total_deductions; ?>');	 //by Rupesh
			...
			if(id == '8'){
				...
				$('#deduct').val(0); //by Rupesh
				$('.loan_d').hide();     //by Rupesh
				$('#emp_bonus').val(0); //by Rupesh
				$('#reduct3').val(0);     //by Rupesh
			}
		}
		if($('#exist_part_day').val()==''){ 
			...
			// $('#reduct').val(0);
				...
				$('#deduct').val('<?php echo $total_deductions; ?>');	 //by Rupesh
				$('.loan_d').show();     //by Rupesh
				//$('#net').val('<?php echo $net_salary; ?>');  //by Rupesh
				$('#net').val('<?php echo $net_salary - $total_loan_amount; ?>');  //by Rupesh
		...
		var work_days = $('#working_days').val();
			...
			if($('#working_days').val() >= parseInt($('#month_last_date').val()) || $('#working_days').val() == '0'){
				...
				//alert(145);
					...
					//var hill_allow = $('#hill_allow').val(); //Commented by Rupesh as it is not available.
					...
					//var gross = parseInt(pay_band)+parseInt(consolidated_pay)+parseInt(grade_pay)+parseInt(da)+parseInt(interim_relief)+parseInt(hra)+parseInt(ma)+parseInt(conv_allow)+parseInt(hill_allow);   //Commented by Rupesh as hill_allow is not available.
					var gross = parseInt(pay_band)+parseInt(grade_pay)+parseInt(da)+parseInt(interim_relief)+parseInt(hra)+parseInt(ma)+parseInt(conv_allow);
					//alert(gross);
			...
			if($('#working_days').val() >= parseInt($('#month_last_date').val()) || $('#working_days').val() == '0'){
				...
				// alert(data);
				 $("#mbody").html(data);
					...
					var conv_allow = $('#conv_allow').val();
					var cpf = $('#cpf').val();
					var bonus = $('#emp_bonus').val();     //bonus added by Rupesh
					//var hill_allaw=$('#hill_allow').val();
...
$(document).ready(function(){
	...
	$("#form").submit(function(){
		//alert(pay_in_payband);
		//alert('98765');
		if($("#salary_type").val() != salary_type)
		{
			alert('Please Enter Valid Salary Type.');
			$("#salary_type").focus();
			return false;
		}
		if((salary_type == 2) && ($("#part_salary_cause_msg").val() != part_salary_cause_msg))
		{
			alert('Please Enter Valid Part Salary Cause.');
			$("#salary_type").focus();
			return false;
		}
		if((salary_type == 2) && ($("#working_days").val() != working_days))
		{
			alert('Please Enter Valid Working Days.');
			$("#working_days").focus();
			return false;
		}
		if((salary_type == 2) && ($("#no_salary_cause_msg").val() != no_salary_cause_msg))
		{
			alert('Please Enter Valid No Salary Cause.');
			$("#no_salary_cause_msg").focus();
			return false;
		}
		if($("#reduction").val() != reduction)
		{
		...
		if(Number($("#grade_pay").val()) != Number(grade_pay))
		{
			alert('Please Enter Valid Grade Pay.');
			$("#grade_pay").focus();
			return false;
		}
		if(Number($("#da").val()) != Number(da))
		{
			alert('Please Enter Valid DA.');
			$("#da").focus();
			return false;
		}
		if(Number($("#hra").val()) != Number(hra))
		{
			alert('Please Enter Valid HRA.');
			$("#hra").focus();
			return false;
		}
		if(Number($("#ma").val()) != Number(ma))
		{
			alert('Please Enter Valid MA.');
			$("#ma").focus();
			return false;
		}
		if(Number($("#conv_allow").val()) != Number(conv_allow))
		{
			alert('Please Enter Valid CONV ALLOW.');
			$("#conv_allow").focus();
			return false;
		}
		if(Number($("#interim_relief").val()) != Number(interim_relief))
		{
			alert('Please Enter Valid Interim Relief.');
			$("#interim_relief").focus();
			return false;
		}
		if(Number($("#emp_bonus").val()) != Number(emp_bonus))
		{
			alert('Please Enter Valid Bonus.');
			$("#emp_bonus").focus();
			return false;
		}
		if(Number($("#gpf").val()) != Number(gpf))
		{
		...
		if(Number($("#p_tax").val()) != Number(p_tax))
		{
			alert('Please Enter Valid P.Tax.');
			$("#p_tax").focus();
			return false;
		}
		if(Number($("#gsli").val()) != Number(gsli))
		{
			alert('Please Enter Valid GSLI.');
			$("#gsli").focus();
			return false;
		}
		if(Number($("#reduct3").val()) != Number(reduct3))
		{
			alert('Please Enter Valid Festival Advance Recovery.');
			$("#reduct3").focus();
			return false;
		}
		if(Number($("#total_loan_deduction").val()) != Number(total_loan_deduction))
		{
