...
$arr=$db->fetch_table("SELECT 
							...
							emp.emp_last_name,
							emp.emp_status,
							emp.interim_relief as emp_ir,
							ir.ir_status,
							ir.interim_relief as ir_ir,
							ir.unlock_req
						FROM 
							prd_employee_master emp
						LEFT JOIN
							(SELECT emp_id_fk,ir_status,interim_relief,unlock_req FROM psemp_interim_relief
								WHERE ir_status IN ('1','2','3','4') AND delete_status=1 ) as ir
						ON emp.emp_id_pk=ir.emp_id_fk
						WHERE 
							emp.emp_status='1' AND emp.zp_id_fk='".$_SESSION['location']['district_id']."' 
						ORDER BY emp.emp_id_const");
                            ...
                            <div class="table-responsive">
                                <table width="100%">
									...
									if(count($arr))
                                    {
										foreach($arr as $item)
										{ ?>
											<input type="hidden" id="id<?=$cnt; ?>" name="id" value="<?=$cryptoGraph->encode($item['emp_id_pk'],4);?>"  />
											<tr>
                                                <td id="show"><?= $cnt;?></td>
                                                <td id="emp_id_const_td<?php echo $cnt;?>"><?= $item['emp_id_const']?></td>
                                                <td id="emp_name_td<?php echo $cnt;?>"><?= $item['emp_first_name'].' '.$item['emp_second_name'].' '.$item['emp_last_name']?></td>	
                                                <?php
													if($item['ir_ir']!='')
													{
														$interim_amount= $item['ir_ir'];
													}
													else
													{
														$interim_amount= $item['emp_ir'];
													}
                                                ?>
                                                <td id="emp_ir_td<?php echo $cnt;?>"><?php echo $interim_amount;?></td>
                                                
                                                <!-------------------------------edit column---------------------------------->
                                                
                                                <?php if($item['ir_status']!='' && $item['ir_status']!='1' && $item['unlock_req']!='2')
												{ ?>
													<td>
                                                        <div class="edit_id<?=$cnt?>">
                                                            <img width="25" src="<?= $config['base_url'] ?>themes/default/image/edit_icon.png" style="opacity:0.5;" alt="Edit">
                                                        </div>
                                                    </td>
												<?php }
                                                else
                                                ...
                                                <?php if($item['ir_status']=='')
												{ ?>
													<td>
                                                    	<div class="save_id<?=$cnt?>">
                                                    		<img width="25" src="<?= $config['base_url'] ?>themes/default/image/save_icon.png" alt="save"  style="opacity:0.5;">
                                                        </div>
                                                        </td>
												<?php }
                                                else if($item['ir_status']=='1')
												{ ?>
                                                	<td class="save" id="save">
                                                        <div class="save_id<?=$cnt?>">
                                                        	<a id="ir_save&<? echo $cryptoGraph->encode($item['emp_id_pk'],4).'&'.$cnt; ?>" onClick="save_ir(this.id);"><img id="img_id<?=$cnt?>" width="25" src="<?= $config['base_url'] ?>themes/default/image/save_icon.png" alt="save"></a>
                                                        </div>
                                                    </td>
												<?php }
												else
                                                ...
                                                <!-------------------------------unlock column---------------------------------->
                                                 ...
                                                 <?php if($item['unlock_req']=='0' && $item['ir_status']=='4')
												{ ?>
...
function submit_ir(k)
{
	...
	var emp_name=$('#emp_name_td'+k).text();
	$('#emp_name').html(emp_name+"'s");
	$('#ir_emp_name').val(emp_name);
	var emp_id_const=$('#emp_id_const_td'+k).text();
	$('#emp_id_const').html(emp_id_const);
	var emp_ir=$('#emp_ir_td'+k).text();
	$('#ir_submit').val(emp_ir);
	$('#ir_submit').css('text-align', 'right')
	$('#edit').modal('show');
}
...
function save_ir(k)
{
	...
	var emp_id_pk = res[1];
	var row_count=res[2];
	var emp_full_name=$('#emp_name_td'+row_count).text();
	
	$.post('<?= $config['base_url'] ?>page/intra_zp/salary_daa/interim_relief/ajax_save_ir.php?postemp='+emp_id_pk, function(data)
	{
		if(data==1)
		{           
			$('#session_msg_print').html('');
			$('.border_val').html('<div class="alert alert-success" style="text-align:center"><strong>The Interim Relief of '+emp_full_name+' has been successfully saved.</strong></div>');
...
function send_ir(k)
{
	...
	$.post('<?= $config['base_url'] ?>page/intra_zp/salary_daa/interim_relief/ajax_send_ir.php?postemp='+emp_id_pk, function(data)
	{
		if(data==1)
		{           
			$('#session_msg_print').html('');
			$('.border_val').html('<div class="alert alert-success" style="text-align:center"><strong>The Interim Relief of '+emp_full_name+' has been successfully send.</strong></div>');
			$('.save_id'+row_count).html('<img width="25" src="<?= $config['base_url'] ?>themes/default/image/saved_icon.png" alt="save">');
			$('.edit_id'+row_count).html('<img width="25" src="<?= $config['base_url'] ?>themes/default/image/edit_icon.png" style="opacity:0.5;" alt="Edit">');
