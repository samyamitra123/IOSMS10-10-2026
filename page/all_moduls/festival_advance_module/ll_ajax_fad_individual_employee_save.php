...
if($emp_id_pk!="")
{
	...
	{
		$query_update=$db->update(" UPDATE prd_festival_advance_employee_details fad SET festival_advance_status=2 
									FROM prd_employee_master emp
									WHERE emp.emp_id_pk=fad.emp_id_fk AND emp.zp_id_fk='".$zp_id_fk."' AND emp_id_fk='".$emp_id_pk."' 
									AND festival_advance_status=1 AND substr(fad_monthyear,1,4)='".date('Y')."'");
		
		$count_emp_not_saved=$db->fetch_table("SELECT count(fad.emp_id_fk) as total_not_saved FROM prd_festival_advance_employee_details fad
												INNER JOIN prd_employee_master emp
												ON emp.emp_id_pk=fad.emp_id_fk
												WHERE fad.festival_advance_status=1 AND emp.zp_id_fk='".$zp_id_fk."'");
	}
	else if($logged_user=='DA')
	{
		...
		$count_emp_not_saved=$db->fetch_table("SELECT count(fad.emp_id_fk) as total_not_saved FROM prd_festival_advance_employee_details fad
												INNER JOIN prd_employee_master emp
												ON emp.emp_id_pk=fad.emp_id_fk
												WHERE fad.festival_advance_status=1 AND emp.ps_id_fk='".$ps_id_fk."'");
	}
	else if($logged_user=='GP')
	{
		...
		$count_emp_not_saved=$db->fetch_table("SELECT count(fad.emp_id_fk) as total_not_saved FROM prd_festival_advance_employee_details fad
												INNER JOIN prd_employee_master emp
												ON emp.emp_id_pk=fad.emp_id_fk
												WHERE fad.festival_advance_status=1 AND emp.gp_id_fk='".$gp_id_fk."'");
	}
}
