...
$db = new database();	
	...
	$sql_query=$db->fetch_table("SELECT 
											block_bill_pk,
											bill_no,
											bill_entry_time,
											salary_monthyear 
									FROM prd_block_bill_details
									WHERE ps_id_fk='".$_SESSION['location']['ps_id']."' and bill_no='".$bill_no."' and salary_monthyear= '".$month_year."' AND requisition_type='".$requisition_type."' AND status='1'");
...
$db = new database();
		...
		$teacher_dtls=$db->fetch_table("SELECT  
											...
											AND fad.bill_id_fk='".$sql_query[0]['block_bill_pk']."'
...
inner join (select salary_monthyear,status_flag,block_code,empcd,ps_id_fk from prd_employee_salary_save where salary_monthyear='".$sql_query[0]['salary_monthyear']."') as sal 
on CAST(emp.empcd AS text)=sal.empcd 
and emp.ps_id_fk=sal.ps_id_fk 
inner join prd_dise_admin ad on ad.block_code=CAST(ps.ps_code as character varying) 
where sal.ps_id_fk='".$_SESSION['location']['ps_id']."' 
and sal.status_flag='3' and emp.emp_status='1' OR emp.emp_status='9'");
...
//$deduc_amt=($i_tax)+($p_tax)+($hra_deduction)+($gsli);
//$deduc_total= ($deduc_amt)+($ag_total);
//$net_amount=($ern_amount)-($deduc_total);
//$net_amount=($net_amount)+($hra_deduction);
