...
{	
	...
	return $dob=='01/01/1900'?'':$dob;
		...
		$financial_year = (date("m") >= 4) ? (date("Y")."-".(date("Y")+1)) : ((date("Y")-1)."-".date("Y"));
	 	...
	 	/*$bill_det=$db->fetch_table("select bill_no,bill_entry_time,salary_monthyear from prd_block_bill_details where block_code='".$schcd."' and bill_no='".$bill."' and salary_monthyear= '".$yemo."' AND requisition_type='".$requisition_type."' AND status='1'");*/
		

		$bill_det=$db->fetch_table("
									SELECT 
											bill.bill_no,
									...
									WHERE bill.block_code='".$_SESSION['location']['block_code']."' and bill.bill_no='".$bill."' 
	...
	for($i=0;$i<count($bill_det);$i++)
	{
		$all_fad_id.=$bill_det[$i]['festival_advance_id_pk'];
		
		if($i!=count($bill_det)-1)
		{
 ...
 $teacher_sql="select  distinct(emp.emp_id_pk),
	...
	where
		fad.festival_advance_id_pk in (".$all_fad_id.")
...
$xml =  new DOMDocument("1.0","UTF-8");
		...
		foreach ($sql_teacher as $key){
			...
			if($key['festival_advance_total_amount'] > 0){
				...
				$amount= $xml->createElement('Amount',$key['festival_advance_total_amount']);
