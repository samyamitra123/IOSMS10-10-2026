...
$db = new database();	
	...
	$sql_query=$db->fetch_table("
									SELECT 
											...
											fad_type.festival_advance_id_pk 
									FROM prd_block_bill_details bill
									INNER JOIN prd_festival_advance_employee_details fad_type
									ON bill.block_bill_pk=fad_type.bill_id_fk
									...
									AND bill.status='1' AND fad_type.festival_advance_status='5'
	");
	
	
	for($i=0;$i<count($sql_query);$i++)
	{
		$all_fad_id.=$sql_query[$i]['festival_advance_id_pk'];
		
		if($i!=count($sql_query)-1)
		{
			$all_fad_id=$all_fad_id.",";
		}
	}
...
/*$teacher_dtls=$db->fetch_table("select distinct(emp.emp_system_code, emp.empcd), 
	...
	where 
		...
		LEFT JOIN prd_festival_advance_employee_details fad 
		ON fad.emp_id_fk=emp.emp_id_pk
		WHERE 
		emp.emp_status in('1','9') 
		AND fad.festival_advance_status='5'
		AND gp.block_id_fk='".$_SESSION['location']['block_id']."'
		AND fad.festival_advance_id_pk in (".$all_fad_id.")
		ORDER BY emp.gp_id_fk ASC");		
/************************************************************************************/											
...
//	     $gross_salary=($gross_salary)+ ($teacher_row['gross_salary']);
		 ...
		 $total_fad_amount=($total_fad_amount)+ ($teacher_row['festival_advance_total_amount']); 
	 }
...
$ern_amount=$total_fad_amount;
$net_amount=$total_fad_amount;
$ag_total=0;
$i_tax=0;
$p_tax=0;
