...
$psemp_ps_profile=$db->fetch_table("select ddo_code,treasury_code from psemp_ps_profile where ps_id_fk='".$_SESSION['location']['ps_id']."'");
	...
	$bill_det=$db->fetch_table("SELECT 
									...
									WHERE ps_id_fk='".$_SESSION['location']['ps_id']."' and bill_no='".$bill."' and salary_monthyear= '".$yemo."' AND requisition_type='".$requisition_type."' AND status='1'");
 ...
 $teacher_sql="select  distinct(emp.emp_id_pk),
	...
	where
		fad.festival_advance_status='5'
		AND emp.emp_status in('1','9')
		AND emp.ps_id_fk='".$_SESSION['location']['ps_id']."'
		AND fad.bill_id_fk='".$bill_det[0]['block_bill_pk']."'
