...
{ 
	$find_bill = $db->fetch_table("
									SELECT * FROM prd_block_bill_details
									WHERE block_code = '".$_SESSION['user_info']['stake_user']."' AND ropa_status='1'
									AND salary_monthyear ='".$yemo."' AND status='1' AND requisition_type='".$requisition_type."' ORDER BY oid DESC
	...
	$checking=	$db->fetch_table("
				SELECT count(*) FROM prd_employee_arrear 
				WHERE ps_id_fk ='".$_SESSION['location']['block_code']."'
				AND is_saved='1' AND delete_status='1' 
				AND bill_id_fk='0'
			");	
...
{ 
	...
	$find_bill = $db->fetch_table("
									SELECT * FROM prd_block_bill_details
									WHERE zp_id_fk = '".$_SESSION['location']['district_id']."' AND ropa_status='1'
									AND salary_monthyear ='".$yemo."' AND status='1' AND requisition_type='".$requisition_type."' ORDER BY oid DESC
...
$(document).ready(function() {
	...
	$('#submit-text').click(function(){
		...
		var new_bill_date=$('#bill_date').val();
			...
			else if(user=='EO' || user=='BDO' || user=='FC&CAO' )
			{
			...
			else if( user=='BDO')
			{
				//alert(11);
				//$('#ifms_form').show();
				$.post('<?php echo $config['base_url'] ?>page/all_moduls/arrear/text_file/ajax_links.php',$(this).closest("form").serialize(), function(data){
				$('.ajax_text_link').html(data);
				$('#submit-text').hide();
...
?>
<?php //var_dump($crypto->decode(1,4)); die; ?>
...

{
	if( $logged_user=='FC&CAO')
	{
		...
		$checking=	$db->fetch_table("
					...
					AND is_saved='1' AND delete_status='1' 
					AND bill_id_fk='0' AND zp_emp_type='".$bill_type."'
				");	
		...
		$get_bill_serial_no = $db->fetch_table("SELECT max(bill_serial_no) AS max_bill_serial_no FROM 
									 ...
									 if($checking[0]['count']>='1' )
										...
										AND status='1' AND ropa_status='1'
...
<?php  }
 ...
 <div class="row mb-3">
    ...
    <div class="col-sm-3">
      <input type="text" class="form-control upper_case" name="bill" id="bill" placeholder=" Arrear Bill Number" onKeyPress="return keyRestrict(event,'abcdefghijklmnopqrstuvwxyz0123456789/');" value="<?= $bill_no;?>"   >
    </div>
    <div class="col-sm-3"></div>
    </div>
...
if($logged_user=='EO')
{
	$data = $db->fetch_table("SELECT * FROM prd_block_bill_details
							WHERE ps_id_fk = '".$_SESSION['location']['ps_id']."'
							AND salary_monthyear ='".date('Ym')."' AND ropa_status='1'
