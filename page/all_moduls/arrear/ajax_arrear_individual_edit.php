...
if($logged_user=='BDO')
{
	$emp_arrear_fetch=$db->fetch_table("
										...
										arrear_from_date, ropa_status
										 FROM prd_employee_arrear WHERE emp_id_fk='".$emp_id_pk."' AND gp_id_fk='".$dise."' AND status_flag='1' AND delete_status='1' AND salary_monthyear is null AND ropa_status='0'
...
else if($logged_user=='EO')
{
	$emp_arrear_fetch=$db->fetch_table("
										SELECT
											...
											arrear_to_date,
											arrear_from_date, ropa_status
										 FROM prd_employee_arrear WHERE emp_id_fk='".$emp_id_pk."' AND ps_id_fk='".$_SESSION['location']['ps_id']."' AND status_flag='1' AND delete_status='1' AND salary_monthyear is null AND ropa_status='0'
...
else if($logged_user=='FC&CAO')
{
	...
	$emp_arrear_fetch=$db->fetch_table("
										SELECT
											...
											arrear_to_date,
											arrear_from_date, ropa_status
										 FROM prd_employee_arrear WHERE emp_id_fk='".$emp_id_pk."' AND zp_emp_type='".$emp_type."' AND zp_id_fk='".$_SESSION['location']['district_id']."' AND status_flag='1' AND delete_status='1' AND salary_monthyear is null AND ropa_status='0'
				...
				{ 
					...
					{?>
                        ...
                        <tr style="background-color: rgb(221, 247, 255);" id="tr<?=$cnt?>">
                            ...
                            <td><input maxlength="6"  type="text" name="grade_pay[]" id="grade_pay<?=$cnt ?>" value="<?php echo round($item['grade_pay']); ?>"  size="6" <?php echo $condition_var;?> /></td>
