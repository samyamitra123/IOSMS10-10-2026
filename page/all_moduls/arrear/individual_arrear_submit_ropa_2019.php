...
/*************************************** END *****************************************/
 ...
 ////////////////////////////////////////////////////////////////////////////// Duplicate Employee Checking //////////////////////////////////////////////////////////////////////////////////
	...
	{
		$upd=$db->update("UPDATE prd_employee_arrear SET delete_status='0' where emp_id_fk='".$emp_ids[0]."' AND gp_id_fk='".$gp_ids[0]."' 
		AND ropa_status='1'and salary_monthyear is null ");
	}
	else if($logged_user=='EO' && $_POST['emp_id_fk']!="")
	{
		$upd=$db->update("UPDATE prd_employee_arrear SET delete_status='0' where emp_id_fk='".$emp_ids[0]."' AND ps_id_fk='".$_SESSION['location']['ps_id']."' 
		AND ropa_status='1' and salary_monthyear is null ");
	}
	else if($logged_user=='FC&CAO' && $_POST['emp_id_fk']!="")
	{
		$upd=$db->update("UPDATE prd_employee_arrear SET delete_status='0' where emp_id_fk='".$emp_ids[0]."' AND zp_id_fk='".$_SESSION['location']['district_id']."' 
		AND ropa_status='1'and salary_monthyear is null ");
	}
		...
		{	
			...
			if($logged_user=='BDO')
			{
				...
				$ins=$db->insert("
									...
									arrear_to_date,
									arrear_from_date,
									ps_id_fk,zp_id_fk,zp_emp_type, ropa_status, ropa_level)
									VALUES
									( 'now()',
									'".$_SERVER['REMOTE_ADDR']."',
									'".$str."',
									...
									'".$arr_to_date."',
									'".$arr_fm_date."',
									'0','0','0','".$ropa_status."','".$ropa_level[$i]."')");
			...
			else if($logged_user=='EO')
			{
				...
				$ins=$db->insert("
									...
									arrear_to_date,
									arrear_from_date,
									ps_id_fk,zp_emp_type, ropa_status, ropa_level)
									VALUES
									( 'now()',
									'".$_SERVER['REMOTE_ADDR']."',
									'',
			...
			else if($logged_user=='FC&CAO')
			{
				...
				$ins=$db->insert("
									...
									ps_id_fk,
									zp_id_fk,
									zp_emp_type, ropa_status, ropa_level,allowance)
									VALUES
									( 'now()',
									'".$_SERVER['REMOTE_ADDR']."',
									'',
									...
									'".$ropa_status."','".$ropa_level[$i]."', '".$allows[$i]."')");
