...
//echo "<pre>";
//print_r($_REQUEST);
//$ps_code = $_SESSION['user_info']['stake_user'];
//echo $block_code;
$mo = $_GET['mo'];
$ye = $_GET['ye'];
...
if($logged_user=='BDO')
{
	...
	$arr2 = $db->fetch_table("
		...
		WHERE block_code ='".$_SESSION['user_info']['stake_user']."' AND
			...
			bill_no = '".$bill."' AND requisition_type='".$requisition_type."'
	");
...
if($logged_user=='EO')
{
	...
	$arr2 = $db->fetch_table("
		SELECT
			...
			salary_monthyear
		FROM prd_block_bill_details
		WHERE ps_id_fk = '".$_SESSION['location']['ps_id']."' AND
			salary_monthyear = '".$ye.$mo."' AND
			bill_no = '".$bill."' AND requisition_type='".$requisition_type."'
	");
		...
		$arr4 = $db->fetch_table("Select treasury_block_code,ddo_code from prd_dise_admin where block_code = '3299001'");
...
if($logged_user=='BDO')
{
	$arr=$db->fetch_table("
						...
						ON
								gp.gp_id_pk=emp.gp_id_fk 
						LEFT JOIN
								prd_employee_arrear arrear 
						ON 
								arrear.emp_id_fk=emp.emp_id_pk
						INNER JOIN 
								prd_dise_admin as admin 	
						ON 
								admin.block_code=arrear.block_code
						WHERE substring(CAST(gp.gp_code AS text),0,8) like '".$_SESSION['user_info']['stake_user']."%' AND (emp.emp_status='1' OR emp.emp_status='9') AND arrear.status_flag='3'
						AND arrear.salary_monthyear = '".$yemo."' AND arrear.delete_status='1' AND arrear.bill_id_fk='".$arr2[0]['block_bill_pk']."'
	...
	$arr = $db->fetch_table("
		SELECT	
					...
					INNER JOIN 
							prd_dise_admin as admin 	
					ON 
							admin.block_code=CAST(ps.ps_code as character varying)
					WHERE ps.ps_id_pk='".$_SESSION['location']['ps_id']."' AND (emp.emp_status='1' OR emp.emp_status='9') AND arrear.status_flag='3'
					AND arrear.salary_monthyear = '".$yemo."' AND arrear.delete_status='1' AND arrear.bill_id_fk='".$arr2[0]['block_bill_pk']."'
	");
...
if(count($arr3)>0 || $arr4>0){
	...
	header('Location:'.$config['base_url'].'page/all_moduls/arrear/text_file.php');
	exit(1);
}
//echo $itax_head;
//echo $ptax_head;
