...
else if($stake_level=='35')
{
	//echo 13; die;
	$id="gp_id_fk='".$gp_id."'";
	$s_id=$crypto->encode($gp_id,4); 
}
else if($stake_level=='51')
{
	//echo 13; die;
...
/*$emp_status_check=$db->fetch_table("select emp_status,emp_id_pk,emp_id_const from prd_employee_master where emp_status in('6') and emp_id_pk='".$emp_id_pk."' AND ps_id_fk='".$_SESSION['location']['ps_id']."' order by emp_first_name");*/
	$arr=$db->fetch_table("select emp_first_name,emp_status,emp_second_name,emp_last_name,emp_form_status,emp_id_pk,emp_desig,emp_status,emp_id_const,update_status from prd_employee_master where emp_status in('1','9')  AND update_status in('2','3','4') AND ".$id." order by emp_first_name");
...
<td class="edit">
  <a href="javascript:void(0)" 
     class="open-edit" 
     id="<?php echo $crypto->encode($item['emp_id_pk'],4) ?>">
...
<div class="modal fade" id="edit_data" tabindex="-1" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content" style="width: 850px; margin-left: -1.5%;">
      <div class="modal-header">
        <h4 class="modal-title" id="myModalLabel">Employee Profile Update</h4>
...
<!---------------------------------------------MODAl------------------------------------->
<form name="unlock_salary" id="unlock_salary" action="up_epension_approve_reject.php" method="post">
    <div class="modal fade bs-example-modal-sm" id="approve_reject" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true" data-modal-parent="#schprfModal">
        <div class="modal-dialog modal-sm">
            <div class="modal-content" >
                ...
                <div class="modal-body"> 
                    <p class="alert alert-warning"><strong><i class="fa fa-exclamation-triangle"></i> Are You Sure To <span id="approve_show" style="display:none;"> Approve</span><span id="reject_show" style="display:none;">Reject</span></strong><strong> Employee Profile?</strong></p>
                    <input type="hidden" id="flag" name="flag"/>
                    <input type="hidden" id="emp_id_k" name="emp_id_k"/>
                     <input type="hidden" id="gp_id" name="gp_id" value="<?=$s_id;?>"/>
                </div>
                <div class="modal-footer">
                    <div class="btn-group">
...
$(document).ready(function() {
  $(document).on('click', '.open-edit', function(e) {
    ...
    $("#mbody").html("<div class='text-center p-3'>Loading...</div>");

    $.ajax({
      type: "POST",
      // url: "<?= $config['base_url'] ?>page/all_moduls/update_epension_emp_profile/up_ajax_employee_view.php?id=" + link,
      url: "<?= $config['base_url'] ?>page/all_moduls/update_epension_emp_profile/up_ajax_employee_view.php?id===AUUJlRVtmSrJGbSVXTWJVU",
      headers: { 'X-Requested-With': 'XMLHttpRequest' },
      success: function(data) {
        console.log(data);
        $("#mbody").html(data);
        var modal = new bootstrap.Modal(document.getElementById('edit_data'));
        modal.show();
      },
      error: function(e) {
        // $("#mbody").html("<div class='alert alert-danger'>Failed to load data.</div>");
        var modal = new bootstrap.Modal(document.getElementById('edit_data'));
        modal.show();
        console.log(e);
      }
