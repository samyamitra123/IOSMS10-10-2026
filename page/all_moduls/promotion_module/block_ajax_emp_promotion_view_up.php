...
$promotion_data = $db->fetch_table("SELECT emp_first_name, emp_second_name, emp_last_name, prd_employee_promotion_details.emp_id_fk,
 prd_employee_promotion_details.emp_pay_scale, prd_employee_promotion_details.emp_pay_in_payband,
  prd_employee_promotion_details.emp_grade_pay, prd_employee_promotion_details.emp_desig, prd_employee_promotion_details.status,
   prd_employee_promotion_details.emp_pay_band, prd_employee_promotion_details.effective_date,prd_employee_promotion_details.dop_doi,approval_status, description ,increment_type,cas_type,prd_employee_promotion_details.increment_amount
FROM prd_employee_promotion_details 
INNER JOIN prd_employee_master ON prd_employee_master.emp_id_pk = prd_employee_promotion_details.emp_id_fk 
INNER JOIN prd_dise_code_master ON prd_dise_code_master.code = CAST(prd_employee_promotion_details.emp_desig as character varying)
									WHERE
        ...
        <div id="attraction">
			...
			<table border="0" width="100%" class="ajax_table">
				<tr>
					...
					<td><?php echo $desig[0]['description']; ?></td>
                    <? } else{?>
                    <td class="text_r"> </td>
					<td></td>
                    <? } ?> 
...
<td class="text_r">Effective Date :</td>
            ...
            <div align="center">
				...
				<? if($approval_status=='3' )
				 {?> 
                		<a class="btn btn-success" data-toggle="modal"  data-target="#approve_k">APPROVE</a>
                <? }?>
				<? if( $approval_status=='2' ||$approval_status=='3') {?> 
                         <a class="btn btn-danger" data-toggle="modal"  data-target="#reject_k">REJECT</a>
                <? }?>
</div>
        </div>
...
?>                                           
       ...
       <input type="hidden" name="emp_status" id="emp_status" value=<?php echo $status; ?> />
