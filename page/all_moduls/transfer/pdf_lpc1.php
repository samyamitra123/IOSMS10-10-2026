...
if($transfer_update && $emp_salary_stop_update)
{
    ...
    if($logged_user=='BDO')
    {
	    ...
	    $email=$info[0]['email_id'];
			    ...
			    INNER JOIN prd_monthly_salary_archive_final as final ON final.emp_id_fk=tr.emp_id_fk where tr.gp_id_fk='".$gp_id."'  and final.salary_monthyear='".$lpc_monthyear."' and final.emp_id_fk='".$emp_id."'"); 
    }
    else if($logged_user=='EO')
    {
	    $db=new database();
	...
	from psemp_ps_profile where ps_id_fk='".$_SESSION['location']['ps_id']."'");
	    ...
	    $email=$info[0]['email'];
		      ...
		      where final.ps_id_fk='".$_SESSION['location']['ps_id']."'  and final.salary_monthyear='".$lpc_monthyear."' and final.emp_id_fk='".$emp_id."'");
    }
	...
	FROM prd_employee_transfer as tr
	INNER JOIN prd_monthly_salary_archive_final as final ON final.emp_id_fk=tr.emp_id_fk 
	where final.zp_id_fk='".$_SESSION['location']['district_id']."'  and final.salary_monthyear='".$lpc_monthyear."' and final.emp_id_fk='".$emp_id."'");
	}
    ...
    {
    $name=fun_dist($_SESSION['location']['district_id']); 
 ...
 if($logged_user=='BDO')
	    ...
	    {	
		...
		else if($logged_user=='AEO')
		    {
				...
				{	
					$g=fun_dis($_SESSION['location']['district_id']).' proceeding on to '.fun_ps($trsfer_gp_id_fk[0]['transfer_ps_id_fk']).',under '.fun_dis($trsfer_gp_id_fk[0]['transfer_district_id_fk']).','.fun_dis($trsfer_gp_id_fk[0]['transfer_district_id_fk']).'.';
					//$trsfer_admin_name="Block Development Officer,".fun_block($trsfer_gp_id_fk[0]['transfer_block_id_fk']).','.fun_dis($trsfer_gp_id_fk[0]['transfer_district_id_fk']).'';
				...
				{
					$g=fun_dis($_SESSION['location']['district_id']).' proceeding on to '.fun_dis($trsfer_gp_id_fk[0]['transfer_district_id_fk']).',under,'.fun_dis($trsfer_gp_id_fk[0]['transfer_district_id_fk']).'.';
     ...
     <table border="0" width="100%"  style="vertical-align: bottom;font-size: 14px;border-collapse: collapse;">
	  ...
	  <table border="0" width="100%"  style="vertical-align: bottom;font-size: 14px;border-collapse: collapse;">
		 <tr>
			    <td>2.He has been paid up to '.fun_lpc($lpc_month)."&nbsp;".$lpc_year.' at the following rates In the revised Pay Band
    of Rs.'.$trsfer_gp_id_fk[0]['pay_payband'].'.00/-+ Grade Pay Rs.'.$trsfer_gp_id_fk[0]['tch_grade_pay'].'.00 ('.fun_payband($trsfer_gp_id_fk[0]['emp_pay_band']).').Scale:'.fun_payscale($trsfer_gp_id_fk[0]['emp_pay_scale']).'
		    ...
		    <tr>
			    <td>3.He made over the Official Charges on '.date('d-m-Y', strtotime($trsfer_gp_id_fk[0]['transfer_date'])).'.
			    </td>
		    </tr>
      </table>',2);
    ...
    $mpdf->WriteHTML('<div class="segment"><br></br>
     <table border="0" width="100%"  style="vertical-align: bottom;font-size: 14px;border-collapse: collapse;">
		    <tr>
		       ...
		       <tr>
				    <td style="width:50%">5. His Permanent Salary Bank A/C No.:'.$trsfer_gp_id_fk[0]['accountno'].'.IFSC CODE:'.$trsfer_gp_id_fk[0]['bank_ifsc'].', He is entitled to draw as mentioned above.</td>
		    </tr>
			    <tr>
				    <td style="width:50%">6.He has been sanctioned --- Leave Proceeding joining time for --- days.</td>
			    </tr>
			    <tr>
				    <td style="width:50%">7.His date of next Increment is:-'.$emp_next_increment_date.'</td>
			    </tr>
			    <tr>
				    <td style="width:50%">8. He enjoyed casual leave for Current Calendar year '.$lpc_year.' :----</td>
			    </tr>
			    <br><br><br></br></br></br>
			    <tr>
