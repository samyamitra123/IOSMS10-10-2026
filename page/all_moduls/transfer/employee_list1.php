...
<td>
    ...
    { ?>
    <button type="button" class="btn btn-danger emp_transfer" id="trans_emp<?=$item['emp_id_pk']?>" data-toggle="modal"  value="<?=$cryptoGraph->encode($item['emp_id_pk'],4)?>" disabled="disabled" style="display: block;">Transfer</button>
    <?php 
    } 
    else 
    {?>
    <button type="button" class="btn btn-danger emp_transfer" id="trans_emp<?=$item['emp_id_pk']?>" data-toggle="modal"  value="<?=$cryptoGraph->encode($item['emp_id_pk'],4)?>" style="display: block;" >Transfer</button>
    <?php 
    }  
	...
	$salary_check=$db->fetch_table("
									SELECT status_flag FROM prd_employee_salary_save WHERE emp_id_fk='".$item['emp_id_pk']."'  AND salary_monthyear='".date('Ym')."' AND is_saved='1' AND delete_status='1'
									");
...
<?php } ?>
  ...
  <!-- <button type="button" class="btn btn-default" id="view_enable<?=$item['emp_id_pk']?>" <?php if ($item['transfer_emp_status']=='0' || $item['lpc_status']=='1') { ?> style="display:block;border:none; background-color:transparent;" <?php } else { ?> style="display:none;border:none; background-color:transparent;" <?php } ?>><a onClick="view_details(this.id);" id="transfer_details<?=$item['emp_id_pk']?>"> <img src="<?= $config['base_url'] ?>themes/default/image/view_icon.png" width="25" alt="view" title="Transfer Details" style="padding-top:6px;" /></a></button>
 

    	<button type="button" class="btn btn-default" id="view_disable<?=$item['emp_id_pk']?>" <?php if ($item['transfer_emp_status']=='0' || $item['lpc_status']=='1') { ?> style="display:none;border:none; background-color:transparent;" <?php } else { ?> style="display:block;border:none; background-color:transparent;" <?php } ?>><img src="<?= $config['base_url'] ?>themes/default/image/view_icon.png" width="25" alt="view" title="Transfer Details" style="padding-top:6px; opacity:0.5;" /></button>-->
    </div>
 </td>  
</tr>
<? $cnt+=1; }} else { ?>
...
<div class="modal fade bs-example-modal-sm" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="min-height:1000px;"data-keyboard="false" data-backdrop="static">
...
<form action="pdf_lpc1.php" method="post" onSubmit="modal_off();">
  ...
  <div class="modal-dialog modal-sm">
    <div class="modal-content" >
     <div class="modal-header">
      ...
      <p class="alert alert-warning"><strong><i class="fa fa-exclamation-triangle"></i> LPC of previous month is being generated. <br/> Salary of Current month will be stopped after LPC generation from this <?php if($logged_user=='BDO') { ?>GP<?php }else if($logged_user=='EO'){ ?>PS<?php } ?>. <br/> Do you want to generate LPC?</strong></p>
