...
if($logged_user=='zpacc' || $logged_user=='DA' || $logged_user=='GP')
{
	...
	if($logged_user=='zpacc')
	{
		...
		$zp_details_fetch=$db->fetch_table(" SELECT tan_no,vill_name,police_station_name,pin_code FROM zpemp_zp_profile WHERE district_id_fk='".$_SESSION['location']['district_id']."' ");
	...
	if($logged_user=='DA')
	{
		...
		$bill_clause=$stake_clause;
		$ps_details_fetch=$db->fetch_table(" SELECT vill_name,police_station_name,pin_code FROM psemp_ps_profile WHERE ps_id_fk='".$_SESSION['location']['ps_id']."' ");
		//$tan_no=$ps_details_fetch[0]['tan_no'];
		$village_name=$ps_details_fetch[0]['vill_name'];
		$police_station=$ps_details_fetch[0]['police_station_name'];
		$pin_code=$ps_details_fetch[0]['pin_code'];
	}
	if($logged_user=='GP')
	{
		...
		$gp_details_fetch=$db->fetch_table(" SELECT vill_name,police_station,pin_code FROM prd_gp_profile WHERE gp_id_fk='".$_SESSION['location']['gp_id']."'");
		$village_name=$gp_details_fetch[0]['vill_name'];
		$police_station=$gp_details_fetch[0]['police_station'];
		$pin_code=$gp_details_fetch[0]['pin_code'];
	}
	...
	if($type=='indi')
	{
		...
		$emp_details_fetch=$db->fetch_table("
												...
												WHERE emp.emp_id_pk='".$emp_id."' and emp.emp_status in('1','9','2')
	...
	else if($type=='all')
	{
		...
		$Query = "SELECT
				...
				LEFT JOIN prd_employee_master emp
				ON emp.emp_id_pk=sal.emp_id_fk AND ".$stake_join."
				WHERE emp.emp_status in('1','9','2')
				AND sal.status_flag in (3,4) AND sal.delete_status='1'
				AND sal.salary_monthyear='".$year.$month."' AND sal.is_saved='1'
				AND sal.lock_status='0' AND sal.ropa_status='".$ropa_status."'
				AND emp.".$stake_clause." ORDER BY emp.emp_first_name";
		//print_r($Query); exit;
			...
			<table style="margin-top:-5%">
				<tr>
					...
					<td class="he2" style="text-align: center;width: 480px;">',2);
						if($logged_user=='GP')
						{
							$mpdf->WriteHTML('<p class="department" style="font-size: 15px;margin: 0px;padding:0px;font-weight:bold;">'.$_SESSION['location']['gp_name']."," .$_SESSION['location']['block_name'].",".$_SESSION['location']['district_name'].'</p>',2);
						}
						else if($logged_user=='DA')
						{
							$mpdf->WriteHTML('<p class="department" style="font-size: 15px;margin: 0px;padding:0px;font-weight:bold;">'. $_SESSION['location']['ps_name'].",".$_SESSION['location']['district_name'].'</p>',2);
						}
						else if($logged_user=='zpacc')
						{
							$mpdf->WriteHTML('<p class="department" style="font-size: 15px;margin: 0px;padding:0px;font-weight:bold;">'. $_SESSION['location']['district_name'].' ZILLA PARISHAD</p>',2);
						}
					$mpdf->WriteHTML('</td>
				...
				<table width="100%" style="border: 1px solid #666; border-radius: 5px; -moz-border-radius: 5px; -webkit-border-radius: 5px;font-size: 12px;vertical-align: bottom;">
					...
					<tr>
						...
						<td style="text-align:right;padding-right: 2%; border-right:1px solid #666;width:25%">
							<table style="font-size:12px;" width="150px;">',2);
								if($key['pf_loan']!='0'  && $key['pf_loan']!='' && ($logged_user=='GP' || $logged_user=='DA'))
								{
									$mpdf->WriteHTML('<tr><td style="font-weight:bold">PF LOAN</td><td align="right">'.$key['pf_loan'].'</td></tr>',2);
									$pf_loan=$key['pf_loan'];
								}
								else if($key['total_loan_deduction']!='0'  && $key['total_loan_deduction']!='' && $logged_user=='zpacc')
								{
