...
if($logged_user == 'BDO')
{
	...
	$arr=$db->fetch_table("select 
								...
								WHERE emp.emp_status in(1) AND emp.gp_id_fk='".$cryptoGraph->decode($_GET['gp_id_fk'],4)."' AND annu.status in('1','2') AND emp.emp_id_pk = annu.emp_id_fk AND  annu.effective_monthyear='".date('Ym')."' ORDER BY emp.emp_id_pk DESC
									");
}
else if($logged_user == 'EO')
{
	$arr=$db->fetch_table("select 
								...
								INNER JOIN prd_employee_annual_increment_details AS annu 
								ON annu.emp_id_fk = emp.emp_id_pk 
								WHERE emp.emp_status in(1) AND ps.ps_id_pk='".$_SESSION['location']['ps_id']."' AND annu.status in('1','2') AND emp.emp_id_pk = annu.emp_id_fk AND  annu.effective_monthyear='".date('Ym')."' ORDER BY emp.emp_id_pk DESC
									");

}
else if($logged_user == 'zpacc' || $logged_user == 'zpsec')
{
	$Query = "select 
								emp.emp_first_name,
								emp.emp_second_name,
								...
								INNER JOIN prd_employee_annual_increment_details AS annu 
								ON annu.emp_id_fk = emp.emp_id_pk 
								WHERE emp.emp_status in(1) AND zp.district_id_pk='".$_SESSION['location']['district_id']."' AND annu.status in('1','2') AND emp.emp_id_pk = annu.emp_id_fk  AND  annu.effective_monthyear='".date('Ym')."' ORDER BY emp.emp_id_pk DESC
									";
	//print($Query); exit;
                                ...
                                <table width="100%">
                                    <thead>
                                        <tr>
                                            ...
                                            <!--<th>Existing Consolidated Pay</th>-->
									...
									{ 
										...
										{?>
                                            <tr>
                                                ...
                                                <?php if($item['status']=='2' || $item['zp_forward_status']=='1') 
												{?>
                                                	<td>
                                                    	<a class="btn btn-success" data-toggle="modal" style="opacity:0.5;">Forward</a>
												...
												{?>
                                                	<td>
                                                    	<a onClick="confirm_accept(<?= $cnt?>,1);"  class="btn btn-success">Forward</a>
...
<form name="approve_promostio"  action="ul_emp_annual_increment_forward_reject.php" method="post">
<div class="modal fade bs-example-modal-sm" id="annu_app_rej" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true" data-modal-parent="#schprfModal">
 ...
 <div class="modal-body"> 
      <p class="alert alert-warning" id="acc_para" style="display:none;"><strong><i class="fa fa-exclamation-triangle"></i> Are You Sure To Forward the Increment Amount ?</strong></p>
