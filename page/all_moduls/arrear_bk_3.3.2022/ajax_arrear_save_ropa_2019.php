...
{
	$arr = $db->fetch_table("SELECT net FROM prd_employee_arrear WHERE status_flag=1 AND delete_status=1  AND ps_id_fk='".$_SESSION['location']['ps_id']."' AND emp_id_fk='".$emp_id_pk."' and ropa_status='1' ");
	
	if($arr[0]['net'])
	{
		$upd = $db->update("UPDATE prd_employee_arrear SET is_saved=1  WHERE ps_id_fk='".$_SESSION['location']['ps_id']."' AND delete_status=1 AND status_flag=1 AND emp_id_fk='".$emp_id_pk."' and ropa_status='1'");
		if($upd)
		{	
			$f=2;				
		}
...
{
	$arr = $db->fetch_table("SELECT net FROM prd_employee_arrear WHERE status_flag=1 AND delete_status=1  AND zp_id_fk='".$_SESSION['location']['district_id']."' AND emp_id_fk='".$emp_id_pk."' and ropa_status='1' ");
	
	if($arr[0]['net'])
	{
		$upd = $db->update("UPDATE prd_employee_arrear SET is_saved=1  WHERE zp_id_fk='".$_SESSION['location']['district_id']."' AND delete_status=1 AND status_flag=1 AND emp_id_fk='".$emp_id_pk."' and ropa_status='1' ");
		if($upd)
		{	
			$f=2;				
		}
