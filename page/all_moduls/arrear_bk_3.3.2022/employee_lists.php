...
function edit_arrear(k)
{
	...
	var link=k.split('&');
		...
		$('#myModal').modal('show');
		$.post('<?= $config['base_url'] ?>page/all_moduls/arrear/ajax_arrear_individual_edit.php?gp_id='+link[0]+'&emp_id='+link[1]+'&emp_type='+link[2], function(data){
...
function save_arrear(k)
{
	...
	$.post('<?= $config['base_url'] ?>page/all_moduls/arrear/ajax_arrear_save.php?gp_id='+link[1]+'&emp_id='+link[2]+'&sec_tok='+sec_token, function(data){
		//alert(data);
...
if($logged_user=='EO')
{
	...
	$emp_arrear_fetch=$db->fetch_table("
										...
										ps_id_fk
										 FROM prd_employee_arrear WHERE ps_id_fk='".$_SESSION['location']['ps_id']."' AND status_flag in (1,2) AND delete_status='1' AND salary_monthyear is null AND ropa_status='0'
										 GROUP BY ps_id_fk,emp_id_fk,emp_id_const,status_flag,is_saved,delete_status,zp_emp_type
...
else if($logged_user=='BDO')
{
	$emp_arrear_fetch=$db->fetch_table("
										...
										sum(CAST (working_days as INTEGER)) as wd_sum
										 ...
										 WHERE block_code='".$_SESSION['location']['block_code']."' AND status_flag in (1,2) AND delete_status='1' AND salary_monthyear is null AND gp_id_fk!=0 AND ropa_status='0'
										 GROUP BY gp_id_fk,emp_id_fk,emp_id_const,status_flag,is_saved,delete_status,zp_emp_type
...
else if($logged_user=='FC&CAO')
{
	...
	$emp_arrear_fetch=$db->fetch_table("
										...
										sum(CAST (working_days as INTEGER)) as wd_sum
										 FROM prd_employee_arrear 
										 WHERE zp_id_fk='".$_SESSION['location']['district_id']."' AND status_flag in (1,2) AND delete_status='1' AND salary_monthyear is null AND zp_id_fk!=0 AND zp_emp_type='".$type."' AND ropa_status='0'
										 GROUP BY zp_id_fk,zp_emp_type,emp_id_fk,emp_id_const,status_flag,is_saved,delete_status
	...
	$save_count=$db->fetch_table("
								...
								COUNT(CASE WHEN status_flag=1 THEN emp_id_fk END) AS total_arrear,
								COUNT(CASE WHEN status_flag=1 AND is_saved='1' THEN emp_id_fk END) AS total_save_arrear
								FROM prd_employee_arrear WHERE delete_status=1 AND salary_monthyear is null AND zp_id_fk='".$_SESSION['location']['district_id']."'  AND zp_emp_type='".$type."' and ropa_status='0'
							");
...
if($logged_user=='EO')
{
	$emp_details_fetch=$db->fetch_table("SELECT net,arrear_id_pk FROM prd_employee_arrear WHERE delete_status='1' AND status_flag='2' AND salary_monthyear is null AND ps_id_fk='".$_SESSION['location']['ps_id']."' AND ropa_status = '0'");   

}
else if($logged_user=='BDO')
{
	$emp_details_fetch=$db->fetch_table("SELECT net,arrear_id_pk FROM prd_employee_arrear WHERE delete_status='1' AND status_flag='2' AND salary_monthyear is null AND block_code='".$_SESSION['location']['block_code']."' AND ropa_status = '0'");   
}
else if($logged_user=='FC&CAO')
{
	$emp_details_fetch =$db->fetch_table("SELECT net,arrear_id_pk FROM prd_employee_arrear WHERE delete_status='1' AND status_flag='2' AND 
											salary_monthyear is null AND zp_id_fk='".$_SESSION['location']['district_id']."' AND zp_emp_type='".$type."' AND ropa_status = '0'"); 
...
if(count($emp_details_fetch)>0)
{
	if($logged_user=='EO')
	{?>
    <a href="<?= $config['base_url'] ?>page/all_moduls/arrear/bill_unlock.php?emp_type=<?php echo $cryptoGraph->encode($type,4); ?>" class="btn btn-danger btn-sm" style="float: right;">Bill Unlock</a>
	<a href="<?= $config['base_url'] ?>page/all_moduls/arrear/text_file_ps.php?emp_type=<?php echo $cryptoGraph->encode($type,4); ?>" class="btn btn-success btn-sm" style="float: right;">Bill Generation </a>
	<?php 
	}
	...
	{	
		...
		{ 
		?>  <a href="<?= $config['base_url'] ?>page/all_moduls/arrear/bill_unlock.php?emp_type=<?php echo $cryptoGraph->encode($type,4); ?>" class="btn btn-danger btn-sm" style="float: right;">Bill Unlock</a>
			<a href="<?= $config['base_url'] ?>page/all_moduls/arrear/text_file_zp.php?emp_type=<?php echo $cryptoGraph->encode($type,4); ?>" class="btn btn-success btn-sm" style="float: right;">Bill Generation</a>
...
<tr>
   ...
   <?php }
    ...
    <!--<td><?php echo dateshow($item['arrear_from_date']); ?></td>
	<td><?php echo dateshow($item['arrear_to_date']); ?></td>-->
	<td><?php echo $item['wd_sum']; ?></td>
	<td><?php echo $item['cp_sum']; ?></td>
	<td><?php echo $item['pb_sum']; ?></td>
	<td><?php echo $item['gp_sum']; ?></td>
...
<div class="row mb-3" id="finalize_show" style="display:none; margin-left: 44%;">
    <div class="col-sm-offset-5 col-sm-7" style="margin-top:1%">
    	 <a href="<?= $config['base_url'] ?>page/all_moduls/arrear/arrear_finalize.php?emp_type=<?=$cryptoGraph->encode($type,4); ?>" class="btn btn-success btn-sm">Arrear Finalize</a>
    </div>
</div>
<?php }else{?>
<div class="row mb-3" id="finalize_show" style="display:none; margin-left: 44%;">
...
<?php if(count($emp_arrear_fetch)>0 && $save_count[0]['total_arrear']==$save_count[0]['total_save_arrear'] && $finalize_count!='1') { ?>
    <div class="row mb-3" id="finalize_show" >
        <div class="col-sm-offset-5 col-sm-7" style="margin-left: 44%;">
            <a href="<?= $config['base_url'] ?>page/all_moduls/arrear/arrear_finalize.php?emp_type=<?=$cryptoGraph->encode($type,4); ?>" class="btn btn-success btn-sm">Arrear Finalize</a>
        </div>
    </div>
<?php }
...
//var_dump($emp_arrear_fetch);
//var_dump($save_count[0]['total_arrear']);
//var_dump($save_count[0]['total_save_arrear']);
//var_dump($finalize_count);.................0 
//die;

if(count($emp_arrear_fetch)>0 && $save_count[0]['total_arrear']==$save_count[0]['total_save_arrear'] && $finalize_count!='1' ) { ?>
    
    <div class="row mb-3" id="finalize_show" style="margin-left: 44%;">
        <div class="col-sm-offset-5 col-sm-7" style="margin-top:1%">
            <a href="<?= $config['base_url'] ?>page/all_moduls/arrear/arrear_finalize.php?emp_type=<?=$cryptoGraph->encode($type,4); ?>" class="btn btn-success btn-sm">Arrear Finalize</a>
        </div>
    </div>
...
function view_arrear()
{
	...
	{	
		...
		{
			$('#myModal').modal('show');
			$.post('<?= $config['base_url'] ?>page/all_moduls/arrear/ajax_arrear_edit_ps.php?emp_type='+emp_type1, function(data){
				// alert(data);
				$("#mbody").html(data);
				});
		}
		...
		{
			$('#myModal').modal('show');
			$.post('<?= $config['base_url'] ?>page/all_moduls/arrear/ajax_arrear_edit.php?emp_type='+emp_type1, function(data){
				// alert(data);
				$("#mbody").html(data);
				});
		}
		...
		{
			$('#myModal').modal('show');
			$.post('<?= $config['base_url'] ?>page/all_moduls/arrear/ajax_arrear_edit_zp.php?emp_type='+emp_type1, function(data){
				 //alert(data);
				$("#mbody").html(data);
				});
		}
