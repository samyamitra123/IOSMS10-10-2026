...
if($emp_type=='366')
{
$emp_type1=$crypto->encode('6',4);
}
elseif($emp_type=='367')
{
$emp_type1=$crypto->encode('7',4);
}
elseif($emp_type=='5')
{
$emp_type1=$crypto->encode('5',4);
}
...
{*/		
	...
	//print_r($basics=$_POST['basic']);
	$pay_in_bands=$_POST['pay_in_band'];
	$ropa_level=$_POST['grade_pay'];
	$das=$_POST['da'];
	//var_dump($das); die;
	$hras=$_POST['hra']; 
	$mas=$_POST['ma'];
	...
	if($total_row!=0)
	{
		for($k=0;$k<$total_row;$k++)
		{
			if($logged_user=='BDO' && $gp_ids[$k]=="")
			{
				...
				header('location:employee_lists_ropa_2019.php');
				exit(0);
			}
			elseif($emp_ids[$k]=="")
			{
				$_SESSION['msg']= '<div class="alert alert-danger" style="text-align:center"><strong>Please Select Employee.</strong></div>';
				header('location:employee_lists_ropa_2019.php?emp_type='.$emp_type1);
				exit(0);
			}
		...
		else 
		{
			...
			if($consolidated_pays[$k]==0)
			{
				...
				$total_allowance=$pay_in_bands[$k]+$das[$k]+$hras[$k]+$mas[$k]+$conv_allows[$k]+$hill_allows[$k]+$interim_reliefs[$k]+$allows[$k];
			
				 $total_deduction=$gpfs[$k]+$pf_loans[$k]+$p_taxs[$k]+$i_taxs[$k]+$gsli[$k]; 
			
	/**************************************** Changed by ANJAN 23/06/2021 start ***************************************/		
			/*	
				if($logged_user=='BDO' || $logged_user=='EO')
				{
...
/*************************************** END *****************************************/
				...
				else if($allows[$k] != 0 && $emp_type == 367){
					$_SESSION['msg']= '<div class="alert alert-danger" style="text-align:center"><strong>Employee not elegible.<strong></div>';
					header('location:employee_lists_ropa_2019.php?emp_type='.$emp_type1);
					exit(0);
				}
				
				else if($allows[$k] != 0  && $emp_type == 366)
				{ //var_dump($allows); die;
					if($allows[$k] > 1200){
						$_SESSION['msg']= '<div class="alert alert-danger" style="text-align:center"><strong>Please enter valid Administrative Allowance.<strong></div>';
						header('location:employee_lists_ropa_2019.php?emp_type='.$emp_type1);
						exit(0);
					}
				}
 ...
 ////////////////////////////////////////////////////////////////////////////// Duplicate Employee Checking //////////////////////////////////////////////////////////////////////////////////
	...
	{
		$_SESSION['msg']= '<div class="alert alert-danger" style="text-align:center"><strong>Sorry !!! Arrear Submission Fails Due To Duplicate Employee Submission.</strong></div>';
		header('location:employee_lists.php');
		exit(0);
	}
	else
	{*/
		...
		/*	if($consolidated_pays[$i]==''){ $consolidated_pay.$i='0'; }
			if($pay_in_bands[$i]==''){ $pay_in_band.$i='0'; }
			if($grade_pays[$i]==''){ $grade_pay.$i='0'; }
			if($das[$i]==''){ $da.$i='0'; }
			if($hras[$i]==''){ $hra.$i='0'; }
			if($mas[$i]==''){ $ma.$i='0'; }
			if($conv_allows[$i]==''){ $conv_allows.$i='0'; }
			if($hill_allows[$i]==''){ $hill_allow.$i='0'; }
			if($interim_reliefs[$i]==''){ $interim_relief.$i='0'; }
			if($grosss[$i]==''){ $gross.$i='0'; }
			if($gpfs[$i]==''){ $gpf.$i='0'; }
			if($pf_loans[$i]==''){ $pf_loan.$i='0'; }
			if($p_taxs[$i]==''){ $p_tax.$i='0'; }
			if($i_taxs[$i]==''){ $i_tax.$i='0'; }
			if($nets[$i]==''){ $net.$i='0'; }*/
			
			
			
			$basics=$pay_in_band+$grade_pay; 

			
			if($logged_user=='BDO')
			{
				$net_check=$db->fetch_table("select net from prd_employee_arrear where emp_id_fk='".$emp_ids[$i]."' AND gp_id_fk='".$gp_ids[$i]."' and salary_monthyear is null");
				...
				$ins=$db->insert("
									...
									arrear_to_date,
									arrear_from_date,
									ps_id_fk,zp_id_fk, ropa_status, ropa_level)
									VALUES
									( 'now()',
									'".$_SERVER['REMOTE_ADDR']."',
									'".$str."',
									...
									'".$arr_to_date."',
									'".$arr_fm_date."',
									'0','0','".$ropa_status."','".$ropa_level[$i]."')");
			
			}
			else if($logged_user=='EO')
			{
				$net_check=$db->fetch_table("select net from prd_employee_arrear where emp_id_fk='".$emp_ids[$i]."' AND ps_id_fk='".$_SESSION['location']['ps_id']."' and salary_monthyear is null");
				...
				$ins=$db->insert("
									...
									'".$arr_to_date."',
									'".$arr_fm_date."',
									'".$_SESSION['location']['ps_id']."','0','".$ropa_status."','".$ropa_level[$i]."')");
			
			}
			
			else if($logged_user=='FC&CAO')
			{
				$net_check=$db->fetch_table("select net from prd_employee_arrear where emp_id_fk='".$emp_ids[$i]."' AND zp_id_fk='".$_SESSION['location']['district_id']."' and salary_monthyear is null");
				$emp_details_fetch=$db->fetch_table("select emp_bank_name,emp_acc_no,emp_ifsc_no,emp_pan_no,emp_id_const from prd_employee_master where emp_id_pk='".$emp_ids[$i]."' AND zp_id_fk='".$_SESSION['location']['district_id']."'");
				...
				$ins=$db->insert("
									...
									arrear_to_date,
									arrear_from_date,
									ps_id_fk,zp_id_fk,zp_emp_type, ropa_status, ropa_level,allowance)
									VALUES
									( 'now()',
									'".$_SERVER['REMOTE_ADDR']."',
									'',
									...
									'".$arr_fm_date."',
									'0',
									'".$_SESSION['location']['district_id']."','".$emp_type."','".$ropa_status."','".$ropa_level[$i]."', '".$allows[$i]."')");
			...
			/*if($net_check[0]['net'])
			{
				$upd=$db->update("UPDATE prd_employee_arrear SET delete_status='0',is_saved=0 where emp_id_fk='".$emp_ids[$i]."' AND gp_id_fk='".$gp_ids[$i]."' and salary_monthyear is null ");
