...
if($logged_user=='EO')
{
	...
	where status_flag=2 AND delete_status=1 AND is_saved=1 AND salary_monthyear is null AND ps_id_fk = '".$_SESSION['location']['ps_id']."' AND zp_emp_type='".$emp_type."' and bill_serial_no='0' AND ropa_status='0'
						");
...
else if($logged_user=='BDO')
{
	...
	$upd = $db->update("
						UPDATE prd_employee_arrear  SET status_flag=1
	where status_flag=2 AND delete_status=1 AND is_saved=1 AND salary_monthyear is null AND block_code = '".$_SESSION['user_info']['stake_user']."' AND zp_emp_type='".$emp_type."' and bill_serial_no='0' AND ropa_status='0'
						");
}
else if($logged_user=='FC&CAO')
{
	...
	$upd = $db->update("
						UPDATE prd_employee_arrear  SET status_flag=1
	where status_flag=2 AND delete_status=1 AND is_saved=1 AND salary_monthyear is null AND zp_id_fk='".$_SESSION['location']['district_id']."' AND zp_emp_type='".$emp_type."' and bill_serial_no='0' AND ropa_status='0'
						");
}
