...
$db = new database();	
	...
	$sql_query=$db->fetch_table("select block_bill_pk,bill_no,bill_entry_time,salary_monthyear from prd_block_bill_details where block_code='".$_SESSION['user_info']['stake_user']."' and bill_no='".$bill_no."' and salary_monthyear= '".$month_year."' AND requisition_type='".$requisition_type."'");
...
$db = new database();
	...
	left join prd_employee_arrear arrear ON (arrear.emp_id_fk=emp.emp_id_pk and CAST(block.block_code as character varying)=arrear.block_code)
	where 
		...
		AND arrear.block_code='".$_SESSION['user_info']['stake_user']."' 
...
$sql_salarymonth=$db->fetch_table("
										...
										inner join (select salary_monthyear,status_flag,block_code,gp_id_fk,bill_id_fk from prd_employee_arrear where salary_monthyear='".$sql_query[0]['salary_monthyear']."') as sal 
										on  emp.gp_id_fk=sal.gp_id_fk 
