...
<div class="form-group">
    ...
    <div class="col-sm-3">
      <input type="text" readonly="readonly" style="background-color:#d3d3d3;" class="form-control upper_case" name="bill" id="bill" placeholder=" Arrear Salary Bill Number" onKeyPress="return keyRestrict(event,'abcdefghijklmnopqrstuvwxyz0123456789/');" value=""  >
    </div>
    <div class="col-sm-3"></div>
    </div>
...
$(document).ready(function() {
	$('#submit-text').click(function(){
			...
			{
				//alert ('ff');
				$.post('<?php echo $config['base_url'] ?>page/all_moduls/ifms_ajax_links/ajax_links_ifms_upload_details.php',$(this).closest("form").serialize(), function(data){
					//alert(data);
...
$('#bill_serial_no').change(function(){
		...
		var bill_serial_no=$("#bill_serial_no").val();
		 $.ajax({
		url:'<?php echo $config['base_url'] ?>page/all_moduls/arrear/text_file/ajax_find_bill_details.php',
		dataType:"json",
		type:'GET',
		data:{
			ye:year,
...
$("#bill_report_month, #bill_report_year").change(function(){ 
	...
	//alert (month);
	//alert (year);
	$.post('<?php echo $config['base_url'] ?>page/all_moduls/arrear/text_file/ajax_bill_serial_list.php?month='+month+'&year='+year, function(data){
		//alert(data);
