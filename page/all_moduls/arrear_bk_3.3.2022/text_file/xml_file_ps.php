...
{	
	...
	return $dob=='01/01/1900'?'':$dob;
		...
		$financial_year = (date("m") >= 4) ? (date("Y")."-".(date("Y")+1)) : ((date("Y")-1)."-".date("Y"));
	 	...
	 	$bill_det=$db->fetch_table("select block_bill_pk,bill_no,bill_entry_time,salary_monthyear from prd_block_bill_details where ps_id_fk = '".$_SESSION['location']['ps_id']."' and bill_no='".$bill."' and salary_monthyear= '".$yemo."' AND requisition_type='".$requisition_type."'");
