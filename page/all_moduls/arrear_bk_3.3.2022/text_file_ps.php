...
{ 
	$find_bill = $db->fetch_table("
									...
									AND salary_monthyear ='".$yemo."' AND status='1' AND requisition_type='".$requisition_type."' AND ropa_status='0' ORDER BY oid DESC
...
{ 
	...
	$find_bill = $db->fetch_table("
									...
									AND salary_monthyear ='".$yemo."' AND status='1' AND requisition_type='".$requisition_type."' AND ropa_status='0' AND zp_emp_type='".$bill_type."'
...
<input type="hidden" id="ropa_status" name="ropa_status" value="<?php echo $crypto->encode(0,4); ?>" />
	...
	if($checking[0]['count']=='0'){
		...
		$get_bill_serial_no = $db->fetch_table("SELECT max(bill_serial_no) AS max_bill_serial_no FROM 
								 ...
								 if($checking[0]['count']>='1' )
									...
									prd_block_bill_details WHERE salary_monthyear='".$yemo."'
									AND status='1' AND ropa_status='0'
...
if($logged_user=='EO')
{
	$data = $db->fetch_table("SELECT * FROM prd_block_bill_details
							WHERE ps_id_fk = '".$_SESSION['location']['ps_id']."'
							AND salary_monthyear ='".date('Ym')."' AND ropa_status='0'
