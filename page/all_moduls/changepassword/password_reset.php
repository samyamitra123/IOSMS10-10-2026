...
{ //var_dump($_SESSION['user_info']["stake_user"]);
//echo 111; die;
	$zp=$db->fetch_table("select district_name, district_id_pk from prd_location_master_district
Where district_code='".$_SESSION['user_info']["stake_user"]."' order by district_name");
}
...
<div class="row" id="cont">
      <div class="col-lg-12 col-md-8 col-sm-8" id="sm-pad"> 
        <div class="col-sm-12">
<h1 class="heading">USER RESET PASSWOARD FOR EMPLOYEE LOGIN </h1>
...
<input type="hidden" name="sec_tok" id="sec_tok" value="<?=$enc_token?>" />
    ...
    <input type="hidden" name="form_flage" id="form_flage" value="<?= $crypto->encode("reset",4); ?>" />
    <div class="row mb-3">
...
{?>
    ...
    <label for="inputPassword3" class="col-sm-3 control-label">ZP Name: <span class="star_color">*</span></label>
    <div class="col-sm-3">
		<input type="text" class="form-control" name="gp" id="gp" value="<?php echo $zp[0]['district_name']; ?>" readonly="readonly" />
    </div>
     <?php } ?>
    ...
    <?php if($_SESSION['user_info']['stake_abbr']=='AEO'){ 
			$db=new database();
			$arr=$db->fetch_table("select emp_first_name,emp_second_name,emp_last_name,emp_id_const from prd_employee_master where zp_id_fk='".$zp[0]['district_id_pk']."' and emp_status in('1','9')");
			?>
			<option value="">-Please Select-</option>
    ...
    <div class="col-sm-3">
      <input type="text" class="form-control" name="e_mobile" readonly id="e_mobile"  />
    </div>
    <div class="col-sm-3">
      </div>
    </div>
    ...
    <div class="col-sm-3">
      <input type="text" class="form-control" name="d_pin" readonly id="d_pin"  />
    </div>
    <div class="col-sm-3">
      </div>
    </div>
...
function show_gp(val)
	...
	function show_name(val)
	{
		//alert(val);
		//return false;
		$.post('<?= $config['base_url'] ?>page/all_moduls/changepassword/ajax_employee_name_reset.php?id='+val, function(data){
