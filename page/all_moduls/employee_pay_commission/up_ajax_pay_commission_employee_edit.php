        ...
        <center><h4 style="color:#03371b;">BASIC PAY DETAILS AS ON <?php echo '31-12-2019'; ?></h4></center> 
               ...
               /* $_SESSION['ppb_2019']=$emp_pay_in_payband;
                ...
                <div class="row">
                                <?
                                $db = new database();
                                //$arr1 = $db->fetch_table("select ropa_2019_effective_date,cause from ropa_2019_emp_pay_scale_master WHERE emp_id_fk = '".$emp_id_pk."'");
  ...
  <div  align="justify"class="col-sm-12  festival_advance_design" <?php if($curr_yr<$year_to_show){echo 'style="display:none;"';} ?> id="year_<?=$curr_yr?>" >
        ...
        <div class="col-sm-2">
		  ...
		  <div class="col-sm-3">
				...
				<div class="col-sm-3">
                        ...
                        <select <?php if(($saved_inc_data['previous_grade_pay']!=0 && ($saved_inc_data['increment_dtls']==2 || $saved_inc_data['increment_dtls']==3)) ){echo 'disabled style="background-color:#d3d3d3;"';}else if($saved_inc_data['previous_grade_pay']==0 && $saved_inc_data['increment_dtls']==1){echo 'style="display:none;"';}else if(($saved_inc_data['increment_dtls']!=1 || $saved_inc_data['increment_dtls']!=2 || $saved_inc_data['increment_dtls']!=3) && ($select_saved_pay_data[0]['year']==$curr_yr)){echo 'disabled style="background-color:#d3d3d3;"';}?> class="form-control gradepay disable_year_<?=$curr_yr?>" name="grade_pay_<?=$curr_yr.'_'.$current_yr_count?>" id="grade_pay_<?=$curr_yr.'_'.$current_yr_count?>" onchange="select_incre_param(<?=$curr_yr?>,<?=$current_yr_count?>)" >
	...
	?>  
	  ...
	  <center><h4 style="color:#03371b;">BASIC PAY DETAILS AS ON <?=date("d-m-Y", strtotime('-1 day', strtotime($ason_date)))?></h4></center> 
            <div class="row">
                    ...
                    <div class="col-sm-4" style="color: #791454;">
                    
                     <?php if ($select_saved_pay_data[0]['edit_status']=='99') { ?>
				<?php /*?><input  type="submit" value="SUBMIT" class="btn btn-info" /><?php */?>
	...
	?>
            ...
            <div class="row">
                ...
                <?php 
                //if($count_salary_data[0]['result_count'] < 1 && $count_salary_final_data[0]['result_count'] < 1)
                //{	
                ?>
                <?php /*?><button type="submit" class="btn btn-info" <?php if($emp_first_join_year!=$get_last_saved_year[0]['min_year']){?>style="display:none;" <?php } ?> id="calculate_ropa_2019" onclick="return calculate_ropa_2019();" >Calculate Revised Pay Structure under ROPA 2019</i></button><?php */?>
            ...
            <div class="row" id="ropa_pay_2019">
...
<div class="clear"></div>
  ...
  <?php
	...
	<script>
		...
		$.get('<?php echo $config['base_url'] ?>page/all_moduls/employee_pay_commission/up_ajax_ropa_2019_calculation.php?emp_id_pk='+emp_id_pk+'&emp_first_join_year='+emp_first_join_year, function(data){
			//alert(data);
	...
	function basic_correction(field_type,emp_id_pk,curr_yr)
	{
		...
		$.get('<?php echo $config['base_url'] ?>page/all_moduls/employee_pay_commission/ajax_basic_pay_correction.php?emp_id_pk='+emp_id_pk+'&curr_yr='+			curr_yr+'&field_type='+field_type+'&ppb='+ppb_year, function(data)
		{
			...
			if(data.trim()==1)
			{
				$.post('<?=$config['base_url'] ?>page/all_moduls/employee_pay_commission/up_ajax_pay_commission_employee_edit.php?id='+emp_id_pk, function(data){
					$("#full_div").html(data);
				});
			}
		});
...
?>
	...
	<div class="row" id="cont">
          ...
          <div class="col-sm-12">
          <center><h3 class="heading">DATE OPTED FOR ROPA 2019 PAY FIXATION</h3></center>
           <h5> ***Please change Date & Reason in case of Employee is not present in duty on 01-01-2016. </h5>
          <br />
          		<div class="col-sm-12 festival_advance_design" style="display:inline !important; padding-top: 10px;">
                	<div class="row">
						<div class="col-sm-2">
                        	<label for="inputPassword3" class="control-label" style="color: #246a8e;">Date<span class="star_color">*</span></label>
                        </div>
                        <div class="col-sm-4">
                        	<input class="form-control" placeholder="DD-MM-YYYY" type="text" id="ropa_2019_date" name="ropa_2019_date" value="<?=$effective_date_ff?>"   <?php if ($selected == '2'){ echo 'readonly=readonly style=background-color:#d3d3d3;cursor:no-drop;pointer-events:none;'; }else{ echo 'readonly style=background-color:#FFF;cursor:pointer;';} ?>/>
                        </div>
                        ...
                        <div class="col-sm-4">
                        	<select class="form-control" name="ropa_2019_reason" id="ropa_2019_reason">
                            	<option value="">-Please Select-</option>
                                 <?php
								 if ($selected == '2')
								 {
								$get_ropa_2019_scenario=$db->fetch_table("SELECT id_pk, rules, status FROM ropa_2019_scenario 
														WHERE id_pk='2' ORDER BY id_pk ASC");
								...
								$get_ropa_2019_scenario=$db->fetch_table("SELECT id_pk, rules, status FROM ropa_2019_scenario 
														WHERE status not in('".$senario_not."') ORDER BY id_pk ASC");
								foreach($get_ropa_2019_scenario as $scenario)
								{
								?>
                                	<option <?php if($selected==$scenario['status']){echo 'selected'; }?> value="<?=$cryp->encode($scenario['id_pk'],4)?>"><?=$scenario['rules']?></option>
