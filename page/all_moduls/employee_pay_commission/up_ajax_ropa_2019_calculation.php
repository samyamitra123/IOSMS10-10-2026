...
else if($stake_level=="FC&CAO")
{
	 ...
	 $user='FC&CAO';
		...
		$percentage_of_basic = $percentage_basic_pay_2016[0]['fitment_factor'];
		 //$basic_pay_on_effective_date."---".$percentage_of_basic; 
...
<?php
	...
	$get_edited_data=$db->fetch_table("SELECT basic_pay, level FROM ropa_2019_emp_pay_scale_master
...
?>
	 <div class="col-sm-12 festival_advance_design" style="display:inline !important;  margin-left: 0% !important;"> 
      ...
      <?php if ($get_ropa_master[0]['edit_status'] != '99' ) { ?>
           ...
           <?php } else if($get_ropa_master[0]['edit_status']== '99' &&  $user=='FC&CAO') { ?>
     ...
     <div class="col-sm-2" style="color: #791454; margin-left:85%; top: 10px;">
          <?php if ($get_ropa_master[0]['edit_status']=='99' &&  $user=='FC&CAO') {  ?>
...
<?php if(($get_ropa_master[0]['status']=='2' || $get_ropa_master[0]['status']=='4') && ($get_ropa_master[0]['sent_status']=='0' || $get_ropa_master[0]['sent_status']=='3') && $user=='ACCOUNTANT' )


{?>
<div class="row">
<div class="col-sm-12" align="center" style="margin-top:12px;">
<a class="btn btn-primary"   onClick="send_action('forward', '<?=$emp_id_fk;?>');">Forward</a>
<a class="btn btn-danger"  onClick="send_action('reject', '<?=$emp_id_fk;?>');">Reject</a> 
</div>
</div>
<?php }?>

<?php if($get_ropa_master[0]['status']=='2' && $get_ropa_master[0]['sent_status']=='1' &&  $user=='FC&CAO' )


{?>
<div class="row">
<div class="col-sm-12" align="center" style="margin-top:12px;">
<a class="btn btn-success"   onClick="send_action('approve', '<?=$emp_id_fk;?>');">Approve</a>
<a class="btn btn-danger"  onClick="send_action('reject', '<?=$emp_id_fk;?>');">Reject</a> 
</div>
</div>
<?php }?>
