...
{ 
  $current_year=date("Y");
	...
	$prev_yrr=$current_year-1;
	$db = new database();
	//print($emp_bonus_category); exit;
	//$emp_bonus_category = 167;
	...
	if($stake_level=='ZP')
	{
		...
		// 										AND pem.emp_id_pk NOT IN ( SELECT emp_id_fk FROM prd_employee_bonus_details WHERE delete_status=1 AND bonus_status not in (1,2,3,4) AND monthyear='".$prev_yrr.$current_year."' AND zp_id_fk = '".$stake_id."')";	
...
//     prd_employee_master AS pem
// LEFT JOIN 
//     prd_monthly_salary_archive_final AS pmsaf ON pem.emp_id_pk = pmsaf.emp_id_fk
// LEFT JOIN 
//     prd_employee_bonus_details AS pbd ON pem.emp_id_pk = pbd.emp_id_fk 
//     AND pbd.delete_status = 1 
//     AND pbd.bonus_status NOT IN (1, 2, 3, 4) 
//     AND pbd.monthyear = '".$prev_yrr.$current_year."' 
//     AND pbd.zp_id_fk = ".$stake_id."
// WHERE
//     pem.emp_religion != '0'
//     AND pem.zp_id_fk = ".$stake_id."
//     AND pmsaf.basic + pmsaf.da <= 44000
...
//     AND pmsaf.status_flag IN (3, 4)
//     AND (
//         (pem.emp_status = '1' AND pem.emp_first_join_date < '2024-10-02') 
//         OR (pem.emp_termination_date >= '2024-09-30' AND pem.emp_termination_date <= '2025-10-31')
//     )
//     AND pbd.emp_id_fk IS NULL
// ;
// ";			
	...
	$employeeQuery = "
	    ...
	    WHERE 
	        ...
	        AND (
	            (emp_status = '1' AND emp_first_join_date < '2024-10-02') 
	            OR (emp_termination_date >= '2024-09-30' AND emp_termination_date <= '2025-10-31')
	        );
	";		
						
		$employee_list = $db->fetch_table($employeeQuery);
		$empIds = array();
		$newEmployeeList = array();
		foreach($employee_list as $employee)
		{
         $empIds[] = $employee['emp_id_pk'];
         $newEmployeeList[$employee['emp_id_pk']] = $employee;
		}
    $empIds = implode(',',$empIds);
		// Step 2: Get filtered employee IDs from prd_monthly_salary_archive_final
		$salaryQuery = "
		    SELECT 
		        emp_id_fk
		    FROM 
		        prd_monthly_salary_archive_final
		    WHERE 
		        salary_monthyear = '202410'
		        AND emp_id_fk IN (".$empIds.")
		        AND basic + da <= 44000
		        AND delete_status = '1'
		        AND status_flag IN (3, 4);
		";		
		$salary_Employee_list = $db->fetch_table($salaryQuery);
		$refineEmployeeList = array();
		$empIds = array();
		foreach($salary_Employee_list as $item)
		{
       $empIds[] = $item['emp_id_fk'];
			 $refineEmployeeList[$item['emp_id_fk']] = $newEmployeeList[$item['emp_id_fk']];
		}
		// Step 3: Get filtered employee IDs from prd_employee_bonus_details
		$bonusQuery = "
		    SELECT 
		        emp_id_fk
		    FROM 
		        prd_employee_bonus_details
		    WHERE 
		        emp_id_fk IN (".$empIds.")
		        AND delete_status = 1
		        AND bonus_status NOT IN (1, 2, 3, 4)
		        AND monthyear = '$prev_yrr$current_year'
		        AND zp_id_fk = $stake_id;
		";		
    $bonus_Employee_list = $db->fetch_table($bonusQuery);
    if(count($bonus_Employee_list))
    {
    	//$bonusRefineList = array();
			foreach($bonus_Employee_list as $item)
			{
	       
				 unset($refineEmployeeList[$item['emp_id_fk']]);
			}  
			
    }

    $employee_list_bonus = $refineEmployeeList;
    
		//print_r($refineEmployeeList); exit;	
...
SELECT emp_id_fk FROM prd_employee_bonus_details WHERE delete_status=1 AND bonus_status not in (1,2,3) AND monthyear='".$prev_yrr.$current_year."' AND ps_id_fk = '".$stake_id."') 	
		...
		$employee_list_bonus = $db->fetch_table("SELECT 
										...
										".$bonus_type."
										AND ps_id_fk = '".$stake_id."'
										AND emp_pay_in_payband + (emp_pay_in_payband*6/100) <= '42000'
										AND ((emp_status = '1' and emp_first_join_date<'2023-10-02' )or (emp_termination_date>='2024-09-30' and emp_termination_date<='2023-10-31' )) 
										AND emp_id_pk in 
(SELECT emp_id_pk FROM prd_employee_master WHERE ".$bonus_type." AND ps_id_fk = '".$stake_id."' AND ((emp_status = '1' and emp_first_join_date<'2023-10-02' )or (emp_termination_date>='2023-09-30' and emp_termination_date<='2024-10-31' )) 
EXCEPT
SELECT emp_id_fk FROM prd_employee_bonus_details WHERE delete_status=1 AND bonus_status not in (1,2,3,4) AND monthyear='".$prev_yrr.$current_year."' AND ps_id_fk = '".$stake_id."') 	
	...
	else if($stake_level=='GP')
	{
		...
		$employee_list_bonus = $db->fetch_table("SELECT 
										...
										AND emp_pay_in_payband + (emp_pay_in_payband*6/100) <= '42000'
										AND ((emp_status = '1' and emp_first_join_date<'2023-10-02' )or (emp_termination_date>='2023-09-30' and emp_termination_date<='2024-10-31' )) 
										AND emp_id_pk in 
(SELECT emp_id_pk FROM prd_employee_master WHERE ".$bonus_type." AND gp_id_fk = '".$stake_id."' AND ((emp_status = '1' and emp_first_join_date<'2023-10-02' )or (emp_termination_date>='2023-09-30' and emp_termination_date<='2024-10-31' )) 
EXCEPT
...
function get_employee_list_for_bonus_not_select($emp_bonus_category, $stake_id, $stake_level)
{
	...
	if($stake_level=='ZP')
	{
			...
			$Query = "SELECT 
												...
												WHERE
										           emp_religion!= '0'
													AND emp_pay_in_payband + (emp_pay_in_payband*3/100) > '39000'
													AND zp_id_fk = '".$stake_id."'
													AND ((emp_status = '1' and emp_first_join_date<'2022-10-02' )or (emp_termination_date>='2022-09-30' and emp_termination_date<='2023-10-31' )) 
												AND emp_id_pk in 
(SELECT emp_id_pk FROM prd_employee_master WHERE  emp_religion!='0' AND zp_id_fk = '".$stake_id."' AND ((emp_status = '1' and emp_first_join_date<'2022-10-02' )or (emp_termination_date>='2022-09-30' and emp_termination_date<='2023-10-31' )) 
EXCEPT
SELECT emp_id_fk FROM prd_employee_bonus_details WHERE delete_status=1 AND bonus_status not in (1,2,3,4) AND monthyear='".$prev_yrr.$current_year."' AND zp_id_fk = '".$stake_id."') 	
										";		
			//print($Query); exit;				
			$employee_list_bonus_for_checking = $db->fetch_table($Query);
...
$Query = "SELECT bonus_type_id_pk,bonus_category,bonus_amount,bonus_monthyear,bonus_name
					FROM prd_bonus_type_details
					WHERE active_status='1' and bonus_monthyear='".$bonus_fin_year."' AND delete_status=1";
//print_r($Query); exit;
$bonus_type_details_fetch=$db->fetch_table($Query);
//print($bonus_type_details_fetch); exit;
...
//$emp_list_for_bonus_not_select_list = implode(', ', array_column($emp_list_for_bonus_not_select, 'emp'));
	...
	$Query = " SELECT 
										...
										AND save.salary_monthyear IN('202309','202310') AND save.basic <= '42000' ";	
	//print($Query); exit;							
	$short_employee_from_not_select =$db->fetch_table($Query);
	//print_r($short_employee_from_not_select); exit;								
