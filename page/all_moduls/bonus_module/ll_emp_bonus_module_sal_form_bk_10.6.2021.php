...
{ 
	...
	if($stake_level=='ZP')
	{
		...
		$employee_list_bonus = $db->fetch_table("SELECT 
												...
												AND emp_id_pk in 
(SELECT emp_id_pk FROM prd_employee_master WHERE ".$bonus_type." AND zp_id_fk = '".$stake_id."' AND ((emp_status = '1' and emp_first_join_date<'2020-10-02' ) or (emp_retirement_date>='2020-09-30' and emp_retirement_date<='2021-10-31' )) 
EXCEPT
...
SELECT emp_id_fk FROM prd_employee_bonus_details WHERE delete_status=1 AND bonus_status not in (1,2,3) AND monthyear='".$prev_yrr.$current_year."' AND ps_id_fk = '".$stake_id."') 	
		...
		$employee_list_bonus = $db->fetch_table("SELECT 
										...
										AND ((emp_status = '1' and emp_first_join_date<'2020-10-02' )or (emp_retirement_date>='2020-09-30' and emp_retirement_date<='2021-10-31' )) 
										AND emp_id_pk in 
(SELECT emp_id_pk FROM prd_employee_master WHERE ".$bonus_type." AND ps_id_fk = '".$stake_id."' AND ((emp_status = '1' and emp_first_join_date<'2020-10-02' )or (emp_retirement_date>='2020-09-30' and emp_retirement_date<='2021-10-31' )) 
EXCEPT
										...
										foreach ($emp_list_for_bonus as $key) 
										{
											...
											{*/
												...
												//$full_amount_for_bonus = $emp_basic_pay + $emp_da;
												 ...
												 if($emp_retirement_date>='2020-09-30' and $emp_retirement_date<='2020-12-31')
												 {
													// echo 11;
														//$emp_grade_pay = fun_grade_pay($key['emp_grade_pay']); 
														$emp_pay_band = $key['emp_pay_in_payband'];
