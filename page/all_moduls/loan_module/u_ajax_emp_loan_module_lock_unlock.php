...
//Page variables
$common['title'] = "Loan Deduction | WBULBHRMS | Govt. of West Bengal ";
...
<!--<input type="hidden" name="desig" id="desig" value=<?= $cryptoGraph->decode($_REQUEST['desig'],4); ?>  />-->
...
<p id="insertinputs"></p>   
 ...
 $loan_deduction_data = $db->fetch_table("SELECT 
									...
									WHERE
										...
										AND approval_status in (3) AND lock_status in (3)");
...
?>
	...
	<div class="form-group" >
     ...
     <div class="form-group" >
		...
		//if(($deductions['lock_status']=='3') && ($count_sal_save[0]['sal_check'] < 1))
		if(($deductions['lock_status']=='3'))
		{
		?>
        
        <a class="btn btn-danger"  style="margin-left:48%;"   data-bs-target="#send"  id="unlock" data-bs-toggle="modal"   onclick="confirmation( '<?php echo $cryptoGraph->encode($deductions['loan_id_pk'],4) ?>','<?php echo $cryptoGraph->encode($deductions['emp_id_fk'],4) ?>');"> UNLOCK</a>
        	<!--<center><input type="submit" class="btn btn-danger" name="loan_deductions_submit" value="REQUEST FOR UNLOCK" /></center>-->
            <?php //echo '<a style="margin-left:48%;" data-bs-toggle="modal"  id="'.$cryptoGraph->encode($deductions['loan_id_pk'],4).'" href="javascript:void(0);" class="btn btn-danger" data-bs-target="#send" onclick="return confirmation(\''.$cryptoGraph->encode($deductions['loan_id_pk'],4).'\',\''.$cryptoGraph->encode($deductions['emp_id_fk'],4).'\');">'; UNLOCK ?><!--Anupam dey 20/3/2017 (encrypt loan_id_pk & emp_id_pk-->
        <?php	
		}
		?>
	</div>
...
$(document).ready(function(){
  ...
  <div class="form-group">
   ...
   <!-- <input type="submit" class="btn btn-info" name="loan_deductions_submit" value="SEND TO DDO" />-->
...
?>  
	...
	function confirm_send()
	{
		...
		$.post('<?= $config['base_url'] ?>page/all_moduls/loan_module/u_loan_module_lock_unlock_insert_submit.php?loan_id_pk='+loan_id_pk+'&emp_id_fk='+emp_id_fk, function(data_request){
			//alert("1234");
