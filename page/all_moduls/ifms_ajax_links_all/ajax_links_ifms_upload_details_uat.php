...
$(document).ready(function() {
	...
	if($('#bill').val()!='' && $('#bill_date').val()!='' && $('#bill_serial_no').val()!='')
		{
			$.post('<?php echo $config['base_url'] ?>page/all_moduls/ifms_ajax_links/ajax_links_ifms_upload_details_send_uat.php',$(this).closest("form").serialize(), function(data){
	...
	$('#submit-text').click(function(){
			...
			{
				//alert ('ff');
				$.post('<?php echo $config['base_url'] ?>page/all_moduls/ifms_ajax_links_all/ajax_links_ifms_upload_details_send_uat.php',$(this).closest("form").serialize(), function(data){
					//alert(data);
				$('.ajax_text_link').html(data);
				$("#submit-text").show();
				});
