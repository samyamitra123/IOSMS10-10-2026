...
function date_frmt_change($original_date){
	...
	if(date("m") == 1)
		...
		$year = date('Y');
	$monthyear=$year.date("m", strtotime("-1 months"));
	...
	$drn_checking = $db->fetch_table("
		...
		if($requisition_type == 1003 || $requisition_type == 1001)
		{
			//print("subikar"); exit;
				...
				$check_count = $db->fetch_table("
				SELECT count(*) FROM prd_employee_salary_save 
				WHERE zp_id_fk = '".$_SESSION['location']['district_id']."'
				AND salary_monthyear = '".$monthyear."' AND requisition_type='".$requisition_type."'  AND status_flag ='4' and delete_status='1' and is_saved='1' AND ropa_status='".$ropa_status."'
				");
...
//var_dump($prd_monthly_salary_archive_nonfinal); die;		
	if($prd_monthly_salary_archive_final==TRUE  &&  $check_count[0]['count']>0 && $prd_monthly_salary_archive_nonfinal==TRUE)
		{ 
				pg_query("commit");
				$Query = " INSERT INTO prd_block_bill_details ( bill_no, status, block_code, bill_entry_time, bill_update_time, ip_addres, salary_monthyear, ps_id_fk, requisition_type, zp_id_fk,bill_serial_no, drn_number, zp_emp_type, ropa_status )
		...
		//var_dump($insert_bonus_bill); die;
	...
	SELECT count(*) FROM prd_employee_salary_save 
	WHERE ps_id_fk = '".$_SESSION['location']['ps_id']."'
	AND salary_monthyear = '".$monthyear."' AND requisition_type='".$requisition_type."'  AND status_flag ='3' and delete_status='1' and is_saved='1' AND ropa_status='".$ropa_status."'
	");
	$prd_monthly_salary_archive_final = $db->insert("
	INSERT INTO prd_monthly_salary_archive_final
	(
...
else if($logged_user=='BDO') // For BDO SECTION Start
	{
   ...
   $Query = "
	...
	";
   //var_dump($Query); die;
	$check_count = $db->fetch_table($Query);
	...
	if($requisition_type == 1003 || $requisition_type == 1001){
		//var_dump(count($prd_monthly_salary_archive_nonfinal)); die;
		/*print($prd_monthly_salary_archive_final);
		echo "<hr/>"; 
		print($check_count[0]['count']);
		echo "<hr/>"; 
		print($prd_monthly_salary_archive_nonfinal); exit; 
		exit; */
		//echo $check_count[0]['count']; die;
	if($prd_monthly_salary_archive_final==TRUE &&  $check_count[0]['count']>0 && $prd_monthly_salary_archive_nonfinal==TRUE)
	{//echo 444; die;
	//$insert_bonus_bill=TRUE;
		$Query = "INSERT INTO prd_block_bill_details (
