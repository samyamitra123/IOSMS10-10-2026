...
$bill_type=$crypto->decode($_POST['emp_type'],4); 
   ...
   {
	   echo $ropa_status=$crypto->decode($_POST['ropa_status'],4); die;
   }*/
   
   
   //var_dump($_POST['ropa_status']); die;
// echo $_POST['bill']; die;
...
if($logged_user=='zpddo')
{
 ...
 $party_code = '008'; 
	...
	if($requisition_type == '1002'|| $requisition_type == '1005' || $requisition_type == '1004' || $requisition_type == '1003'){
	...
	else if($requisition_type != '1002'|| $requisition_type != '1005' || $requisition_type != '1004' || $requisition_type != '1003'){
		$check_duplicate_bill_no_check=$db->fetch_table("
		SELECT count(*) FROM prd_block_bill_details 
		WHERE zp_id_fk ='".$_SESSION['location']['district_id']."'
		AND bill_no = '".$_POST['bill']."'
...
else 
{
	...
	if($check_duplicate_bill_no_check[0]['count']=='0' || $check_duplicate_bill_no_check[0]['count']== NULL)		
	{
		...
		pg_query('BEGIN');
			...
			if($requisition_type == 1003 ){
			//$prd_monthly_salary_archive_final=TRUE;
		$prd_monthly_salary_archive_final = $db->insert("INSERT INTO prd_monthly_salary_archive_final
									(slno, gp_id_fk, empcd, bankname, accountno , basic , da, hra, ma , pf_loan, p_tax , i_tax , net ,
									bank_ifsc , sal_source, spl_pay , pf_deduct , code, spl_alo, status_flag, salary_monthyear, category_id,
									block_code, emp_id_fk, pay_payband , tch_grade_pay, hill_allowance, gpf, gross_salary , is_saved, conv_allow,
									overdrawn, salary_type, cause, gp_code, gsli, delete_status, consolidated_pay, other_deduction_cause ,
									other_deduction, festival_loan , festival_loan_cause, part_salary_cause, no_salary_cause, interim_relief,
									lock_status, archive_final_entry_time, archive_final_entry_ip, advance_amount, requisition_type, part_day,
									zp_id_fk, ropa_status )
									SELECT slno, gp_id_fk, empcd, bankname, accountno , basic , da, hra, ma , pf_loan, p_tax , i_tax , net ,
									bank_ifsc , sal_source, spl_pay , pf_deduct , code, spl_alo, status_flag, salary_monthyear, category_id,
									block_code, emp_id_fk, pay_payband , tch_grade_pay, hill_allowance, gpf, gross_salary , is_saved, conv_allow,
									overdrawn, salary_type, cause, gp_code, gsli, delete_status, consolidated_pay, other_deduction_cause ,
									other_deduction, festival_loan , festival_loan_cause, part_salary_cause, no_salary_cause, interim_relief,
									lock_status, now(), '" . $_SESSION['user_agent']['USER_IP'] . "', advance_amount, requisition_type, part_day,
									zp_id_fk, ropa_status
									FROM prd_employee_salary_save
									WHERE zp_id_fk ='" . $_SESSION['location']['district_id'] . "'
									AND salary_monthyear ='" . $monthyear . "'
									AND requisition_type='".$requisition_type."'
									AND status_flag = 4 and delete_status='1' and is_saved='1' AND ropa_status='".$ropa_status."'
									");
		...
		$prd_monthly_salary_archive_nonfinal = $db->insert("INSERT INTO prd_monthly_salary_archive_nonfinal(
									slno, entry_time, entry_ip_address, gp_id_fk, empcd, salary_monthyear, bankname, accountno, basic, da,
									interim_relief, hra, ma, cpf, pf_loan, p_tax , i_tax, net, bank_ifsc, sal_source, spl_pay, pf_deduct,
									code, spl_alo, status_flag, delete_status, emp_id_fk, block_code, festival_loan, festival_loan_cause,
									consolidated_pay, conv_allow, hill_allowance, gpf, overdrawn, gsli, gp_code, pay_payband, tch_grade_pay,
									gross_salary, archive_nonfinal_entry_time, archive_nonfinal_entry_ip, requisition_type, part_day, zp_id_fk,
									ropa_status)
									SELECT slno, latestupdate_time, latestupdate_ip_address, gp_id_fk, empcd, salary_monthyear, bankname,
									accountno, basic, da, interim_relief, hra, ma, cpf, pf_loan, p_tax, i_tax, net, bank_ifsc, sal_source,
									spl_pay, pf_deduct, code, spl_alo, status_flag, '1' AS delete_status, emp_id_fk, block_code, festival_loan,
									festival_loan_cause, consolidated_pay, conv_allow, hill_allowance, gpf, overdrawn, gsli, gp_code, pay_payband,
									tch_grade_pay, gross_salary, now(), '" . $_SESSION['user_agent']['USER_IP'] . "', requisition_type, part_day,
									zp_id_fk, ropa_status
									from prd_employee_salary_save where
		...
		if($requisition_type == 1003 ){
			...
			{ 
				...
				$insert_bonus_bill = $db->insert(" INSERT INTO prd_block_bill_details ( bill_no, status, block_code, bill_entry_time, bill_update_time, ip_addres, salary_monthyear, ps_id_fk, requisition_type, zp_id_fk,bill_serial_no, drn_number, zp_emp_type,
				ropa_status	)
		...
		else if($requisition_type== 1005)
		{
			...
			$bonus_details_update=$db->update("UPDATE prd_festival_advance_employee_details SET 
											festival_advance_status=5,
											bill_id_fk='".$bill_details_fetch[0]['block_bill_pk']."',
											bill_serial_no='".$bill_serial_no."'
											WHERE festival_advance_status=4 
										    AND fad_monthyear='".$monthyear."' 
											AND zp_id_fk ='".$_SESSION['location']['district_id']."'");
			...
			$fav_bill_fetch = $db->fetch_table("SELECT detail.emp_id_fk, detail.festival_advance_id_pk, sal.festival_advance_instalment_no FROM 
									...
									AND bill.zp_id_fk='".$_SESSION['location']['district_id']."'
			...
			$bonus_details_update=$db->update("UPDATE prd_employee_arrear arr
											SET 
											salary_monthyear='".$monthyear."',
											status_flag='3',
											bill_id_fk='".$bill_details_fetch[0]['block_bill_pk']."',
											...
											AND arr.zp_id_fk = '".$_SESSION['location']['district_id']."' 
											 AND arr.zp_emp_type='".$bill_type."' AND ropa_status='".$ropa_status."' ");
		}
		...
		//var_dump($bonus_details_update); die;
			...
			foreach($get_emp_id_loan as $emp_id_fk_loan)
			{
				if($emp_id_fk_loan['salary_type'] != 8)			
				{
					...
					foreach($get_loan_data as $loan_data)
					{
						...
						loan_id_pk, total_amount, no_of_installment, reminder_amount, status, approval_status , counter , emp_id_fk,
						zp_id_fk, created_by , create_date , who, update_time , ip , installment_amount , emp_unique_id , loan_master_id_fk ,
						dise_code_fk, deduction_loan_type_variable, lock_status, edit_status, delete_status, loan_id_fk, due_amount,
						last_deduction_time, last_deduction_by, loan_name, deduction_by_who, deduction_date, transaction_month_year,
						monthly_deduction_archive_status )
						SELECT loan_id_pk, total_amount, no_of_installment, reminder_amount, status, approval_status , counter , emp_id_fk,
						zp_id_fk, created_by , create_date , who, update_time , ip , installment_amount , emp_unique_id , loan_master_id_fk ,
						dise_code_fk, deduction_loan_type_variable, lock_status, edit_status, delete_status, loan_id_fk, due_amount,
						last_deduction_time, last_deduction_by, loan_name, '".$_SESSION['user_info']['stake_user']."', now(), '".date('Ym')."', '1'
...
if($check_duplicate_bill_no_check[0]['count']=='0' || $check_duplicate_bill_no_check[0]['count']== NULL)		
	{
		...
		if($requisition_type == 1003 ){
			...
			//$prd_monthly_salary_archive_final=TRUE;
			$prd_monthly_salary_archive_final = $db->insert(" INSERT INTO prd_monthly_salary_archive_final
				( slno , ps_id_fk, empcd, bankname, accountno , basic , da, hra, ma , pf_loan, p_tax , i_tax , net , bank_ifsc , sal_source,
				spl_pay , pf_deduct , code, spl_alo, status_flag, salary_monthyear, category_id, block_code, emp_id_fk, pay_payband , tch_grade_pay,
				hill_allowance, gpf, gross_salary , is_saved, conv_allow, overdrawn, salary_type, cause, gsli, delete_status, consolidated_pay,
				other_deduction_cause , other_deduction, festival_loan , festival_loan_cause, part_salary_cause, no_salary_cause, interim_relief,
				lock_status, archive_final_entry_time, archive_final_entry_ip, gp_id_fk, requisition_type, hra_deduction, part_day, ropa_status )
				SELECT slno, ps_id_fk, empcd, bankname, accountno , basic , da, hra, ma , pf_loan, p_tax , i_tax , net , bank_ifsc , sal_source,
				spl_pay , pf_deduct , code, spl_alo, status_flag, salary_monthyear, category_id, block_code, emp_id_fk, pay_payband , tch_grade_pay,
				hill_allowance, gpf, gross_salary , is_saved, conv_allow, overdrawn, salary_type, cause, gsli, delete_status, consolidated_pay,
				other_deduction_cause , other_deduction, festival_loan , festival_loan_cause, part_salary_cause, no_salary_cause,
				interim_relief, lock_status, now(), '".$_SESSION['user_agent']['USER_IP']."', gp_id_fk, '".$requisition_type."', hra_deduction,
				part_day, ropa_status
					FROM prd_employee_salary_save
					WHERE ps_id_fk ='".$_SESSION['location']['ps_id']."'
					AND salary_monthyear ='".$monthyear."'
			...
			$prd_monthly_salary_archive_nonfinal = $db->insert("INSERT INTO prd_monthly_salary_archive_nonfinal(
				slno, entry_time, entry_ip_address, ps_id_fk, empcd, salary_monthyear, bankname, accountno, basic, da, interim_relief,
				hra, ma, cpf, pf_loan, p_tax , i_tax, net, bank_ifsc, sal_source, spl_pay, pf_deduct, code, spl_alo, status_flag, delete_status,
				emp_id_fk, block_code, festival_loan, festival_loan_cause, consolidated_pay, conv_allow, hill_allowance, gpf, overdrawn, gsli,
				pay_payband, tch_grade_pay, gross_salary, archive_nonfinal_entry_time, archive_nonfinal_entry_ip, requisition_type, gp_id_fk,
				hra_deduction, part_day, ropa_status )
				SELECT slno, latestupdate_time, latestupdate_ip_address, ps_id_fk, empcd, salary_monthyear, bankname, accountno, basic, da,
				interim_relief, hra, ma, cpf, pf_loan, p_tax, i_tax, net, bank_ifsc, sal_source, spl_pay, pf_deduct, code, spl_alo, status_flag,
				delete_status, emp_id_fk, block_code, festival_loan, festival_loan_cause, consolidated_pay, conv_allow, hill_allowance, gpf,
				overdrawn, gsli, pay_payband, tch_grade_pay, gross_salary, now(), '".$_SESSION['user_agent']['USER_IP']."', '".$requisition_type."',
				gp_id_fk, hra_deduction, part_day, ropa_status
					from prd_employee_salary_save 
					where salary_monthyear='".$monthyear."' AND ps_id_fk ='".$_SESSION['location']['ps_id']."'  
					AND requisition_type='".$requisition_type."' AND ropa_status='".$ropa_status."' AND status_flag not in('3')");
...
//var_dump($drn_number); die;
	...
	{ 
		...
		if($requisition_type == 1003 ){
			//$prd_monthly_salary_archive_final=TRUE;
		$prd_monthly_salary_archive_final = $db->insert(" INSERT INTO prd_monthly_salary_archive_final
				( slno , gp_id_fk, empcd, bankname, accountno , basic , da, hra, ma , pf_loan, p_tax , i_tax , net , bank_ifsc ,
				sal_source, spl_pay , pf_deduct , code, spl_alo, status_flag, salary_monthyear, category_id, block_code, emp_id_fk,
				pay_payband , tch_grade_pay, hill_allowance, gpf, gross_salary , is_saved, conv_allow, overdrawn, salary_type, cause,
				gp_code, gsli, delete_status, consolidated_pay, other_deduction_cause , other_deduction, festival_loan , festival_loan_cause,
				part_salary_cause, no_salary_cause, interim_relief, lock_status, archive_final_entry_time, archive_final_entry_ip, advance_amount,
				requisition_type, part_day, ropa_status )
				SELECT  slno, gp_id_fk, empcd, bankname, accountno , basic , da, hra, ma , pf_loan, p_tax , i_tax , net , bank_ifsc ,
				sal_source, spl_pay , pf_deduct , code, spl_alo, status_flag, salary_monthyear, category_id, block_code, emp_id_fk, pay_payband ,
				tch_grade_pay, hill_allowance, gpf, gross_salary , is_saved, conv_allow, overdrawn, salary_type, cause, gp_code, gsli, delete_status,
				consolidated_pay, other_deduction_cause , other_deduction, festival_loan , festival_loan_cause, part_salary_cause,
				no_salary_cause, interim_relief, lock_status, now(), '" . $_SESSION['user_agent']['USER_IP'] . "', advance_amount,
				requisition_type, part_day, ropa_status
					FROM prd_employee_salary_save
					WHERE block_code ='" . $_SESSION['location']['block_code'] . "'
					AND salary_monthyear ='" . $monthyear . "'
					AND requisition_type='".$requisition_type."'
					AND status_flag = 3 and delete_status='1' and is_saved='1' AND ropa_status='".$ropa_status."'
					");
															//$prd_monthly_salary_archive_nonfinal=TRUE;
		
		$prd_monthly_salary_archive_nonfinal = $db->insert("INSERT INTO prd_monthly_salary_archive_nonfinal(
				slno, entry_time, entry_ip_address, gp_id_fk, empcd, salary_monthyear, bankname, accountno, basic, da, interim_relief,
				hra, ma, cpf, pf_loan, p_tax , i_tax, net, bank_ifsc, sal_source, spl_pay, pf_deduct, code, spl_alo, status_flag, delete_status,
				emp_id_fk, block_code, festival_loan, festival_loan_cause, consolidated_pay, conv_allow, hill_allowance, gpf, overdrawn,
				gsli, gp_code, pay_payband, tch_grade_pay, gross_salary, archive_nonfinal_entry_time, archive_nonfinal_entry_ip,
				requisition_type, part_day, ropa_status )
				SELECT slno, latestupdate_time, latestupdate_ip_address, gp_id_fk, empcd, salary_monthyear, bankname, accountno, basic, da,
				interim_relief, hra, ma, cpf, pf_loan, p_tax, i_tax, net, bank_ifsc, sal_source, spl_pay, pf_deduct, code, spl_alo, status_flag,
				'1' AS delete_status, emp_id_fk, block_code, festival_loan, festival_loan_cause, consolidated_pay, conv_allow, hill_allowance,
				gpf, overdrawn, gsli, gp_code, pay_payband, tch_grade_pay, gross_salary, now(), '" . $_SESSION['user_agent']['USER_IP'] . "',
				requisition_type, part_day, ropa_status
					from prd_employee_salary_save where
					salary_monthyear='" .$monthyear . "'
		...
		if($requisition_type == 1003 ){
			...
			{
				//$insert_bonus_bill=TRUE;
				$insert_bonus_bill = $db->insert("INSERT INTO prd_block_bill_details (
