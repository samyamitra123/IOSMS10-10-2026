...
$query = "SELECT zp_id from prd_gpf_subscriber_master
          WHERE zp_id != '0' GROUP BY zp_id";

$zpSalarySubscriptionStatus = $db->fetch_obj($query); 
//print_r($gpSalarySubscriptionStatus); exit;
$zp_id = array();

foreach($zpSalarySubscriptionStatus as $value)
{
	$zpId[] = "'".$value->zp_id."'";  
}
$zpId = implode(',', $zpId);

$Query = "Select district_name,district_code from prd_location_master_district WHERE district_code IN (".$zpId.")";
$psInfo = $db->fetch_obj($Query); 
$psData = array();
foreach($psInfo as $item)
{
	$psData[$item->district_code] = $item->district_name;
}
//print_r($psData); exit;
$query = "SELECT a.drn_number, b.salary_monthyear, a.zp_id from prd_gpf_subscriber_master as a
          LEFT JOIN prd_block_bill_details as b
          ON a.drn_number = b.drn_number
          WHERE a.zp_id IN (".$zpId.") ";
$psSalarySubscriptionStatus = $db->fetch_obj($query);
$newpsSubscriptionData = array();
foreach($psSalarySubscriptionStatus as $item)
{
	$newpsSubscriptionData[$item->zp_id][$item->salary_monthyear] = $item;
}
