...
if (!empty($_REQUEST['first_join_date'])) {
    $first_join = explode('-', $_REQUEST['first_join_date']);

    if (count($first_join) === 3 && is_numeric($first_join[0]) && is_numeric($first_join[1]) && is_numeric($first_join[2])) {
        $first_join_date = $first_join[2] . '-' . $first_join[1] . '-' . $first_join[0];
    } else {
        $first_join_date = '0001-01-01';
    }
} else {
    $first_join_date = '0001-01-01';
}
...
if (!empty($_REQUEST['present_post_date'])) {
    $post_date = explode('-', $_REQUEST['present_post_date']);

   
    if (count($post_date) === 3 && is_numeric($post_date[0]) && is_numeric($post_date[1]) && is_numeric($post_date[2])) {
        $present_post_date = $post_date[2] . '-' . $post_date[1] . '-' . $post_date[0];
    } else {
        $present_post_date = '0001-01-01';
    }
} else {
    $present_post_date = '0001-01-01';
}

if (!empty($_REQUEST['present_office_date'])) {
    $office_date = explode('-', $_REQUEST['present_office_date']);

    if (count($office_date) === 3 && is_numeric($office_date[0]) && is_numeric($office_date[1]) && is_numeric($office_date[2])) {
        $present_office_date = $office_date[2] . '-' . $office_date[1] . '-' . $office_date[0];
    } else {
        $present_office_date = '0001-01-01';
    }
} else {
    $present_office_date = '0001-01-01';
}
...
//echo "prd_stake_epension_employee_profile";exit;
 ...
 else
 {
 	...
 	//echo "prd_stake_epension_employee_profile1";exit;

 	//echo $memo_no;exit;
		/*echo "INSERT INTO prd_stake_epension_employee_profile
								(
									first_momo_no,
									first_momo_wef_date ,
									stake_level_id_fk,
...
/*echo "UPDATE prd_employee_master set
							...
							entry_ip='".$_SERVER['REMOTE_ADDR']."'
                            WHERE
						    emp_id_pk='$emp_id_pk'";exit;*/

//echo $emp_id_pk;exit;
//echo "prd_stake_epension_employee_profile3";exit;
...
if($query_insert){
	//echo 'Location:profile_entry_sal.php?desig='.$cryptoGraph->encode($_REQUEST['vice_desig'],4).'&emp_id_pk='.$cryptoGraph->encode($emp_id_pk,4).'&confirm=success';exit;
if($emp_status == 11)
  {
	...
	header('Location:employee_ret_edit_details.php');
	exit(0);

  }	
else 
  {  
//echo 'Location:profile_ret_entry_sal.php?desig='.$cryptoGraph->encode($_REQUEST['vice_desig'],4).'&emp_id_pk='.$cryptoGraph->encode($emp_id_pk,4).'&confirm=success';exit;
header('Location:profile_ret_entry_per.php?desig='.$cryptoGraph->encode($_REQUEST['vice_desig'],4).'&emp_id_pk='.$cryptoGraph->encode($emp_id_pk,4).'&confirm=success');
exit(0);
  }
}
else{
header('Location:profile_ret_entry_prof.php?desig='.$cryptoGraph->encode($_REQUEST['vice_desig'],4).'&emp_id_pk='.$cryptoGraph->encode($emp_id_pk,4).'confirm=false');
exit(0);
}
}
}
