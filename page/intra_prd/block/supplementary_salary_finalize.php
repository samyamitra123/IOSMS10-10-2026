...
<script>
    	$(document).ready(function(){
		  ...
		  $.post('<?php echo $config['base_url'] ?>page/intra_prd/block/supplementary_salary_finalize/not_finalize.php',function(data){
			  
			  $(".emplist").html(data);
		  });
		   });
		   $("#finalize_all").click(function(){
			  $.post('<?php echo $config['base_url'] ?>page/intra_prd/block/supplementary_salary_finalize/lock_gp.php',function(data){
...
$arr=$db->fetch_table("
						SELECT gp_id_pk,gp.gp_code,gp_name,status_flag,count(distinct(emp.emp_id_pk)) 
						as total_emp,count(distinct(sal.emp_id_fk)) as                        
						total_finalized_emp from prd_location_master_gp gp
						...
						AND status_flag in('2','3') and delete_status='1' and is_saved='1'
...
$lock_button = $db->fetch_table("SELECT count(*) FROM prd_employee_salary_save WHERE status_flag =2 AND
		block_code = '".$_SESSION['location']['block_code']."' 
		AND salary_monthyear='".date('Ym')."' and delete_status='1' and is_saved='1' AND requisition_type='".$requisition_type."'");
		//var_dump($lock_button); die;
/*
...
$emp_check=$db->fetch_table("
						SELECT gp_id_pk,gp.gp_code,gp_name from prd_location_master_gp gp inner JOIN
 prd_employee_master emp on emp.gp_id_fk=gp.gp_id_pk  
...
<?php if($item['status_flag']=='2'){ ?>
  <a class="btn btn-sm btn-success" id="unlock_active" href="<?php echo $config['base_url'] ?>page/intra_prd/block/supplementary_salary_finalize/unlock_gp.php?gp_code=<?php echo $cryptoGraph->encode($item['gp_code'],4); ?>">Unlock</a>
  <a class="btn btn-sm btn-success" id="unlock_active" href="<?php echo $config['base_url'] ?>page/intra_prd/block/supplementary_salary_finalize/view_salary_requisition.php?gp_code=<?php echo $cryptoGraph->encode($item['gp_id_pk'],4); ?>">View Requisition</a> 
