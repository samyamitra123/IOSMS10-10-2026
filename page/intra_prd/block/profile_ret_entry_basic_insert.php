...
if (isset($_POST['location_type'])) {
    ...
    if ($location_type == 'gp' && !empty($_POST['gp_id'])) {
        $gp_id_fk = $_POST['gp_id'];
    } elseif (!empty($location_type)) {
        // If not 'gp', assume it's PS and assign PS id from location_type value
        $ps_id_fk = $location_type; // Since PS value is in location_type
    }
}

$_SESSION['location']['gp_id'] = $gp_id_fk;
...
//echo "<pre>";print_r($_SESSION['location']['gp_id']);exit;
			...
			include 'profile_retired_entry_basic_form.php';
...
$arr = $db->fetch_table("select max(empcd) as entry_rec_tchcd from prd_employee_master where gp_id_fk='".$gp_id_fk."'");
...
$sql=$db->fetch_table("SELECT emp_id_pk FROM prd_employee_master WHERE empcd='".$empcd."' and gp_id_fk='".$gp_id_fk."'");

//echo "SELECT emp_id_pk FROM prd_employee_master WHERE empcd='".$empcd."' and gp_id_fk='".$gp_id_fk."'";
...
header('Location:profile_entry_ret_prof.php?emp_date_retirement='.$cryptoGraph->encode($emp_date_retirement,4).'&emp_id_pk='.$cryptoGraph->encode($emp_id_pk,4).'&confirm=success');
...
header('Location:profile_ret_entry_basic.php?confirm=false');
