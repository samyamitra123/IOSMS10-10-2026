...
$arr=$db->fetch_table(" SELECT emp_id_fk,gp_id_fk from prd_employee_salary_save where gp_code='$gp_code' AND ropa_status='1'");
...
$update = $db->update("
		...
		SET status_flag ='1'
		WHERE status_flag ='2' AND gp_code = '".$gp_code."' AND salary_monthyear='".date('Ym')."' 
		and delete_status='1' and is_saved='1' AND requisition_type='".$requisition_type."'
		AND ropa_status='1'");
}
...
if($security)
{
	$_SESSION['msg']='<div class="alert alert-success" style="text-align:center"><strong>GP successfully unlocked.</strong></div>';
	 ...
	 header('Location:'.$config['base_url'].'page/intra_prd/block/salary_finalize_ropa_2019.php');
	 exit(0);
