...
<script>
    	$(document).ready(function(){
		  ...
		  $.post('<?php echo $config['base_url'] ?>page/intra_prd/block/salary_finalize/salary_finalize.php',function(data){
			  $(".emplist").html(data);
		  });
		   })
		});
...
/*$arr=$db->fetch_table("
						...
						LEFT JOIN prd_employee_salary_save sal on CAST(emp.gp_id_fk AS text)=sal.gp_id_fk AND status_flag='1' OR status_flag IS NULL
						WHERE CAST(gp.gp_code AS text) like '".$_SESSION['location']['block_code']."%'  GROUP BY gp_name,gp_id_pk,gp.gp_code,status_flag order by gp_name
