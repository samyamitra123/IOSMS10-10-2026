    	...
    	$(document).ready(function(){
		  ...
		  $.post('<?php echo $config['base_url'] ?>page/intra_prd/block/salary_finalize/not_finalize_ropa_2019.php',function(data){
		   ...
		   $("#finalize_all").click(function(){
			  $.post('<?php echo $config['base_url'] ?>page/intra_prd/block/salary_finalize/lock_gp_ropa_2019.php',function(data){
...
$arr=$db->fetch_table(" SELECT gp_id_pk,gp.gp_code,gp_name,status_flag,
...
$arrQuery = " SELECT gp_id_pk,gp.gp_code,gp_name,status_flag 
						from prd_location_master_gp gp
						inner JOIN prd_employee_master emp on emp.gp_id_fk=gp.gp_id_pk 
...
$arr_check=$db->fetch_table($arrQuery);		

$empQuery = " SELECT gp_id_pk,gp.gp_code,gp_name from prd_location_master_gp gp 
								...
								GROUP BY gp_name,gp_id_pk,gp.gp_code order by gp_name
								";
$emp_check=$db->fetch_table($empQuery);	
//print_r($emp_check); exit
...
<div class="content">
<?php //echo  count($arr_check).'---'.count($emp_check);?>
...
<?php if($item['status_flag']=='2'){ ?>
  <a class="btn btn-sm btn-success" id="unlock_active" href="<?php echo $config['base_url'] ?>page/intra_prd/block/salary_finalize/unlock_gp_ropa_2019.php?gp_code=<?php echo $cryptoGraph->encode($item['gp_code'],4); ?>">Unlock</a>
  <a class="btn btn-sm btn-success" id="unlock_active" href="<?php echo $config['base_url'] ?>page/intra_prd/block/salary_finalize/view_salary_requisition_ropa_2019.php?gp_code=<?php echo $cryptoGraph->encode($item['gp_id_pk'],4); ?>">View Requisition</a> 
