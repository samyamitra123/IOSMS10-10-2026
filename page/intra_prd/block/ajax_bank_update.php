...
$gp_id=$gp[0]['gp_id_fk'];
if(trim($cryptoGraph->decode($_REQUEST['flag'],4))=='transfer'){
$update_transfer=$db->update("update prd_employee_master set bank_upd_status=2
where emp_id_pk='".$id."'");
}
else if(trim($cryptoGraph->decode($_REQUEST['flag'],4))=='reject'){
$update_reject=$db->update("update prd_employee_master set bank_upd_status='0'
where emp_id_pk='".$id."'");
}
if($update_transfer){
	$_SESSION['bank_msg']='<div class="alert alert-success" style="text-align:center"><strong>Request For Bank Details Update Successfully Transfer To Admin.</strong></div>';
