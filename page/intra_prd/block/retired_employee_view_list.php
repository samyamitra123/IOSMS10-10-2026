...
$Query = "select emp_id_const,emp_first_name,emp_second_name,emp_last_name,emp_status,emp_id_pk,emp_system_code from prd_employee_master where emp_status in('99','100','7') AND block_id='".$_SESSION['location']['block_id']."' order by emp_id_const";
//print_r($Query); exit;
...
<td><?= $status; ?></td>
<input type="hidden" id="id" name="id" value="<?= $cryptoGraph->encode($item['emp_id_pk'],4);?>"  />
...
<script>
		...
		//alert(link);
//alert('<?= $config['base_url'] ?>page/intra_prd/block/ajax_ret_emp_view.php?id='+link);
		$.post('<?= $config['base_url'] ?>page/intra_prd/block/ajax_ret_emp_view.php?id='+link, function(data){
			alert(data);
				  $(".mbody").html(data);
			  	});
		});
	...
	$.post('<?= $config['base_url'] ?>page/intra_prd/block/ajax_ret_finalize_reject.php?id='+link1+'&flag=finalize', function(data){
	...
	$.post('<?= $config['base_url'] ?>page/intra_prd/block/ajax_ret_finalize_reject.php?id='+link2+'&flag=reject', function(data){
