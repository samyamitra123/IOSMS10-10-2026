...
<?php $db = new database(); 
$arr_gp = $db->fetch_table("select gp_id_pk,gp_name from prd_location_master_gp where block_id_fk='".$_SESSION['location']['block_id']."'");
...
<? require '../../../page/common_back_btns.php'; ?>
  ...
  <div class="welcome_msg">
                      ...
                      <?php
                      //echo "<pre>";print_r($_SESSION['location']['block_id']);exit;
...
<form class="form-horizontal" id="first_form" method="post" action="profile_ret_entry_basic_insert.php" onsubmit="return valid_code();"> 
...
<div class="row mb-3">
  ...
  <div class="row mb-3">
<div class="col-md-3">
		<label class="form-label">Select Type <span class="star_color">*</span></label><br>
  <div class="form-check form-check-inline">
    <input class="form-check-input" type="radio" name="location_type" id="gpRadio" value="gp" checked onclick="toggleGpDropdown(true)">
    <label class="form-check-label" for="gpRadio">GP</label>
  </div>
  <div class="form-check form-check-inline">
    <input class="form-check-input" type="radio" name="location_type" id="psRadio" value="<?php echo $_SESSION['user_info']['stake_user']; ?>" onclick="toggleGpDropdown(false)">
    <label class="form-check-label" for="psRadio">PS</label>
  </div>
</div>
  <!-- GP Dropdown -->
  <div class="col-md-3">
    <label for="gpSelect" class="form-label">Gram Panchayat <span class="star_color">*</span></label>
    <select class="form-control" name="gp_id" id="gpSelect">
      <option value="">-Please Select-</option>
      <?php foreach($arr_gp as $key){ ?>
        <option value="<?= $key['gp_id_pk']; ?>">
          <?= $key['gp_name']; ?>
        </option>
      <?php } ?>
    </select>
  ...
  <!-- Date of Birth -->
  <div class="col-md-3">
    <label for="tch_dob" class="form-label">Date Of Birth <span class="star_color">*</span></label>
    <input type="text" class="form-control" id="tch_dob" name="tch_dob"
           onChange="calculate_retireyear(this.value)"
           value="<?= ($tch_dob == '0001-01-01') ? '' : dateshow($dob); ?>"
           placeholder="DD-MM-YYYY" readonly style="background-color:#FFF;cursor:pointer;" />
  </div>

  <!-- Sex Dropdown -->
  <div class="col-md-3">
    <label for="drpSex" class="form-label">Sex <span class="star_color">*</span></label>
    <?php
      $db = new database();
    ...
    <select class="form-control" name="drpSex" id="drpSex">
      ...
      <?php foreach($arr as $key){ ?>
        <option value="<?= $key['code']; ?>" <?= (!empty($drpSex) && $drpSex == $key['code']) ? 'selected' : '' ?>>
          <?= $key['description']; ?>
        </option>
      <?php } ?>
    </select>
...
<script>
	...
	function toggleGpDropdown(enable) {
  const gpSelect = document.getElementById('gpSelect');
  gpSelect.disabled = !enable;

  if (!enable) {
    gpSelect.selectedIndex = 0; // Clear selection if PS is selected
  }
}
