...
function get_emp(){ 
	...
	$data = $db->fetch_table("select emp_first_name,emp_second_name,emp_last_name,emp_form_status,emp_id_pk,emp_desig from prd_employee_master where emp_form_status in('1','2','3','4','5','99') AND emp_status='6' AND block_id='".$_SESSION['location']['block_id']."' order by emp_first_name");
...
function send_to_ddo($pk){
	...
	$data = $db->update("
							...
							WHERE emp_id_pk = '".$pk."'
								...
								AND emp_status in (10,7,11,99)
								AND emp_form_status = '99'
    	...
    	$(document).ready(function(){
		  ...
		  $("#dialog").dialog({
			...
			$(".sendtoddo a").click(function() {
			    ...
			    $(".emplist").load('<?= $config['base_url'] ?>page/intra_prd/gp/ajax_ret_employee_edit_details.php?id='+link, function(responseTxt,statusTxt,xhr){
			      if(statusTxt=="error"){
...
<td><? if($item['emp_form_status']=='1' || $item['emp_form_status']=='2' || $item['emp_form_status']=='3' || $item['emp_form_status']=='4' || $item['emp_form_status']=='5'||$item['emp_form_status']=='99'){ ?>
...
<td><? if($item['emp_form_status']=='4' || $item['emp_form_status']=='4' || $item['emp_form_status']=='5' || $item['emp_form_status']=='99'){ ?>
<a href="profile_ret_entry_per.php?emp_id_pk=<?= $cryptoGraph->encode($item['emp_id_pk'],4) ?>&desig=<?=$cryptoGraph->encode($item['emp_desig'],4) ?>">
<img width="25" src="<?= $config['base_url'];?>themes/default/image/edit_icon.png" alt="Edit"  />
</a><? } ?>
</td>

<td><? if($item['emp_form_status']=='5'|| $item['emp_form_status']=='99'){ ?>
<a href="profile_ret_entry_contact.php?emp_id_pk=<?= $cryptoGraph->encode($item['emp_id_pk'],4) ?>">
<img width="25" src="<?= $config['base_url'];?>themes/default/image/edit_icon.png" alt="Edit"  />
</a><? } ?>
</td>

<td class="sendtoddo"><? if($item['emp_form_status']=='5' || $item['emp_status']=='99'){ ?><a href="<?php echo $cryptoGraph->encode($item['emp_id_pk'],4) ?>"><img width="25" src="<?= $config['base_url'];?>themes/default/image/send.png" alt="Edit"/></a></a>
<? } else{ ?>
