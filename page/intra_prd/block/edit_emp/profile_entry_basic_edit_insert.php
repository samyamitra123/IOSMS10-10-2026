...
else
{
	...
	$birth_date=explode('-',$_REQUEST['tch_dob']);
	...
	else if($validator->blank_select($_REQUEST['tch_dob']) == FALSE || $_REQUEST['tch_dob'] == '0001-01-01')
	{
	...
	else if($validator->blank_select($_REQUEST['tch_dob']) == TRUE && $validator->valid_age($_REQUEST['tch_dob']) == FALSE)
	{
	...
	/*echo "select emp_id_pk,emp_voter_id from prd_employee_master where emp_voter_id !=
	...
	else
	{
		...
		//echo $cryptoGraph->encode($gp_id,4);exit;
		...
		if($query_insert && $update_id=='true')
		{
			...
			{
				header('Location:profile_entry_prof.php?emp_date_retirement='.$cryptoGraph->encode($emp_date_retirement,4).'&emp_id_pk='.$cryptoGraph->encode($emp_id_pk,4).'&gp_id='.$cryptoGraph->encode($gp_id,4).'&confirm=success');
				exit(0);
			}
			else
			{
				header('Location:profile_entry_prof.php?emp_id_pk='.$cryptoGraph->encode($emp_id_pk,4).'&gp_id='.$cryptoGraph->encode($gp_id,4).'&confirm=success');
				exit(0);
			}
		}
		...
		{
			header('Location:profile_entry_basic_edit.php?emp_id_pk='.$cryptoGraph->encode($emp_id_pk,4).'&gp_id='.$cryptoGraph->encode($gp_id,4).'&confirm=false');
			exit(0);
		}
	}
}
