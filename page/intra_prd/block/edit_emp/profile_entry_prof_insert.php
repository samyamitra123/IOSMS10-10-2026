		...
		/*if($emp_id_pk=='9373')
{
	...
	$emp_date_termination=date("Y-m-d", strtotime($_POST['emp_date_retirement']));
	...
	$increment_amount=($_REQUEST['increment_amount']);
 ...
 else
 {
		...
		if($query)
		{
			...
			on mas.emp_id_pk=stake.emp_id_fk where stake.emp_id_fk='$emp_id_pk' ");
		...
		{
		//echo $update_id;exit;
			header('Location:profile_entry_sal.php?desig='.$cryptoGraph->encode($_REQUEST['vice_desig'],4).'&emp_id_pk='.$cryptoGraph->encode($emp_id_pk,4).'&gp_id='.$cryptoGraph->encode($gp_id,4).'&confirm=success');
			exit(0);
		}
		else if($update_id=='false')
		{
			header('Location:profile_entry_sal.php?desig='.$cryptoGraph->encode($_REQUEST['vice_desig'],4).'&emp_id_pk='.$cryptoGraph->encode($emp_id_pk,4).'&gp_id='.$cryptoGraph->encode($gp_id,4));
			exit(0);
		}
		else
		{
			header('Location:profile_entry_prof.php?desig='.$cryptoGraph->encode($_REQUEST['vice_desig'],4).'&emp_id_pk='.$cryptoGraph->encode($emp_id_pk,4).'&gp_id='.$cryptoGraph->encode($gp_id,4).'confirm=false');
			exit(0);
		}
	}
}
