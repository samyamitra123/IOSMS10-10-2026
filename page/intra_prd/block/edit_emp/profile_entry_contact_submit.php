...
else
{
	...
	if($insrt_contact && $update_id=='true')
	{
		header('Location:edit_employee.php?emp_id_pk='.$cryptoGraph->encode($emp_id_pk,4).'&gp_id='.$cryptoGraph->encode($gp_id,4).'&confirm=success');
		exit(0);
	}
	else if($update_id=='false')
	{
		header('Location:edit_employee.php?emp_id_pk='.$cryptoGraph->encode($emp_id_pk,4).'&gp_id='.$cryptoGraph->encode($gp_id,4));
		exit(0);
	}
	else
	{
