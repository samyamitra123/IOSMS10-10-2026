...
{
	$.post('<?= $config['base_url'] ?>page/intra_prd/block/edit_emp/ajax_payscale_details.php?payband='+val, function(data){
		$(".payscale").html(data);	 
		$.post('<?= $config['base_url'] ?>page/intra_prd/block/edit_emp/ajax_gradepay_details.php?payband='+val, function(data){
			$(".gradepay").html(data);	 
		});
	});
}
            ...
            <div class="col-sm-12" style="width:98%;">
            	...
            	<form class="form-horizontal" id="loginForm" method="post" action="profile_entry_salary_insert.php" onsubmit="return valid_code();">
					...
					{ ?>
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-4">
                            	<input type="text" class="form-control" name="cons_pay" id="cons_pay" maxlength="5" placeholder="Consolidated Pay" autocomplete="off" value="<?= $emp_cosolidated_pay?>" onKeyPress="return keyRestrict(event,'0123456789');">
                            </div>
                        </div>
                    <?php } 
					else 
				...
				<div class="row mb-3">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-4">
                                ...
                                <select class="form-control" name="pay_band" id="pay_band" onchange="payband(this.value)">
                                    ...
                                    <option value="<?= $key['payband_code']; ?>" <? if($emp_pay_band==$key['payband_code']){ echo "selected";}?>><?= $key['payband_name']; ?></option>
                                    <? } ?>
                                </select>
                            </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-4">
                            	<input type="text" class="form-control" name="pay_in_payband" id="pay_in_payband" placeholder="PAY IN PAYBAND" autocomplete="off" value="<?=$emp_pay_in_payband ?>" onKeyPress="return keyRestrict(event,'0123456789');" maxlength="5">
                            </div>
                            ...
                            <div class="col-sm-4">
                                ...
                                ?>
                                <select class="form-control gradepay" name="grade_pay" id="grade_pay" placeholder="Grade Pay">
                                    <option value="">-Please Select-</option>
                                    <? foreach($arr as $key){ $key['grade_code']. '<br />'; ?>
                                    <option value="<?= $key['grade_code']; ?>" <? if($emp_grade_pay==$key['grade_code']){ echo "selected";}?>><?= $key['grade_amount']; ?></option>
                                    <? } ?>
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-4">
                            ...
                            <select class="form-control" id="txt_bank" name="txt_bank">
                                ...
                                <option value="<?= $key['bank_code']; ?>" <? if($emp_bank_name==$key['bank_code']){ echo "selected";}?>><?= $key['bank_name']; ?></option>
                                <? } ?>
                            </select>
                        </div>
                    ...
                    <div class="row mb-3">
                        <div class="col-sm-12">
                            ...
                            <a href="profile_entry_prof.php?emp_id_pk=<? if(!empty($_GET['emp_id_pk'])){ echo $_GET['emp_id_pk']; } else { echo $emp_id;} ?>&gp_id=<? if(!empty($_GET['gp_id'])){ echo $_GET['gp_id']; } else { echo $gpid;} ?>" class="btn btn-danger" style="float:left;"> <i class="fa fa-chevron-left"></i> PREVIOUS</a>
                        </div>
                    </div>
                </form>
            </div>
...
<script>
	...
	{		
		...
		var emp_cosolidated_pay= '<?php if($emp_cosolidated_pay=='0'){ echo "";}else{ echo $emp_cosolidated_pay;} ?>';
		var emp_pay_band= '<?php if($emp_pay_band=='0'){ echo "";}else{ echo $emp_pay_band;} ?>';
		var emp_pay_in_payband= '<?php if($emp_pay_in_payband=='0'){ echo "";}else{ echo $emp_pay_in_payband;} ?>';
		var emp_grade_pay= '<?php if($emp_grade_pay=='0'){ echo "";}else{ echo $emp_grade_pay;} ?>';
		var emp_pay_scale= '<?php if($emp_pay_scale=='0'){ echo "";}else{ echo $emp_pay_scale;} ?>';
		var emp_bank_name= '<?php echo $emp_bank_name; ?>';
		var emp_bank_branch= '<?php echo $emp_bank_branch; ?>';
		var emp_branch_code= '<?php echo $emp_branch_code; ?>';
		var emp_micr_no= '<?php echo $emp_micr_no; ?>';
		var emp_acc_no= '<?php echo $emp_acc_no; ?>';
		var emp_ifsc_no= '<?php echo $emp_ifsc_no; ?>';
		
		if($('#desig').val()=='1120' || $('#desig').val()=='1124' || $('#desig').val()=='1125')
		{
			var val_emp_cosolidated_pay= $('#cons_pay').val();
			var val_emp_pay_band= "";
			var val_emp_pay_in_payband= "";
			var val_emp_grade_pay= "";
			var val_emp_pay_scale= "";
		}
		else
		{
			var val_emp_cosolidated_pay= "";
			var val_emp_pay_band= $('#pay_band').val();
			var val_emp_pay_in_payband= $('#pay_in_payband').val().trim();
			var val_emp_grade_pay= $('#grade_pay').val();
			var val_emp_pay_scale= $('#pay_scale').val();
		}
		var val_emp_bank_name= $('#txt_bank').val();
		var val_emp_bank_branch= $('#tch_bank_branch').val().trim();
		var val_emp_branch_code= $('#tch_branch_code').val().trim();
		var val_emp_micr_no= $('#tch_micr_code').val().trim();
		var val_emp_acc_no= $('#txt_account').val().trim();
		var val_emp_ifsc_no= $('#txt_ifsc').val().trim();
		
		if(emp_cosolidated_pay!=val_emp_cosolidated_pay || emp_pay_band!=val_emp_pay_band || emp_pay_in_payband!=val_emp_pay_in_payband || emp_grade_pay!=val_emp_grade_pay || emp_pay_scale!=val_emp_pay_scale || emp_bank_name!=val_emp_bank_name || emp_bank_branch!=val_emp_bank_branch || emp_branch_code!=val_emp_branch_code || emp_micr_no!=val_emp_micr_no || emp_acc_no!=val_emp_acc_no || emp_ifsc_no!=val_emp_ifsc_no)
		{
		...
		if(emp_pay_in_payband!=val_emp_pay_in_payband || emp_grade_pay!=val_emp_grade_pay)
		{
