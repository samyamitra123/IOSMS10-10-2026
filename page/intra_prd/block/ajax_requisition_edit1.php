...
function getAmount($dise,$emp_id_pk,$type,$basic){
			...
			if($type=='conv'){
				...
				if($arr[0]['conv_allow']){
                 ...
                 /* $dedact = $db->fetch_table("select id,deduction_type from prd_salary_deduction_master where id=1"); 
                   $dedact[0]['id']; */
...
$paychange = $db->fetch_table("
       ...
       overdrawn, salary_type, cause,gp_code, part_day,gsli,consolidated_pay,festival_loan,festival_loan_cause from prd_employee_salary_save where status_flag=1 AND delete_status=1 AND salary_monthyear='".date('Ym')."'  AND emp_id_fk='$emp_id_pk' AND gp_id_fk='".$dise."'");
       ...
       category_id, block_code, emp_id_fk, pay_payband, tch_grade_pay, 
       hill_allowance, gpf, cpf_deduct, gross_salary, is_saved, conv_allow, 
       overdrawn, salary_type, cause,gp_code, part_day,gsli,consolidated_pay,festival_loan,festival_loan_cause from prd_employee_salary_save where status_flag=3 AND delete_status=1 AND salary_monthyear='".date('Ym',strtotime('-1 month'))."'  AND emp_id_fk='$emp_id_pk' AND gp_id_fk='".$dise."'");
	...
	$('#no_ovd').click(function(){
                ...
                <table>
                	<tr>
                        ...
                        <td>
							...
							");
							$sal_caus = $db->fetch_table("select overdrawn,salary_type,cause,part_day,cpf,gpf,gsli,part_salary_cause,no_salary_cause from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='".$tch[0]['emp_id_pk']."' AND gp_id_fk='".$dise."'");
							?>
               ...
               <span id="limit_holder"> <span id="limit" style="font-weight:bold; color:#F00;">500</span><span> <strong>of 500 maximum character.</strong></span></span>
                      ...
                      <?php 
							...
							$dedact_cause = $db->fetch_table("select festival_loan_cause from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='".$tch[0]['emp_id_pk']."' AND gp_id_fk='".$dise."'");
		...
		var work_days = $('#working_days').val();
			...
			if($('#working_days').val() >= parseInt($('#month_last_date').val()) || $('#working_days').val() == '0'){
				...
				//	alert(145);
					...
					var ma = $('#ma').val();
					var part_ma = Math.round((parseInt(<?php echo $ma; ?>)/parseInt(exist_part_day))*parseInt(work_days));
					$('#ma').val(part_ma);
