...
$db = new database();	
	...
	$sql_query=$db->fetch_table("select bill_no,bill_entry_time,salary_monthyear from prd_block_bill_details where block_code='".$_SESSION['location']['block_code']."' and bill_no='".$bill_no."' and salary_monthyear= '".$month_year."' AND requisition_type='".$requisition_type."' AND status='1'");
...
/********************************************************************************************/
/*$sql_salarymonth=$db->fetch_table("
select DISTINCT(sal.salary_monthyear), ad.lead_bank, ad.lead_branch, sal.salary_monthyear from prd_employee_master as emp inner join prd_location_master_gp as gp on emp.gp_id_fk=gp.gp_id_pk inner join prd_employee_salary_save as sal on CAST(emp.empcd AS text)=sal.empcd and CAST(emp.gp_id_fk AS character varying)=sal.gp_id_fk inner join prd_dise_admin ad on ad.block_code=sal.block_code
		where sal.block_code='".$_SESSION['user_info']['stake_user']."' 
					...
					and 
						sal.status_flag='3'
					and 
						emp.emp_status='1' OR emp.emp_status='9'");
						*/


$sql_salarymonth=$db->fetch_table("
select DISTINCT(sal.salary_monthyear), ad.lead_bank, ad.lead_branch from prd_employee_master as emp 
inner join prd_location_master_gp as gp on emp.gp_id_fk=gp.gp_id_pk 
inner join (select salary_monthyear,status_flag,block_code,empcd,gp_id_fk,requisition_type from prd_employee_salary_save where salary_monthyear='".$sql_query[0]['salary_monthyear']."') as sal 
on CAST(emp.empcd AS text)=sal.empcd 
