...
//echo $spouse_medical_allowance; die;
...
if($form_status>4){
	...
	$emp_form_status=4;
			...
			include 'profile_ret_entry_personal_form.php';
			exit;
			}
			...
			{
				$error_msg='<div class="alert alert-danger" style="text-align:center"><strong>Please Enter Numeric Value In Height Field.</strong></div>';
				$emp_id=$cryptoGraph->encode($emp_id_pk,4);
				include 'profile_ret_entry_personal_form.php';
				exit;
			}
...
/*		echo "UPDATE prd_employee_master set
		...
		if($query_insert){
		header('Location:profile_ret_entry_contact.php?emp_id_pk='.$cryptoGraph->encode($emp_id_pk,4).'&confirm=success');
		exit(0);
		}
		else{
		header('Location:profile_ret_entry_per.php?emp_id_pk='.$cryptoGraph->encode($emp_id_pk,4).'&desig='.$cryptoGraph->encode($_REQUEST['desig'],4).'&confirm=false');
		exit(0);
		}
	}
}
