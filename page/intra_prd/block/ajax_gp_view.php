...
?>
<!--<div class="downpdf" align="right"><a href="<?= $config['base_url']?>page/intra_prd/block/pdf_gp_details.php?id=<?=$_REQUEST['id']?>"><img src="<?= $config['base_url'] ?>themes/default/image/pdf_download.png"/></a></div>-->
...
<td><strong>NAME OF PARDHAN:</strong></td>
...
<? if($flag=='1'){ ?>
<div>
<a class="btn btn-success" style="margin-left:43%" href="<?= $config['base_url']?>page/intra_prd/block/gp_profile_finalize.php?id=<?=$_REQUEST['id']?>&action=<?= $cryptoGraph->encode('approve',4)?>">Approve</a>
<a class="btn btn-danger" href="<?= $config['base_url']?>page/intra_prd/block/gp_profile_finalize.php?id=<?=$_REQUEST['id']?>&action=<?= $cryptoGraph->encode('reject',4)?>">Reject</a>  
</div>
<? } ?>