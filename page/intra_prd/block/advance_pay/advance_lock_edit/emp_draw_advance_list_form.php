        ...
        <input type="hidden" name="emp_id" id="emp_id<?php echo $m; ?>" value="<?php echo $crypto->encode($key['emp_id_pk'],4) ?>" />
         ...
         <? 
		 	...
		 	$emp_data = $db->fetch_table("
							SELECT status_flag
							FROM prd_employee_salary_save where emp_id_fk='".$key['emp_id_pk']."' and is_saved='1' and delete_status='1' and status_flag='1' and salary_monthyear='".date('Ym')."'
	
						");
		 
		 ?>
              <td>
              <? if((!$emp_data) || ($emp_data[0]['status_flag']==1)) { ?>
              
               <button type="submit" class="btn btn-info" onclick="submit_data(<?php echo $m; ?>)" >Submit</button></td>
              <? } else { ?>
      			
                <button type="submit" class="btn btn-info" disabled >Submit</button></td>
...
function submit_data(k)
{
		...
		$.post('<?= $config['base_url'] ?>page/intra_prd/block/advance_pay/advance_lock_edit/entry_draw_advance_list_insert.php?id='+link+'&gp_id='+link1+'&emp_id='+link2+'&desig='+link3+'&adance_type='+link4, function(data){
				//alert(data);
				if(data=='success')
			 {
			 ...
			 if(data=='fail')
			 {
				$('#sess_msg').html('<div class="alert alert-danger" style="text-align:center"><strong> Sorrry!!! '+emp_name+'\'s Data Updation Failed.. </strong></div>');
			 }
