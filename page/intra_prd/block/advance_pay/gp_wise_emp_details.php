...
$arr=$db->fetch_table( "
						...
						AND advance_monthyear='".date('Ym')."' and adv.status_flag in('3')
...
$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('D5', 'Employee Id')
            ->setCellValue('E5', 'Desgnation')
			->setCellValue('F5', 'Grade pay')
			->setCellValue('G5', 'Willing to Draw advance');

			
			cellColor('A5:G5', 'C6E9F4');
...
foreach ($arr as $key) {
	...
	$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue('C'.$count,$key['emp_first_name'].' '.$key['emp_second_name'].' '.$key['emp_last_name'])
				->setCellValue('D'.$count,$key['emp_id_const'])
				->setCellValue('E'.$count,fun_common($key['emp_desig'],$code_data))
				->setCellValue('F'.$count,func_gradepay($key['emp_grade_pay'],$code_data))
				->setCellValue('G'.$count, $key['advance_amount']);
