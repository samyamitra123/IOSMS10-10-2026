...
$block_id=$db->fetch_table("select block_id from prd_employee_master where emp_id_pk='".$cryptoGraph->decode($_REQUEST['id'], 4)."'");
$gp=$gp_id['0']['gp_id_fk'];
$block = $block_id['0']['block_id'];
...
if($_REQUEST['flag']=='finalize'){
	...
	$empid_const=$fun_store->check_ret_unique_emp_const_id();
	//echo $_POST['id']; exit;
	$db = new database();
...
$msg='<p class="alert alert-success" style="text-align:center"><strong>Employee Has Been Successfully Finalized.</strong></p>';
	?>
    <script>
    window.location.href="<?=$config['base_url'] ?>page/intra_prd/block/retired_employee_view_list.php?id=<?=$cryptoGraph->encode($block, 4)?>&msg=<?=$cryptoGraph->encode($msg,4);?>";
    </script>
	<? } else{ 
