...
/*$get_gp_id=$db->fetch_table("
							SELECT 
							gp_id_fk
							FROM prd_employee_master
							WHERE emp_id_pk = '".$obj_crpto->decode($_GET['emp_id_pk'],4)."'");*/


//echo $_GET['emp_id_pk'];exit;
//$decoded_id = $obj_crpto->decode($_GET['emp_id_pk'], 4); 

//echo $decoded_id;exit;

//echo "SELECT gp_id_fk FROM prd_employee_master WHERE emp_id_pk = '" . $decoded_id . "'"; exit;
...
function get_emp($id){
	...
	$data = $db->fetch_table("
							...
							and block_id = '".$_SESSION['location']['block_id']."'
...
else
{
   $emp_data_addl = get_emp_addl($emp_id);
		...
		$emp_status = $emp_data[0]['emp_status'];
...
<form class="form-horizontal" name="teacher_info" id="teacher_info" method="post" action="profile_ret_entry_prof_insert.php" onsubmit="return validateCode();">
    ...
    <div class="col-sm-3">
      ...
      <option value="">-Please Select-</option>
                      <? 
						...
						<? 
 						if($emp_desig==$key['code'] || $vice_desig==$key['code']){ echo "selected";}?>><?= $key['description']; ?></option>
                         <? } ?>
      </select>
    </div>
    <label for="inputPassword3" class="col-sm-3 control-label">Date of First Joining in Service</label>
    <div class="col-sm-3">
      <input type="text" class="form-control" name="first_join_date" id="first_join_date"  placeholder="DD-MM-YYYY" autocomplete="off" value="<?=$emp_first_join_date=='0001-01-01' ? '' : dateshow($emp_first_join_date);?>" readonly="readonly" style="background-color:#FFF;cursor:pointer;">
    </div>
  </div>
  
  
  <div class="row mb-3">
  <label for="inputPassword3" class="col-sm-3 control-label">Date of Confirmation in Service Applicable</label>
    <div class="col-sm-3">
  ...
  <div class="row mb-3">
     <label for="inputPassword3" class="col-sm-3 control-label confirm_date_label" style="display:none">Date of Confirmation in Service </label>
    <div class="col-sm-3 confirm_date_div" style="display:none"> 
      <input type="text" class="form-control" name="confirm_date" id="confirm_date" placeholder="DD-MM-YYYY" autocomplete="off" value="<?=$emp_conf_join_date=='0001-01-01' ? '' : dateshow($emp_conf_join_date);?>" readonly="readonly" style="background-color:#FFF;cursor:pointer;">
  </div>
  <label for="inputPassword3" class="col-sm-3 control-label">Date of Joining in the Present Post </label>
    <div class="col-sm-3">
      <input type="text" class="form-control" name="present_post_date" id="present_post_date" placeholder="DD-MM-YYYY" autocomplete="off"  value="<?=$emp_join_prsnt_post_date=='0001-01-01' ? '' : dateshow($emp_join_prsnt_post_date);?>" readonly="readonly" style="background-color:#FFF;cursor:pointer;"/>
  </div>
  </div>
  <div class="row mb-3">
    <label for="inputPassword3" class="col-sm-3 control-label">Date of Joining in the Present Office </label>
    <div class="col-sm-3">
      <input type="text" class="form-control" name="present_office_date" id="present_office_date" placeholder="DD-MM-YYYY" autocomplete="off"  value="<?=$emp_join_prsnt_office_date=='0001-01-01' ? '' : dateshow($emp_join_prsnt_office_date);?>" readonly="readonly" style="background-color:#FFF;cursor:pointer;">
    </div>
    <label for="inputPassword3" class="col-sm-3 control-label">Date of Retirement / Termination </label>
    <div class="col-sm-3">
      <input type="text" class="form-control" name="emp_date_retirement" id="emp_date_termination" readonly="readonly" placeholder="DD-MM-YYYY" autocomplete="off"  value="<?  echo dateshow($emp_retirement_date); ?>">
    </div>
  </div>
  ...
  <div class="row mb-3" >
                            <label for="inputPassword3" class="col-sm-3 control-label"> Present Joining Memo no <span class="star_color">(Memo No. of joining in the present post in present office)</span>:</label>
                            <div class="col-sm-3">
                            ...
                            <label for="inputPassword3" class="col-sm-3 control-label"> Present Joining Memo Date</label>
                            <div class="col-sm-3">
                ...
                <div class="row mb-3" >
                    <label for="inputPassword3" class="col-sm-3 control-label"> First Joining Memo no:</label>
                    <div class="col-sm-3">
                    ...
                    <label for="inputPassword3" class="col-sm-3 control-label"> First Joining Memo Date</label>
                    <div class="col-sm-3">
                    <input type="text" class="form-control" id="tch_date" name="tch_date"   value="<?php if(count($arr)==0){echo  $wef_date="";}else { echo  dateshow($wef_date);}?>" Date of Next Increment style="background-color:#FFF;cursor:pointer;" />
                    </div>
                </div>
                        
                <div class="row mb-3" >
                
                <label for="inputPassword3" class="col-sm-3 control-label">First Joining In</label>
                <div class="col-sm-3">
                <?php 
 ...
 <label for="inputPassword3"  id="district_show" class="col-sm-3 control-label  " <?php echo $display;?>>District</label>
 ...
 <div class="col-sm-3">
  ...
  <label for="inputPassword3"  class="col-sm-3 control-label" id="ps_show" <?php echo $display1;?>>PS</label>
  ...
  <?php $arr = $db->fetch_table("select ps_id_pk,ps_name,ps_code,district_id_fk from prd_location_master_panchayat_samiti where district_id_fk='". $district_id."'");?>
    ...
    <label for="inputPassword3"  id="district_show_gp" class="col-sm-3 control-label  " <?php echo $display2;?>>District</label>
    ...
    <label for="inputPassword3"  class="col-sm-3 control-label" id="block_show" <?php echo $display2;?>>BLOCK</label>
...
<? } ?>
    ...
    <label for="inputPassword3"  id="gp_show" class="col-sm-3 control-label  " <?php echo $display2;?>>GP</label>
...
<? } ?>
  ...
  <div class="row mb-3">
    <label for="inputPassword3" class="col-sm-3 control-label" id="deput_label" style="display:none">Whether on Deputation </label>
    <div class="col-sm-3" id="deput_field" style="display:none">
      <select class="form-control" name="deputation" id="deputation">
      <option value="">Please Select</option>
      <option value="1" <?php if($emp_status_deputation=='1' || $deputation=='1') { echo "selected" ;} ?>>YES</option>
      <option value="0" <?php if($emp_status_deputation=='0' || $deputation=='0') { echo "selected" ;} ?>>NO</option>
      </select>
    </div>
    <label for="inputPassword3" class="col-sm-3 control-label" id="group_label" style="display:none">Employee Group </label>
    ...
    <div class="col-sm-3">
      ...
      <option value="">-Please Select-</option>
                      <? 
                        ...
                        <option value="<?= $key['code']; ?>" 
						<? 
 						if($emp_desig_first_app==$key['code'] || $first_desig==$key['code']){ echo "selected";}?>><?= $key['description']; ?></option>
                         <? } ?>
      </select>
    </div>
    <label for="inputPassword3" class="col-sm-3 control-label">Date of Next Increment </label>
    <?php if(count($arr)==0){echo  $wef_date="";}else { echo  dateshow($wef_date);}?>
    <div class="col-sm-3">
   <input type="text" class="form-control" name="increment_date" id="increment_date" placeholder="DD-MM-YYYY" autocomplete="off" value="<?php echo !empty($emp_next_increment_date) ? $emp_next_increment_date : ''; ?>">

  </div>
  <div class="row mb-3">
    <label for="inputPassword3" class="col-sm-3 control-label">Amount of Increment <span class="star_color">(On Basic Pay)</span></label>
    <div class="col-sm-3">
    ...
    <div class="row mb-3">
    <label for="inputPassword3" class="col-sm-3 control-label">Employee Appointment Type <span class="star_color">(Promoted/Direct)</span> </label>
    <div class="col-sm-3">
  ...
  <?php if($emp_status == 11){?>
  <div class="row mb-3">
    <div class="col-sm-12">
      <button type="submit" class="btn btn-info" style="float:right">SAVE<i class="fa fa-chevron-right"></i></button>
...
/*if($('#recruitment_type').val() == '' || $('#recruitment_type').val() =='0')
			{
				alert('Please Enter Designation.');
				$('#recruitment_type').focus();
				return false;
			}
