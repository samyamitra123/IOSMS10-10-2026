...
if($sec_time_token!=$enc_session)
{
	...
	include 'profile_ret_entry_salary_form.php';
	exit;
}
else{
	...
	if($ropa_status=='1' || $emp_first_join_date>='2020-01-01' )
		{
			...
			else if($validator->pattern_match_alphanumeric($txt_ifsc) == FALSE)
			{
				...
				if($query_insert){
				header('Location:profile_ret_entry_per.php?desig='.$cryptoGraph->encode($_REQUEST['desig'],4).'&emp_id_pk='.$cryptoGraph->encode($emp_id_pk,4).'&confirm=success');
				exit(0);
				}
				else{
				header('Location:profile_ret_entry_sal.php?desig='.$cryptoGraph->encode($_REQUEST['desig'],4).'&emp_id_pk='.$cryptoGraph->encode($emp_id_pk,4).'&confirm=false');
				exit(0);
				}
}
}
