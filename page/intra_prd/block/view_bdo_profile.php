...
<?
$db=new database();
$arr=$db->fetch_table("SELECT block_status from prd_location_master_block WHERE block_code='".$_SESSION['location']['block_code']."'");
//echo $arr[0]['flag'];
?>
<style>
h1 {
...
<div class="content">
      <div class="col-lg-12 col-md-8 col-sm-8" id="sm-pad"> 
        <div class="col-sm-12">
<h1 class="heading">VIEW BDO PROFILE</h1>
...
<?
if($arr[0]['block_status']=='0'){
	$status='<a data-bs-toggle="modal" data-bs-target="#send" style="cursor:pointer;"><p class="text-primary" style="font-weight:bold">SEND TO DPRDO</p></a>';
}
else if($arr[0]['block_status']=='1'){
	$status='<p class="text-warning" style="font-weight:bold">WAITING FOR FINALIZATION</p>';
}
else if($arr[0]['block_status']=='2') {
	$status='<p class="text-success" style="font-weight:bold">APPROVED</p>';
}
else if($arr[0]['block_status']=='3') {
	$status='<p class="text-danger" style="font-weight:bold">PROFILE REJECTED</p>';
}
else if($arr[0]['block_status']=='5') {
	$status='<p class="text-warning" style="font-weight:bold">WAITING FOR PROFILE UNLOCK</p>';
}
else if($arr[0]['block_status']=='6') {
...
<td><?= $status; ?></td>
<td class="view_prof"><a href="" data-bs-toggle="modal" data-bs-target="#bdoprofModal">
<img src="<?= $config['base_url'] ?>themes/default/image/view_icon.png" width="25" alt="view" /></a></td>
<? if($arr[0]['block_status']=='0' || $arr[0]['block_status']=='3' || $arr[0]['block_status']=='4' || $arr[0]['block_status']=='6') { ?>
<td><a href="<?= $config['base_url'] ?>page/intra_prd/block/bdo_profile_form.php"><img src="<?= $config['base_url'] ?>themes/default/image/edit_icon.png" width="25" alt="view" /></a></td>
<? } else { ?>
<td><img src="<?= $config['base_url'] ?>themes/default/image/edit_icon.png" width="25" alt="view" style="opacity:0.5" /></td>
<? } ?>
<? if($arr[0]['block_status']=='2') { ?>
<td><a class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#unlock" style="cursor:pointer;"><i class="fa fa-lock"></i></a></td>
<? }else if($arr[0]['block_status']=='6'){ ?>
<td><a class="btn btn-primary disabled"><i class="fa fa-unlock-alt"></i></a></td>
<? }else{ ?>
<td><a class="btn btn-primary disabled"><i class="fa fa-lock"></i></a></td>
<? } ?>
</tr>
...
<script>
		...
		$.post('<?= $config['base_url'] ?>page/intra_prd/block/ajax_block_view.php', function(data){
			//alert(data);
				  $(".mbody").html(data);
			  	});
		});
