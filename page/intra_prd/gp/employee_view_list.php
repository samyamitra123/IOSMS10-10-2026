...
<?
$db=new database();
$Query = "select emp_id_const,emp_first_name,emp_second_name,emp_last_name,emp_status,emp_id_pk,emp_system_code from prd_employee_master where emp_status in('6','1','7') AND gp_id_fk='".$_SESSION['location']['gp_id']."' order by emp_id_const";
//print_r($Query); exit;
...
<script>
		...
		$.post('<?= $config['base_url'] ?>page/intra_prd/gp/ajax_emp_view.php?id='+link, function(data){
			//alert(data);
				  $(".mbody").html(data);
			  	});
		});
