...
function getAmount($dise,$emp_id_pk,$type,$basic){
			$db = new database();
			if($type=='cpf'){
				$arr = $db->fetch_table("select cpf from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
				if($arr[0]['cpf']){
					return $arr[0]['cpf'];
				}else{
					return 0;
			...
			if($type=='gpf'){
				$arr = $db->fetch_table("select gpf from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
				if($arr[0]['gpf']){
					return $arr[0]['gpf'];
				}else{
					$gpf_amt = ($basic/100)*6;
			...
			if($type=='pfl'){
				$arr = $db->fetch_table("select pf_loan from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
				if($arr[0]['pf_loan']){
			...
			if($type=='itax'){
				$arr = $db->fetch_table("select i_tax from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
				if($arr[0]['i_tax']){
					return $arr[0]['i_tax'];
				}else{
					return 0;
			...
			if($type=='ovd'){
				$arr = $db->fetch_table("select overdrawn from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
				if($arr[0]['overdrawn']){
			...
			if($type=='gsli'){
				$arr = $db->fetch_table("select gsli from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
				if($arr[0]['gsli']){
					return $arr[0]['gsli'];
				}else{
					return 0;
			...
			if($type=='conv'){
				$arr = $db->fetch_table("select conv_allow from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
				if($arr[0]['conv_allow']){
...
$paychange = $db->fetch_table("
       ...
       overdrawn, salary_type, cause,gp_code, part_day,gsli,consolidated_pay,festival_loan,festival_loan_cause from prd_employee_salary_save where status_flag=1 AND delete_status=1 AND salary_monthyear='".date('Ym')."'  AND emp_id_fk='$emp_id_pk' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
       ...
       category_id, block_code, emp_id_fk, pay_payband, tch_grade_pay, 
       hill_allowance, gpf, cpf_deduct, gross_salary, is_saved, conv_allow, 
       overdrawn, salary_type, cause,gp_code, part_day,gsli,consolidated_pay,festival_loan,festival_loan_cause from prd_employee_salary_save where status_flag=3 AND delete_status=1 AND salary_monthyear='".date('Ym',strtotime('-1 month'))."'  AND emp_id_fk='$emp_id_pk' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
		...
		$dop_doi = $promotion_data_cal[0]['dop_doi'];
		 ...
		 if($emp_effective_day_for_promotion_cal != 01 && $emp_id_for_promotion_part != "" && $dop_doi == 3)
		 {
		 ...
		 else if($emp_effective_day_for_promotion_cal != 01 && $emp_id_for_promotion_part != "" && ($dop_doi == 4 || $dop_doi == 5))
		 {
			 $count_days_pre_part_sal = (($emp_effective_day_for_promotion_cal)-'1');
			 $count_days_post_part_sal = ($max_days_of_current_month - $count_days_pre_part_sal);
			 $part_grade_pay_for_pre_days = round(($pre_emp_grade_pay_for_promotion_cal/$max_days_of_current_month)*$count_days_pre_part_sal);
	   	...
	   	$emp_suspend=$db->fetch_table("
								SELECT 
									...
									FROM
										...
										INNER JOIN prd_employee_master em1 on sus.emp_id_fk=em1.emp_id_pk
									WHERE
										sus.gp_id_fk = '".$dise."' and sus.emp_id_fk='".$cryp->decode($_REQUEST['id'],4)."' and em1.emp_status='9' and sus.delete_status='0'
		                      ");
...
*/							else{
				...
				/*	if($sal_save[0]['overdrawn']){
					...
					$overdrawn = getAmount($dise,$tch[0]['tchcd'],'ovd','');
					 ...
					 $overdrawn=0;
					 $total_deduct = $gpf+$pfl+$ptax+$itax+$gsli; 
					 $net_salary = $gross_salary-$total_deduct;
	...
	$('#no_ovd').click(function(){
                ...
                <table>
                	<tr>
                        ...
                        <td>
							...
							");
							$sal_caus = $db->fetch_table("select overdrawn,salary_type,cause,part_day,cpf,gpf,gsli,part_salary_cause,no_salary_cause from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='".$tch[0]['emp_id_pk']."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
							?>
               ...
               <span id="limit_holder"> <span id="limit" style="font-weight:bold; color:#F00;">500</span><span> <strong>of 500 maximum character.</strong></span></span>
                      ...
                      <?php 
							...
							$dedact_cause = $db->fetch_table("select festival_loan_cause from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='".$tch[0]['emp_id_pk']."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
          ...
          <select name="reduction" id="reduction" class="form-control" style="width:150px;border: 1px solid #3E4255">
                      ...
                      <option id="yes_ovd1" value="yes_ovd1" <?php if($dedact_cause[0]['festival_loan_cause']==1){ echo 'selected';} ?> >YES</option>   
                    ...
                    <td width="65%" style="text-align:left;"><!--<p><input type="checkbox" name="n7" id="reduction_type1" class="cls6" value="1" <?php  if($dedact_cause[0]['cooperative_loan_cause']==1) { ?>checked="checked" <?php } ?>>&nbsp;&nbsp;&nbsp;<b>Co-operative Loan Recovery</b></p>
                   
  <p><input type="checkbox" name="n8" id="reduction_type2" class="cls7" value="1" <?php  if( $dedact_cause[0]['hbl_loan_cause']==1) { ?>checked="checked" <?php } ?>>&nbsp;&nbsp;&nbsp;<b>HBL Recovery</b></p>-->
               ...
               <div class="school">
                ...
                <table width="100%">
					...
					<th>NAME OF EMPLOYEE</th>
						...
						<th>M.A</th>
						<th>CONV<br>ALLOW</th>
						<!--<th>HILL<br>ALLOW<br>(<?php //echo $hill_allowance_per; ?>%)</th>
						<th>C.P.F<br>(max <?php echo $cpf_per; ?>%)</th>-->
