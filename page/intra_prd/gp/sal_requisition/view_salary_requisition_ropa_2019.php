...
$tch= $db->fetch_table("SELECT sal.gp_id_fk,tch.emp_status, sal.empcd, 
		...
		sal.overdrawn, sal.salary_type, sal.cause, sal.gp_code, sal.part_day, sal.gsli,sal.consolidated_pay,
		sal.other_deduction,sal.cooperative_loan,sal.hbl_loan,sal.hbl_loan,festival_loan,sal.interim_relief,
		tch.emp_first_name, tch.emp_second_name, tch.emp_last_name, tch.emp_pay_in_payband, tch.ropa_level
			FROM prd_employee_salary_save as sal
			INNER JOIN prd_employee_master as tch
			ON sal.emp_id_fk =tch.emp_id_pk AND sal.gp_id_fk= CAST(tch.gp_id_fk AS text)
			WHERE (tch.emp_status='1' OR tch.emp_status='9')
			AND sal.gp_id_fk = '".$_SESSION['location']['gp_id']."'
			AND sal.delete_status=1
			AND (sal.status_flag = 1 OR sal.status_flag = 2 OR sal.status_flag = 3 OR sal.status_flag = 4) 
			AND sal.delete_status='1' AND salary_monthyear='".date('Ym')."' AND sal.ropa_status = '1' 
			AND is_saved='1' AND sal.requisition_type='".$requisition_type."'
