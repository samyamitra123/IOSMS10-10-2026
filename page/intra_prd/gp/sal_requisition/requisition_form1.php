...
//-------------------------------------------------------------------
		...
		function getAmount($dise,$empcd,$type,$basic){
			$db = new database();
			if($type=='cpf'){
				$arr = $db->fetch_table("select cpf from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
				if($arr[0]['cpf']){
					return $arr[0]['cpf'];
				}else{
					return 0;
			...
			if($type=='gpf'){
				$arr = $db->fetch_table("select gpf from prd_employee_salary_save where status_flag=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
				if($arr[0]['gpf']){
					return $arr[0]['gpf'];
				}else{
			...
			if($type=='pfl'){
				$arr = $db->fetch_table("select pf_loan from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
				if($arr[0]['pf_loan']){
			...
			if($type=='itax'){
				...
				$arr = $db->fetch_table("select i_tax from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
				if($arr[0]['i_tax']){
					return $arr[0]['i_tax'];
				}else{
					return 0;
			...
			if($type=='ovd'){
				$arr = $db->fetch_table("select overdrawn from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
				if($arr[0]['overdrawn']){
			...
			if($type=='gsli'){
				$arr = $db->fetch_table("select gsli from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
				if($arr[0]['gsli']){
					return $arr[0]['gsli'];
				}else{
					return 0;
       ...
       overdrawn, salary_type, cause,gp_code, part_day, gsli,consolidated_pay,other_deduction,festival_loan,festival_loan_cause,interim_relief from prd_employee_salary_save where status_flag=1 AND delete_status=1 AND salary_monthyear='".date('Ym')."'  AND emp_id_fk='$emp_id_pk' AND gp_id_fk='".$key['gp_id_fk']."' AND requisition_type='".$requisition_type."'");
			...
			//----------- End Gpf=0 before retirement ---------------------
				...
				if($sal_chk[0]['salary_type']=='1'){
					...
					//$full_pay_band = $key['pay_in_pay_band'];
