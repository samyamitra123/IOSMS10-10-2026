...
function getEmpAmount($type,$dise,$emp_id_pk)
{
	...
	if($type == 'grade_pay')
	{
		...
		$arr = $db->fetch_table("select emp_grade_pay,grade_amount from prd_employee_master as emp
									...
									where emp_id_pk='".$emp_id_pk."' AND gp_id_fk='".$dise."' AND ropa_status='2' ");
...
function getAmount($dise,$emp_id_pk,$type,$basic,$requisition_type)
{
	...
	if($type=='cpf')
	{
		$arr = $db->fetch_table("select cpf from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='2'");
		if($arr[0]['cpf'])
		{
			return $arr[0]['cpf'];
		}
	...
	if($type=='gpf')
	{
		$arr = $db->fetch_table("select gpf from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='2'");
		if($arr[0]['gpf'])
		{
			return $arr[0]['gpf'];
		}
	...
	/*if($type=='bas')
	{
		s
			...
			and is_saved='1' and salary_type='1' and delete_status='1'  AND emp_id_fk='$emp_id_pk' AND AND salary_monthyear between '201903' and '202001' requisition_type='".$requisition_type."'  order by salary_monthyear ASC LIMIT 1 ");
			
			 $basic_gpf=$arr[0]['basic']; 
			
			return round($basic_gpf);
	...
	if($type=='pfl')
	{
		$arr = $db->fetch_table("select pf_loan from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='2'");
		if($arr[0]['pf_loan'])
		{
	...
	if($type=='itax')
	{
		$arr = $db->fetch_table("select i_tax from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='2'");
		if($arr[0]['i_tax'])
		{
			return $arr[0]['i_tax'];
		}
	...
	if($type=='ovd')
	{
		$arr = $db->fetch_table("select overdrawn from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='2'");
		if($arr[0]['overdrawn'])
		{
	...
	if($type=='gsli')
	{
		$arr = $db->fetch_table("select gsli from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='2'");
		if($arr[0]['gsli'])
		{
			return $arr[0]['gsli'];
		}
	...
	if($type=='conv')
	{
		$arr = $db->fetch_table("select conv_allow from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='2'");
		if($arr[0]['conv_allow'])
		{
...
$paychange = $db->fetch_table("
							SELECT paychange_id_pk, paychange_ip, paychange_fromdate, paychange_todate, 
									...
									WHERE flag = 't' AND ropa_year = '2009' ");
...
$sal_chk= $db->fetch_table("select slno, latestupdate_time, latestupdate_ip_address, gp_id_fk, empcd, 
							...
							WHERE status_flag=1 AND delete_status=1 AND salary_monthyear='".date('Ym')."'  
...
if($sal_chk[0]['gp_id_fk'] && $sal_chk[0]['emp_id_fk'] && ($sal_chk[0]['salary_type']=='1' || $sal_chk[0]['salary_type']=='2'))
{
	...
	if($tch[0]['emp_desig']=='1120' || $tch[0]['emp_desig']=='1124')
	{
		...
		if(count($promotion_data_cal) > 0){	
			...
			else
			{
				...
				else
				{
					...
					//1111$prv_bas= getAmount($dise,$emp_id_pk,'bas',$full_basic,$requisition_type);
				}
			}
			...
			{
				$full_itax = getAmount($dise,$emp_id_pk,'itax','',$requisition_type).'';
				//$full_itax = $sal_chk[0]['i_tax'];
			}
...
else if($sal_chk[0]['salary_type']=='8')
{
	...
	//11$prv_bas= getAmount($dise,$tch[0]['emp_id_pk'],'bas',$full_basic,$requisition_type);
	...
	$emp_suspend=$db->fetch_table("
								SELECT 
									...
									WHERE
										...
										and em1.emp_status='9' and sus.delete_status='0' AND em1.ropa_status='2'
		                      ");
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	else
	{
		...
		else
		{
			...
			//11$prv_bas = getAmount($dise,$emp_id_pk,'bas',$basic,$requisition_type);
		}
	}
	...
	{
		$pfl = getAmount($dise,$emp_id_pk,'pfl','',$requisition_type);
	}
...
// $max_ptax = empPtax($gross_salary);
	...
	{
		$itax = getAmount($dise,$emp_id_pk,'itax','',$requisition_type);
	}
	...
	{
		$gsli = getAmount($dise,$emp_id_pk,'gsli','',$requisition_type);
	}
...
<script>
	<?php if($tch[0]['emp_desig']=='1120' || $tch[0]['emp_desig']=='1124' ){?>
...
<script>
	...
	$(document).ready(function (e1) 
	{
		...
		/////// Code has been commented by Anannya on 07-06-2018 ////////
		/*if($('#reduct3').val()>0)
		{
			$("#reduction_type").show();
			$('#reduct3').removeAttr('readonly');
...
<form id="form" method="post" action="ajax_rq_submit.php" enctype="multipart/form-data">
    	...
    	<div class="table-responsive">
            <table>
                ...
                <td>
                    ...
                    $dedact_cause = $db->fetch_table("select festival_loan_cause, other_deduction_cause from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='".$tch[0]['emp_id_pk']."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='2' ");
			...
			else
			{
				...
				else
				{
					...
					if(desig!='1120' && desig!='1124' )
					{
						...
						{
							var min_gpf_amt = ((parseInt(pay_band)+parseInt(consolidated_pay)+parseInt(grade_pay))/100)*6;
							var min_gpf = Math.round(min_gpf_amt);
							$('#gpf').val(min_gpf);
						}
					} 
