...
$arr_emp_cnt = $db->fetch_table("select emp_id_pk from prd_employee_master as emp
								...
								where emp.gp_id_fk = '".$_SESSION['location']['gp_id']."' AND save.requisition_type='".$requisition_type."' AND save.status_flag=1 AND save.delete_status=1 AND save.is_saved=1 AND save.ropa_status ='1'  and save.salary_monthyear='".date('Ym')."' 
...
$upd = $db->update("UPDATE prd_employee_salary_save sal SET status_flag=2
					FROM prd_employee_master emp 
					WHERE 
						sal.emp_id_fk=emp.emp_id_pk 
						AND sal.gp_id_fk=CAST(emp.gp_id_fk as character varying) 
...
if($upd && $security){
	//echo "in if";
		...
		$_SESSION['msg']= '<div class="alert alert-success" style="text-align:center"><strong>Salary requisitions has been sent to BDO successfully.</strong></div>';
		header('Location:'.$config['base_url'].'page/intra_prd/gp/sal_requisition/emp_sal_requisition_ropa_2019.php');
		exit(0);
}
	...
	//echo "in else";
		pg_query("rollback"); 
		$_SESSION['msg']= '<div class="alert alert-danger" style="text-align:center"><strong>Salary requisitions has been sent to  Unsuccessfully.</strong></div>';
		header('Location:'.$config['base_url'].'page/intra_prd/gp/sal_requisition/emp_sal_requisition_ropa_2019.php');
		exit(0);
} ?>
