...
{ 
	...
	if($type=='cpf')
	{
		...
		$arr = $db->fetch_table("select cpf from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='2'");
		if($arr[0]['cpf'])
		{
			return $arr[0]['cpf'];
		}
	...
	if($type=='gpf')
	{
		$arr = $db->fetch_table("select gpf from prd_employee_salary_save where status_flag=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='2'");
		if($arr[0]['gpf'])
		{
			return $arr[0]['gpf'];
		}
		...
		{
			/*echo ("select basic from prd_employee_salary_save where where  status_flag in('3','4') and salary_monthyear between '".date('Y')-1."' and '".date('Ym')."'
			
			and is_saved='1' and salary_type='1' and delete_status='1' AND requisition_type='".$requisition_type."'  order by salary_monthyear DESC LIMIT 1 '");die;*/
	...
	if($type=='pfl')
	{
		$arr = $db->fetch_table("select pf_loan from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='2'");
		if($arr[0]['pf_loan'])
		{
	...
	if($type=='itax')
	{
		$arr = $db->fetch_table("select i_tax from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='2'");
		if($arr[0]['i_tax'])
		{
			return $arr[0]['i_tax'];
		}
	...
	if($type=='ovd')
	{
		$arr = $db->fetch_table("select overdrawn from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='2'");
		if($arr[0]['overdrawn'])
		{
	...
	if($type=='gsli')
	{
		$arr = $db->fetch_table("select gsli from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='2'");
		if($arr[0]['gsli'])
		{
			return $arr[0]['gsli'];
		}
...
if(count($tch_final) == 0 )
{
	...
	$employee_for_promotion_increment = $db->fetch_table("SELECT 
														...
														AND (tch.emp_status='1' OR tch.emp_status='9') AND tch.ropa_status='2'");	
			...
			if((date("Ym") >= $promotion_eff_yearmonth))    //Date of promotion
			{
				...
				{	
					...
					$update_employee_data = $db->update("UPDATE prd_employee_master 
														...
														".$update_emp_desig."
														where emp_id_pk='".$employees_for_promotion_increment['emp_id_pk']."' AND ropa_status='2'
														");
	...
	function getEmpAmount($type,$dise,$emp_id_pk)
	{
		...
		{
			$arr = $db->fetch_table("select emp_pay_in_payband from prd_employee_master where emp_id_pk='".$emp_id_pk."' AND gp_id_fk='".$dise."' AND ropa_status='2'");
	...
	<table width="100%">
        ...
        if(count($tch))
		{
			...
			foreach ($tch as $key) 
			{
				...
				{
					if($key['emp_desig']=='1120' || $key['emp_desig']=='1124')
					{
					...
					if(count($promotion_data_cal) > 0){	
							...
							else
							{
								...
								$emp_suspend=$db->fetch_table("
																...
																WHERE
																sus.gp_id_fk = '".$_SESSION['location']['gp_id']."' and sus.emp_id_fk='$emp_id_pk' and em1.emp_status='9' and sus.delete_status='0' AND em1.ropa_status='2'
								");
							...
							else
							{
								...
								if(strtotime($suspend_effect_date)<=strtotime(date('Y-m-d')) && strtotime($suspend_effect_date)>0)
								{
                                     // Temporary Modification without full knowledge..
									 //$full_gross_salary_sus =round($full_pay_in_band_sus+$full_grade_pay_sus+$full_da_sus+$interim_relief+$hra+$ma+$conveyance_allowance);
									  $full_gross_salary_sus =round($full_pay_in_band+$full_grade_pay+$full_da+$interim_relief+$hra+$ma+$conveyance_allowance);
									  $full_ptax = empPtax($full_gross_salary_sus);
									 // print_r("Pay In Band".$full_pay_in_band_sus.'Grade Pay:'.$full_grade_pay_sus."DA".$full_da_sus.'I Relief'.$interim_relief.'HRA'.$hra.'MA'.$ma.'CA'.$conveyance_allowance); 
								}
								else
								{
									  $full_ptax = empPtax($full_gross_salary);
				...
				else
				{
					$sal_save=	$db->fetch_table("select gp_id_fk, empcd, 
												...
												AND gp_id_fk='".$key['gp_id_fk']."' AND emp_id_fk='$emp_id_pk' AND requisition_type='".$requisition_type."' AND ropa_status='2'");	
...
/*************************************** END ************************************************/
		...
		/**************************************** Changed by ANJAN for error log Start ******************************/		
...
<script>
	$(document).ready(function(){
		...
		$(".edit a").click(function() {	
			...
			var link1= $("#gp_id").val();
			$.post('<?= $config['base_url'] ?>page/intra_prd/gp/sal_requisition/ajax_requisition_edit.php?id='+link+'&gp_id='+link1, function(data){
				$("#mbody").html(data);
			});
		});
