...
else
{
	...
	/*			if($validator->blank_select($pay_in_band) == FALSE){
	$_SESSION['msg']= '<div class="alert alert-danger" style="text-align:center"><strong>Please enter valid Pay in band.<strong></div>';
	header('location:emp_sal_requisition.php'.$query_string);
	exit(0);
	}
	if($validator->blank_select($pay_in_band) == FALSE || $validator->pattern_number($pay_in_band) == FALSE){
	$_SESSION['msg']= '<div class="alert alert-danger" style="text-align:center"><strong>Please enter valid Pay in band.<strong></div>';
	header('location:emp_sal_requisition.php'.$query_string);
	exit(0);
	}
	...
	*/			
	...
	$arr = $db->fetch_table("select pay_payband,salary_type from prd_employee_salary_save where emp_id_fk='$emp_id_pk' AND gp_id_fk='".$schcd."' AND salary_monthyear='".date('Ym')."' AND requisition_type='".$requisition_type."' AND ropa_status='2'");
	...
	{ 
		...
		requisition_type,emp_desig, emp_group, emp_pay_scale, emp_pay_band, ropa_status)
		VALUES ('$rank', 'now()', '$user_sip', '".$schcd."', '$tchcd', 
		'$bankname', '$accountno', '$basic', '$da', '$hra', '$ma', '0', '$pf_loan', '$p_tax', 
		'$i_tax', '$net', '$bank_ifsc', 'NA', '0', '$p_tax', '$code', '0', '1', '$salary_monthyear', 
		'1', '".$_SESSION['location']['block_code']."', '$teacher_id_pk', '$pay_in_band', '$grade_pay', '$hill_allow','$gpf', '0', 
		...
		'$part_salary_cause_msg','$no_salary_cause_msg','$interim_relief','".$requisition_type."',$emp_desig,'".$emp_detail[0]['emp_group']."',
		'".$emp_detail[0]['emp_pay_scale']."','".$emp_detail[0]['emp_pay_band']."', '$ropa_status')");			
	...
	else
	{
		...
		cooperative_loan_cause,hbl_loan_cause,festival_loan_cause,part_salary_cause,no_salary_cause,interim_relief,
		requisition_type,emp_desig, emp_group, emp_pay_scale, emp_pay_band, ropa_status)
		VALUES ('$rank', 'now()', '$user_ip', '".$schcd."', '$tchcd', 
		'$bankname', '$accountno', '$basic', '$da', '$hra', '$ma', '0', '$pf_loan', '$p_tax', 
		'$i_tax', '$net', '$bank_ifsc', 'NA', '0', '$p_tax', '$code', '0', '1', '$salary_monthyear', 
		'1', '".$_SESSION['location']['block_code']."', '$teacher_id_pk', '$pay_in_band', '$grade_pay', '$hill_allow','$gpf', '0', 
		'$gross', '0','$conv_allow','$overdrawn','$salary_type','$cause_msg','".$_SESSION['user_info']['stake_user']."','$part_day',
