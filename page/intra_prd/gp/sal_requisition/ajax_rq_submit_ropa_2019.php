...
if($sec_time_token!=$enc_session)
{
	...
	header('location:emp_sal_requisition_ropa_2019.php');
	exit(0);
}
else
{
	...
	//$o_deduction=0;
	//echo $o_deduction_cause1;exit;
	if($cooperative_loan=='')
	{
		$cooperative_loan=0;
		$cooperative_loan_val=0;
	}
	else
	{
		$cooperative_loan_val = $_POST['reduct1']; 
	}
	...
	$total_allowance=$pay_in_band+$da+$hra+$ma+$conv_allow+$hill_allow+$interim_relief;
	...
	if($validator->blank_select($tchcd) == FALSE || $validator->pattern_number($tchcd) == FALSE)
	{
		...
		header('location:emp_sal_requisition_ropa_2019.php'.$query_string);
		exit(0);
	}
	...
	*/
	...
	/*else if(($validator->blank_select($cpf) == FALSE || $validator->pattern_number($cpf) == FALSE))
	{
		$_SESSION['msg']= '<div class="alert alert-danger" style="text-align:center"><strong>CPF amount is not valid.<strong></div>';
		header('location:emp_sal_requisition.php'.$query_string);
		exit(0);
	}
	else if(($validator->blank_select($cpf_deduct) == FALSE || $validator->pattern_number($cpf_deduct) == FALSE))
	{
		$_SESSION['msg']= '<div class="alert alert-danger" style="text-align:center"><strong>CPF deduct amount is not valid.<strong></div>';
		header('location:emp_sal_requisition.php'.$query_string);
		exit(0);
	}*/
	...
	else if($emp_desig=='1120' || $emp_desig=='1124' )
	{
		...
		else if($pay_in_band!='0' || $da!='0' || $hra!='0' || $ma!='0' || $conv_allow!='0' || $hill_allow!='0' || $interim_relief!='0' || $gpf!='0' || $pf_loan!='0' || $i_tax!='0' || $gsli!='0' || $overdrawn!='0' ) //|| $festival_val!='0'
		{
	...
	else if($emp_desig!='1120' && $emp_desig!='1124' && $emp_desig!='1125' )
	{
	...
	$arr = $db->fetch_table("select pay_payband,salary_type from prd_employee_salary_save where emp_id_fk='$emp_id_pk' AND gp_id_fk='".$schcd."' AND salary_monthyear='".date('Ym')."' AND requisition_type='".$requisition_type."' AND ropa_status = '1'");
	...
	{ 
		...
		WHERE emp_id_fk='$emp_id_pk' AND gp_id_fk='".$schcd."' AND salary_monthyear='".date('Ym')."' AND requisition_type='".$requisition_type."'
		AND ropa_status = '1'
		");	
		...
		cause,gp_code,part_day,gsli,delete_status,consolidated_pay,cooperative_loan,hbl_loan,festival_loan,
		cooperative_loan_cause,hbl_loan_cause,festival_loan_cause,part_salary_cause,no_salary_cause,interim_relief,
		requisition_type,emp_desig, emp_group, emp_pay_scale, emp_pay_band, ropa_level, ropa_status)
		VALUES ('0', 'now()', '$user_sip', '".$schcd."', '$tchcd', 
		'$bankname', '$accountno', '$basic', '$da', '$hra', '$ma', '0', '$pf_loan', '$p_tax', 
		'$i_tax', '$net', '$bank_ifsc', 'NA', '0', '$p_tax', '$code', 
		'0', '1', '$salary_monthyear', 
		'1', '".$_SESSION['location']['block_code']."', '$teacher_id_pk', '$pay_in_band', '0', '$hill_allow','$gpf', '0', 
		'$gross', '0','$conv_allow','$overdrawn','$salary_type','$cause_msg','".$_SESSION['user_info']['stake_user']."','$part_day',
		'$gsli','1','$consolidated_pay','$cooperative_loan_val','$hbl_val','$festival_val','$cooperative_loan','$hbl','$festival',
		'$part_salary_cause_msg','$no_salary_cause_msg','$interim_relief','".$requisition_type."',$emp_desig,'".$emp_group."',
		'".$emp_pay_scale."','".$emp_pay_band."', '$grade_pay', $ropa_status)");			
	...
	else
	{
		...
		//$upd = $db->update("UPDATE ehrms_teacher_salary_save_primary SET status_flag=0,is_saved=0 WHERE schcd='".$schcd."' AND tchcd='$tchcd' AND code='$code'");
		pg_query("begin");
		//echo 12; die;
		...
		gp_code,part_day,gsli,delete_status,consolidated_pay,cooperative_loan,hbl_loan,festival_loan,
		cooperative_loan_cause,hbl_loan_cause,festival_loan_cause,part_salary_cause,no_salary_cause,interim_relief,
		requisition_type,emp_desig, emp_group, emp_pay_scale, emp_pay_band, ropa_level, ropa_status)
