...
$arr_emp_cnt = $db->fetch_table("select emp_id_pk from prd_employee_master as emp
	...
	where emp.gp_id_fk = '".$_SESSION['location']['gp_id']."' AND save.requisition_type='".$requisition_type."' AND save.ropa_status='2'");
...
$upd = $db->update("
					...
					UPDATE prd_employee_salary_save sal SET status_flag=2
					FROM 
...
mail_function($mail_body,$email,$emp_sub);*/
					...
					header('Location:'.$config['base_url'].'page/intra_prd/gp/sal_requisition/emp_sal_requisition.php');
					exit(0);
}
	...
	//echo "in else";
					pg_query("rollback"); 
					$_SESSION['msg']= '<div class="alert alert-danger" style="text-align:center"><strong>Salary requisitions has been sent to DPSC successfully.</strong></div>';
					header('Location:'.$config['base_url'].'page/intra_prd/gp/sal_requisition/emp_sal_requisition.php');
					exit(0);
} ?>
