...
//echo $requisition_type."<br>";exit;
...

	{
		...
		$arr = $db->fetch_table("select cpf from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='1'");

		//print_r($arr);exit;

		//echo "select cpf from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='1'";exit;
...

	{
		...
		$arr = $db->fetch_table("select gpf from prd_employee_salary_save where status_flag=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='1'");
		...
		/*	$arr = $db->fetch_table("select basic from prd_employee_salary_save where status_flag in('3','4') 
			...
			AND emp_id_fk='".$empcd."' order by salary_monthyear DESC LIMIT 1 ");
			...
			*/
			...
			$arr_emp = $db->fetch_table("select ropa_9_emp_pay_in_payband,emp_grade_pay, emp_pay_in_payband from prd_employee_master where ropa_status=1  AND emp_id_pk='".$empcd."' AND gp_id_fk='".$dise."'  AND ropa_status='1'");

			

				//$grade_pay=func_gradepay($arr_emp[0]['emp_grade_pay']);
...

	{
		...
		$arr = $db->fetch_table("select pf_loan from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='1'");
...

	{
		...
		$arr = $db->fetch_table("select i_tax from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='1'");
...

	{
		...
		$arr = $db->fetch_table("select overdrawn from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='1'");
...

	{
		...
		$arr = $db->fetch_table("select gsli from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND gp_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status='1'");
...

{
	...
	// {
	 	...
	 	//echo "Subikar"; print_r($Query); exit;
	...
	if(count($arr) > 0){
		...
		// Need to fix this sql Later this month
...
$tch_final = $db->fetch_table("
								...
								gp_id_fk = '".$dise."' and salary_monthyear='".date('Ym')."' AND requisition_type='".$requisition_type."' 
...

{
	...
	on p.emp_id_fk=tch.emp_id_pk
														...
														AND (tch.emp_status='1' OR tch.emp_status='9') AND tch.ropa_status='1'");
...

	{
...

		{
			...
			//&& date("Ym")==date("Y").'07'  && ( $increment_type=='2' ||  $increment_type=='1')
		...
		//var_dump($employees_for_promotion_increment['emp_id_pk']);
	...
	$Query = "SELECT save.empcd,prd_emp.emp_desig,save.emp_id_fk FROM prd_employee_salary_save 
									...
									AND salary_monthyear='".date('Ym')."' AND save.ropa_status = '1'

									AND requisition_type='".$requisition_type."'

									";

	$sal_saved = $db->fetch_table($Query);
	...
	<table width="100%">
...

		{
...

			{
				...
				overdrawn, salary_type, cause,gp_code, part_day, gsli,consolidated_pay,other_deduction,festival_loan,festival_loan_cause,interim_relief from prd_employee_salary_save where status_flag=1 AND delete_status=1 AND salary_monthyear='".date('Ym')."' AND emp_id_fk='$emp_id_pk' AND gp_id_fk='".$key['gp_id_fk']."' AND requisition_type='".$requisition_type."' AND ropa_status='1'");
...

				{
					...
					if($key['emp_desig']=='1120' || $key['emp_desig']=='1124' || $key['emp_desig']=='1125')
...

					{
                        ...
                        //print($sal_chk[0]['salary_type']); exit;
...

						{
							...
							$promotion_data_cal = $db->fetch_table($Query);

					if(count($promotion_data_cal) > 0){	

						//print_r($promotion_data_cal); exit;
                        ...
                        // if($emp_id_pk == 7553)
					    //    { print($emp_effective_day_for_promotion_cal); exit; }
...

							{
								...
								";
								$emp_suspend=$db->fetch_table($Query);
                                // if($emp_id_pk == 7553)
								//   {
								//   	print_r($Query); exit;
                                //   }
...

				{
					...
					$sal_save=	$db->fetch_table("select gp_id_fk, empcd, 
												...
												overdrawn, salary_type, cause,gp_code, part_day,gsli,consolidated_pay,festival_loan,festival_loan_cause,interim_relief  

												from prd_employee_salary_save 

												where status_flag IN(3,4) AND delete_status=1 AND salary_monthyear='".date('Ym',strtotime('-1 month'))."' 

												AND gp_id_fk='".$key['gp_id_fk']."' AND emp_id_fk='$emp_id_pk' AND requisition_type='".$requisition_type."' AND ropa_status='1'");	
...
/*************************************** END ************************************************/
					...
					//var_dump($promotion_data_cal); die;
...

					{
						...
						// new $count_days_total_grade_pay_for_promotion = func_gradepay($key['emp_grade_pay']);
					...
					$emp_suspend=$db->fetch_table("
													...
													sus.gp_id_fk = '".$_SESSION['location']['gp_id']."' and sus.emp_id_fk='$emp_id_pk' 
					...
					$suspend_effect_date=$emp_suspend[0]['suspend_effect_date']; // Suspended Effect Date is changing from next month

				  // if($emp_id_pk == 7553)
				// 		{
				// 			print_r($emp_suspend); exit;
				// 		}
...

					{
...

						{
							...
							//28.12.2018 $interim_relief = $key['interim_relief'];
						...
						//$festival_loan = festival_adv_recovery($emp_id_pk);	//05_06_2018 // Subikar uncommented

						$gross_salary = round($pay_in_band+$da+$interim_relief+$hra+$ma+$conveyance_allowance+$cpf+$hill_allowance);
						...
						$total_deduct = $gpf+$pfl+$cpf_deduct+$ptax+$itax+$gsli+$festival_loan;
						...
						//$total_deduct = $gpf+$pfl+$cpf_deduct+$ptax+$itax+$gsli+$overdrawn+$reduct1+$reduct2+$reduct3+$festival_loan; // Error log code
				...
				//$_SESSION['salary_source'.$emp_id_pk] = $crypto->encode($key['salary_source'],4);  // Error code 
				...
				<tr>
                    ...
                    <td id="show"><?php echo $count; //echo $emp_id_pk; ?></td>
                    ...
                    <td><?php echo $festival_loan;//if($festival_loan){echo $festival_loan;} else {echo 0;} ?></td>
	...
	<div class="form-group" id="sal_save" style="display:none; margin-left:48%">
        ...
        <div class="col-sm-offset-5 col-sm-7" style="margin-top:2%">
        	...
        	<a href="<?= $config['base_url'] ?>page/intra_prd/gp/sal_requisition/ajax_rq_finalize_ropa_2019.php" class="btn btn-success btn-sm">Salary Finalize</a>
	...
	//echo count($tch).'--'.count($sal_saved);
...
{ ?>
	...
	<script>
		...
		window.location.replace("<?php echo $config['base_url'] . 'page/intra_prd/gp/sal_requisition/view_salary_requisition_ropa_2019.php'; ?>");
...
<script>
	...
	$(document).ready(function(){
		...
		$(".edit a").click(function() {	
			...
			$.post('<?= $config['base_url'] ?>page/intra_prd/gp/sal_requisition/ajax_requisition_edit_ropa_2019.php?id='+link+'&gp_id='+link1, function(data){
		...
		$(".save a").click(function() {	
			...
			$.post('<?= $config['base_url'] ?>page/intra_prd/gp/sal_requisition/ajax_rq_save_ropa_2019.php?emp_id_pk='+emp_id_pk+'&empcd='+empcd+'&sec_tok='+sec_tok, function(data)

			{
				...
				{ 
					...
					$('.border_val').html('<div class="alert alert-danger" style="text-align:center; width:98%;"><strong>Time Out!..Please Try Again.<strong></div>');
				...
				{ 
					...
					$('.border_val').html('<div class="alert alert-success" style="text-align:center; width:98%;"><strong>The salary requisition of '+name+' has been successfully saved.</strong></div>');
				...
				{ 
					...
					$('.border_val').html('<div class="alert alert-danger" style="text-align:center; width:98%;"><strong>Sorry!!! Salary Requisition Not Saved. Please try again.</strong></div>');
		...
		$("#finalize").click(function() {	
			...
			$.post('<?= $config['base_url'] ?>page/intra_prd/gp/sal_requisition/ajax_rq_finalize_ropa_2019.php', function(data){
