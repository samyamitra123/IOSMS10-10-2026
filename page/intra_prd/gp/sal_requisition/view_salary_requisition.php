...
$tch= $db->fetch_table("SELECT sal.gp_id_fk,tch.emp_status, sal.empcd, 
       ...
       sal.overdrawn, sal.salary_type, sal.cause, sal.gp_code, sal.part_day, sal.gsli,sal.consolidated_pay,sal.other_deduction,sal.cooperative_loan,sal.hbl_loan,sal.hbl_loan,festival_loan,sal.interim_relief,
										tch.emp_first_name,
										tch.emp_second_name,
										tch.emp_last_name,
									...
									INNER JOIN 
										...
										ON sal.emp_id_fk =tch.emp_id_pk AND sal.gp_id_fk= CAST(tch.gp_id_fk AS text)
										WHERE
											(tch.emp_status='1' OR tch.emp_status='9') AND
											sal.gp_id_fk = '".$_SESSION['location']['gp_id']."'
											AND sal.delete_status=1 AND sal.ropa_status='2'
											AND (sal.status_flag = 1 OR sal.status_flag = 2 OR sal.status_flag = 3 OR sal.status_flag = 4) AND sal.delete_status='1' AND salary_monthyear='".date('Ym')."' AND is_saved='1' AND sal.requisition_type='".$requisition_type."'
											order by 		        									
											tch.emp_first_name ;
								");
			...
			<br>
				<?php if(count($tch)){
					...
					elseif($tch[0]['status_flag'] == 2){ ?>
						<div class="statusf">
                        <div class="alert alert-success" style="text-align:center"><strong>Salary Requisition Finalized by GP</strong></div>
                        </div>
						
					<?php } elseif($tch[0]['status_flag'] == 3){ ?>
						<div class="alert alert-success" style="text-align:center"><strong>Salary Requisition Finalized by Block</strong></div>
					<?php } } ?>
                 <div class="emplist">
				<div class="school">	
