...
$(document).ready(function(e) {
	...
	$.post('<?= $config['base_url'] ?>page/intra_prd/gp/ajax_send_promotional_emp.php?id='+link, function(data){
				  //alert(data);
				  $('.result').html(data);
			  	});
	});
...
<?php
	...
	if($pro_emp_chk)
     {
		...
		$exists=$db->fetch_table("select emp_first_name,gp_id_fk from prd_employee_master where emp_id_const='".$id."' AND gp_id_fk='".$_SESSION['location']['gp_id']."'");
	if(count($exists)=='1')
	 {
	...
	{
		pg_query('BEGIN');
		$update=$db->update("UPDATE prd_employee_master SET ps_id_fk='0',gp_id_fk='".$_SESSION['location']['gp_id']."',emp_status='10',emp_form_status='1' WHERE emp_id_const='".$id."' AND emp_status='2'");
	...
	if($update )
	{
		...
		$gp_name=$db->fetch_table("select gp_name,gp_code from  prd_location_master_gp where gp_id_pk='".$arr[0]['gp_id_fk']."'");
					...
					<? echo '<div class="alert alert-success" style="text-align:center"><strong>Employee Successfully Added To '.$_SESSION['location']['gp_name'] .' GP</strong></div>'; ?>
					<div class="emplist">
					<div class="school">
				 ...
				 <table width="100%">
					...
					<tr>
                        ...
                        <td id="">
                         <? if($arr[0]['emp_form_status']=='5'){ ?>
                        <a href="<?= $config['base_url'] ?>page/intra_prd/gp/ajax_send_promotional_emp.php?id=<?php echo $cryptoGraph->encode($arr[0]['emp_id_pk'],4) ?>"><img width="25" src="<?= $config['base_url'];?>themes/default/image/send.png" alt="sent"/></a>
                        <? } ?>
                        </td>
					</tr>
				</table>
