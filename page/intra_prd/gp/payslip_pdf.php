...
$ropa_status= $crypto->decode($Arguments['ropa'],4);

//var_dump($ropa_status); die;

if($ropa_status=='0' || $ropa_status=='')
{
	$emp_id_const_db=$db->fetch_table("Select emp_first_name,emp_second_name,emp_pay_in_payband,emp_pay_scale,emp_group,emp_pay_band,emp_pan_no,emp_last_name,emp_id_const,emp_desig, gp_id_fk from prd_employee_master
WHERE emp_id_pk='".$Arguments['user_id']."' and emp_status in('1','9','2')");
...
//print_r($emp_id_const_db); exit;
$check_data=$db->fetch_table("Select count(emp_id_fk) AS cnt from prd_employee_salary_save
WHERE salary_monthyear='".$monthno."' AND status_flag IN ('3', '4') AND emp_id_fk='".$Arguments['user_id']."'");
...
$key= $db->fetch_table(" select
										  ...
										  gp.gp_code,
										  gp.gp_name
   FROM prd_employee_salary_save as sal INNER JOIN prd_employee_master as tch ON sal.emp_id_fk = tch.emp_id_pk 
    INNER JOIN prd_location_master_gp as gp ON tch.gp_id_fk=gp.gp_id_pk
    WHERE
										tch.emp_id_pk='".$Arguments['user_id']."'
	...
	$key= $db->fetch_table("SELECT 
										  ...
										  gp.gp_code,
										  gp.gp_name
   FROM prd_monthly_salary_archive_final as sal INNER JOIN prd_employee_master as tch ON sal.emp_id_fk = tch.emp_id_pk 
    INNER JOIN prd_location_master_gp as gp ON tch.gp_id_fk=gp.gp_id_pk
    WHERE
						 	            tch.emp_id_pk='".$Arguments['user_id']."'
						  				AND tch.emp_status in('1','9','2')  
...
$Query = "
		...
		where gp.gp_code='".$gpCode."'


		";
$circle = $db->fetch_table($Query);
...
$amt_net_total=number_to_words($key[0]['net']);
 ...
 $total_deduct=$key[0]['gpf']+$key[0]['pf_loan']+$key[0]['p_tax']+$key[0]['i_tax']+$key[0]['gsli']+$key[0]['overdrawn']+$key[0]['cooperative_loan']+$key[0]['festival_loan']+$key[0]['hbl_loan'];
					...
					<table style="margin-top:-5%">
						<tr>
							...
							<td class="he2" style="text-align: center;width: 480px;">
								...
								<p class="school" style="color: #004080;font-size: 12px;font-weight:bold;">'. $circle[0]['gp_name']."," .$circle[0]['block_name'].",".$circle[0]['district_name'].'</p>
...
$mpdf->WriteHTML('<div class="segment">
						...
						<tr> 
						<td>DESIGNATION: </b>'.fun_common($emp_id_const_db[0]['emp_desig']).'</td>',2);
...
$mpdf->WriteHTML('<td><b>PAN:</b> '.$emp_id_const_db[0]['emp_pan_no'].'</td>
					...
					<br><table width="100%" style="border: 1px solid #666; border-radius: 5px; -moz-border-radius: 5px; -webkit-border-radius: 5px;font-size: 12px;vertical-align: bottom;">
						  ...
						  <tr>
                              ...
                              <td style="padding-left: 5px; padding-right: 2%; border-right:1px solid #666;width:25%">',2);
								  ...
								  if($key[0]['pf_loan']!='0'){
                                      ...
                                      $mpdf->WriteHTML('<tr>
                                        <td style="font-weight:bold">CPF DEDUCT</td>
                                        <td align="right">'.$key[0]['cpf_deduct'].'</td>
                                      </tr>',2);
								  }if($key[0]['p_tax']!='0'){
                                      $mpdf->WriteHTML('<tr>
						  ...
						  <p style="font-size:11px;">GSLI: Group Savings Linked Insurance, IR: Interim Relief .</p>
						  </div>
						  <br />
