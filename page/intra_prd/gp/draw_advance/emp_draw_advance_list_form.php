...
function fun_common($tcode, $code){
	...
	$tch= $db->fetch_table("
		...
		count(distinct CASE WHEN '1' then adv1.emp_id_fk else null end) as total_emp
                                from 
								prd_adavance_pay adv1 where gp_id_fk = '".$_SESSION['location']['gp_id']."'");
...
<table width="100%">
        ...
        <input type="hidden" name="emp_id" id="emp_id<?php echo $m; ?>" value="<?php echo $crypto->encode($key['emp_id_pk'],4) ?>" />
         <input type="hidden" name="desig" id="desig<?php echo $m; ?>" value="<?php echo $crypto->encode($key['emp_desig'],4) ?>" />
              <td> <button type="submit" class="btn btn-info" onclick="submit_data(<?php echo $m; ?>)" <? if($status_flag==2 or $status_flag==3){ echo disabled; } ?>>Submit</button></td>
...
function submit_data(k)
{
		...
		$.post('<?= $config['base_url'] ?>page/intra_prd/gp/draw_advance/entry_draw_advance_list_insert.php?id='+link+'&gp_id='+link1+'&emp_id='+link2+'&desig='+link3+'&adance_type='+link4, function(data){
				if(data=='<div class="alert alert-success" style="text-align:center"><strong>Data Has Been Succesfully Inserted.</strong></div>')
			 {
				$('#sess_msg').html(data);
			 }
			 if(data=='<div class="alert alert-danger" style="text-align:center"><strong>Sorry !!! Data insertion Fails.</strong></div>')
			 {
				$('#sess_msg').html(data);
			 }
			 if(data=='<div class="alert alert-success" style="text-align:center"><strong>Data Has Been Succesfully Updated.</strong></div>')
			 {
				$('#sess_msg').html(data);
			 }
			 if(data=='<div class="alert alert-success" style="text-align:center"><strong>Sorry !!! Data updation Fails.</strong></div>')
			 {
				$('#sess_msg').html(data);
			 }
		       });
