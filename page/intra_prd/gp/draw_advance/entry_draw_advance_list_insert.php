...
$block = $db->fetch_table("SELECT   
									...
									FROM
										prd_location_master_gp where gp_id_pk='".$_SESSION['location']['gp_id']."'
								");
...
$block_code=substr($block[0]['gp_code'],0,7);		
				...
				if($query_update){
				echo '<div class="alert alert-success" style="text-align:center"><strong>Data Has Been Succesfully Updated.</strong></div>';exit;
				//$_SESSION['msg']= '<div class="alert alert-success" style="text-align:center"><strong>Data Has Been Succesfully Inserted.</strong></div>';
					
				}
				else{
					echo '<div class="alert alert-danger" style="text-align:center"><strong>Sorry !!! Data updation Fails.</strong></div>';exit;
