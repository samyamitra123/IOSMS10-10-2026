...
<script>
    	$(document).ready(function(){
		  ...
		  $.post('<?= $config['base_url'] ?>page/intra_prd/gp/ajax_bank_view.php?id='+link, function(data){
		  $(".empshow").html(data);
		  });
		  });
		});
...
<?
$db=new database();
$arr=$db->fetch_table("select emp_id_const, emp_first_name,emp_second_name,emp_last_name,emp_bank_name, emp_id_pk, emp_acc_no,emp_ifsc_no,emp_status,bank_upd_status from prd_employee_master where emp_status='1' AND gp_id_fk='".$_SESSION['location']['gp_id']."' order by emp_id_const");
...
<?php if($item['bank_upd_status']=='0' || $item['bank_upd_status']=='4'){ ?>
                    <td style="color:#996600" align="center">Request Not Send/Rejected</td>
                    <?php } 
					 else if($item['bank_upd_status']=='1'){ ?>
                    <td style="color:#660066" align="center">Request Sent To BDO</td>
                    <?php } 
                    ...
                    <?php }*/ ?>
<td class="show"><a href="" id="<?= $cryptoGraph->encode($item['emp_id_pk'],4);?>" data-toggle="modal" data-target="#view"><img src="<?= $config['base_url'] ?>themes/default/image/view_icon.png" width="25" alt="view" /></a></td></tr>
<?php $cnt++;} } else { ?>
<tr>
