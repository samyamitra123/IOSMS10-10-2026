...
$arr=$db->fetch_table("select 
			...
			emp.bank_upd_status in(1,2,3) AND CAST(gp_code AS text) like '".$cryptoGraph->decode($_REQUEST['block_code'],4)."%' 
...
<td><?= $item['gp_name']?></td>
<td><?= $item['total'] ?> </td>
<td><a href="<?= $config['base_url'] ?>page/intra_prd/district/update_bank_details.php?gp_id=<?= $cryptoGraph->encode($item['gp_id_pk'],4);?>"><img src="<?= $config['base_url'] ?>themes/default/image/view_icon.png" width="25" alt="view" /></a></td></tr>
<?php $cnt++;} ?> 
<tr>
<td></td>
<td></td>
