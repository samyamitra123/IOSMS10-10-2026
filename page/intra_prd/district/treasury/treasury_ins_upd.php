...
$sec_time_token=$_POST['sec_tok'];
	...
	if($sec_time_token!=$enc_session){
		...
		header('Location: '. $config['base_url'] . "page/intra_prd/district/treasury_details.php?msg_er=".$error_msg);
	...
	if($validator->blank_select($district_name) == FALSE)
			{
				...
				header('Location: '. $config['base_url'] . "page/intra_prd/district/treasury/treasury_details.php?msg_er=".$error_msg);
				//include 'treasury_details_for_secondary.php' ;
				exit;
	...
	$block = $db->fetch_table("SELECT distcd, block_code, treasury_block_code, treasury_name, lead_bank,lead_branch, entry_time, ip_add, 	desig_block,app_duration, address_line2,contact_no,email,admin_id_pk FROM prd_dise_admin where block_code='$block_name'") ; 
	   //echo count($block);exit;
	if(count($block)>0){
		$update = $db->update("update prd_dise_admin set treasury_block_code ='$treasury_block_code', address_line2='$line', treasury_name='$treasury_name', lead_bank='$lead_bank_branch',lead_branch='$branch_name',app_duration='$dpp',contact_no='$tan_no',desig_block='$desig_block',email='$email' where block_code='".$block_code."'");
		...
		/*echo "INSERT INTO prd_dise_admin(
