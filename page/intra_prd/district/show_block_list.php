...
$arr=$db->fetch_table("
						select DISTINCT(block.block_code),(block_id_pk),block_name,count(emp.emp_id_pk) as total,(save.status_flag) as status,(save.lock_status) as lock_status,

count(DISTINCT(gp.gp_id_pk)) as total_gp from prd_location_master_gp gp
...
INNER JOIN (select emp_id_fk,status_flag,lock_status from prd_employee_salary_save

 where delete_status=1 and is_saved=1  and status_flag=3 AND salary_monthyear='".date('Ym')."' AND lock_status=1 ) as save ON emp.emp_id_pk=save.emp_id_fk WHERE emp.emp_status=1 
