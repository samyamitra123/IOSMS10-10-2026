...
<div class="btn-group">
<? if($arr[0]['bank_upd_status']=='2'){ ?>
  <a type="button" class="btn btn-success" href="<?= $config['base_url']?>page/intra_prd/district/ajax_bank_update.php?flag=<?= $cryptoGraph->encode('approve',4);?>&id=<?= $cryptoGraph->encode($arr[0]['emp_id_pk'],4);?>">Approve</a>
  <? }if($arr[0]['bank_upd_status']=='1' || $arr[0]['bank_upd_status']=='2'){ ?> 
  <a type="button" class="btn btn-danger" href="<?= $config['base_url']?>page/intra_prd/district/ajax_bank_update.php?flag=<?= $cryptoGraph->encode('reject',4);?>&id=<?= $cryptoGraph->encode($arr[0]['emp_id_pk'],4);?>">Reject Request</a>
  <? } ?>
