...
/*	else if($validator->blank_select($reason)==FALSE)
	...
	$db->update("UPDATE prd_employee_salary_save SET status_flag=2,lock_status=0 WHERE block_code='".trim($block_code)."' AND salary_monthyear='".date('Ym')."' AND status_flag=3");
	if($db){
			$date=date("Ym");
