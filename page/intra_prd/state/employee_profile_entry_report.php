...
count(distinct CASE WHEN (em1.emp_status='7') then em1.emp_id_pk else null end) as emp_rej,
 count(distinct CASE WHEN (em1.emp_status='10') and (em1.emp_form_status<=4) then em1.emp_id_pk else null end) as emp_incomplete,
...
LEFT JOIN prd_location_master_gp gp ON SUBSTRING (CAST(gp_code AS text),1,4)=CAST(district_code AS text)

LEFT JOIN prd_employee_master as em1 on em1.gp_id_fk=gp.gp_id_pk where district_code<>'3299' and district_code<>'3298'
