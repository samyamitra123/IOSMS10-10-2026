...
$date=date("Y-m-d");
	...
	if(!isset($_POST['ptax_order_submit'])){
		//echo 1111;exit;
		header('Location: '. $config['base_url'] . "page/intra_prd/state/paychange/paychange_order_edit.php");
		exit;
	...
	if($sec_time_token!=$enc_session){
		...
		if(($validator->blank_select($activation_date)==FALSE) ){
			...
			header('Location: '. $config['base_url'] . "page/intra_prd/state/paychange/paychange_order_edit.php?order_id=".$cryptography->encode($ptax_id_pk,3)."&msg=".$cryptography->encode($error_msg1,3) );
			//include 'paychange_order_edit.php';
		    exit;
		...
		$insert=$db->insert("		
		 UPDATE prd_admin_paychange
				...
				if($insert)
				{
					...
					header('Location: '. $config['base_url'] . "page/intra_prd/state/paychange/paychange_order_edit.php?order_id=".$cryptography->encode($ptax_id_pk,3)."&msg=".$cryptography->encode($msg,3));
			        exit;
				}
				else
				{
					...
					//header('Location: '. $config['base_url'] . "page/intra_prd/state/paychange/ptax_order_form.php?msg=".$error_msg1);
 					//include 'ptax_order_form.php';
					header('Location: '. $config['base_url'] . "page/intra_prd/state/paychange/paychange_order_edit.php?order_id=".$cryptography->encode($ptax_id_pk,3)."&msg=".$cryptography->encode($msg,3));
			        exit;
				}
	  
		//header('Location: '. $config['base_url'] . "page/intra_prd/state/paychange/paychange_order_details.php?msg=".$msg);
		//exit;
	}
?>
