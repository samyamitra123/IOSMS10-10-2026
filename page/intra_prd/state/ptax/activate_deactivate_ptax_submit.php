...
if($validator->blank_select($ptax_orderfile_pk)==FALSE || $validator->pattern_number($ptax_orderfile_pk)==FALSE)
{
	$_SESSION['msg']='<div class="alert alert-danger" style="text-align:center;"><strong>Updation Failed.</strong></div>';
		header('Location:'.$config['base_url'].'page/intra_prd/state/ptax/ptax_order_details.php');
		exit(0);
} 

if($_REQUEST['flag']=='activate')
{
	$update_ptax=$db->update("UPDATE prd_ptax_order_file
   									SET active_status='1'
 									WHERE ptax_orderfile_pk='".$ptax_orderfile_pk."'");
									
	$update_ptax_de=$db->update("UPDATE prd_ptax_order_file
   									SET active_status='0'
 									WHERE ptax_orderfile_pk<>'".$ptax_orderfile_pk."'");	
...
{
	$update_paychange=$db->update("UPDATE prd_ptax_order_file
   									SET active_status='0'
 									WHERE ptax_orderfile_pk='".$ptax_orderfile_pk."'");	
	if($update_paychange){
