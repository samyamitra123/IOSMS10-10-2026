...
//				else if(($promotion_data[0]['dop_doi'] == 5) && ($promotion_data[0]['annual_increment_date'] != $promotion_data[0]['effective_date']))
	...
	$supplementary_employee_fetch=$db->fetch_table("
															...
															FROM
																prd_employee_master
															WHERE 
																emp_status=1 AND ps_id_fk = '".$dise."'
															EXCEPT
															SELECT
																emp_id_fk
															FROM
																prd_employee_salary_save
															WHERE 
																status_flag=3 AND delete_status=1 AND is_saved=1 AND ps_id_fk = '".$dise."' AND salary_monthyear='".date('Ym')."' AND requisition_type='1001'
			...
			foreach ($tch as $key) 
			{
				...
				if(count($promotion_data_cal) > 0){	
					...
					if($key['emp_desig']=='9012' || $key['emp_desig']=='9013' || $key['emp_desig']=='9014' || $key['emp_desig']=='9015' || $key['emp_desig']=='9016' || $key['emp_desig']=='9017')
					{
						...
						$total_deduct = $gpf+$pfl+$cpf_deduct+$ptax+$itax+$gsli+$overdrawn+$festival_loan+$hra_deduction+$festival_loan;
    ...
    <a href="<?= $config['base_url'] ?>page/intra_ps/da/supplementary_sal/ajax_ps_rq_finalize.php" class="btn btn-success btn-sm">Salary Finalize</a>
    </div>
    </div>
...
{ ?>
	<script>
		...
		window.location.replace("<?php echo $config['base_url'] . 'page/intra_ps/da/supplementary_sal/view_ps_salary_requisition.php'; ?>");
		},3000);
    </script>
...
$.post('<?= $config['base_url'] ?>page/intra_ps/da/supplementary_sal/ajax_ps_requisition_edit1.php?id='+link+'&ps_id='+link1, function(data){                    
$("#mbody").html(data);
});
});
...
$.post('<?= $config['base_url'] ?>page/intra_ps/da/supplementary_sal/ajax_ps_rq_save1.php?emp_id_pk='+emp_id_pk+'&empcd='+empcd+'&sec_tok='+sec_tok, function(data){
//alert(data);
...
$.post('<?= $config['base_url'] ?>page/intra_ps/da/supplementary_sal/ajax_ps_rq_finalize1.php', function(data){
//alert(data);
//$("#mbody").html(data);
});
});
