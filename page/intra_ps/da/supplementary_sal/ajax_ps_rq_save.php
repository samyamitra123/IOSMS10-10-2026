<?php
	...
	$arr = $db->fetch_table("select pay_payband,emp_id_fk from prd_employee_salary_save where status_flag=1 AND delete_status=1 AND ps_id_fk='$gpcd' AND salary_monthyear='".date('Ym')."' AND emp_id_fk='$emp_id_pk' AND requisition_type='".$requisition_type."'");
	...
	if($arr[0]['emp_id_fk']){
		...
		$upd = $db->update("UPDATE prd_employee_salary_save SET is_saved=1  WHERE ps_id_fk='$gpcd' AND delete_status=1 AND emp_id_fk='$emp_id_pk' AND code='".$crypto->decode($_SESSION['code'.$emp_id_pk],4)."' AND requisition_type='".$requisition_type."'");
		if($upd){
		...
		$ins = $db->insert("INSERT INTO prd_employee_salary_save(slno,latestupdate_time, latestupdate_ip_address, ps_id_fk, empcd, bankname, 
		accountno, basic, da, hra, ma, cpf, pf_loan, p_tax, i_tax, net, bank_ifsc, sal_source, spl_pay, pf_deduct, code, spl_alo, status_flag, 
		salary_monthyear, category_id, block_code, emp_id_fk, pay_payband, tch_grade_pay, hill_allowance, gpf, cpf_deduct, gross_salary, is_saved,
		conv_allow,overdrawn, salary_type, cause,part_day,gsli,delete_status,consolidated_pay,festival_loan,festival_loan_cause,interim_relief,
		requisition_type,gp_id_fk,hra_deduction )
		...
		'".$crypto->decode($_SESSION['ptax'.$emp_id_pk],4)."', '".$crypto->decode($_SESSION['code'.$emp_id_pk],4)."','0', '1', '$salary_monthyear',
		'1', '0', '".$crypto->decode($_SESSION['emp_id_pk'.$emp_id_pk],4)."', '".$crypto->decode($_SESSION['pay_in_band'.$emp_id_pk],4)."', 
		...
		'".$crypto->decode($_SESSION['gsli'.$emp_id_pk],4)."','1','".$crypto->decode($_SESSION['consolidated_pay'.$emp_id_pk],4)."',
		'".$crypto->decode($_SESSION['festival_loan'.$emp_id_pk],4)."','".$crypto->decode($_SESSION['festival_loan_cause'.$emp_id_pk],4)."',
		'0','".$requisition_type."','0','".$crypto->decode($_SESSION['hra_deduc'.$emp_id_pk],4)."')");
...
inner join prd_location_master_panchayat_samiti ps on ps.ps_id_pk=prd_emp.ps_id_fk
									WHERE
									prd_emp.emp_status in(1,9) and ps.ps_id_pk='".$_SESSION['location']['ps_id']."'
									");*/
									
									
									$sal_emp=$db->fetch_table("SELECT
																emp_id_pk
															FROM
																prd_employee_master
															WHERE 
																emp_status in(1,9) AND ps_id_fk='".$_SESSION['location']['ps_id']."'
															EXCEPT
															SELECT
																emp_id_fk
															FROM
															...
															WHERE 
																status_flag=3 AND delete_status=1 AND is_saved=1 AND ps_id_fk='".$_SESSION['location']['ps_id']."' AND salary_monthyear='".date('Ym')."' AND requisition_type='1001'");			
										
								$sal_emp_count=count($sal_emp);		
