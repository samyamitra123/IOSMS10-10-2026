<?php
	...
	$arr = $db->fetch_table("select pay_payband,emp_id_fk from prd_employee_salary_save where status_flag=1 AND delete_status=1 AND ps_id_fk='$gpcd' AND salary_monthyear='".date('Ym')."' AND emp_id_fk='$emp_id_pk' AND requisition_type='".$requisition_type."' AND ropa_status ='1' ");
	
	$emp_desig_fetch = $db->fetch_table("select emp_desig, emp_group, emp_pay_scale, emp_pay_band from prd_employee_master where emp_id_pk= '$emp_id_pk' AND (ropa_status = '1' OR emp_cosolidated_pay!='0') ");
	...
	if($arr[0]['emp_id_fk']){
		...
		$upd = $db->update("UPDATE prd_employee_salary_save SET is_saved=1  WHERE ps_id_fk='$gpcd' AND delete_status=1 AND emp_id_fk='$emp_id_pk' AND code='".$crypto->decode($_SESSION['code'.$emp_id_pk],4)."' AND requisition_type='".$requisition_type."' AND ropa_status ='1' ");
		if($upd){
		...
		requisition_type,gp_id_fk,hra_deduction, emp_desig, emp_group, emp_pay_scale, emp_pay_band, ropa_level, ropa_status)
		...
		'".$crypto->decode($_SESSION['pay_in_band'.$emp_id_pk],4)."', '0',
		...
		'".$emp_group."', '".$emp_pay_scale."', '".$emp_pay_band."',
		'".$crypto->decode($_SESSION['ropa_level'.$emp_id_pk],4)."', '1')");
	...
	$sal_emp_count = $db->fetch_table("SELECT count(emp_id_pk) as emp_cnt 
										...
										WHERE (prd_emp.emp_status=1 OR prd_emp.emp_status=9) and ps.ps_id_pk='".$_SESSION['location']['ps_id']."'
	...
	$sal_saved = $db->fetch_table("SELECT count(emp_id_fk) as sal_cnt FROM 
											...
											WHERE
											         (prd_emp.emp_status=1 OR prd_emp.emp_status=9)
