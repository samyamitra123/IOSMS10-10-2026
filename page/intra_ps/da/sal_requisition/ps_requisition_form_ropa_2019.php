...
function getAmount($dise,$empcd,$type,$basic,$requisition_type)
{
	...
	if($type=='cpf')
	{
		$arr = $db->fetch_table("select cpf from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND ps_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status = '1' ");
	...
	if($type=='gpf')
	{ 
		$arr = $db->fetch_table("select gpf from prd_employee_salary_save where status_flag=1  AND emp_id_fk='$empcd' AND salary_monthyear='".date('Ym')."' AND ps_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status = '1'");
		...
		{
				$arr_emp = $db->fetch_table("select ropa_9_emp_pay_in_payband,emp_grade_pay, emp_pay_in_payband from prd_employee_master where ropa_status=1  AND emp_id_pk='".$empcd."' AND ps_id_fk='".$dise."'  AND ropa_status='1'");
	...
	if($type=='pfl')
	{
		$arr = $db->fetch_table("select pf_loan from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND ps_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status = '1'");
		if($arr[0]['pf_loan'])
		{
	...
	if($type=='itax')
	{
		$arr = $db->fetch_table("select i_tax from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND ps_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status = '1'");
		if($arr[0]['i_tax'])
		{
			return $arr[0]['i_tax'];
		}
	...
	if($type=='ovd')
	{
		$arr = $db->fetch_table("select overdrawn from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND ps_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status = '1'");
		if($arr[0]['overdrawn'])
		{
	...
	if($type=='gsli')
	{
		$arr = $db->fetch_table("select gsli from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND empcd='$empcd' AND salary_monthyear='".date('Ym')."' AND ps_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status = '1'");
		if($arr[0]['gsli'])
		{
			return $arr[0]['gsli'];
		}
...
$Query = "SELECT ps_id_fk
						...
						";
//print_r($Query); exit;						
$tch_final = $db->fetch_table($Query);
...
if(count($tch_final) == 0 )
{
	...
	on p.emp_id_fk=tch.emp_id_pk
													WHERE
													tch.ps_id_fk = '".$dise."' AND (tch.ropa_status = '1' )
...
//				else if(($promotion_data[0]['dop_doi'] == 5) && ($promotion_data[0]['annual_increment_date'] != $promotion_data[0]['effective_date']))
	...
	$sal_saved = $db->fetch_table("SELECT save.empcd,prd_emp.emp_desig,save.emp_id_fk FROM prd_employee_salary_save as save 
								...
								AND salary_monthyear='".date('Ym')."' AND requisition_type='".$requisition_type."' AND save.ropa_status = '1' 
								");
        ...
        if(count($tch))
		{
			...
			foreach ($tch as $key) 
			{
				...
				$sal_chk = $db->fetch_table("select slno, latestupdate_time, latestupdate_ip_address, save.ps_id_fk, save.empcd, 
											...
											where (emp_status=1 OR emp_status=9) and status_flag=1 AND delete_status=1 
											AND salary_monthyear='".date('Ym')."'  AND emp_id_fk='$emp_id_pk' AND save.ropa_status = '1'
				...
				if($sal_chk[0]['ps_id_fk'] && $sal_chk[0]['empcd'])
				{
					...
					else
					{
						...
						if(count($promotion_data_cal) >0){	
							...
							$full_gross_salary =round($pay_in_band+$da+$hill_allowance+$hra+$ma+$conveyance_allowance);
							...
							else
							{
								if(strtotime($suspend_effect_date)<=strtotime(date('Y-m-d')) && strtotime($suspend_effect_date)>0)
								{
									$full_gross_salary_sus =round($full_pay_in_band_sus+$full_da_sus+$hra+$ma+$conveyance_allowance);
				...
				else
				{
					...
					overdrawn, salary_type, cause,gp_code, part_day,gsli,consolidated_pay,festival_loan,festival_loan_cause,interim_relief  from prd_employee_salary_save where status_flag IN (3,4) AND delete_status=1 AND salary_monthyear='".date('Ym',strtotime('-1 month'))."' AND ps_id_fk='".$key['ps_id_fk']."' AND emp_id_fk='$emp_id_pk' AND requisition_type='".$requisition_type."'
					AND ropa_status='1'");	
				...
				if(count($promotion_data_cal) > 0){	
					...
					else
					{
						...
						$gross_salary = round($pay_in_band+$da+$hra+$ma+$conveyance_allowance+$cpf+$hill_allowance);
						if($key['conv_allow_status']=='1')
						{
							$ptax = 0;
						}
						else
						{
							if(strtotime($suspend_effect_date)<=strtotime(date('Y-m-d')) && strtotime($suspend_effect_date)>0)
							{
								$gross_salary_sus=round($pay_in_band+$da_sus+$hra+$ma+$conveyance_allowance+$cpf+$hill_allowance);
    ...
    <a href="<?= $config['base_url'] ?>page/intra_ps/da/sal_requisition/ajax_ps_rq_finalize_ropa_2019.php" class="btn btn-success btn-sm">Salary Finalize</a>
    </div>
    </div>
...
{ ?>
	<script>
		...
		window.location.replace("<?php echo $config['base_url'] . 'page/intra_ps/da/sal_requisition/view_ps_salary_requisition_ropa_2019.php'; ?>");
		},3000);
    </script>
...
var link1= $("#ps_id").val();
$.post('<?= $config['base_url'] ?>page/intra_ps/da/sal_requisition/ajax_ps_requisition_edit_ropa_2019.php?id='+link+'&ps_id='+link1, function(data){                    
$("#mbody").html(data);
});
});
...
$.post('<?= $config['base_url'] ?>page/intra_ps/da/sal_requisition/ajax_ps_rq_save_ropa_2019.php?emp_id_pk='+emp_id_pk+'&empcd='+empcd+'&sec_tok='+sec_tok, function(data){
//alert(data);
...
//alert('in function');
$.post('<?= $config['base_url'] ?>page/intra_ps/da/sal_requisition/ajax_ps_rq_finalize_ropa_2019.php', function(data){
//alert(data);
//$("#mbody").html(data);
});
});
