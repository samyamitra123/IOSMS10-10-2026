...
<div class="border_val" style="width: 98%;"></div>
	...
	<div class="emplist" style="width: 99%;">
		...
		<?php
			$db = new database();
			$id_const= $db->fetch_table(" SELECT * FROM prd_employee_master 
	...
	{		
		foreach ($id_const as $const) {
			if(strlen($const['emp_first_memo_no']) <= 1 || strlen($const['emp_present_memo_no']) <= 1 || $const['recruitment_type'] == '0')
				$appMemo[] = $const['emp_first_name'].' '.$const['emp_second_name'].' '.$const['emp_last_name'].' ('.$const['emp_id_const'].')';
		}
	}	
		?>
		...
		?></b></p> </div>    
		 <?php }?> 
			...
			<div class="table-responsive">
				...
				if(count($appMemo) > 0)
				  {
				  	    include_once('../../../intra_prd/common/appo_type.php');
						  	echo "
						    <ul>
						  	";
					...
					require_once 'ps_requisition_form_ropa_2019.php'; 
