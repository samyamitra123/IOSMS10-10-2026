...
{ //var_dump($type);
	...
	if($type=='cpf')
	{
		$arr = $db->fetch_table("select cpf from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='".$emp_id_pk."' AND salary_monthyear='".date('Ym')."' AND ps_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status ='1' ");
		if($arr[0]['cpf'])
		{
			return $arr[0]['cpf'];
		}
	...
	if($type=='gpf')
	{
		...
		$arr = $db->fetch_table("select gpf from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='".$emp_id_pk."' AND salary_monthyear='".date('Ym')."' AND ps_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status ='1' ");
		if($arr[0]['gpf'])
		{
			return $arr[0]['gpf'];
		}
		...
		$arr_emp = $db->fetch_table("select ropa_9_emp_pay_in_payband,emp_grade_pay, emp_pay_in_payband from prd_employee_master where ropa_status=1  AND emp_id_pk='".$emp_id_pk."' AND ps_id_fk='".$dise."'  AND ropa_status='1'");
	...
	if($type=='pfl')
	{
		$arr = $db->fetch_table("select pf_loan from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND ps_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status ='1' ");
		if($arr[0]['pf_loan'])
		{
	...
	if($type=='itax')
	{
		$arr = $db->fetch_table("select i_tax from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND ps_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status ='1' ");
	...
	if($type=='ovd')
	{
		$arr = $db->fetch_table("select overdrawn from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND ps_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status ='1' ");
		if($arr[0]['overdrawn'])
		{
	...
	if($type=='gsli')
	{
		$arr = $db->fetch_table("select gsli from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND ps_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status ='1' ");
		if($arr[0]['gsli'])
		{
			return $arr[0]['gsli'];
		}
	...
	if($type=='conv')
	{
		$arr = $db->fetch_table("select conv_allow from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='$emp_id_pk' AND salary_monthyear='".date('Ym')."' AND ps_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status ='1' ");
		if($arr[0]['conv_allow'])
		{
...
function festival_adv_recovery($emp_id_pk)
{
	// for festival advance by nd on 04072017	
		//die;
	$db = new database();
	$arr = $db->fetch_table("select festival_loan from prd_employee_salary_save where status_flag=1 AND delete_status=1 AND salary_monthyear='".date('Ym')."' AND emp_id_fk = '".$emp_id_pk."' AND ropa_status ='1' "); 
...
$sal_chk= $db->fetch_table("SELECT slno, latestupdate_time, latestupdate_ip_address, ps_id_fk, empcd, 
						...
						hra_deduction,overdrawn, salary_type, cause,gp_code, part_day,gsli,
						consolidated_pay,festival_loan,festival_loan_cause 
						FROM prd_employee_salary_save 
						WHERE status_flag=1 AND delete_status=1 AND salary_monthyear='".date('Ym')."'  AND ropa_status ='1' 
						AND emp_id_fk='$emp_id_pk' AND ps_id_fk='".$dise."' AND requisition_type='".$requisition_type."'");
...
if($sal_chk[0]['ps_id_fk'] && $sal_chk[0]['emp_id_fk'] && ($sal_chk[0]['salary_type']=='1' || $sal_chk[0]['salary_type']=='2'))
{
	...
	if($tch[0]['emp_desig']=='9012' || $tch[0]['emp_desig']=='9013' || $tch[0]['emp_desig']=='9014' || $tch[0]['emp_desig']=='9015' || $tch[0]['emp_desig']=='9016' || $tch[0]['emp_desig']=='9017')
	{
		...
		if(count($promotion_data_cal) > 0){
			...
			else
			{
				...
				if(strtotime($suspend_effect_date)<=strtotime(date('Y-m-d')) && strtotime($suspend_effect_date)>0)
				{
					...
					$full_da = round(($full_basic_sus/100)*$da_per);//04.12.2020//
					$full_interim_relief =0;
				}
				else
				{
					...
					$full_basic = $full_pay_band;
					$full_da = round(($full_basic/100)*$da_per);//04.12.2020//
					//$full_interim_relief = $tch[0]['interim_relief']28.12.2018;
					$full_interim_relief=0;
...
else
{
	...
	$sal_save=	$db->fetch_table("SELECT slno, latestupdate_time, latestupdate_ip_address, ps_id_fk, empcd, 
								...
								category_id, block_code, emp_id_fk, pay_payband, tch_grade_pay, 
								hill_allowance, gpf, cpf_deduct, gross_salary, is_saved, conv_allow, 
								overdrawn, salary_type, cause,gp_code, part_day,gsli,consolidated_pay,festival_loan,
								festival_loan_cause,hra_deduction,consolidated_pay 
								from prd_employee_salary_save 
	...
	$emp_suspend=$db->fetch_table("SELECT 
									...
									INNER JOIN prd_employee_master em1 on sus.emp_id_fk=em1.emp_id_pk
									WHERE
									sus.ps_id_fk = '".$dise."' AND sus.emp_id_fk='".$cryp->decode($_REQUEST['id'],4)."' AND em1.emp_status='9' 
									AND sus.delete_status='0'
	");
	...
	if(strtotime($suspend_effect_date)<=strtotime(date('Y-m-d')) && strtotime($suspend_effect_date)>0)
	{
		...
		$da = round(($basic_sus/100)*$da_per);//04.12.2020//
		$interim_relief =0;
	}
	else
	{
	...
	//$cpf_deduct = $cpf*2;
	/**************************************** Changed By ANJAN for erroe log Start ************************************/
	/*
	if($key['emp_diff_able']=='1')
	{
		$ptax = 0;
	...
	*/
	...
	else
	{
		if(strtotime($suspend_effect_date)<=strtotime(date('Y-m-d')) && strtotime($suspend_effect_date)>0)
		{
			$gross_salary_sus=round($pay_in_band+$da_sus+$interim_relief+$hra+$ma+$conveyance_allowance+$cpf+$hill_allowance);
	...
	$overdrawn=0;
	$total_deduct = $gpf+$pfl+$ptax+$itax+$gsli+$hra_deduction+$festival_loan; 
	$net_salary = $gross_salary-$total_deduct;
...
<script>
	...
	$(document).ready(function(e) 
	{
		...
		{
			var no_of_part_days=$('#num_part_days').val();
			var cause_for_part_sal=$('#cause_part_sal').val();
			var id=$(this).val();
			if($('#is_overdrawn').val()=='no_ovd')
			{
				var overdrawn = $('#overdrawn').val();
			...
			if($('#exist_part_day').val()!='')
			{
				if(id == '1')
				{
					...
					if(desig!='9012' && desig!='9013' && desig!='9014' && desig!='9015' && desig!='9016' && desig!='9017')
					{
				...
				if(id == '2')
				{
					...
					//$('#cause_holder').show();
					$('#working_days').val(no_of_part_days);
					$('#part_salary_cause_msg').val(cause_for_part_sal);
					...
					if(desig!='9012' && desig!='9013' && desig!='9014' && desig!='9015' && desig!='9016' && desig!='9017')
					{
						...
						$('#gross').val('<?php echo $ross_salary; ?>');
...
<form id="form" method="post" action="ajax_ps_rq_submit_ropa_2019.php" enctype="multipart/form-data">
            ...
            <table>
                <tr>
                    ...
                    <td>
                        ...
                        ");
                        $sal_caus = $db->fetch_table("select overdrawn,salary_type,cause,part_day,cpf,gpf,gsli,part_salary_cause,no_salary_cause from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='".$tch[0]['emp_id_pk']."' AND ps_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND salary_monthyear='".date('Ym')."'
						AND ropa_status='1'");
                        $part_day_cal=$sal_caus[0]['part_day']; 
                        ?>
                        <input type="hidden" name="num_part_days" id="num_part_days" value="<?php echo $part_day_cal; ?>" />
                        <input type="hidden" name="cause_part_sal" id="cause_part_sal" value="<?php echo $sal_caus[0]['part_salary_cause']; ?>" />
                ...
                <tr id="is_reduct"  <?php if($sal_caus[0]['salary_type']!= '8'){ ?><?php }else{ ?> style="display:none;" <?php } ?>>
                    ...
                    <td>
						<?php 
                        $dedact_cause = $db->fetch_table("select festival_loan_cause,hra_loan_cause from prd_employee_salary_save where status_flag=1 AND delete_status=1  AND emp_id_fk='".$tch[0]['emp_id_pk']."' AND ps_id_fk='".$dise."' AND requisition_type='".$requisition_type."' AND ropa_status ='1' ");
                ...
                <tr id="reduction_type" >
                    ...
                    <p><input type="checkbox" name="n9" id="reduction_type3" class="cls8" value="1" onclick="return false;" <?php  if($dedact_cause[0]['festival_loan_cause']==1|| $festival_loan_n>0) { ?>checked="checked" <?php } ?>>&nbsp;&nbsp;&nbsp;<b>Festival Advance Recovery
                    </b></p>
                    </td>
                </tr>
            </table>
		...
		<div class="table-responsive">
            <table width="100%">
                ...
                <tr style="background-color: rgb(221, 247, 255);">
                    ...
                    <td>
                        <input maxlength="5" style='background-color: #EEE;' type="text" id="consolidated_pay" name="consolidated_pay" value="<?php echo round($consolidated_pay); ?>" readonly size="5" />
	...
	<script>
		$('#working_days').keyup(function(e){
			...
			else
			{
				...
				{
					$('#pay_in_band').val('<?php echo $pay_in_band; ?>');
					$('#consolidated_pay').val('<?php echo getEmpConsolidated('consolidated_pay',$dise,$emp_id_pk); ?>');
					$('#grade_pay').val('<?php echo $ropa_level; ?>');
					$('#da').val('<?php echo $da; ?>');
					$('#interim_relief').val('<?php echo $interim_relief; ?>');
					$('#hra').val('<?php echo $hra; ?>');
					...
					var gross = parseInt(consolidated_pay)+parseInt(pay_band)+parseInt(da)+parseInt(interim_relief)+parseInt(hra)+parseInt(ma)+parseInt(conv_allow)+parseInt(hill_allaw);
					//alert(gross);
					$('#gross').val(gross);
					var gpf = $('#gpf').val();
					var pfl = $('#pf_loan').val();
				...
				else
				{
					...
					if(pay_in_pay_band!=0)
					{
						var part_pay_band = Math.round((parseInt(<?php echo getEmpAmount('pay_in_pay_band',$dise,$emp_id_pk); ?>)/parseInt(max_days_of_month))*parseInt(work_days));
					...
					{
						var part_consolidated_pay = Math.round((parseInt(<?php echo getEmpConsolidated('consolidated_pay',$dise,$emp_id_pk); ?>)/parseInt(max_days_of_month))*parseInt(work_days));
						$('#consolidated_pay').val(part_consolidated_pay);
						part_pay_band=0;
					}
					if(desig!='9012' && desig!='9013' && desig!='9014' && desig!='9015' && desig!='9016' && desig!='9017')
					{
						var grade_pay = $('#grade_pay').val();
						var part_grade_pay = Math.round((parseInt(<?php echo getEmpAmount('grade_pay',$dise,$emp_id_pk); ?>)/parseInt(max_days_of_month))*parseInt(work_days));
					} else 
					{
						part_grade_pay=0;	
					}
					...
					var gross = parseInt(part_consolidated_pay)+parseInt(pay_band)+parseInt(da)+parseInt(interim_relief)+parseInt(hra)+parseInt(ma)+parseInt(conv_allow)+parseInt(hill_allaw);
