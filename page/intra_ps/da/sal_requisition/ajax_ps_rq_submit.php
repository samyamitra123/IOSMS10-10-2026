...
else
{
	...
	else if($emp_desig=='9012' || $emp_desig=='9013' || $emp_desig=='9014' || $emp_desig=='9015' || $emp_desig=='9016' || $emp_desig=='9017')
	{
		...
		else if($pay_in_band!='0' || $grade_pay!='0' || $da!='0' || $hra!='0' || $ma!='0' || $conv_allow!='0' || $hill_allow!='0' || $interim_relief!='0' || $gpf!='0' || $pf_loan!='0' || $i_tax!='0' || $gsli!='0' || $overdrawn!='0' || $festival_val!='0'|| $hra_deduction!='0')
		{
	...
	$arr = $db->fetch_table("SELECT ps_id_fk,emp_id_fk 
							...
							AND salary_monthyear='".date('Ym')."' AND requisition_type='".$requisition_type."' AND ropa_status='2' ");
	...
	if($arr[0]['emp_id_fk'] && $arr[0]['ps_id_fk'])
	{
		...
		$ins = $db->insert("INSERT INTO prd_employee_salary_save
							...
							hill_allowance, gpf, cpf_deduct, gross_salary, is_saved,
							conv_allow,overdrawn,salary_type,cause,
							part_day,gsli,delete_status,consolidated_pay,
							festival_loan,festival_loan_cause,part_salary_cause,
							no_salary_cause,interim_relief,requisition_type,gp_id_fk,hra_deduction,hra_loan_cause,
							emp_desig, emp_group, emp_pay_scale, emp_pay_band, ropa_status)
							...
							'1', '0', '$teacher_id_pk', '$pay_in_band', '$grade_pay', '$hill_allow','$gpf', '0', 
							'$gross', '0','$conv_allow','$overdrawn','$salary_type','$cause_msg','$part_day',
							'$gsli','1','$consolidated_pay','$festival_val',
							'$festival','$part_salary_cause_msg','$no_salary_cause_msg',
							'$interim_relief','".$requisition_type."','0','".$hra_deduction."','".$hra_loan."',
							$emp_desig,'".$emp_detail[0]['emp_group']."', '".$emp_detail[0]['emp_pay_scale']."', '".$emp_detail[0]['emp_pay_band']."',
							'$ropa_status')");			
	...
	{ 
		...
		$ins = $db->insert("INSERT INTO prd_employee_salary_save
							...
							cause,part_day,gsli,delete_status,consolidated_pay,
							festival_loan,festival_loan_cause,part_salary_cause,no_salary_cause,
							interim_relief,requisition_type,gp_id_fk,hra_deduction,hra_loan_cause,
							emp_desig, emp_group, emp_pay_scale, emp_pay_band, ropa_status)
							VALUES ('0', 'now()', '$user_ip', '".$schcd."', '$tchcd', 
							'$bankname', '$accountno', '$basic', '$da', '$hra', '$ma', '0', '$pf_loan', '$p_tax', 
							'$i_tax', '$net', '$bank_ifsc', 'NA', '0', '$p_tax', '$code', 
							'0', '1', '$salary_monthyear', '1', '0', '$teacher_id_pk', '$pay_in_band', 
							'$grade_pay', '$hill_allow','$gpf', '0', '$gross','0','$conv_allow',
							'$overdrawn','$salary_type','$cause_msg','$part_day','$gsli',
							'1','$consolidated_pay','$festival_val','$festival','$part_salary_cause_msg',
							'$no_salary_cause_msg','$interim_relief','".$requisition_type."','0','".$hra_deduction."','".$hra_loan."',
							$emp_desig,'".$emp_detail[0]['emp_group']."', '".$emp_detail[0]['emp_pay_scale']."' , '".$emp_detail[0]['emp_pay_band']."',
							'$ropa_status')");
