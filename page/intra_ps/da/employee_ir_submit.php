...
if (
  ...
  /*if($validator->blank_select($ir) == FALSE)
	...
	$emp_id_pk_check=$db->fetch_table("SELECT 
											emp_id_fk,
											delete_status,unlock_req 
										FROM 
											psemp_interim_relief 
										WHERE 
											emp_id_fk='".$emp_id_fk."' AND ps_id_fk='".$_SESSION['location']['ps_id']."' AND ir_status in('1','4') AND delete_status='1' AND unlock_req in('0','2')");
	...
	{ 
		...
		$ir_submit=$db->insert("INSERT INTO psemp_interim_relief
	       (emp_id_fk,ir_status,entry_time,interim_relief,ps_id_fk,delete_status)
		VALUES('".$emp_id_fk."','1','now()','".$ir."','".$_SESSION['location']['ps_id']."','1')");
