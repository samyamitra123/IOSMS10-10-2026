...
$arr=$db->fetch_table("SELECT 
						...
						FROM 
							prd_employee_master 
						WHERE 
							emp_status='1' AND ps_id_fk='".$_SESSION['location']['ps_id']."' AND emp_desig not in ('9012','9013')
...
$ir_saved = $db->fetch_table("SELECT 
								...
								WHERE
									ir_status IN ('1','2','3','4') AND delete_status='1' AND ps_id_fk='".$_SESSION['location']['ps_id']."'
...
function save_ir(k)
{
	...
	$.post('<?= $config['base_url'] ?>page/intra_ps/da/ajax_save_ir.php?postemp='+emp_id_pk+'&emp_full_name='+emp_full_name, function(data){
...
function send_ir(k)
{
	...
	$.post('<?= $config['base_url'] ?>page/intra_ps/da/ajax_send_ir.php?postemp='+emp_id_pk+'&emp_full_name='+emp_full_name, function(data){
	var result = $.parseJSON(data);
			if(result[0]==1)
			{           
				$('#session_msg_print').html('');
  ...
  <div class="modal-dialog modal-sm">
    <div class="modal-content" >
     <div class="modal-header">
      ...
      <div class="modal-body"> 
      <p class="alert alert-warning"><strong><i class="fa fa-exclamation-triangle"></i> Are You Sure To Send IR Unlock Request to Executive Officer?</strong></p>
