...
/////////////////////////////////////////////////////////////////////////////// SERVER SIDE VALIDATION  ///////////////////////////////////////////////////////////////////////////////////////////////
	...
	else
	{
		...
		if(empty($lms_query))
		{
			$query=$db->insert("INSERT INTO psemp_ps_profile
								(
									ps_id_fk,
									district_id_fk,
									exe_officer_name, 
									...
									ip_address,
									vill_name,
									ddo_code,treasury_code,
									pl_code_pf,t_code_pf)
									VALUES 
										('".$ps_id."',
										'".$district_id."',
										...
										'".$_SERVER['REMOTE_ADDR']."',
										'$vill_name',
										'$ddo_code','$t_code','$pl_code_pf','$t_code_pf')");
						
		} 
		else
		{
			$upd=$db->update("UPDATE prd_location_master_panchayat_samiti SET ps_status='0' WHERE ps_id_pk='".$ps_id."' AND district_id_fk='".$district_id."'");
			...
			if($upd)
			{
				$query=$db->update("UPDATE psemp_ps_profile SET
												...
												vill_name='$vill_name',
												ddo_code='$ddo_code',
												treasury_code='$t_code',
												pl_code_pf='$pl_code_pf',
												t_code_pf='$t_code_pf'
												WHERE ps_id_fk='".$ps_id."'");
			}
    	}
		
		if($query)
		{
			$_SESSION["msg"]='<div class="alert alert-success" style="text-align:center;"><strong>PS profile submitted Successfully...</strong></div>';
			header('Location:ps_profile_form.php');
		}
		else
		{
			$_SESSION["msg"]='<div class="alert alert-success" style="text-align:center;"><strong>PS profile submitted failed...</strong></div>';
			header('Location:ps_profile_form.php');
		}
	}
}
