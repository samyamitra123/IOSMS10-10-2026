...
WHERE emp_id_pk='".$Arguments['user_id']."' and emp_status in('1','9','2')  ");
   ...
   FROM prd_monthly_salary_archive_final as sal LEFT JOIN prd_employee_master as tch ON sal.emp_id_fk = tch.emp_id_pk 
    ...
    WHERE
						 	            sal.emp_id_fk='".$Arguments['user_id']."'
   ...
   $ps_id_pk = $key[0]['ps_id_pk'];
  // print_r($key); exit;
if($key[0]['requisition_type']=='1001')
{
	$bill_details_fetch=$db->fetch_table(" SELECT bill_no, bill_entry_time FROM prd_block_bill_details WHERE ps_id_fk='".$ps_id_pk."' AND salary_monthyear='".$monthno."' AND requisition_type='1001' AND status='1' AND ropa_status='".$ropa_status."' ");
}
elseif($key[0]['requisition_type']=='1003')
{
	$bill_details_fetch=$db->fetch_table(" SELECT bill_no, bill_entry_time FROM prd_block_bill_details WHERE ps_id_fk='".$ps_id_pk."' AND salary_monthyear='".$monthno."' AND requisition_type='1003' AND status='1' AND ropa_status='".$ropa_status."' ");
}
if(count($key)>0){
