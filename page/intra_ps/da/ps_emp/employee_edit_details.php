...
if($id!='')
{
	...
	$arr=$db->fetch_table("select emp_first_name,emp_status,emp_second_name,emp_last_name,emp_form_status,emp_id_pk,emp_desig,emp_status from prd_employee_master where emp_status in('4','7','10') AND emp_id_pk='".$emp_id_pk."' AND ps_id_fk='".$_SESSION['location']['ps_id']."' order by emp_first_name");
}
else
{	
	$db=new database();
	$arr=$db->fetch_table("select emp_first_name,emp_status,emp_second_name,emp_last_name,emp_id_pk,emp_form_status,emp_id_pk,emp_desig,emp_status from prd_employee_master where emp_form_status in('1','2','3','4','5') AND emp_status in('10','7') AND ps_id_fk='".$_SESSION['location']['ps_id']."' order by emp_first_name");
}
...
{
	$status='<a value="'.$item['emp_id_pk'].'" id="show_rsn'.$cnt.'" onclick="show_reason('.$cnt.')" class="reason_view" data-bs-toggle="modal" data-bs-target="#reject_reason" style="cursor:pointer;"><span style="color:RED;font-weight:bold">PROFILE REJECTED </span></a>';
}
...
<td class="sendtoddo"><? if($item['emp_form_status']=='5'){ ?><a id="<?php echo $cryptoGraph->encode($item['emp_id_pk'],4) ?>" onClick="show_confirmation(this.id);"><img width="25" src="<?= $config['base_url'];?>themes/default/image/send.png" alt="Edit"/></a>
<? } else{ ?>
...
<form method="post" action="send_employee_to_eo.php" > 
