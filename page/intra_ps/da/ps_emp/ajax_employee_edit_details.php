...
function get_emp(){ 
	...
	$emp_status_check=$db->fetch_table("select emp_status,emp_id_pk,emp_id_const from prd_employee_master where emp_status in('6') and emp_id_pk='".$cryptoGraph->decode($_GET['id'], 4)."' AND ps_id_fk='".$_SESSION['location']['ps_id']."' order by emp_first_name");
	
	
	if(($emp_status_check['0']['emp_id_const']!='0'))
{
...
$data = $db->fetch_table("select emp_first_name,emp_second_name,emp_last_name,emp_form_status,emp_status,emp_id_pk,emp_desig from prd_employee_master where emp_id_pk='".$cryptoGraph->decode($_GET['id'], 4)."' emp_form_status in('5') AND emp_status in('6') AND ps_id_fk='".$_SESSION['location']['ps_id']."' order by emp_first_name");
	...
	$data = $db->fetch_table("select emp_first_name,emp_second_name,emp_last_name,emp_form_status,emp_status,emp_id_pk,emp_desig from prd_employee_master where emp_form_status in('1','2','3','4','5') AND emp_status in('10','7','4') AND ps_id_fk='".$_SESSION['location']['ps_id']."' order by emp_first_name");
...
if($item['emp_status']=='10' & $item['emp_form_status']=='5'){
	...
	$status='<a value="'.$item['emp_id_pk'].'" id="show_rsn'.$cnt.'" onclick="show_reason('.$cnt.')" class="reason_view" data-toggle="modal" data-target="#reject_reason" style="cursor:pointer;"><span style="color:RED;font-weight:bold">PROFILE REJECTED </span></a>';
}
...
if($item['emp_status']=='7')
{
		...
		$arr_reason=$db->fetch_table("select 
							reason from psemp_employee_profile_update_status where emp_id_fk='".$item['emp_id_pk']."' order by date desc limit 1");
...
<td class="sendtoddo"><? if($item['emp_form_status']=='5'){ ?><a id="<?php echo $cryptoGraph->encode($item['emp_id_pk'],4) ?>" onClick="show_confirmation(this.id);"><img width="25" src="<?= $config['base_url'];?>themes/default/image/send.png" alt="Edit"/></a></a>
<? } else{ ?>
