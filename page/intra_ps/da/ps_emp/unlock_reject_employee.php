...
if($cryptoGraph->decode($_POST['value_unlock'],4)==1)
{
			...
			//echo $emp_id_pk; die;
			 ...
			 $security = $db->insert("
 			 ...
 			 $query_unlock=$db->update("UPDATE prd_employee_master SET emp_status='4'
	     						 WHERE ps_id_fk = '".$_SESSION['location']['ps_id']."'
								  AND emp_status='3' AND emp_id_pk='".$emp_id_pk."'"); 
...
if($cryptoGraph->decode($_POST['value_reject'],4)==2)
{
 ...
 $security = $db->insert("
  ...
  $query_reject=$db->update("UPDATE prd_employee_master SET emp_status='1'
	     						 WHERE ps_id_fk = '".$_SESSION['location']['ps_id']."'
								  AND emp_status='3' AND emp_id_pk='".$emp_id_pk."'"); 
