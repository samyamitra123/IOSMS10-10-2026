...
else
{
	...
	if($desig !='9012' && $desig !='9013' && $desig !='9014' && $desig !='9015' && $desig !='9016' && $desig !='9017')
	{
		if($ropa_status=='1' || $emp_first_join_date>='2020-01-01' )
		{
			...
			{
			$error_msg='<div class="alert alert-danger" style="text-align:center"><strong>Please LEVEL.</strong></div>';
			$emp_id=$cryptoGraph->encode($emp_id_pk,4);
			include 'profile_entry_salary_form.php';
			exit;
			}
	...
	else
	{
		...
		if(($emp_status[0]['emp_status']=='4' && $update_id=='true')|| ($emp_status[0]['emp_status']=='10' && $emp_status[0]['emp_id_const']!='0'))
		{
			$query_archive=$db->insert("Insert into prd_employee_master_archive
										...
										profile_update_time,profile_update_ip_address,spouse_medical_allowance,conv_allow_status,ropa_status,ropa_level) 
										SELECT 
										emp_id_pk, gp_id_fk, emp_first_name, emp_second_name, emp_last_name, 
										emp_dob, emp_sex, emp_caste, emp_voter_id, emp_aadhar_no, emp_edu_quali, 
										emp_desig, emp_first_join_date, emp_conf_join_date, emp_join_prsnt_post_date, 
										...
										emp_status, entry_time, entry_ip, empcd, emp_grade_pay, emp_id_const,
										bank_upd_status,bank_msg_flag,conf_dt_flag,interim_relief,ps_id_fk,
										'now()','".$_SERVER['REMOTE_ADDR']."',spouse_medical_allowance,conv_allow_status,ropa_status,ropa_level
	...
	if($query_fetch_bank[0]['count']!='0'  )
	{
		...
		{
			header('Location:profile_entry_per.php?desig='.$cryptoGraph->encode($_POST['desig'],4).'&emp_id_pk='.$cryptoGraph->encode($emp_id_pk,4));
			exit(0);
		}
		else
		{
