...
function payband(val){
 $.post('<?= $config['base_url'] ?>page/intra_ps/da/ps_emp/ajax_payscale_details.php?payband='+val, function(data){
	 $(".payscale").html(data);	 
	 $.post('<?= $config['base_url'] ?>page/intra_ps/da/ps_emp/ajax_gradepay_details.php?payband='+val, function(data){
	 $(".gradepay").html(data);	 
 });
 });
}
        ...
        <div class="col-lg-12 col-md-8 col-sm-8" id="sm-pad"> 
            <div class="col-sm-12" style="width:98%;">
                ...
                <form class="form-horizontal" id="loginForm" method="post" action="profile_entry_salary_insert.php" onsubmit="return valid_code();">
					...
					<?php if($emp_desig=='9012' || $emp_desig=='9013' || $emp_desig=='9014' || $emp_desig=='9015' || $emp_desig=='9016' || $emp_desig=='9017') 
					{ ?>
                        <div class="row mb-3">
			...
			if($ropa_status == 1 || $emp_first_join_date>='2020-01-01'){?>
						...
						<div class="row mb-3">
                            ...
                            <div class="col-sm-4">
								...
								$get_column=$db->fetch_table("SELECT level1,level2,level3,level4,level5,level6,level7,level8,level9,level10,
								level11,level12,level13,level14,level15,level16,level17,level19 from ropa_2019 order by level ");
                                ?>
                                <select class="form-control" name="ropa_level" id="ropa_level" onChange="show_basic(this.value);">
                                    <option value="">-Please Select-</option>
                                    <? foreach($get_column[0] as $key=>$value){ //var_dump($ropa_level);?>
...
<script>
	function valid_code()
	{
		...
		if($('#desig').val()!='9012' && $('#desig').val()!='9013' && $('#desig').val()!='9014' && $('#desig').val()!='9015' && $('#desig').val()!='9016' && $('#desig').val()!='9017')
		{
			if($('#pay_band').val()=='')
			{
