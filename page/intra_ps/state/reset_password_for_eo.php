...
<script>
  ...
  if(document.getElementById('pass_chk').value!='yes'){
	...
	function fun_block(district_id){
		   //alert(district_code);
		   
		   $('#loader').show();
			$.post('<?= $config['base_url']?>page/intra_prd/state/block_list.php',{district_id:district_id},function(data){
			    //alert(data) ;
...
<div class="row" id="cont">
      <div class="col-lg-12 col-md-8 col-sm-8" id="sm-pad"> 
        <div class="col-sm-12">
<h1 class="heading">Reset Password For EO </h1>
...
<input type="hidden" name="sec_tok" id="sec_tok" value="<?=$enc_token?>" />
    ...
    <label for="inputPassword3" class="col-sm-3 control-label">EO CODE: <span class="star_color">*</span></label>
    <div class="col-sm-3">
      <select class="form-control upper_case" name="dist" id="dist" onchange="return fun_block(this.value)">
      <option value="">-Please Select-</option>
						<?php
							
							$dist=$db->fetch_table("Select district_id_pk,district_name from prd_location_master_district");
							foreach($dist as $key)
							{
								
						?>
                        <option value="<?php echo $key['district_id_pk']?>"><?php echo $key['district_name']; ?></option>
                  		 <?php
							}
						?>
      </select>
    ...
    <div class="col-sm-3">
      <select class="form-control upper_case" name="block" id="block_name">
      <option value="">-Please Select-</option>
      </select>
    </div>
    <div class="col-sm-3"> <span style="display:none;" id="loader">&nbsp; <img src="<?php echo $config['base_url']; ?>themes/default/image/unlock_load.gif"  alt="loading" style="width: 60px;margin-top: -18px;" /></span></div>
    </div>
    ...
    <div class="col-sm-offset-5 col-sm-7">
      <input type="submit" name="change_block" value="SUBMIT" class="btn btn-info">
    </div>
  </div>
