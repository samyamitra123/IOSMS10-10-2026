...
$sec_time_token=$_POST['sec_tok'];
	...
	if(!isset($_POST['paychange_submit'])){
		header('Location: '. $config['base_url'] . "page/intra_ps/state/paychange/paychange_insert.php");
		exit;
		...
		$hill_allowance=$_POST['hill_allowance'];
		$cpf=$_POST['cpf'];
		$date=date("Y-m-d");
	...
	$cryptography= new cryptography();
		...
		if((!$validation->blank_select($da))){
			...
			header('Location: '. $config['base_url'] . "page/intra_ps/state/paychange/paychange_insert.php?error_msg=".$error_msg);
			exit;
		}
...
//--------------------------------------------------------------------------------------------------------------------------------------------		
       if((!$validation->blank_select($hra))){
		...
		if($validation->blank_select($hill_allowance)==FALSE || $validation->pattern_number($hill_allowance)==FALSE || $validation->float_val($hill_allowance)==FALSE)
		{
			$error_msg=$cryptography->encode('<div class="alert alert-danger" style="text-align:center;"><strong>Invalid Hill ALLOW.</strong></div>',3);
			header('Location: '. $config['base_url'] . "page/intra_ps/state/paychange/paychange_insert.php?error_msg=".$error_msg);
			exit;
	 ...
	 $db=new database();
		...
		$insert=$db->insert("		
				...
				if($insert){
											...
											UPDATE psemp_admin_paychange SET paychange_todate='".$todate."' WHERE flag='t' 
...
//========================new setup======= 18300=======================================================================================		
		...
		header('Location: '. $config['base_url'] . "page/intra_ps/state/paychange/paychange_insert.php?msg=".$msg);
		exit;
