...
{	
	$_SESSION['msg']='<div class="alert alert-danger" style="text-align:center;"><strong>Updation Failed.</strong></div>';
		header('Location:'.$config['base_url'].'page/intra_ps/state/paychange/paychange_order_details.php');
		exit(0);
} 
...
if($_REQUEST['flag']=='activate')
{
	...
	$update_paychange=$db->update("UPDATE psemp_admin_paychange
   									SET flag='TRUE'
 									WHERE paychange_id_pk='".$paychange_id_pk."'");
									
	$update_paychange_de=$db->update("UPDATE psemp_admin_paychange
   									SET flag='FALSE'
 									WHERE paychange_id_pk<>'".$paychange_id_pk."'");	
