...
<?php 
	...
	if(!isset($_POST['paychange_order_submit'])){
		header('Location: '. $config['base_url'] . "page/intra_ps/state/paychange/paychange_order_insert.php");
		exit;
...
($_FILES['order_file']['size'] <= 41943040)) {*/
    ...
    /*}
	 ...
	 $search_db=$db->fetch_table('select paychange_id_pk from psemp_admin_paychange');
	 if(count($search_db)>0){
		...
		$insert=$db->insert("		
		 INSERT INTO psemp_admin_paychange
									(entrydate,
									paychange_fromdate,
									paychange_ip,
									 order_file_name,
		...
		header('Location: '. $config['base_url'] . "page/intra_ps/state/paychange/paychange_order_insert.php?msg=".$msg);
		exit;
	}
?>
