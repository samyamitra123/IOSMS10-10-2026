...
<?php
		...
		$db = new database();
		/*echo "select pd.ptax_id_pk,pd.mn_amount,pd.mx_amount,pd.ptax_amount,pd.ptax_order_id_fk from psemp_ptax_deduction as pd 
													INNER JOIN psemp_ptax_order_file as pf ON pd.ptax_order_id_fk=pf.ptax_orderfile_pk  WHERE pf.ptax_orderfile_pk='".$ptax_orderfile_pk."' ORDER BY pd.ptax_id_pk";exit;*/
					
					$ptax_deduction = $db->fetch_table("select pd.ptax_id_pk,pd.mn_amount,pd.mx_amount,pd.ptax_amount,pd.ptax_order_id_fk,pf.active_status from psemp_ptax_deduction as pd 
													INNER JOIN psemp_ptax_order_file as pf ON pd.ptax_order_id_fk=pf.ptax_orderfile_pk  WHERE pf.ptax_orderfile_pk='".$ptax_orderfile_pk."' ORDER BY pd.ptax_id_pk
												");
												...
												if(count($ptax_deduction)<1)
												{
													...
													INNER JOIN psemp_ptax_order_file as pf ON pd.ptax_order_id_fk=pf.ptax_orderfile_pk  WHERE pf.active_status=0 ORDER BY pd.ptax_id_pk
												");
...
$enc_token=md5('369'.$time_token);
					...
					$check_date=$db->fetch_table("SELECT ptax_orderfile_pk,activation_date,active_status AS max_date FROM psemp_ptax_order_file WHERE ptax_orderfile_pk='$ptax_orderfile_pk' AND active_status='1'");
