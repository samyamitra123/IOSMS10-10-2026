...
<?php
	...
	if(!isset($_POST['ptax_deduction_submit'])){
		header('Location: '. $config['base_url'] . "page/intra_ps/state/ptax/ptax_deduction_insert.php");
		exit;
...
//========================non ssa school======18101=======================================================================================		
	...
	$ptax_order = $db->fetch_table("select ptax_orderfile_pk from psemp_ptax_order_file 
												...
												{
	$ptax_order_id_fk= $db->fetch_table("SELECT ptax_id_pk FROM psemp_ptax_deduction WHERE ptax_order_id_fk='".$ptax_order[0]['ptax_orderfile_pk']."' ORDER BY ptax_id_pk");	
												}
...
while($p!=0)
		...
		$insert=$db->insert("		
		 INSERT INTO psemp_ptax_deduction
									(ptax_order_id_fk,
									 mn_amount,
									 mx_amount,
									 ptax_amount
...
$msg=$cryptography->encode('<div class="alert alert-success" style="text-align:center;"><strong>Data Successfully Entered.</strong></div>',3);
		...
		{ 
				...
				$insert=$db->update("		
									 UPDATE psemp_ptax_deduction SET 
...
//========================new setup======= 18300=======================================================================================		
		...
		header('Location: '. $config['base_url'] . "page/intra_ps/state/ptax/ptax_deduction_insert.php?msg=".$msg);
		exit;
?>
