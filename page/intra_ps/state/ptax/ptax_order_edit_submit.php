...
<?php 
	...
	if(!isset($_POST['ptax_order_submit'])){
		...
		if(!$validator->blank_select($activation_date) ){
			...
			header('Location: '. $config['base_url'] . "page/intra_ps/state/ptax/ptax_order_edit.php?order_id=".$cryptography->encode($ptax_id_pk,3)."&msg=".$cryptography->encode($error_msg1,3));
		    exit;
		...
		$insert=$db->insert("		
		  UPDATE psemp_ptax_order_file
				...
				$msg='<div class="alert alert-success" style="text-align:center;"><strong>Data Successfully Updated. </strong></div>';
 					//include 'ptax_order_form.php';
					header('Location: '. $config['base_url'] . "page/intra_ps/state/ptax/ptax_order_edit.php?order_id=".$cryptography->encode($ptax_id_pk,3)."&msg=".$cryptography->encode($msg,3));
			        exit;
				}
				else
				{
