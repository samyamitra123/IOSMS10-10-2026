...
$ps_id=$crypto->decode($_GET['ps_id'],4);
$enc_ps=$_GET['ps_id'];


$emp_list_fetch=$db->fetch_table("
									SELECT 
										distinct(emp_first_name),
										emp_second_name,
										emp_last_name,
										transfer_date,
										emp_id_fk,
									...
									WHERE
										ps_id_fk='".$ps_id."' AND transfer_emp_status not in (2)
								");
...
{
	var ps=<?php echo '"'.$enc_ps.'"'; ?>;
	var emp= ''+$('#lpc_emp_id').val()+'';
	window.location.href="pdf_lpc.php?emp_id="+emp+"&ps_id="+ps;
