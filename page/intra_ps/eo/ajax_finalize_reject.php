...
//	$rand_code_nw=check_rand_code();
	...
	if(!empty($check_id)){
		...
		$empid_const=$empid.$inc;
	}
	else{
		$empid_const=str_pad($empid,12,'0',STR_PAD_LEFT) ;
	}*/
	...
	$status_insert = $db->insert("INSERT into psemp_employee_profile_update_status(
															ip_address,
															date,
															ps_id_fk,
															emp_id_fk,
...
$_SESSION['msg']='<p class="alert alert-success" style="text-align:center"><strong>Employee Has Been Successfully Approved.</strong></p>';
	?>
    <script>
    window.location.href="<?=$config['base_url'] ?>page/intra_ps/eo/employee_list.php?msg=<?=$cryptoGraph->encode($msg,4);?>";
    </script>
	<? } else{ 
...
if($_REQUEST['flag']=='reject'){
	...
	if($finalize_update){ 
		...
		$_SESSION['msg']='<p class="alert alert-danger" style="text-align:center"><strong>Employee Has Been Successfully Rejected.</strong></p>';
		?>
        <script>
    window.location.href="<?=$config['base_url'] ?>page/intra_ps/eo/employee_list.php?msg=<?=$cryptoGraph->encode($msg,4);?>";
    </script>
