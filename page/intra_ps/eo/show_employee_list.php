...
$arr=$db->fetch_table("
							...
							FROM 
								prd_employee_master 
							WHERE 
								emp_status IN('1','2','9') AND ps_id_fk='".$_SESSION['location']['ps_id']."'
									");
...
$(document).ready(function(e) {
	...
	$('.stop_salary').click(function(e) {
		...
		var emp_id=$(this).val();
					...
					$.post('<?= $config['base_url'] ?>page/intra_ps/eo/ajax_reason_stop_sal.php?id='+emp_id, function(data){
						//alert(data);
	...
	$('#confirm_delete').click(function(e) {
		...
		$.post('<?= $config['base_url'] ?>page/intra_ps/eo/stop_start_suspend_submit.php?id='+arr[0]+'&flag=delete'+'&ps_id_fk='+arr[1], function(data){
						//alert(data);
...
function confirm_start_emp()
{
	...
	$.post('<?= $config['base_url'] ?>page/intra_ps/eo/stop_start_suspend_submit.php?id='+start_emp_id+'&flag=start'+'&gp_id_fk='+start_gp_id, function(data){
...
<? $cnt=1; if(count($arr)){ foreach($arr as $item){
 ...
 $arr_transfer=$db->fetch_table("select transfer_id_pk from prd_employee_transfer where transfer_emp_status='0' AND ps_id_fk='".$_SESSION['location']['ps_id']."' and emp_id_fk='".$item['emp_id_pk']."'");
?>
<tr>
<td><?= $cnt;?></td>
...
<div class="btn-group" role="group">
  ...
  <button type="button" class="btn btn-success start_salary" id="start_sal<?=$item['emp_id_pk']?>" value="<?=$cryptoGraph->encode($item['emp_id_pk'],4).'&'.$cryptoGraph->encode($item['ps_id_fk'],4).'&'.$item['emp_id_pk']?>" disabled="disabled">Start</button>
<?php } else { ?>
  <button type="button" class="btn btn-success start_salary" id="start_sal<?=$item['emp_id_pk']?>" value="<?=$cryptoGraph->encode($item['emp_id_pk'],4).'&'.$cryptoGraph->encode($item['ps_id_fk'],4).'&'.$item['emp_id_pk']?>">Start</button>
  <?php } 
  if ($item['emp_status']=='2' || count($arr_transfer)!='0' ) { ?>
  <button type="button" class="btn btn-danger stop_salary" id="stop_sal<?=$item['emp_id_pk']?>" data-bs-toggle="modal"  value="<?=$cryptoGraph->encode($item['emp_id_pk'],4)?>" disabled="disabled" style="display: block;">Stop</button>
  <?php } else {?>
  <button type="button" class="btn btn-danger stop_salary" id="stop_sal<?=$item['emp_id_pk']?>" data-bs-toggle="modal"  value="<?=$cryptoGraph->encode($item['emp_id_pk'],4)?>" style="display: block;" >Stop</button>
  <?php } 
  if ($item['emp_status']=='9'  || count($arr_transfer)!='0') {?>
  <button type="button" class="btn btn-info suspend_salary" id="suspend_sal<?=$item['emp_id_pk']?>" value="<?=$cryptoGraph->encode($item['emp_id_pk'],4).'&'.$cryptoGraph->encode($item['ps_id_fk'],4).'&'.$item['emp_id_pk']?>" disabled="disabled">Suspend</button>
  <?php } else { ?>
  <button type="button" class="btn btn-info suspend_salary" id="suspend_sal<?=$item['emp_id_pk']?>" value="<?=$cryptoGraph->encode($item['emp_id_pk'],4).'&'.$cryptoGraph->encode($item['ps_id_fk'],4).'&'.$item['emp_id_pk']?>">Suspend</button>
  <?php } 
  
//  $db=new database();
//$status_chk=$db->fetch_table("select status_flag from prd_employee_salary_save where emp_id_fk='".$item['emp_id_pk']."' AND salary_monthyear='".date('Ym')."' and delete_status='1'");
//
//$status_flag=$status_chk[0]['status_flag'];  
// 
// if( !$status_chk || $status_flag==2 || $status_flag==3)
// {
  ?>
<!--  <button type="button" class="btn btn-warning" id="delete_sal<?=$item['emp_id_pk']?>" value="<?=$cryptoGraph->encode($item['emp_id_pk'],4).'&'.$cryptoGraph->encode($item['ps_id_fk'],4).'&'.$item['emp_id_pk']?>" disabled="disabled">Delete</button>-->
  <?php// } else { ?>
<!--   <button type="button" class="btn btn-warning" id="delete_sal<?=$item['emp_id_pk']?>" value="<?=$cryptoGraph->encode($item['emp_id_pk'],4).'&'.$cryptoGraph->encode($item['ps_id_fk'],4).'&'.$item['emp_id_pk']?>" onClick="del_salary(this.value)">Delete</button>-->
   <?php //} ?>
</div>
</td>
</tr>
...
<div class="modal fade bs-example-modal-sm" id="myModal1" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="min-height:1000px;">
  <div class="modal-dialog modal-lg" style="height:">
    <div class="modal-content" style="width: max-content; margin-left: -22%;">
      <div class="modal-header">
