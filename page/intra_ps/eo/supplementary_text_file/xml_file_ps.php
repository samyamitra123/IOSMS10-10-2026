...
$psemp_ps_profile=$db->fetch_table("select ddo_code,treasury_code from psemp_ps_profile where ps_id_fk='".$_SESSION['location']['ps_id']."'");
		...
		//echo $ddo_code_name;exit;
	 	...
	 	$bill_det=$db->fetch_table("select bill_no,bill_entry_time,salary_monthyear from prd_block_bill_details where ps_id_fk='".$_SESSION['location']['ps_id']."' and bill_no='".$bill."' and salary_monthyear= '".$yemo."' AND requisition_type='".$requisition_type."'");
...
$teacher_sql="select distinct(emp.emp_id_pk),
	...
	where
		...
		AND save.ps_id_fk='".$_SESSION['location']['ps_id']."'
...
$xml =  new DOMDocument("1.0","UTF-8");
		...
		foreach ($sql_teacher as $key){
			...
			if($key['net'] > 0){
				...
				$ifsccd = $xml->createElement('ifscode',$key['bank_ifsc']);
		...
		//	MONTH(10)YEAR(2015)TR_TYPE(31)DDO_CODE(NPCEDS002)ANY_SERIAL_NO(132)CREATED_DATE(07102015)_Benf(1)
			...
			$filename = substr($bill_det[0]['salary_monthyear'],4).substr($bill_det[0]['salary_monthyear'],0,4).$treassury_code.$ddo_code.$billdate."_Benf.xml";
