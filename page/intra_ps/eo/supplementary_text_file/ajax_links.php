...
?>
	<div class="link_p">
		<a href="<?php echo $config['base_url'] ?>page/intra_prd/block/supplementary_text_file/personal.php?mo=<?php echo $_POST['bill_report_month']; ?>&ye=<?php echo $_POST['bill_report_year']; ?>&bill=<?php echo $_POST['bill']; ?>">Generate Personal Details</a>
	</div>
	<div class="link_p">
		<a href="<?php echo $config['base_url'] ?>page/intra_prd/block/supplementary_text_file/salarybill.php?mo=<?php echo $_POST['bill_report_month']; ?>&ye=<?php echo $_POST['bill_report_year']; ?>&bill=<?php echo $_POST['bill']; ?>">Generate Bill Summery</a>
	</div>
<?php }
else {*/
$db = new database();
...
$requisition_type=$requisition[0]['code'];
        ...
        /*echo "SELECT count(*) FROM prd_block_bill_details 
		...
		//echo "<pre>";
		//print_r($check_dpsc_bill);
		//echo $check_dpsc_bill[0]['count'];
		...
		$prd_monthly_salary_archive_final = $db->insert("
				INSERT INTO prd_monthly_salary_archive_final
					...
					WHERE ps_id_fk ='".$_SESSION['location']['ps_id']."'
					AND salary_monthyear ='".$monthyear."'
					AND status_flag = 3 and delete_status='1' and is_saved='1' AND requisition_type='".$requisition_type."'
		");
		 ...
		 $prd_monthly_salary_archive_nonfinal=$db->insert("INSERT INTO prd_monthly_salary_archive_nonfinal(
					...
					SELECT 
						...
						hra_deduction,
						part_day 
	    from prd_employee_salary_save where ps_id_fk ='".$_SESSION['location']['ps_id']."' AND salary_monthyear='".date('Ym')."' AND requisition_type='".$requisition_type."' AND status_flag <> 3 ");
		
	/*	$query_update=$db->update("UPDATE prd_employee_salary_save set
									status_flag=4
								 WHERE
		...
		*/
		   ...
		   $delete_save=$db->delete("DELETE FROM prd_employee_salary_save
 			WHERE salary_monthyear='".date("Ym", strtotime("-1 months"))."'
...
mail_function($mail_body,$email,$emp_sub);
	...
	*/
		...
		<a class="btn btn-info btn-sm" href="<?php echo $config['base_url'] ?>page/intra_ps/eo/supplementary_text_file/personal.php?mo=<?php echo $_POST['bill_report_month']; ?>&ye=<?php echo $_POST['bill_report_year']; ?>&bill=<?php echo $_POST['bill']; ?>"><i class="fa fa-file-text"></i> Generate Personal Details</a>
	</div>
    </div>
    <br/>-->
    <div class="form-group">
    <div class="col-sm-offset-5 col-sm-7">
		<a class="btn btn-info btn-sm" href="<?php echo $config['base_url'] ?>page/intra_ps/eo/supplementary_text_file/salarybill.php?mo=<?php echo $_POST['bill_report_month']; ?>&ye=<?php echo $_POST['bill_report_year']; ?>&bill=<?php echo $_POST['bill']; ?>" style="width: 29.5%;text-align: left;"><i class="fa fa-file-text"></i> Generate Bill Summery</a>
	</div>
    </div>
    <br />
    <div class="form-group">
    <div class="col-sm-offset-5 col-sm-7">
		<a class="btn btn-info btn-sm" href="<?php echo $config['base_url'] ?>page/intra_ps/eo/supplementary_text_file/xml_file.php?mo=<?php echo $_POST['bill_report_month']; ?>&ye=<?php echo $_POST['bill_report_year']; ?>&bill=<?php echo $_POST['bill']; ?>" style="width: 29.5%;text-align: left;"><i class="fa fa-file-text"></i> Generate ECS</a>
	</div>
    </div>
	<?php
		...
		<?php	}
			...
			pg_query("rollback");
			//echo "Monthly not salary inserted<br />";
			?>
   ...
   <?php
	...
	?>
<!--		<div class="alert alert-danger"style="width: 23%;margin-left: 43%;text-align: center;"><strong>Duplicate Bill No. Found</strong></div>
-->
