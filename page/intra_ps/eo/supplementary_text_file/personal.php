...
/*	$dpsc_sql = $db->fetch_table("Select treasury_block_code,ddo_code from prd_dise_admin where block_code = '".$_SESSION['user_info']['stake_user']."'");
		...
		$arr4 = $db->fetch_table("Select ddo_code from psemp_ps_profile where ps_id_fk='".$_SESSION['location']['ps_id']."'");
...
$arr=$db->fetch_table("
					...
					LEFT JOIN
							prd_employee_salary_save save 
					ON 
							save.emp_id_fk=emp.emp_id_pk
					
					WHERE ps.ps_id_pk='".$_SESSION['location']['ps_id']."' AND (emp.emp_status='1' OR emp.emp_status='9') AND save.status_flag='3'
					AND save.salary_monthyear = '".$yemo."' AND save.delete_status='1' AND requisition_type='".$requisition_type."' 
