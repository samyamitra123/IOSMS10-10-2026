...
$db = new database();	
	...
	$sql_query=$db->fetch_table("select bill_no,bill_entry_time,salary_monthyear from prd_block_bill_details where ps_id_fk='".$_SESSION['location']['ps_id']."' and bill_no='".$bill_no."' and salary_monthyear= '".$month_year."' AND requisition_type='".$requisition_type."'");
...
left join prd_employee_salary_save sal ON (sal.emp_id_fk=emp.emp_id_pk and ps.ps_id_pk=sal.ps_id_fk)
	where 
...
//MONTH(10)YEAR(2015)TR_TYPE(31)DDO_CODE(NPCEDS002)ANY_SERIAL_NO(132)CREATED_DATE(07102015)_BillSummary
   ...
   $filename=substr($sql_query[0]['salary_monthyear'],4).substr($sql_query[0]['salary_monthyear'],0,4).$treasury_code.$dpsc_code.$billdate."_BillSummary.txt";
