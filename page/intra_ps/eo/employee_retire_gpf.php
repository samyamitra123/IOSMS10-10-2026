...
$Query = "select  * from prd_monthly_salary_archive_final as pmsaf 
                        ...
                        WHERE pmsaf.ps_id_fk='".$ps_id."' AND pmsaf.salary_monthyear='202307'";
$Query = "select  * from prd_monthly_salary_archive_final 
                        WHERE ps_id_fk='".$ps_id."' AND salary_monthyear='202307'";                        
...
$Query = "select  
                 ...
                 pgrm.full_response,
                 pgrm.request
          from prd_employee_master_ngipf as pem
          LEFT JOIN prd_gpf_request_master as pgrm on pem.emp_id_const= pgrm.emp_id_const
          where emp_termination_date > '2023-04-01' AND emp_status in('2') AND ps_id_fk='".$ps_id."' 
...
<h1 class="heading">RETIRE EMPLOYEE LIST FOR GPF (<?php echo $ps_id; ?>) as on 01/04/2023</h1>
...
<? $cnt=1; if(count($arr)){ foreach($arr as $item)
{
	...
	$action = ($request->req->action == 'M')?'Modification ':'Insert ';
	$action = ($request->req->action == 'C')?'Modification ':$action;
...
<td><?= $status; ?> <?php echo $item['status']; ?></td>
<td>
	  <?php 
	      echo $action;
        if($item['status']==0)
