...
$(document).ready(function() {
	...
	$('#submit-text').click(function(){
			...
			else
			{
				//alert(11);
				//$('#ifms_form').show();
				$.post('<?php echo $config['base_url'] ?>page/intra_ps/eo/supplementary_text_file/ajax_links.php',$(this).closest("form").serialize(), function(data){
	...
	$('#bill_report_month , #bill_report_year').change(function(){
		...
		//alert(111);
				...
				var month = $('#bill_report_month').val();
				 $.ajax({
				url:'<?php echo $config['base_url'] ?>page/intra_ps/eo/supplementary_text_file/find_bill.php',
				dataType:"json",
            	type:'GET',
				data:{
					ye:year,
     ...
     <select name="bill_report_month" id="bill_report_month" class="form-control" style="margin-left: -15%;">
		<?php
							...
							for ($m=1; $m<=12; $m++) {
								...
								?>
						<option value ="<?php echo $crypto->encode($num, 4) ?>"<?php if(date("m") == $num){ echo " selected";}else { echo "disabled"; } ?> ><?php echo $month; ?></option>
								<?php
							     }
							?>				
     </select>
     ...
     <select name="bill_report_year" id="bill_report_year" class="form-control" >
		<?php
							...
							for ($i=$cryear; $i > $cryear-$le; $i--) { 
								?>
								<option value="<?php echo $crypto->encode($i, 4) ?>"<?php if(date("Y") == $i){ echo " selected";}else { echo "disabled"; } ?>><?php echo $i; ?></option>
								<?php
							}
