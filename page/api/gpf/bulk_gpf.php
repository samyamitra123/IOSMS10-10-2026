...
require_once('ngipfFunction.php');
  ...
  $Query = "SELECT * FROM prd_ngipf_request_cron WHERE send_status = 0 LIMIT 10 offset 0";
  $EmployeePending = $db->fetch_table($Query);
  $Query = "SELECT count(*) as pendingrequest FROM prd_ngipf_request_cron WHERE send_status = 0";
  $EmployeePendingCount = $db->fetch_table($Query);
  //print_r($EmployeePendingCount); exit; 
  $emp = array();
  
  foreach($EmployeePending as $empData)
  {
  	  $emp[] = $empData['emp_id_fk'];
  	  //echo '<p>'.$ps['ps_name'].' PS-CODE: '.$ps['ps_code'].' | PS IOSMS ID:'.$ps['ps_id_pk'].'</p>';
  }
  //print_r($emp); exit; 
  // $Query = "SELECT * from prd_employee_master 
  //           WHERE emp_id_pk IN (".implode(',',$emp).") AND ropa_status IN (1,2) AND emp_cosolidated_pay=0
	//             ORDER BY emp_id_pk ASC";
  $Query = "SELECT * from prd_employee_master 
            WHERE emp_id_pk IN (".implode(',',$emp).") AND emp_cosolidated_pay=0
	            ORDER BY emp_id_pk ASC";	            
  $EmployeeData = $db->fetch_table($Query);
  //print_r($EmployeeData); exit;
  $countofEmployeeDate = count($EmployeeData);
  echo "Count Of Data: ".$countofEmployeeDate.'- Total Left: '.$EmployeePendingCount[0]['pendingrequest']; 
  $status="I";
  foreach($EmployeeData as $employee_dtls)
     {
					  

if(count($employee_dtls) < 1)
{
	  $Query = "UPDATE prd_ngipf_request_cron SET send_status = 9 WHERE emp_id_fk='".$employee_dtls['emp_id_pk']."'";
  	$db->update($Query);
	  continue;
}
...
if(count($prevEmployeeDetails) > 0 && $debug == 0)
  {
      ...
      */
     if($prevEmployeeDetails[0]['pfaccno'] != "")
       {
				  $Query = "UPDATE prd_ngipf_request_cron SET send_status = 9 WHERE emp_id_fk='".$employee_dtls['emp_id_pk']."'";
			  	$db->update($Query);  
			  	continue;     	
...
$emp_id_const = $employee_dtls['emp_id_const'];
$emp_id_fk = $employee_dtls['emp_id_pk'];
...
$emp_present_memo_date = ($emp_present_memo_date = '//')?"":$emp_present_memo_date;
//print($emp_present_memo_date); exit;
...
?>
<?php if($countofEmployeeDate > 0) { ?>
<script type="text/javascript">
	setTimeout(function(){
     window.location.reload();
	},10000);
</script>
<?php } else { echo "Your Job Finished! Close"; } ?>