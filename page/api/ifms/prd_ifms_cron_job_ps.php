...
///////////////////////////////////////////////////////////////////////  PS START ///////////////////////////////////////////////////////////////////////
//echo $config['sftp_ip_ps'].$config['sftp_user_name_ps'].$config['sftp_user_password_ps'];die;
$sftp_ps=sftp_login($config['sftp_ip_ps'],$config['sftp_user_name_ps'],$config['sftp_user_password_ps']);
	if($sftp_ps!=0)
	{
		echo $bill_details_fetch_ps=$db->fetch_table(" SELECT 
												bill.block_bill_pk,
						...
						if($sftp_ps->get($remote_directory_003.$wrong_data_file_name, $local_directory_003.$wrong_data_file_name))
						{
							...
							for($j=0;$j<count($emp_id_fetch);$j++)
							{
								foreach($emp_wrdt_failure_arr as $key3=>$value3)
								{
									...
									{
										 $ins_sql.="('".$bill_details_fetch_ps[$i]['block_bill_pk']."','".$bill_details_fetch_ps[$i]['sftp_benf_id_pk']."','".$emp_id_fetch[$j]['emp_id_pk']."','".$value3[0]."','".$value3[1]."','".$value3[2]."','".$value3[3]."','".$value3[4]."','".$value3[5]."','".$key3."','".$value3[6]."','".$value3[7]."','".$value3[8]."','','9','now();','".$_SERVER['REMOTE_ADDR']."','556','1')"; 
