...
$stack=$_POST["stack"]; 
	...
	$month1=$crypto->decode($_POST["month"],4);
	$year1=$crypto->decode($_POST["year"],4);
	$stack1=$crypto->decode($_POST["stack"],4);
	$current_monthyr=$year1.$month1; 
//var_dump($month1); die;
...
if($stack1 =='PS')
{
	...
	if($sftp_ps!=0)
	{
		...
		$bill_details_fetch_ps=$db->fetch_table(" SELECT 
											...
											INNER JOIN prd_sftp_benf_upload_response sftp
											ON bill.block_bill_pk=sftp.bill_id_fk
											WHERE bill.salary_monthyear in ('".$current_monthyr."') AND bill.status='1' AND ps_id_fk!='0'
											AND sftp.active_status='1' AND sftp.sftp_benf_sending_status='4' 
											AND (sftp.sftp_benf_response_status not in ('6','7','8') or sftp.sftp_benf_response_status is null)");
...
if($stack1 =='ZP')
{
	...
	if($sftp_zp!=0)
	{
		...
		$bill_details_fetch_zp=$db->fetch_table(" SELECT 
											...
											INNER JOIN prd_sftp_benf_upload_response sftp
											ON bill.block_bill_pk=sftp.bill_id_fk
											WHERE bill.salary_monthyear in ('".$current_monthyr."') AND bill.status='1' AND zp_id_fk!='0'
											AND sftp.active_status='1' AND sftp.sftp_benf_sending_status='4' 
											AND (sftp.sftp_benf_response_status not in ('6','7','8') or sftp.sftp_benf_response_status is null)");
...
<script>
	...
	window.location.href="<?=$config['base_url'] ?>page/all_moduls/webmaster/ifms_block_view.php?stack="+stack+"&month="+month+"&year="+year+"&req_type="+req_type;
