...
if($ifms_track_insart)
{
	...
	$trsc = $xml->createElement('TREASURY_CODE','BUA');
	$trsc = $container->appendChild($trsc);
	
	$ddc = $xml->createElement('DDO_CODE','BUAPRA011');
	...
	$hoa = $xml->createElement('HEAD_OF_ACCOUNT','182047001030010107V');
	$hoa = $container->appendChild($hoa);
	
	$ga = $xml->createElement('GROSS_AMOUNT','2000000');
	$ga = $container->appendChild($ga);
	
	$na = $xml->createElement('NET_AMOUNT','2000000');
	$na = $container->appendChild($na);
	
	$billno = $xml->createElement('BILL_NO','656');
	$billno = $container->appendChild($billno);
	
	$billdate = $xml->createElement('BILL_DATE','19/06/2018');
