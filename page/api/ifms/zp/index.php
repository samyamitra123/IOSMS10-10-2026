...
$bill_file_existance_checking=$db->fetch_table("SELECT response_code FROM prd_block_bill_details WHERE drn_number='".$drn_number."'");
...
else
{
	...
	//$fest_adv_head='0000000000000000000';
	//%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	if($bill_type == '1001')
{
	...
	foreach($teacher_dtls as $teacher_row)
	{
		...
		$allowance=($allowance)+ ($teacher_row['allowance']);
...
else if($bill_type == '1003')
{
	...
	{
	   $Query = "select bill_no,bill_entry_time,salary_monthyear, block_bill_pk, zp_emp_type from prd_block_bill_details where zp_id_fk='" . $_SESSION['location']['district_id'] . "' and salary_monthyear= '". $monthyear."' AND requisition_type='".$bill_type."' AND status='1' AND drn_number='".$drn_number."'";
	   $bill_det=$db->fetch_table($Query);
	}
	
	else if($bill_type == '1004' || $bill_type == '1005' ||  $bill_type == '1003')
	{
		$bill_det=$db->fetch_table("select bill_no,bill_entry_time,salary_monthyear, block_bill_pk, zp_emp_type from prd_block_bill_details where zp_id_fk='" . $_SESSION['location']['district_id'] . "' and salary_monthyear= '". $monthyear."' AND requisition_type='".$bill_type."' AND status='1' AND drn_number='".$drn_number."' ");
	}
	...
	{
		echo 'Bill Number Missing.'.$billno;
		exit;
	}
	...
	if($ifms_track_insart)
	{
	    ...
	    $ngipfSal = ($bill_det[0]['zp_emp_type'] == 366)?'N':$ngipfSal;
		...
		$bthoa = $xml->createElement('BT_HOA',$fest_adv_head);
		$bthoa = $bynsfer->appendChild($bthoa);
		
		$bt_effect = $xml->createElement('BT_EFFECT','T');
		$bt_effect = $bynsfer->appendChild($bt_effect);
		
		$bammount = $xml->createElement('BT_AMOUNT',$festival_loan);
		$bammount = $bynsfer->appendChild($bammount);*/
...
/*if($_SESSION['location']['district_id']=='21')
		...
		print_r($data); die;
		 ...
		 //$get_data = callAPI('GET', 'http://192.168.1.59:12:90/ifms_testing/data.php', $data);
