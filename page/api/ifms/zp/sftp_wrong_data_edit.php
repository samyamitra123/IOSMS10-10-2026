...
<script>
	function edit_individual(k)
	{
		...
		$.post('<?= $config['base_url'] ?>page/api/ifms/zp/ajax_bank_view.php?id='+k, function(data){
		  $(".empshow").html(data);
		  });
	...
	function finalize_all_emp()
	{
		...
		$.post('<?= $config['base_url'] ?>page/api/ifms/zp/ajax_finz_all.php?all_id='+all_id+'&benf_id='+benf_id, function(data){
			if(data=='1')
			{
