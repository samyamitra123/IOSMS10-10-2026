...
$bill_det=$db->fetch_table("SELECT block_bill_pk,bill_no,bill_entry_time,salary_monthyear FROM prd_block_bill_details WHERE zp_id_fk='".$_SESSION['location']['district_id']."' and drn_number='".$drn_number."' and salary_monthyear= '".$monthyear."' AND requisition_type='".$bill_type."' AND status='1'  AND bill_serial_no='".$bill_serial_no."'");
	...
	$path_sign = $_SERVER['DOCUMENT_ROOT'].'/readwrite/xml_file/ifmszp/sign_file/';
...
if(isset($name)){
 ...
 $filename_zip=$_SERVER['DOCUMENT_ROOT'].'/readwrite/xml_file/ifmszp/ePayment_Files_006/'.$new_name.".zip";

  if ($zip->open($filename_zip, ZipArchive::CREATE)!==TRUE) {
    exit("cannot open <$filename_zip>\n");
  }
//$file='../ACKTISTTS0010062506201801007796.xml';
  $dir = $_SERVER['DOCUMENT_ROOT'].'/readwrite/xml_file/ifmszp/ePayment_Files_006/';
  $path_sign = $_SERVER['DOCUMENT_ROOT'].'/readwrite/xml_file/ifmszp/sign_file/';
