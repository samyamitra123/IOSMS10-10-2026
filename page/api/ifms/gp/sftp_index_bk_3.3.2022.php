...
{*/
	...
	$cfile_xm_file_name = realpath($path.$name); 
	
	$hash_xm_file=hash_file("sha256",$cfile_xm_file_name); 
	
	$path_sign = $_SERVER['DOCUMENT_ROOT'].'/readwrite/xml_file/ifmsgp/sign_file/';
	
	//$db = new database();
	
	//$key=$db->fetch_table("SELECT private_key,public_key from prd_ssl_key");
	
	//$privKey=$key[0]['private_key']; 
	//$pubKey=$key[0]['public_key']; 
	
	//$private_encription=openssl_private_encrypt($hash_xm_file, $encrypted, openssl_pkey_get_private($privKey, "api@123")); 
	//$encrypted_has_val=$encrypted;
...
//file_put_contents('private_key.pem', $hash_xm_file);
...
$username = 'gen006';
//$password_hash1 = 'Api@123';

/*$password_hash1 = 'gn@lldp+';
$password_hash2 = 'Api@123';*/
//$password_hash = 'gn@lldp+Api@123';
$password_hash = 'gengraWb!fm506';

$password = hash("sha512", $password_hash);
//$password = hash("sha512", $password_hash1.$password_hash2);
$fileType = 'BENF';
$flag='1';
 $zipped_name= $new_name.".zip"; 

$request_id=request_id_generation($party_code);
 //$request_id='4444444444444444';
$party_code_api='gen006';

/*echo $zipped_name.'--'.$filename.'---'.$local_directory.'--'.$username.'---'.$password.'---'.$fileType.'--'.$flag.'---'.$zipped_name.'---'.$party_code_api.'--'.$request_id; die;*/
...
//var_dump($ben_upload); die;
 ...
 $configData = json_decode($ben_upload, true);
	//echo $configData['status']; die;
	
	
	if($configData['status']=='0' && $configData['status']!='')
	{
...
else if($response_code!=404 && $response_code!='0')
	{
		//echo "Sorry! Beneficiary Upload Fails";
		echo 0;
	}
