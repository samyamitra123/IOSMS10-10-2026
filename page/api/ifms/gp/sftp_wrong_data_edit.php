...
$wrong_employee_data_fetch=$db->fetch_table("  SELECT
												...
												WHERE benf.active_status in ('1','2','3') AND benf.response_from='9' 
												AND emp.gp_id_fk!='0' AND benf.sftp_benf_id_fk='".$benf_id."'
...
<script>
	function edit_individual(k)
	{
	    ...
	    var benf_id=$('#benf_id').val();
	  
		$('#indi_edit').modal('show');
		$.post('<?= $config['base_url'] ?>page/api/ifms/gp/ajax_bank_view.php?id='+k+'&monthyear='+monthyear+'&benf_id='+benf_id, function(data){
		  $(".empshow").html(data);
		  });
	...
	function finalize_all_emp()
	{
		...
		$.post('<?= $config['base_url'] ?>page/api/ifms/gp/ajax_finz_all.php?all_id='+all_id+'&benf_id='+benf_id, function(data){
			if(data=='1')
			{
...
<div id="msg">
    ...
    <input type="hidden" value="<?php echo $monthyear; ?>" id="monthyear">
     <input type="hidden" value="<?php echo $_GET['benf_id']; ?>" id="benf_id">
</div>
