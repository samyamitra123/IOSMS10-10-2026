...
$treasury_dts = $db->fetch_table("select ddo_code,treasury_block_code,operator_code_pf,t_code_pf from 
									prd_dise_admin
									WHERE 
									block_code='" . $_SESSION['user_info']['stake_user'] . "'");
...
$treasury_code=$treasury_dts[0]['treasury_block_code'];
 ...
 $operator_code_pf=$treasury_dts[0]['operator_code_pf']; 
  $t_code_pf=$treasury_dts[0]['t_code_pf']; 
...
$data = array (
	...
	else
	{
                $error_type_fetch=$db->fetch_table("SELECT description FROM prd_ifms_response_code_master WHERE code='".trim($configData['RESPONSE'])."'");
				//echo $error_type_fetch[0]['description'];
				echo "Error".trim($configData['RESPONSE']);		
	}
