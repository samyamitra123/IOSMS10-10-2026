...
<script>
	function edit_individual(k)
	{
		...
		$.post('<?= $config['base_url'] ?>page/api/ifms/gp/ajax_bank_epayment_view.php?id='+k, function(data){
		  $(".empshow").html(data);
		  });
	...
	function finalize_all_emp()
	{
		...
		$.post('<?= $config['base_url'] ?>page/api/ifms/gp/ajax_finz_all_epayment.php?all_id='+all_id+'&benf_id='+benf_id, function(data){
			if(data=='1')
			{
