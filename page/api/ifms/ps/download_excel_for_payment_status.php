...
//-------------------------------------------------------QUERY-----------------------------------------------------------------
	...
	");
			...
			$tch= $db->fetch_table(" 
					SELECT emp.ps_id_fk, emp.emp_first_name,emp.emp_second_name,emp.emp_last_name,emp.emp_desig,emp.emp_id_const,payment_status,payment_date
					FROM
					prd_employee_master emp
					INNER JOIN prd_employee_salary_save sal
					ON sal.emp_id_fk=emp.emp_id_pk AND sal.ps_id_fk=emp.ps_id_fk 
					WHERE emp.ps_id_fk='".$_SESSION['location']['ps_id']."' AND sal.status_flag='3' AND sal.is_saved='1' 
...
<table width="200" border="1">
  <tr>
  <th colspan="7" style="text-align:center;"><br /><br />PAYMENT STATUS EXCEL FOR THE EMPLOYEES UNDER <?= $_SESSION['location']['ps_name']?>  FOR THE MONTH OF <?= GetMonthString(date('m')).", ".date('Y') ?><br /><br /></th>
  </tr>
  <tr>
