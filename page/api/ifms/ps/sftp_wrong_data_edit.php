...
{
	$db= new database();
	$dist_data2 = @$db->fetch_table("select ps_name,ps_code from prd_location_master_panchayat_samiti where ps_id_pk='".$val."' ");
	return $dist_data2[0]['ps_name'];
}
...
$wrong_employee_data_fetch=$db->fetch_table("  SELECT
												...
												WHERE benf.active_status in ('1','2','3') And emp.ps_id_fk='".$_SESSION['location']['ps_id']."' AND emp.ps_id_fk!='0' AND sftp_benf_id_fk='".$benf_id."' AND benf.response_from='9'" );
...
<script>
	function edit_individual(k)
	{
		...
		$.post('<?= $config['base_url'] ?>page/api/ifms/ps/ajax_bank_view.php?id='+k, function(data){
		 $(".empshow").html(data);
		  });
	...
	function finalize_all_emp()
	{
		...
		$.post('<?= $config['base_url'] ?>page/api/ifms/ps/ajax_finz_all.php?all_id='+all_id+'&benf_id='+benf_id, function(data){
			//alert(data);
			if(data=='1')
			{
...
<div id="msg"></div>
    <div class="table-responsive">
        <table width="100%">
			...
			if(count($wrong_employee_data_fetch)>0)
			{
				...
				{ ?>
                    <tr id="">
                    	<td><?php echo fun_ps($key['ps_id_fk']); ?></td>
                        ...
                        <td style="color:#E86769; font-weight:bold;">
						<?php if($key['active_status']=='1') {echo $key['reason'];} else {echo "WRONG DATA UPDATED";} ?>
                        </td>
                        <td><?php if($key['active_status']!='3')
						{ ?>
	...
	{ ?>
        <div class="col-md-6 col-md-offset-5" style="padding-top:20px;">
