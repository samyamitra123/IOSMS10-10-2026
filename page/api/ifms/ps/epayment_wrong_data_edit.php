...
$wrong_employee_data_fetch=$db->fetch_table("  SELECT
												...
												WHERE benf.active_status in ('1','2','3') And emp.ps_id_fk='".$_SESSION['location']['ps_id']."' AND emp.ps_id_fk!='0' AND sftp_benf_id_fk='".$benf_id."' AND benf.response_from='10'" );
...
<script>
	function edit_individual(k)
	{
		...
		$.post('<?= $config['base_url'] ?>page/api/ifms/ps/ajax_bank_epayment_view.php?id='+k, function(data){
		 $(".empshow").html(data);
		  });
	...
	function finalize_all_emp()
	{
		...
		$.post('<?= $config['base_url'] ?>page/api/ifms/ps/ajax_finz_all_epayment.php?all_id='+all_id+'&benf_id='+benf_id, function(data){
			//alert(data);
			if(data=='1')
			{
