...
else
{
	...
	$bill_det=$db->fetch_table("select bill_no,bill_entry_time,salary_monthyear, block_bill_pk, ropa_status from prd_block_bill_details where ps_id_fk='" . $_SESSION['location']['ps_id'] . "' and salary_monthyear= '".$monthyear."' AND requisition_type='".$bill_type."' AND status='1' AND drn_number='".$drn_number."'");
	...
	$ddo_sql = $db->fetch_table("Select ddo_code,treasury_code,pl_code_pf,t_code_pf from psemp_ps_profile where ps_id_fk = '".$_SESSION['location']['ps_id']."'");
	...
	$schm_code='999998';
	$operator_code_pf=$ddo_sql[0]['pl_code_pf']; 
  $t_code_pf=$ddo_sql[0]['t_code_pf']; 
	...
	else if($validator->blank_select($operator_code_pf) == FALSE || $operator_code_pf == '0')
	{
	echo 'Operator Code for PF subscription Missing.';
	exit;
	}
	else if($validator->blank_select($t_code_pf) == FALSE || $t_code_pf == '0')
	{
	echo 'Treasury Code for PF subscription Missing.';
	exit;
	}
