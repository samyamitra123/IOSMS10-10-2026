...
/*if (session_start()) {*/
//	setcookie($sess_name, session_id(), time()+(86400*100), '/', null, null, true);
...
//echo $pass; die;
	...
	*/
	...
	//echo $count;exit;	
if($count>200 && $securitybypass == 0)
{
	//echo "1";exit;	
echo '<div class="alert alert-danger" style="text-align:center;"><strong>Excessive traffic congesstion! Please try login after some time.'.$securitybypass.'</strong></div>';
...
/*$arr = $db->fetch_table("
								...
								$arr = $db->fetch_table("
                                SELECT 
								user_log.stake_level_id_fk,user_log.stake_user,user_log.stake_credential_flag			,user_log.new_stake_password,stk_lvl.stake_level_master_password_new
								FROM 
								prd_stack_user_login AS user_log 
								INNER JOIN prd_stake_level AS stk_lvl ON 
								user_log.stake_level_id_fk=stk_lvl.stake_level_id_pk
...
//}
	...
	$fetch_password=$arr[0]['new_stake_password']; 
	$stake_fetch_password=$arr[0]['stake_level_master_password_new']; 
	$salt1='wbprd!@#123';
	//$salt='wbprd!@#123';
	$salt=hash("sha256",$salt1);
	
	$check_password= hash("sha256", $fetch_password.$salt); 
	$stake_check_password= hash("sha256", $stake_fetch_password.$salt); 
	//echo $pass.'---'.$check_password; die;
	
        
		
			if(($pass==$check_password) ||($pass==$stake_check_password))
			{
				//echo "4";exit;	
...
function prd_get_block($state_code, $district_code, $block_code){
	...
	$arr = $db->fetch_table("
                SELECT prd_location_master_block.block_code,
                        prd_location_master_block.ngipf_status,
                        prd_location_master_block.prev_year_sal,
                        prd_location_master_block.salary_status,
                        prd_location_master_block.ngipf_sal,
        ...
        if(count($arr)){
            $returnarray = array(
                ...
                'district_name'     => $arr[0]['district_name'],
                'block_name'        => $arr[0]['block_name'],
                'ngipf_status'      => $arr[0]['ngipf_status'],
                'salary_status'     => $arr[0]['salary_status'],
                'prev_year_sal'     => $arr[0]['prev_year_sal'],
                'ngipf_sal'         => $arr[0]['ngipf_sal'],
...
function prd_get_ps($state_code, $district_code, $block_code){
	require_once '../includes/library/database.class.php';
        ...
        $arr = $db->fetch_table("
                ...
                prd_location_master_panchayat_samiti.ngipf_status,
                prd_location_master_panchayat_samiti.salary_status,
                prd_location_master_panchayat_samiti.prev_year_sal,
                prd_location_master_panchayat_samiti.ngipf_sal,
...
prd_location_master_district.ngipf_status,               
prd_location_master_district.district_name,
prd_location_master_district.salary_status,
prd_location_master_district.district_id_pk,
prd_location_master_district.ngipf_sal,
...
global $securitybypass;
$securitybypass = ($password == '1ca08a6d5b7fa785029b4e848298e6ee702e00c91e6214ec0eae251442dcccdd')?1:0;
...
if($select_val==""){
	...
	//echo "7";exit;
		...
		if ($security_code == "" && $securitybypass != 1) {
...
// print($securitybypass); exit;
if (isset($security_code) ) {

	if ((md5($captcha_decode) == $security_code) || $securitybypass == 1) {
		...
		if (
						...
						if($stake==9){
							...
							// by nirupam for 1st time login orf AEO
								...
								//echo $config['base_url'] . "/page/dashboard.php";exit;
