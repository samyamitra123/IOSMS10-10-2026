...
// Complete
$Query = "Select count(app_id) from intra_pri_cg_profile_master WHERE gp_id_fk IN(block_id_pk - gp_ids) || ps_id_fk(block_id_pk - ps_ids)  AND status=2"

// Submitted Not forwarded
$Query = "Select application_id from intra_pri_cg_profile_master WHERE district_level='ID' AND pre_block_id= block_id_pk AND status=0"

// Pending District
$Query = "Select application_id from intra_pri_cg_profile_master WHERE district_level='ID' AND pre_block_id= block_id_pk AND status=1"
